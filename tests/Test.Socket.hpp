...
//     virtual void socketRead ( Socket *socket, const MsgPtr& msg, const
...
//         : socket ( T::connect ( this, IpAddrPort ( address, port ) ) ), timer
//         ( this )
//     {
//         timer.start ( timeout );
//         if ( keepAlive )
...
//         done = 0; \
//         struct TestSocket : public BaseTestSocket<T, KEEP_ALIVE, TIMEOUT> { \
//             void socketAccepted ( Socket *serverSocket ) override { accepted
...
//             TestSocket ( const string& address, uint16_t port ) :
//             BaseTestSocket ( address, port ) {                \
//                 buffer = ::Protocol::encode ( new TestMessage ( "Hello
//                 server!" ) );                                \
