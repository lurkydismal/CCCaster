...
struct BaseTestSocket : public Socket::Owner, public Timer::Owner {
    ...
    BaseTestSocket( const string& address, uint16_t port )
        : socket( T::connect( this, IpAddrPort( address, port ) ) ),
          timer( this ) {
        timer.start( timeout );
        if ( keepAlive )
            socket->getAsUDP().setKeepAlive( keepAlive );
    ...
    TEST( T, Connect ) {                                                      \
        ...
        struct TestSocket : public BaseTestSocket< T, KEEP_ALIVE, TIMEOUT > { \
            ...
            TestSocket( uint16_t port ) : BaseTestSocket( port ) {            \
                ...
                socket->setCheckSumFail( FAIL );                              \
            }                                                                 \
            TestSocket( const string& address, uint16_t port )                \
                : BaseTestSocket( address, port ) {                           \
                socket->setPacketLoss( LOSS );                                \
                socket->setCheckSumFail( FAIL );                              \
            }                                                                 \
        };                                                                    \
...
#define TEST_SEND( T, LOSS, FAIL, KEEP_ALIVE, TIMEOUT )                       \
    TEST( T, Send ) {                                                         \
        ...
        if ( server.msg.get() ) {                                             \
            ...
            EXPECT_EQ( "Hello server!",                                       \
                       server.msg->getAs< TestMessage >().str );              \
        }                                                                     \
        EXPECT_TRUE( client.socket.get() );                                   \
        if ( client.socket.get() )                                            \
            EXPECT_TRUE( client.socket->isConnected() );                      \
        ...
        if ( client.msg.get() ) {                                             \
            ...
            EXPECT_EQ( "Hello client!",                                       \
                       client.msg->getAs< TestMessage >().str );              \
        }                                                                     \
        EXPECT_EQ( 2, done );                                                 \
        SocketManager::get().deinitialize();                                  \
        TimerManager::get().deinitialize();                                   \
    ...
    TEST( T, SendPartial ) {                                                  \
        struct TestSocket : public BaseTestSocket< T, 0, 1000 > {             \
            ...
            TestSocket( const string& address, uint16_t port )                \
                : BaseTestSocket( address, port ) {                           \
                buffer =                                                      \
                    ::Protocol::encode( new TestMessage( "Hello server!" ) ); \
