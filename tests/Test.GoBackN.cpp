...
struct TestClass : public GoBackN::Owner,
                   public Socket::Owner,
                   public Timer::Owner {
...
TEST( GoBackN, SendAndRecv ) {
    ...
    struct TestSocket : public TestClass {
        ...
        void timerExpired( Timer* timer ) override {
            if ( !sent ) {
                gbn.sendViaGoBackN( new TestMessage(
                    socket->isClient() ? "Client 1" : "Server 1" ) );
                gbn.sendViaGoBackN( new TestMessage(
                    socket->isClient() ? "Client 2" : "Server 2" ) );
                gbn.sendViaGoBackN( new TestMessage(
                    socket->isClient() ? "Client 3" : "Server 3" ) );
                gbn.sendViaGoBackN( new TestMessage(
                    socket->isClient() ? "Client 4" : "Server 4" ) );
                gbn.sendViaGoBackN( new TestMessage(
                    socket->isClient() ? "Client 5" : "Server 5" ) );
                sent = true;
                timer->start( LONG_TIMEOUT );
            } else {
                LOG( "Stopping because of timeout" );
...
TEST( GoBackN, Timeout ) {
    ...
    struct TestSocket : public TestClass {
        ...
        TestSocket( uint16_t port )
            : socket( UdpSocket::bind( this, port ) ),
              gbn( this, DEFAULT_SEND_INTERVAL, 1000 ),
              timer( this ),
              properTimeout( false ),
              stage( 0 ) {
            timer.start( 1000 );
