...
HttpDownload::HttpDownload( Owner* owner,
                            const string& url,
                            const string& file )
    : owner( owner ), url( url ), file( file ) {}

void HttpDownload::httpResponse( HttpGet* httpGet,
                                 int code,
                                 const string& data,
...
void HttpDownload::httpProgress( HttpGet* httpGet,
...
void HttpDownload::start() {
    ...
    _httpGet.reset(
        new HttpGet( this, url, DEFAULT_GET_TIMEOUT, HttpGet::Incremental ) );
    _httpGet->start();
}
