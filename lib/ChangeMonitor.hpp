...
public:
    ...
    // Add a RefChangeMonitor, returns a pointer that can be used to remove the
    // monitor
    template < typename O, typename K, typename T >
    ChangeMonitor::Interface* addRef( O* owner, K key, const T& ref );

    // Add a PtrToRefChangeMonitor, returns a pointer that can be used to remove
    // the monitor
    template < typename O, typename K, typename T >
    ChangeMonitor::Interface* addPtrToRef( O* owner,
                                           K key,
                                           const T*& ptr,
                                           T nullPtrValue );

    // Remove a ChangeMonitor::Interface, returns true only if something was
    // removed
...
public:
    ...
    RefChangeMonitor( Owner* owner, K key, const T& ref )
        : owner( owner ), _key( key ), _current( ref ), _previous( ref ) {}
...
template < typename O, typename K, typename T >
ChangeMonitor::Interface* ChangeMonitor::addRef( O* owner,
                                                 K key,
                                                 const T& ref ) {
    typedef typename RefChangeMonitor< K, T >::Owner Owner;

    ChangeMonitorPtr monitor( new RefChangeMonitor< K, T >(
        static_cast< Owner* >( owner ), key, ref ) );
...
// This monitors for changes of a pointer to a reference (the reference not the
// pointer)
template < typename K, typename T >
...
template < typename O, typename K, typename T >
ChangeMonitor::Interface* ChangeMonitor::addPtrToRef( O* owner,
                                                      K key,
                                                      const T*& ptr,
                                                      T nullPtrValue ) {
    typedef typename PtrToRefChangeMonitor< K, T >::Owner Owner;

    ChangeMonitorPtr monitor( new PtrToRefChangeMonitor< K, T >(
        static_cast< Owner* >( owner ), key, ptr, nullPtrValue ) );
