...
#define LOG_UDP_SOCKET( SOCKET, FORMAT, ... ) \
    LOG_SOCKET( SOCKET, "type=%s; " FORMAT, _type, ##__VA_ARGS__ )

UdpSocket::UdpSocket( Socket::Owner* owner,
                      uint16_t port,
                      const Type& type,
                      bool isRaw )
    : Socket( owner, IpAddrPort( "", port ), Protocol::UDP, isRaw ),
      ...
      _gbn( this,
            ...
            isConnectionLess() ? 0 : DEFAULT_KEEP_ALIVE_TIMEOUT ) {
...
UdpSocket::UdpSocket( Socket::Owner* owner,
    ...
    : Socket( owner, address, Protocol::UDP, isRaw ),
      ...
      _gbn( this,
            DEFAULT_SEND_INTERVAL,
            isConnectionLess() ? 0 : connectTimeout ) {
...
UdpSocket::UdpSocket( Socket::Owner* owner, const SocketShareData& data )
    ...
    _fd = WSASocket( data.info->iAddressFamily, SOCK_DGRAM, IPPROTO_UDP,
                     data.info.get(), 0, 0 );
    ...
    switch ( _type.value ) {
        ...
        case Type::Server:
            ...
            for ( const auto& kv : data.childSockets ) {
                UdpSocket* socket =
                    new UdpSocket( ChildSocket, this, kv.first, kv.second );
                _childSockets.insert(
                    make_pair( socket->address, SocketPtr( socket ) ) );

                LOG( "child: address='%s'; keepAlive=%d", socket->address,
                     socket->_keepAlive );
...
UdpSocket::UdpSocket( ChildSocketEnum,
                      UdpSocket* parentSocket,
                      const IpAddrPort& address )
    : Socket( 0, address, Protocol::UDP, parentSocket->_isRaw ),
      _type( Type::Child ),
      _gbn( this,
            parentSocket->getSendInterval(),
            parentSocket->_connectTimeout ),
...
SocketPtr UdpSocket::connect( Socket::Owner* owner,
                              const IpAddrPort& address,
                              uint64_t connectTimeout ) {
    return SocketPtr(
        new UdpSocket( owner, address, Type::Client, false, connectTimeout ) );
}

SocketPtr UdpSocket::bind( Socket::Owner* owner, uint16_t port, bool isRaw ) {
    return SocketPtr(
        new UdpSocket( owner, port, Type::ConnectionLess, isRaw ) );
}

SocketPtr UdpSocket::bind( Socket::Owner* owner,
                           const IpAddrPort& address,
                           bool isRaw ) {
    return SocketPtr( new UdpSocket( owner, address, Type::ConnectionLess,
                                     isRaw, DEFAULT_CONNECT_TIMEOUT ) );
}

SocketPtr UdpSocket::shared( Socket::Owner* owner,
...
bool UdpSocket::send( SerializableMessage* message,
                      const IpAddrPort& address ) {
    _gbn.delayKeepAliveOnce();
    return sendRaw( MsgPtr( message ), address );
}

bool UdpSocket::send( SerializableSequence* message,
                      const IpAddrPort& address ) {
...
bool UdpSocket::send( const MsgPtr& msg, const IpAddrPort& address ) {
    if ( isConnectionLess() ||
         msg.get() == 0
         // Send the UdpControl::Disconnect message immediately instead of using
         // GoBackN
...
bool UdpSocket::sendRaw( const MsgPtr& msg, const IpAddrPort& address ) {
    ...
    if ( isChild() && _parentSocket )
        return _parentSocket->Socket::send(
...
void UdpSocket::goBackNRecvMsg( GoBackN* gbn, const MsgPtr& msg ) {
    ...
    if ( isChild() ) {
        ASSERT( _parentSocket->_childSockets.find( getRemoteAddress() ) !=
                _parentSocket->_childSockets.end() );
        ASSERT( _parentSocket->_childSockets[ getRemoteAddress() ].get() ==
                this );
        ...
        switch ( msg->getMsgType() ) {
            case MsgType::UdpControl:
                if ( isConnecting() ) {
                    switch ( msg->getAs< UdpControl >().value ) {
                        case UdpControl::ConnectRequest:
                            // UdpControl::ConnectRequest should be responded to
                            // with a Reply
                            send( new UdpControl( UdpControl::ConnectReply ) );
                            return;

                        case UdpControl::ConnectFinal:
                            // UdpControl::ConnectFinal means the client
                            // connected properly and is now accepted
                            _state = State::Connected;

                            LOG_UDP_SOCKET( this, "socketAccepted" );

                            _parentSocket->_acceptedSocket =
                                _parentSocket
                                    ->_childSockets[ getRemoteAddress() ];

                            _gbn.setKeepAlive( _keepAlive );

                            if ( _parentSocket->owner )
                                _parentSocket->owner->socketAccepted(
                                    _parentSocket );
                            return;
                ...
                if ( msg->getAs< UdpControl >().value ==
                     UdpControl::Disconnect ) {
                ...
                LOG_UDP_SOCKET( this, "Unexpected '%s' from '%s'", msg,
                                address );
                return;
        ...
        switch ( msg->getMsgType() ) {
            case MsgType::UdpControl:
                // UdpControl::ConnectReply while connecting indicates this
                // socket is now connected
                if ( isConnecting() && msg->getAs< UdpControl >().value ==
                                           UdpControl::ConnectReply ) {
                    _state = State::Connected;
...
void UdpSocket::socketRead( const MsgPtr& msg, const IpAddrPort& address ) {
    if ( isConnectionLess() ) {
        ...
        // Handle the UdpControl::Disconnect message immediately instead of
        // using GoBackN
        if ( msg.get() != 0 && msg->getMsgType() == MsgType::UdpControl &&
             msg->getAs< UdpControl >().value == UdpControl::Disconnect ) {
...
void UdpSocket::socketReadAddressed( const MsgPtr& msg,
                                     const IpAddrPort& address ) {
    UdpSocket* socket;
    ...
    if ( it != _childSockets.end() ) {
    ...
    } else if ( msg.get() && msg->getMsgType() == MsgType::UdpControl &&
                msg->getAs< UdpControl >().value ==
                    UdpControl::ConnectRequest ) {
        ...
        LOG_UDP_SOCKET( this, "Unexpected '%s' from '%s'", msg, address );
        LOG_UDP_SOCKET( this, "Ignoring because no child socket for '%s'",
                        address );
        return;
    }
...
MsgPtr UdpSocket::share( int processId ) {
    ...
    switch ( _type.value ) {
        ...
        case Type::Client:
            ...
            data->getAs< SocketShareData >().gbnState.reset(
                new GoBackN( _gbn ) );
            break;
        ...
        case Type::Server:
            ...
            for ( const auto& kv : _childSockets ) {
                LOG( "child: address='%s'; keepAlive=%d", kv.first,
                     kv.second->getAsUDP()._keepAlive );
                kv.second->getAsUDP()._gbn.logSendList();

                data->getAs< SocketShareData >().childSockets[ kv.first ] =
                    kv.second->getAsUDP()._gbn;
                kv.second->getAsUDP()._gbn.reset(); // Reset to stop the GoBackN
                                                    // timers from firing
            }
            break;
