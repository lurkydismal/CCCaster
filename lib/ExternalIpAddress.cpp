...
void ExternalIpAddress::httpResponse( HttpGet* httpGet,
                                      int code,
                                      const string& data,
    ...
    if ( code != 200 ||
         data.size() <
             7 ) // Min IPv4 length, eg "1.1.1.1" TODO actually validate this
    {
        httpFailed( httpGet );
        return;
    }
...
void ExternalIpAddress::httpFailed( HttpGet* httpGet ) {
    ...
    _httpGet.reset(
        new HttpGet( this, ExternalIpServices[ _nextQueryIndex++ ] ) );
    _httpGet->start();
}
