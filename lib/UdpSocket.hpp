...
class UdpSocket : public Socket, private GoBackN::Owner {
   public:
    ...
    // Can only be used on a connection-less socket, where address.addr is
    // empty. Changes the type to a message-based, UDP server socket.
    void listen();

    // Connect to the address.
    // Can only be used on a connection-less socket, where address.addr is NOT
    // empty. Changes the type to a message-based, UDP client socket.
    void connect();
