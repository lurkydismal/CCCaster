...
class Logger {
   public:
    ...
    void initialize( const std::string& filePath = "",
                     uint32_t options = LOG_DEFAULT_OPTIONS );
    void deinitialize();
...
#define LOG_TO( LOGGER, FORMAT, ... )                             \
    do {                                                          \
        LOGGER.log( __BASE_FILE__, __LINE__, __PRETTY_FUNCTION__, \
                    format( FORMAT, ##__VA_ARGS__ ).c_str() );    \
    } while ( 0 )

#define LOG( FORMAT, ... )                                               \
    do {                                                                 \
        Logger::get().log( __BASE_FILE__, __LINE__, __PRETTY_FUNCTION__, \
                           format( FORMAT, ##__VA_ARGS__ ).c_str() );    \
    } while ( 0 )
