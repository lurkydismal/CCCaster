...
std::shared_ptr< addrinfo > getAddrInfo( const std::string& addr,
                                         uint16_t port,
                                         bool isV4,
                                         bool passive = false );
...
class IpAddrPort : public SerializableSequence {
   public:
    ...
    bool empty() const { return ( addr.empty() && !port ); }
...
// Stream operator
inline std::ostream& operator<<( std::ostream& os, const IpAddrPort& a ) {
    return ( os << a.str() );
}
