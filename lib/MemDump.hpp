...
class MemDumpBase {
   ...
   protected:
    ...
    static std::vector< MemDumpPtr > addOffsets(
        const std::vector< MemDumpPtr >& ptrs,
        size_t addSrcOffset );
    static std::vector< MemDumpPtr > concat(
        const std::vector< MemDumpPtr >& a,
        const std::vector< MemDumpPtr >& b );
};
...
inline MemDumpBase::MemDumpBase( size_t size,
                                 const std::vector< MemDumpPtr >& ptrs )
...
class MemDumpList {
   public:
    ...
    void append( const MemDump& mem, size_t addAddrOffset ) {
        addrs.push_back(
            MemDump( mem.addr + addAddrOffset, mem.size, mem.ptrs ) );
    }
    ...
    // Update the list of memory dumps: merge continuous address ranges, then
    // compute total size
    void update();
