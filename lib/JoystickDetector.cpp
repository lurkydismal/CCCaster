...
static LRESULT CALLBACK joystickCallback( HWND hwnd,
                                          UINT message,
                                          WPARAM wParam,
                                          LPARAM lParam ) {
    switch ( message ) {
...
public:
    ...
    void run() override {
        ...
        if ( FAILED( comInitRet ) )
            THROW_EXCEPTION( "CoInitializeEx failed: 0x%08x",
                             ERROR_CONTROLLER_INIT, comInitRet );
        ...
        if ( !RegisterClassEx( &windowClass ) )
            THROW_WIN_EXCEPTION( GetLastError(), "RegisterClassEx failed",
                                 ERROR_CONTROLLER_INIT );

        HWND windowHandle = CreateWindowEx( 0, "JoystickThread", 0, 0, 0, 0, 0,
                                            0, HWND_MESSAGE, 0, 0, 0 );
        if ( !windowHandle )
            THROW_WIN_EXCEPTION( GetLastError(), "CreateWindowEx failed",
                                 ERROR_CONTROLLER_INIT );
        ...
        HDEVNOTIFY notifyHandle = RegisterDeviceNotification(
            windowHandle, &dbh, DEVICE_NOTIFY_WINDOW_HANDLE );
        if ( !notifyHandle )
            THROW_WIN_EXCEPTION( GetLastError(),
        ...
        for ( ;; ) {
            ...
            if ( ( ret = GetMessage( &message, windowHandle, 0, 0 ) ) != 0 ) {
                if ( ret == -1 ) {
                    THROW_WIN_EXCEPTION( GetLastError(), "GetMessage failed",
                                         ERROR_CONTROLLER_CHECK );
                } else {
...
void JoystickDetector::socketRead( Socket* socket,
                                   const MsgPtr& msg,
...
void JoystickDetector::start() {
    ...
    sendSocket =
        UdpSocket::bind( 0, { "127.0.0.1", recvSocket->address.port } );
