...
template < typename T >
inline void printToString( char* buffer,
                           size_t len,
                           const char* fmt,
                           const T& val,
                           bool2type< false > ) {
...
void splitFormat( const std::string& fmt,
                  std::string& first,
                  std::string& rest );
...
inline std::string format( const std::string& fmt, const T& val, V... vals ) {
    ...
    printToString( buffer, sizeof( buffer ), first.c_str(), val,
                   bool2type < std::is_arithmetic< T >::value ||
...
// Normalize a path to Windows backslash format and make sure there is a
// trailing backslash
