...
#define LOG_CONTROLLER( CONTROLLER, FORMAT, ... )                           \
    LOG( "%s: controller=%08x; state=%08x; " FORMAT, CONTROLLER->getName(), \
         CONTROLLER, CONTROLLER->_state, ##__VA_ARGS__ )
...
struct JoystickMappings : public SerializableSequence {
    ...
    //
    // The zero value is mapped to a bit mask of all the mapped values on the
    // same axis index.
    //
    // Example:
    //   Axis up      -> 0b0001
    ...
    //
    // The 5 value is mapped to a bit mask of all the mapped values on the same
    // hat index.
    //
    // Example:
    ...
    PROTOCOL_MESSAGE_BOILERPLATE( JoystickMappings,
                                  name,
                                  axes,
                                  hats,
                                  buttons,
...
class Controller : public KeyboardManager::Owner {
   ...
   private:
    ...
    // Private constructor, instances should only be created by
    // ControllerManager
    Controller();
