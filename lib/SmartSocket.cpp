...
#define LOG_SMART_SOCKET( SOCKET, FORMAT, ... ) \
    LOG_SOCKET( SOCKET, FORMAT, ##__VA_ARGS__ )
...
/* Tunnel protocol
    ...
    1 - Host opens a TCP socket to the server and sends its TypedHostingPort.
        Host should maintain the socket connection; reconnect and resend if
  needed.

    2 - Client opens a TCP socket to the server and sends its
  TypedConnectionAddress.

    2 - Server tries to match-make:
        If a matching host if found, the server sends MatchInfo to host AND
  client over TCP. Otherwise disconnects the client if no matching host exists.

    3 - On match, host and client both create a new UDP socket bound to any
  port, and start repeatedly sending UdpData to the server's UDP port.

    4 - Server recvs UdpData from the client and sends TunInfo ONCE to the host
  over TCP. Server recvs UdpData from the host and sends TunInfo ONCE to the
  client over TCP.
  ...
  Binary formats (little-endian):
    ...
    TypedHostingPort is a char followed by a single uint16_t. The char must by
  'T' for TCP or 'U' for UDP.

    TypedConnectionAddress is a NON-null-terminated string, eg. "T<ip>:<port>".
  The first char is the socket type.

    MatchInfo is "MatchInfo" followed by the matchId.

    UdpData is a uint8_t followed by the matchId. The uint8_t is a boolean flag
  indicating isClient.

    TunInfo is "TunInfo" followed by the matchId, followed by a NULL-terminated
  address string (for easier parsing).
...
struct MatchInfo {
    static uint32_t decode( const char* buffer, size_t len, size_t& consumed ) {
        ...
        if ( len < header.size() + sizeof( uint32_t ) ||
             string( buffer, header.size() ) != header ) {
            consumed = 0;
            return 0;
        }
...
struct TunInfo {
    ...
    static TunInfo decode( const char* buffer, size_t len, size_t& consumed ) {
        ...
        for ( i = 0; i < 22;
              ++i ) // max string length ("255.255.255.255:65535\0")
        {
            if ( start + i >= len )
                break;
        ...
        consumed = end + 1;
        return TunInfo( *( uint32_t* )&buffer[ header.size() ],
                        string( buffer + start, end - start ) );
    }
};

SmartSocket::SmartSocket( Owner* owner,
                          uint16_t port,
...
void SmartSocket::socketAccepted( Socket* serverSocket ) {
    ASSERT( serverSocket == _directSocket.get() ||
            serverSocket == _tunSocket.get() );
...
void SmartSocket::socketDisconnected( Socket* socket ) {
    if ( socket == _directSocket.get() && isServer() ) {
        _directSocket.reset();
    } else if ( socket == _directSocket.get() &&
                ( isConnecting() || ( isConnected() && !gotGoodRead() ) ) ) {
        ...
        _vpsAddress = relayServers.cbegin();
        _vpsSocket =
            TcpSocket::connect( this, *_vpsAddress, true ); // Raw socket

        if ( owner )
            ( ( SmartSocket::Owner* )owner )->smartSocketSwitchedToUDP( this );
    } else if ( ( socket == _directSocket.get() && isConnected() &&
                  gotGoodRead() ) ||
                socket == _tunSocket.get() ) {
...
void SmartSocket::socketRead( Socket* socket,
                              const MsgPtr& msg,
                              const IpAddrPort& address ) {
    if ( owner )
...
void SmartSocket::socketRead( Socket* socket,
    ...
    LOG( "Read [ %u bytes ] from '%s'; %u bytes remaining in buffer", len,
         address, _vpsSocket->_readPos );
    ...
    for ( ;; ) {
        id = MatchInfo::decode( &_vpsSocket->_readBuffer[ 0 ],
                                _vpsSocket->_readPos, consumed );
        ...
        tun = TunInfo::decode( &_vpsSocket->_readBuffer[ 0 ],
                               _vpsSocket->_readPos, consumed );

        if ( tun.matchId ) {
            LOG_SMART_SOCKET( this, "gotTunInfo ( %u, '%s' )", tun.matchId,
                              tun.address );
    ...
    if ( timer == _connectTimer.get() ) {
        ...
        if ( isServer() ) {
            for ( const auto& kv : _pendingClients ) {
                ...
                _tunSocket->send( data.buffer, sizeof( data.buffer ),
                                  *_vpsAddress );
        ...
        if ( it != _pendingClients.end() ) {
            LOG_SMART_SOCKET( this,
                              "matchId=%u; address='%s'; Client timed out",
...
SocketPtr SmartSocket::connectTCP( Owner* owner,
                                   const IpAddrPort& address,
                                   bool forceTunnel ) {
    const string addr = getAddrFromSockAddr(
        address.getAddrInfo()->ai_addr ); // Resolve IP address first
    return SocketPtr( new SmartSocket( owner, { addr, address.port },
                                       Socket::Protocol::TCP, forceTunnel ) );
}

SocketPtr SmartSocket::connectUDP( Owner* owner,
                                   const IpAddrPort& address,
                                   bool forceTunnel ) {
    const string addr = getAddrFromSockAddr(
        address.getAddrInfo()->ai_addr ); // Resolve IP address first
    return SocketPtr( new SmartSocket( owner, { addr, address.port },
                                       Socket::Protocol::UDP, forceTunnel ) );
}

bool SmartSocket::isTunnel() const {
    return ( isClient() && _tunSocket &&
             !_tunSocket->getAsUDP().isConnectionLess() &&
             _tunSocket->isConnected() );
}
...
bool SmartSocket::send( const char* buffer,
                        size_t len,
                        const IpAddrPort& address ) {
    BOILERPLATE_SEND( buffer, len, address );
}

bool SmartSocket::send( SerializableMessage* message,
                        const IpAddrPort& address ) {
    BOILERPLATE_SEND( message, address );
}

bool SmartSocket::send( SerializableSequence* message,
                        const IpAddrPort& address ) {
    BOILERPLATE_SEND( message, address );
}
