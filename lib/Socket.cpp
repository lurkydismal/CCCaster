...
#define SET_NON_BLOCKING_MODE( VALUE )                                    \
    do {                                                                  \
        ...
        if ( ioctlsocket( _fd, FIONBIO, &flag ) != 0 ) {                  \
            ...
            closesocket( _fd );                                           \
            _fd = 0;                                                      \
            THROW_WIN_EXCEPTION( WSAGetLastError(),                       \
                                 "ioctlsocket(FIONBIO, %u) failed",       \
...
void Socket::init() {
    ...
    for ( ; res; res = res->ai_next ) {
        ...
        if ( _fd == INVALID_SOCKET ) {
            exc = WinException( WSAGetLastError(), "socket failed",
                                ERROR_NETWORK_GENERIC );
            _fd = 0;
        ...
        if ( enableForceReusePort && ( isServer() || isUDP() ) ) {
            ...
            if ( setsockopt( _fd, SOL_SOCKET, SO_REUSEADDR, &yes, 1 ) ==
                 SOCKET_ERROR ) {
                exc = WinException( WSAGetLastError(), "setsockopt failed",
                                    ERROR_NETWORK_GENERIC );
                LOG_SOCKET( this, "%s", exc );
        ...
        if ( isClient() ) {
            if ( isTCP() ) {
                if ( ::connect( _fd, res->ai_addr, res->ai_addrlen ) ==
                     SOCKET_ERROR ) {
                    ...
                    exc = WinException( error, "connect failed",
                                        ERROR_NETWORK_GENERIC );
                    LOG_SOCKET( this, "%s", exc );
                    closesocket( _fd );
                    _fd = 0;
                ...
                if ( ::bind( _fd, res->ai_addr, res->ai_addrlen ) ==
                     SOCKET_ERROR ) {
                    exc = WinException( WSAGetLastError(), "bind failed",
                                        ERROR_NETWORK_GENERIC );
                    LOG_SOCKET( this, "%s", exc );
                    closesocket( _fd );
                    _fd = 0;
            ...
            if ( ::bind( _fd, res->ai_addr, res->ai_addrlen ) ==
                ...
                exc = WinException(
                    WSAGetLastError(),
                    format( ERROR_NETWORK_PORT_BIND, address.port ), "" );
                LOG_SOCKET( this, "%s", exc );
                closesocket( _fd );
                _fd = 0;
                continue;
            ...
            if ( isTCP() && ( ::listen( _fd, SOMAXCONN ) == SOCKET_ERROR ) ) {
                exc = WinException( WSAGetLastError(), "listen failed",
                                    ERROR_NETWORK_GENERIC );
                LOG_SOCKET( this, "%s", exc );
                closesocket( _fd );
                _fd = 0;
    ...
    if ( address.port == 0 ) {
        ...
        if ( getsockname( _fd, ( sockaddr* )&sas, &saLen ) == SOCKET_ERROR ) {
            ...
            closesocket( _fd );
            _fd = 0;
            THROW_WIN_EXCEPTION( WSAGetLastError(), "getsockname failed",
                                 ERROR_NETWORK_GENERIC );
        }
...
bool Socket::send( const char* buffer, size_t len ) {
    ...
    while ( totalBytes < len || len == 0 ) {
        ...
        if ( isTCP() ) {
            ...
            sentBytes =
                ::sendto( _fd, buffer, len, 0, address.getAddrInfo()->ai_addr,
                          address.getAddrInfo()->ai_addrlen );
        }
...
bool Socket::send( const char* buffer, size_t len, const IpAddrPort& address ) {
    ...
    while ( totalBytes < len || len == 0 ) {
        ...
        if ( sentBytes == SOCKET_ERROR ) {
            LOG_SOCKET( this, "%s; sendto failed",
                        WinException::getLastSocketError() );
            return false;
        }
...
int Socket::recvfrom( char* buffer, size_t& len, IpAddrPort& address ) {
    ...
    int recvBytes =
        ::recvfrom( _fd, buffer, len, 0, ( sockaddr* )&sas, &saLen );
...
void Socket::socketRead() {
    ...
    if ( error ) {
        LOG_SOCKET( this, "[%d] %s; %s failed", error,
        ...
        // WSAECONNRESET does not mean the UDP socket is dead, it just means
    ...
    _readPos += bufferLen;
    LOG( "Read [ %u bytes ] from '%s'; %u bytes remaining in buffer", bufferLen,
         address, _readPos );
    ...
    if ( _readPos >= sizeof( MsgType ) &&
         !::Protocol::checkMsgType( *( MsgType* )&_readBuffer[ 0 ] ) ) {
    ...
    for ( ;; ) {
        ...
        MsgPtr msg =
            ::Protocol::decode( &_readBuffer[ 0 ], _readPos, consumedBytes );
        ...
        LOG( "Decoded '%s' using [ %u bytes ]; %u bytes remaining in buffer",
             msg, consumedBytes, _readPos );
...
MsgPtr Socket::share( int processId ) {
    ...
    if ( WSADuplicateSocket( _fd, processId, info.get() ) )
        THROW_WIN_EXCEPTION( WSAGetLastError(), "WSADuplicateSocket failed",
                             ERROR_NETWORK_GENERIC );
    ...
    return MsgPtr( new SocketShareData( address, protocol, _readBuffer,
                                        _readPos, _state, info ) );
}
