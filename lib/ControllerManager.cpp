...
static const Guid PS4_CONTROLLER_GUID( { 0x4c, 0x05, 0xc4, 0x05, 0x00, 0x00,
                                         0x00, 0x00, 0x00, 0x00, 0x50, 0x49,
                                         0x44, 0x56, 0x49, 0x44 } );

static const Guid P3_WIRED_GAMEPAD( { 0x12, 0x0c, 0x30, 0x0c, 0x00, 0x00, 0x00,
                                      0x00, 0x00, 0x00, 0x50, 0x49, 0x44, 0x56,
                                      0x49, 0x44 } );
...
static inline uint8_t mapHatValue( uint32_t value ) {
    ...
    // The original value has range [0,36000), starting with 0 as north, and
    // rotating clockwise. So we need to round the value down to the range [0,
    // 8).
    value %= 36000;
    value /= 4500;
...
bool ControllerManager::check() {
    ...
    for ( auto it = joysticks.begin(); it != joysticks.end(); ) {
        ...
        // Note: this is independent of the prevState property, which is used to
        // detect edge events per frame.
        ...
        if ( FAILED( result ) ) {
            LOG_CONTROLLER( controller,
                            "IDirectInputDevice8_Poll failed: 0x%08x", result );
        ...
        if ( FAILED( result ) ) {
            LOG_CONTROLLER( controller,
                            "IDirectInputDevice8_GetDeviceState failed: 0x%08x",
                            result );
            LOG( "Detaching joystick: %s", it->first );
            detachJoystick( ( it++ )->first );
            continue;
        ...
        if ( axisMask & 0x02u )
            axes[ axis++ ] = mapAxisValue(
                -djs.lY, deadzone ); // DirectInput, like most APIs,
                                     // reports inverted Y-axis
        if ( axisMask & 0x04u )
        ...
        if ( axisMask & 0x10u )
            axes[ axis++ ] = mapAxisValue(
                -djs.lRy, deadzone ); // DirectInput, like most APIs, reports
                                      // inverted Y-axis
        if ( axisMask & 0x20u )
        ...
        for ( axis = 0; axis < info.numAxes; ++axis ) {
            ...
            LOG_CONTROLLER( controller, "axis%u: %u -> %u", axis,
                            prevAxes[ axis ], axes[ axis ] );
        ...
        for ( uint8_t hat = 0; hat < info.numHats; ++hat ) {
            ...
            LOG_CONTROLLER( controller, "hat%u: %u -> %u", hat, prevHats[ hat ],
                            hats[ hat ] );
        ...
        for ( uint8_t button = 0; button < info.numButtons; ++button ) {
            ...
            if ( ( buttons & ( 1u << button ) ) ==
                 ( prevButtons & ( 1u << button ) ) )
                continue;

            LOG_CONTROLLER( controller, "raw=%08X; button%u: %s", buttons,
                            button, ( value ? "0 -> 1" : "1 -> 0" ) );
...
static BOOL CALLBACK enumJoystickAxes( const DIDEVICEOBJECTINSTANCE* ddoi,
                                       void* userPtr ) {
...
#define CHECK_ADD_AXIS( INDEX, GUID_CONSTANT, NAME )             \
    ...
    if ( PS4_CONTROLLER_GUID == info.guid || P3_WIRED_GAMEPAD == info.guid ) {
        // Workaround for the PS4 pad, because it maps some buttons to the X/Y
        // rotation axes as well. These buttons are mapped to min/max values so
        // they end up eating all the inputs. So we ignore them...
    } else {
    ...
    result = IDirectInputDevice8_SetProperty(
        ( IDirectInputDevice8* )info.device, DIPROP_RANGE, &range.diph );
    if ( FAILED( result ) )
        LOG( "IDirectInputDevice8_SetProperty DIPROP_RANGE failed: 0x%08x",
             result );
    ...
    result = IDirectInputDevice8_SetProperty(
        ( IDirectInputDevice8* )info.device, DIPROP_DEADZONE, &deadzone.diph );
    if ( FAILED( result ) )
        LOG( "IDirectInputDevice8_SetProperty DIPROP_DEADZONE failed: 0x%08x",
             result );
...
void ControllerManager::attachJoystick( const Guid& guid, JoystickInfo& info ) {
    ...
    // Create the DirectInput device, NOTE the pointer returned is only
    // temporarily used
    ...
    result = IDirectInputDevice8_QueryInterface( tmp, IID_IDirectInputDevice8,
                                                 ( void** )&device );
    ...
    result = IDirectInputDevice8_SetCooperativeLevel(
        device, ( HWND )windowHandle, JOYSTICK_FLAGS );
    if ( FAILED( result ) )
        LOG( "IDirectInputDevice8_SetCooperativeLevel failed: 0x%08x",
             result ); // Non-fatal
    ...
    result = IDirectInputDevice8_EnumObjects( device, enumJoystickAxes, &info,
                                              DIDFT_AXIS );
    if ( FAILED( result ) ) {
    ...
    if ( it != mappings.mappings.end() &&
         it->second->getMsgType() == MsgType::JoystickMappings ) {
...
void ControllerManager::detachJoystick( const Guid& guid ) {
    ...
    Controller* controller = it->second.get();
    IDirectInputDevice8* const device =
        ( IDirectInputDevice8* )controller->_joystick.info.device;
...
static BOOL CALLBACK enumJoysticks( const DIDEVICEINSTANCE* ddi,
                                    void* userPtr ) {
    unordered_map< Guid, JoystickInfo >& activeJoysticks =
        *( unordered_map< Guid, JoystickInfo >* )userPtr;
    ...
    LOG( "%s: guidInstance='%s'; guidProduct='%s'", info.name, guidInstance,
         info.guid );
...
void ControllerManager::refreshJoysticks() {
    ...
    HRESULT result = IDirectInput8_EnumDevices(
        dinput, DI8DEVCLASS_GAMECTRL, enumJoysticks, ( void* )&activeJoysticks,
        DIEDFL_ATTACHEDONLY );

    if ( FAILED( result ) )
        THROW_EXCEPTION( "IDirectInput8_EnumDevices failed: 0x%08x",
                         ERROR_CONTROLLER_CHECK, result );
...
void ControllerManager::initialize( Owner* owner ) {
    ...
    if ( FAILED( comInitRet ) )
        THROW_EXCEPTION( "CoInitializeEx failed: 0x%08x", ERROR_CONTROLLER_INIT,
                         comInitRet );

    HRESULT result =
        CoCreateInstance( CLSID_DirectInput8, 0, CLSCTX_INPROC_SERVER,
                          IID_IDirectInput8, ( void** )&dinput );
    if ( FAILED( result ) )
        THROW_EXCEPTION( "CoCreateInstance failed: 0x%08x",
                         ERROR_CONTROLLER_INIT, result );

    result = IDirectInput8_Initialize( dinput, GetModuleHandle( 0 ),
                                       DIRECTINPUT_VERSION );
    if ( FAILED( result ) )
        THROW_EXCEPTION( "IDirectInput8_Initialize failed: 0x%08x",
                         ERROR_CONTROLLER_INIT, result );
...
void ControllerMappings::load( cereal::BinaryInputArchive& ar ) {
    ...
    for ( size_t i = 0; i < count; ++i ) {
        ...
        mappings[ name ] =
            Protocol::decode( &buffer[ 0 ], buffer.size(), consumed );

        ASSERT( consumed == buffer.size() );
    }
}

size_t ControllerManager::saveMappings( const string& folder,
...
size_t ControllerManager::loadMappings( const string& folder,
                                        const string& ext ) {
    LOCK( mutex );
...
bool ControllerManager::saveMappings( const string& file,
                                      const MsgPtr& mappings ) {
...
MsgPtr ControllerManager::loadMappings( const string& file ) {
    ...
    if ( good ) {
        ...
        if ( !msg )
            LOG( "Failed to decode %u bytes from file %s", buffer.size(),
                 file.c_str() );
        else if ( consumed != buffer.size() )
            LOG( "Warning: consumed bytes %u != buffer size %u", consumed,
                 buffer.size() );
    }
