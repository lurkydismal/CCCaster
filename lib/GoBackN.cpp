...
string formatSerializableSequence( const MsgPtr& msg ) {
    ...
    return format( "%u:'%s'",
                   msg->getAs< SerializableSequence >().getSequence(), msg );
}
...
void GoBackN::timerExpired( Timer* timer ) {
    ...
    if ( _sendList.empty() && !_keepAlive ) {
        ...
        LOG( "Sending '%s'; sequence=%u; sendSequence=%d", msg,
             msg->getAs< SerializableSequence >().getSequence(),
             _sendSequence );
    ...
    if ( _keepAlive ) {
        LOG( "this=%08x; keepAlive=%llu; countDown=%d", this, _keepAlive,
             _countDown );

        if ( _countDown ) {
            --_countDown;
        } else {
            LOG( "owner->goBackNTimeout ( this=%08x ); owner=%08x", this,
                 owner );
...
void GoBackN::sendViaGoBackN( const MsgPtr& msg ) {
    ...
    ASSERT( msg->getBaseType() == BaseType::SerializableSequence );
    ASSERT( _sendList.empty() ||
            _sendList.back()->getAs< SerializableSequence >().getSequence() ==
                _sendSequence );
    ASSERT( owner != 0 );
    ...
    if ( msg->getAs< SerializableSequence >().getSequence() != 0 ) {
        ...
        if ( bytes.size() <= MTU ) {
            ...
            const uint32_t count =
                ( bytes.size() / MTU ) + ( bytes.size() % MTU == 0 ? 0 : 1 );

            for ( uint32_t pos = 0, i = 0; pos < bytes.size();
                  pos += MTU, ++i ) {
                SplitMessage* splitMsg = new SplitMessage(
                    msg->getMsgType(), bytes.substr( pos, MTU ), i, count );
...
void GoBackN::recvFromSocket( const MsgPtr& msg ) {
    ...
    if ( msg->getMsgType() == MsgType::AckSequence ) {
        ...
        LOG( "Got AckSequence; sequence=%u; sendSequence=%u", sequence,
             _sendSequence );

        // Remove messages from sendList with sequence <= the ACKed sequence
        while (
            !_sendList.empty() &&
            _sendList.front()->getAs< SerializableSequence >().getSequence() <=
                sequence )
            _sendList.pop_front();
    ...
    if ( sequence != _recvSequence + 1 ) {
        owner->goBackNSendRaw( this,
                               MsgPtr( new AckSequence( _recvSequence ) ) );
        return;
    }

    LOG( "Received '%s'; sequence=%u; recvSequence=%u", msg, sequence,
         _recvSequence );
    ...
    if ( msg->getMsgType() == MsgType::SplitMessage ) {
        ...
        if ( splitMsg.isLastMessage() ) {
            size_t consumed = 0;
            MsgPtr msg = ::Protocol::decode( &_recvBuffer[ 0 ],
                                             _recvBuffer.size(), consumed );

            if ( !msg.get() || msg->getMsgType() != splitMsg.origMsgType ||
                 consumed != _recvBuffer.size() ) {
                LOG( "Failed to recreate '%s' from [ %u bytes ]",
                     splitMsg.origMsgType, _recvBuffer.size() );
                msg.reset();
            }
...
GoBackN::GoBackN( Owner* owner, const GoBackN& state )
    : owner( owner ), _sendListPos( _sendList.cend() ) {
    *this = state;
}
