...
typedef HRESULT( __stdcall* PFN_DX9_RESET )( IDirect3DDevice9* pDevice,
                                             LPVOID );
typedef HRESULT( __stdcall* PFN_DX9_PRESENT )( IDirect3DDevice9* pDevice,
                                               const RECT*,
                                               const RECT*,
                                               HWND,
                                               LPVOID );
...
EXTERN_C ULONG __declspec( dllexport ) __stdcall DX9_AddRef(
    IDirect3DDevice9* pDevice ) {
    // New AddRef function
    g_iRefCount = s_D3D9_AddRef( pDevice );
    // DEBUG_TRACE(("DX9_AddRef: called (m_iRefCount = %d)." LOG_CR,
    // g_iRefCount));
    return g_iRefCount;
}

EXTERN_C ULONG __declspec( dllexport ) __stdcall DX9_Release(
    IDirect3DDevice9* pDevice ) {
    // New Release function
    // a "fall-through" case
    if ( ( g_iRefCount > g_iRefCountMe + 1 ) && s_D3D9_Release ) {
        g_iRefCount = s_D3D9_Release( pDevice );
        // DEBUG_TRACE(("DX9_Release: called (m_iRefCount = %d)." LOG_CR,
        // g_iRefCount));
        return g_iRefCount;
    }
    ...
    DEBUG_TRACE(("DX9_Release: VTABLE[0] = %08x" LOG_CR,
    ((UINT_PTR*)(*((UINT_PTR*)pDevice)))[0])); DEBUG_TRACE(("DX9_Release:
    VTABLE[1] = %08x" LOG_CR, ((UINT_PTR*)(*((UINT_PTR*)pDevice)))[1]));
    DEBUG_TRACE(("DX9_Release: VTABLE[2] = %08x" LOG_CR,
    ((UINT_PTR*)(*((UINT_PTR*)pDevice)))[2]));
    */
...
void DX9_HooksVerify( IDirect3DDevice9* pDevice ) {
    // It looks like at certain points, vtable entries get restored to its
    // original values. If that happens, we need to re-assign them to our
    // functions again. NOTE: we don't want blindly re-assign, because there can
    // be other programs hooking on the same methods. Therefore, we only
    // re-assign if we see that original addresses are restored by the system.
    ...
    if ( pVTable[ INTF_Release ] == ( UINT_PTR )s_D3D9_Release ) {
        ...
        // DEBUG_MSG(( "DX9_HooksVerify: pDevice->Release() re-hooked."
        // LOG_CR));
    }
}

EXTERN_C HRESULT __declspec( dllexport ) __stdcall DX9_Reset(
    IDirect3DDevice9* pDevice,
    LPVOID params ) {
    // New Reset function
...
EXTERN_C HRESULT __declspec( dllexport ) __stdcall DX9_Present(
    ...
    // remember IDirect3DDevice9::Release pointer so that we can clean-up
    // properly.
...
string InitDirectX( void* hwnd ) {
    // get the offset from the start of the DLL to the interface element we
    // want. step 1: Load d3d9.dll
    HRESULT hRes = g_DX9.LoadDll( TEXT( "d3d9" ) );
    if ( IS_ERROR( hRes ) ) {
        _com_error err( hRes );
        return "Failed to load d3d9.dll: [" + toHexString( hRes ) + "] " +
               err.ErrorMessage();
    }

    // step 2: Get IDirect3D9
    DIRECT3DCREATE9 pDirect3DCreate9 =
        ( DIRECT3DCREATE9 )g_DX9.GetProcAddress( "Direct3DCreate9" );
    ...
    if ( FAILED( hRes ) ) {
        _com_error err( hRes );
        return "GetAdapterDisplayMode failed: [" + toHexString( hRes ) + "] " +
               err.ErrorMessage();
    }
    ...
    hRes = pD3D->CreateDevice( D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, ( HWND )hwnd,
                               ...
                               IREF_GETPPTR( pD3DDevice, IDirect3DDevice9 ) );
    if ( FAILED( hRes ) ) {
        _com_error err( hRes );
        return "CreateDevice failed: [" + toHexString( hRes ) + "] " +
               err.ErrorMessage();
    }

    // step 4: store method addresses in out vars
    UINT_PTR* pVTable =
        ( UINT_PTR* )( *( ( UINT_PTR* )pD3DDevice.get_RefObj() ) );
    assert( pVTable );
...
string HookDirectX() {
    // This function hooks two IDirect3DDevice9 methods, using code overwriting
    // technique. hook IDirect3DDevice9::Present(), using code modifications at
    // run-time. ALGORITHM: we overwrite the beginning of real
    // IDirect3DDevice9::Present with a JMP instruction to our routine
    // (DX9_Present). When our routine gets called, first thing that it does -
    // it restores the original bytes of IDirect3DDevice9::Present, then
    // performs its pre-call tasks, then calls IDirect3DDevice9::Present, then
    // does post-call tasks, then writes the JMP instruction back into the
    // beginning of IDirect3DDevice9::Present, and returns.
    ...
    if ( !m_Hook_Present.InstallHook( ( void* )s_D3D9_Present,
                                      ( void* )DX9_Present ) )
        return "m_Hook_Present failed";

    if ( !m_Hook_Reset.InstallHook( ( void* )s_D3D9_Reset,
                                    ( void* )DX9_Reset ) )
