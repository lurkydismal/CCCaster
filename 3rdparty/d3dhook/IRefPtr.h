...
protected:
    ...
    void AssertIREF( _TYPE* pObj ) const {
        ...
        // assert( PTR_CAST(IUnknown,pObj)); // ambigious in the case of
        // multiple non virtual IUnknown bases !!! and fails !
    }
#else
    void AssertIREF( _TYPE* pObj ) const {}
#endif
...
public:
    ...
    int ReleaseRefObj() {
        if ( m_pObj ) {
            ...
            IRef_Log( pObj, typeid( pObj ), -1, m_pszFile, m_iLine );
#endif
            m_pObj = NULL;          // make sure possible destructors called in
                                    // DecRefCount don't reuse this.
    ...
    // Accessor ops.
    operator _TYPE*() const { return ( m_pObj ); }
