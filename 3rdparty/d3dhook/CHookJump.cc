...
bool CHookJump::InstallHook( LPVOID pFunc, LPVOID pFuncNew ) {
    ...
    // DEBUG_TRACE(("InstallHook: pFunc = %08x, pFuncNew = %08x" LOG_CR,
    // (UINT_PTR)pFunc, (UINT_PTR)pFuncNew ));
    m_Jump[ 0 ] = 0;
    ...
    m_Jump[ 0 ] = 0xe9;
    DWORD dwAddr = ( DWORD )( ( UINT_PTR )pFuncNew - ( UINT_PTR )pFunc ) -
                   sizeof( m_Jump );
...
void CHookJump::RemoveHook( LPVOID pFunc ) {
    ...
    try {
        ...
        ::VirtualProtect( pFunc, 8, m_dwOldProtection,
                          &dwOldProtection ); // restore protection.
