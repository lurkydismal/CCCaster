    ...
    for ( int a = 0; a < h; ++a ) {
        for ( int b = 0; b < w; ++b ) {
            ...
            if ( is_8bpp ) {
                // 8bpp -> 8bpp
                unsigned char* src =
                    ( ( unsigned char* )m_data ) + cell->start + cell->offset;
                ...
                unsigned int* ldest = ( unsigned int* )dest;
                unsigned int* src =
                    ( unsigned int* )( m_data + cell->start + cell->offset );
                int cellw = cell->width;
...
Texture* MBAACC_CG::draw_texture( unsigned int n,
                                  unsigned int* palette,
                                  bool to_pow2_flg,
                                  bool draw_8bpp ) {
    const MBAACC_CG_Image* image = get_image( n );
    ...
    for ( unsigned int i = 0; i < image->align_len; ++i, ++align ) {
        copy_cells( image, align, pixels, x1, y1, width, height, palette,
                    is_8bpp );
    }
        ...
        for ( unsigned int j = 0; j < image->align_len; ++j, ++align ) {
            ...
            for ( int a = 0; a < h; ++a ) {
                ...
                for ( int b = 0; b < w; ++b, ++cell ) {
                    ...
                    cell->offset =
                        ( b * 0x10 ) + ( a * align->width * 0x10 ) * mult;
...
bool MBAACC_CG::load( MBAACC_Pack* pack, const char* name ) {
    ...
    // alignment data
    int align_count =
        ( size - indices[ 3000 ] ) / sizeof( MBAACC_CG_Alignment );
