...
static unsigned int* fd_frame_AT_load( unsigned int* data,
...
static unsigned int* fd_frame_AS_load( unsigned int* data,
                                       const unsigned int* data_end,
                                       MBAACC_Frame_AS* AS ) {
    AS->speed_flags = 0;
    AS->ASMV = -1;
...
static unsigned int* fd_frame_EF_load( unsigned int* data,
                                       const unsigned int* data_end,
                                       MBAACC_Frame_EF* EF ) {
    while ( data < data_end ) {
        unsigned int* buf = data;
...
static unsigned int* fd_frame_IF_load( unsigned int* data,
                                       const unsigned int* data_end,
                                       MBAACC_Frame_IF* IF ) {
    while ( data < data_end ) {
        unsigned int* buf = data;
...
static unsigned int* fd_frame_AF_load( unsigned int* data,
                                       const unsigned int* data_end,
                                       MBAACC_Frame* frame ) {
    frame->AF.active = 1;
    frame->AF.frame = -1;
...
static unsigned int* fd_frame_load( unsigned int* data,
                                    const unsigned int* data_end,
                                    MBAACC_Frame* frame,
                                    TempInfo* info ) {
...
static unsigned int* fd_sequence_load( unsigned int* data,
                                       const unsigned int* data_end,
                                       MBAACC_Sequence* seq ) {
    TempInfo temp_info;
    bool initialized = 0;
...
static unsigned int* fd_main_load( unsigned int* data,
                                   const unsigned int* data_end,
...
bool MBAACC_FrameData::load_move_list( MBAACC_Pack* pack,
                                       const char* filename ) {
    if ( !m_loaded ) {
        return 0;
    }
    ...
    while ( ( line = split_line( &d ) ) != 0 ) {
        ...
        if ( sscanf( line, "%d %19s %19s %19s %d %d %d %d %d", &move_id,
                     move_input, move_props, move_props_2, &move_seq,
                     &move_meter, &move_unk1, &move_unk2, &move_unk3 ) == 9 ) {
            MBAACC_Sequence* seq;
            ...
            if ( seq ) {
                ...
                if ( strlen( move_props ) >= 8 ) {
                    if ( move_props[ 6 ] == '1' && move_props[ 5 ] == '0' &&
                         move_props[ 7 ] == '0' ) {
                        input = "j." + input;
                    }
                }
