...
void MBAACC_Character::set_render_properties( const MBAACC_Frame* frame,
                                              Texture* texture ) {
...
void MBAACC_Character::render( const RenderProperties* properties,
                               int seq_id,
                               int frame_id ) {
    if ( !m_loaded ) {
        return;
    ...
    if ( properties->display_sprite && frame->AF.active &&
         frame->AF.frame >= 0 ) {
        // render sprite
        if ( !m_texture || m_last_sprite_id != frame->AF.frame ) {
            ...
            m_texture = m_cg.draw_texture(
                frame->AF.frame, m_palettes[ m_active_palette ], 1, 0 );
        ...
        if ( m_texture ) {
            ...
            m_texture->draw( ( frame->AF.offset_x - 128 ) * 2,
                             ( frame->AF.offset_y - 224 ) * 2, 2 );
        }
    }
    ...
    if ( properties->display_collision_box ) {
        if ( frame->hitboxes[ 0 ] ) {
            ...
            render_boxes( BOX_COLLISION, &rect, 1,
                          properties->display_solid_boxes );
        }
    }
    ...
    if ( properties->display_hit_box ) {
        ...
        if ( nrects > 0 ) {
            render_boxes( BOX_HIT, rects, nrects,
                          properties->display_solid_boxes );
        }
    }
    ...
    if ( properties->display_attack_box ) {
        ...
        if ( nrects > 0 ) {
            render_boxes( BOX_ATTACK, rects, nrects,
                          properties->display_solid_boxes );
        }
    }
    ...
    if ( properties->display_clash_box ) {
        ...
        if ( nrects > 0 ) {
            render_boxes( BOX_CLASH, rects, nrects,
                          properties->display_solid_boxes );
        }
    }
}
...
Clone* MBAACC_Character::make_clone( int seq_id, int fr_id ) {
    ...
    if ( frame->AF.active && frame->AF.frame >= 0 ) {
        ...
        texture = m_cg.draw_texture( frame->AF.frame,
                                     m_palettes[ m_active_palette ], 1, 0 );

        if ( texture ) {
            set_render_properties( frame, texture );

            clone->init_texture( texture, ( frame->AF.offset_x - 128 ) * 2,
                                 ( frame->AF.offset_y - 224 ) * 2, 2 );
        }
    }
...
void MBAACC_Character::render_frame_properties( bool detailed,
                                                int scr_width,
                                                int scr_height,
                                                int seq_id,
                                                int fr_id ) {
    ...
    if ( frame->AF.active ) {
        y += render_shaded_text( x, y, "$yDuration:$. %d", frame->AF.duration );
    }
    ...
    if (frame->AF.jump_to_seq >= 0) {
            y += render_shaded_text(x, y, "$y-$. Go to seq %3.3d",
    frame->AF.jump_to_seq); } else if (frame->AS->move_to_frame >= 0) { y +=
    render_shaded_text(x, y, "$y-$. Go to state %d", frame->AS->move_to_frame +
    1); } else { y += render_shaded_text(x, y, "$y-$.");
    }
     */
    ...
    if ( frame->AT ) {
        y += render_shaded_text(
            ...
            : frame->AT->guard_flags & 4   ? " Y "
                                           : " N ",
            frame->AT->guard_flags & 0x100 ? " - "
            : frame->AT->guard_flags & 1   ? " Y "
                                           : " N ",
            frame->AT->guard_flags & 0x200 ? " - "
            : frame->AT->guard_flags & 2   ? " Y "
                                           : " N ",
            frame->AT->damage, frame->AT->red_damage,
...
bool MBAACC_Character::load( MBAACC_Pack* pack,
                             const char* name,
                             int sub_type ) {
    if ( m_loaded ) {
        return 0;
...
const char* MBAACC_Character::get_sequence_move_name( int seq_id,
                                                      int* dmeter ) {
    if ( !m_loaded ) {
        return 0;
    }
...
int MBAACC_Character::get_subframe_length( int seq_id, int fr_id ) {
    ...
    if ( ( unsigned int )fr_id < seq->nframes &&
         seq->frames[ fr_id ].AF.active ) {
...
const char* MBAACC_Character::get_current_sprite_filename( int seq_id,
                                                           int fr_id ) {
    if ( !m_loaded ) {
        return 0;
    }
...
bool MBAACC_Character::do_sprite_save( int id, const char* filename ) {
    ...
    if ( texture ) {
        retval =
            texture->save_to_png( filename, m_palettes[ m_active_palette ] );
...
bool MBAACC_Character::save_current_sprite( const char* filename,
                                            int seq_id,
                                            int fr_id ) {
    if ( !m_loaded ) {
        return 0;
