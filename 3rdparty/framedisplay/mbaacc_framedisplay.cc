...
void MBAACC_FrameDisplay::set_frame( int n ) {
    ...
    m_subframe_next =
        m_character_data.get_subframe_length( m_sequence, m_frame );
}
...
void MBAACC_FrameDisplay::set_active_character( int n ) {
    ...
    if ( m_character >= 0 && m_character < mbaacc_ncharacter_info ) {
        if ( !strcmp( mbaacc_character_info[ m_character ].file_name,
                      mbaacc_character_info[ n ].file_name ) ) {
            need_gfx = 0;
        }
    }
...
void MBAACC_FrameDisplay::render_frame_properties( bool detailed,
                                                   int scr_width,
                                                   int scr_height ) {
    if ( !m_initialized ) {
        return;
    ...
    m_character_data.render_frame_properties( detailed, scr_width, scr_height,
                                              m_sequence, m_frame );
}

void MBAACC_FrameDisplay::command( FrameDisplayCommand command,
                                   const void* param ) {
    if ( !m_initialized ) {
        return;
    }
    ...
    switch ( command ) {
        case COMMAND_CHARACTER_NEXT:
            if ( m_character == -1 ) {
                ...
                set_active_character( ( m_character + 1 ) %
                                      ( mbaacc_ncharacter_info ) );
            }
            break;
        case COMMAND_CHARACTER_PREV:
            if ( m_character == -1 ) {
                ...
                set_active_character(
                    ( m_character + ( mbaacc_ncharacter_info )-1 ) %
                    mbaacc_ncharacter_info );
            }
            break;
        case COMMAND_CHARACTER_SET:
            ...
            set_active_character( ( int )( *( unsigned int* )param ) %
                                  ( mbaacc_ncharacter_info ) );
            break;
...
bool MBAACC_FrameDisplay::save_current_sprite( const char* filename ) {
    ...
    return m_character_data.save_current_sprite( filename, m_sequence,
                                                 m_frame );
}
