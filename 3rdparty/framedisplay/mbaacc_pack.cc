...
void decrapt( unsigned char* data,
              unsigned int size,
              unsigned int xorkey,
              unsigned int xormod ) {
    union {
...
bool MBAACC_Pack::open_pack( const char* filename ) {
    ...
    for ( unsigned int i = 0; i < header.folder_count; ++i ) {
        decrapt( folder_index[ i ].filename, 256, header.xor_key,
                 folder_index[ i ].size );
    ...
    for ( unsigned int i = 0; i < header.file_count; ++i ) {
        decrapt( file_index[ i ].filename, 32, header.xor_key,
                 file_index[ i ].size );
    }
...
bool MBAACC_Pack::read_file( const char* filename,
                             char** dest,
                             unsigned int* dsize ) {
    ...
    while ( n < n_end ) {
        if ( !strcasecmp( ( char* )m_file_index[ n ].filename, filename ) ) {
            // Found it. Read it in.
            unsigned char* data =
                new unsigned char[ m_file_index[ n ].size + 3 ];
