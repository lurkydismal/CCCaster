        ...
        for ( int i = 0; i < 4; ++i ) {
            ...
            for ( int j = 0; j < nhitboxes; ++j ) {
                if ( hitboxes[ j ].type == box_type ) {
                    memcpy( &m_rects[ base + n ], &hitboxes[ j ].rect,
                            sizeof( rect_t ) );
...
void Clone::render( const RenderProperties* properties ) {
    ...
    if ( m_rects && m_nrects > 0 ) {
        ...
        if ( properties->display_collision_box && m_nrect_collision > 0 ) {
            render_boxes( BOX_COLLISION, m_rects + n, m_nrect_collision,
        ...
        if ( properties->display_hit_box && m_nrect_hit > 0 ) {
            render_boxes( BOX_HIT, m_rects + n, m_nrect_hit,
                          properties->display_solid_boxes );
        }
        ...
        if ( properties->display_attack_box && m_nrect_attack > 0 ) {
            render_boxes( BOX_ATTACK, m_rects + n, m_nrect_attack,
                          properties->display_solid_boxes );
        }
        ...
        if ( properties->display_clash_box && m_nrect_clash > 0 ) {
            render_boxes( BOX_CLASH, m_rects + n, m_nrect_clash,
                          properties->display_solid_boxes );
        }
...
bool Clone::in_box( int x, int y ) {
    ...
    int count =
        m_nrect_collision + m_nrect_attack + m_nrect_hit + m_nrect_clash;
