...
#define ANT_SET_ROTO_CURSOR( _Num ) \
    g_TwMgr->SetCursor( g_TwMgr->m_RotoCursors[ _Num ] )
...
bool IsCustomType( int _Type ) {
    return ( g_TwMgr && _Type >= TW_TYPE_CUSTOM_BASE &&
...
bool IsEnumType( int _Type ) {
    return ( g_TwMgr && _Type >= TW_TYPE_ENUM_BASE &&
...
CTwVarAtom::~CTwVarAtom() {
    if ( m_Type == TW_TYPE_BOOL8 || m_Type == TW_TYPE_BOOL16 ||
         m_Type == TW_TYPE_BOOL32 || m_Type == TW_TYPE_BOOLCPP ) {
        if ( m_Val.m_Bool.m_FreeTrueString &&
             m_Val.m_Bool.m_TrueString != NULL ) {
            free( m_Val.m_Bool.m_TrueString );
            m_Val.m_Bool.m_TrueString = NULL;
        }
        if ( m_Val.m_Bool.m_FreeFalseString &&
             m_Val.m_Bool.m_FalseString != NULL ) {
            free( m_Val.m_Bool.m_FalseString );
            m_Val.m_Bool.m_FalseString = NULL;
        }
    } else if ( m_Type == TW_TYPE_CDSTDSTRING &&
                m_GetCallback == CTwMgr::CCDStdString::GetCB &&
                m_ClientData != NULL && g_TwMgr != NULL ) {
        // delete corresponding g_TwMgr->m_CDStdStrings element
        const CTwMgr::CCDStdString* CDStdString =
            ( const CTwMgr::CCDStdString* )m_ClientData;
        // if( &(*CDStdString->m_This)==CDStdString )
        //   g_TwMgr->m_CDStdStrings.erase(CDStdString->m_This);
        for ( list< CTwMgr::CCDStdString >::iterator it =
                  g_TwMgr->m_CDStdStrings.begin();
              it != g_TwMgr->m_CDStdStrings.end(); ++it )
            if ( &( *it ) == CDStdString ) {
    ...
    /*
    else if( m_Type==TW_TYPE_ENUM8 || m_Type==TW_TYPE_ENUM16 ||
    m_Type==TW_TYPE_ENUM32 )
    {
...
void CTwVarAtom::ValueToString( string* _Str ) const {
    ...
    char Tmp[ 1024 ];
    if ( m_Type == TW_TYPE_UNDEF || m_Type == TW_TYPE_HELP_ATOM ||
         m_Type == TW_TYPE_HELP_GRP ||
         m_Type == TW_TYPE_BUTTON ) // has no value
    {
        *_Str = "";
        return;
        ...
        *_Str = "SHORTCUTS";
        return;
    } else if ( m_Type == TW_TYPE_SHORTCUT ) // special case for help bar:
                                             // display shortcut
    {
        *_Str = "";
        if ( m_ReadOnly && m_Val.m_Shortcut.m_Incr[ 0 ] == 0 &&
             m_Val.m_Shortcut.m_Decr[ 0 ] == 0 )
            ( *_Str ) = "(read only)";
        else {
            if ( m_Val.m_Shortcut.m_Decr[ 0 ] > 0 )
                TwGetKeyString( _Str, m_Val.m_Shortcut.m_Decr[ 0 ],
                                m_Val.m_Shortcut.m_Decr[ 1 ] );
            if ( m_Val.m_Shortcut.m_Incr[ 0 ] > 0 ) {
                ( *_Str ) += "  ";
                TwGetKeyString( _Str, m_Val.m_Shortcut.m_Incr[ 0 ],
                                m_Val.m_Shortcut.m_Incr[ 1 ] );
            }
        }
        return;
    ...
    switch ( m_Type ) {
        case TW_TYPE_BOOLCPP: {
            ...
            if ( UseGet )
                ...
                Val = *( bool* )m_Ptr;
            if ( Val )
                *_Str = ( m_Val.m_Bool.m_TrueString != NULL )
                            ? m_Val.m_Bool.m_TrueString
                            : "1";
            else
                *_Str = ( m_Val.m_Bool.m_FalseString != NULL )
                            ? m_Val.m_Bool.m_FalseString
                            : "0";
        } break;
        case TW_TYPE_BOOL8: {
            char Val = 0;
        ...
        case TW_TYPE_ENUM32:
            {
                ...
                if( m_Val.m_Enum.m_Entries!=NULL )
                {
                    UVal::CEnumVal::CEntries::iterator It =
        m_Val.m_Enum.m_Entries->find(d); if( It!=m_Val.m_Enum.m_Entries->end() )
                    {
                        *_Str = It->second;
                        Found = true;
                    }
        ...
        */
        default:
            if ( IsEnumType( m_Type ) ) {
                ...
                CTwMgr::CEnum& e =
                    g_TwMgr->m_Enums[ m_Type - TW_TYPE_ENUM_BASE ];
                ...
                if ( Val != NULL )
                    ...
                    *_Str = "";
            } else if ( m_Type == TW_TYPE_CDSTRING ||
...
void CTwVarAtom::MinMaxStepToDouble( double* _Min,
                                     double* _Max,
                                     double* _Step ) const {
...
const CTwVar* CTwVarAtom::Find( const char* _Name,
                                CTwVarGroup** _Parent,
                                int* _Index ) const {
...
int CTwVar::SetAttrib( int _AttribID,
                       const char* _Value,
                       TwBar* _Bar,
                       struct CTwVarGroup* _VarParent,
                       int _VarIndex ) {
    switch ( _AttribID ) {
        case V_LABEL:
        case V_HELP:
            if ( _Value && strlen( _Value ) > 0 ) {
                /*
                if( IsGroup() && static_cast<CTwVarGroup
                *>(this)->m_StructValuePtr!=NULL )
                {
                    int Idx = static_cast<CTwVarGroup
                *>(this)->m_StructType-TW_TYPE_STRUCT_BASE; if( Idx>=0 &&
                Idx<(int)g_TwMgr->m_Structs.size() ) if( _AttribID==V_LABEL )
                ...
                */
                {
                    ...
                    if ( this == ThisVar && Parent != NULL &&
                         Parent->m_StructValuePtr != NULL ) {
                        int Idx = Parent->m_StructType - TW_TYPE_STRUCT_BASE;
                        if ( Idx >= 0 &&
                             Idx < ( int )g_TwMgr->m_Structs.size() ) {
                            size_t nl = m_Name.length();
                            for ( size_t im = 0;
                                  im <
                                  g_TwMgr->m_Structs[ Idx ].m_Members.size();
                                  ++im ) {
                                size_t ml = g_TwMgr->m_Structs[ Idx ]
                                                .m_Members[ im ]
                                                .m_Name.length();
                                if ( nl >= ml &&
                                     strcmp( g_TwMgr->m_Structs[ Idx ]
                                                 .m_Members[ im ]
                                                 .m_Name.c_str(),
                                             m_Name.c_str() + ( nl - ml ) ) ==
                                         0 ) {
                                    // TODO: would have to be applied to other
                                    // vars already created
                                    if ( _AttribID == V_LABEL ) {
                                        g_TwMgr->m_Structs[ Idx ]
                                            .m_Members[ im ]
                                            .m_Label = _Value;
                                        //                                    m_Label
                                        //                                    =
        ...
        case V_GROUP: {
            ...
            if ( _VarParent != NULL && _VarIndex >= 0 ) {
                _VarParent->m_Vars.erase( _VarParent->m_Vars.begin() +
                                          _VarIndex );
                if ( _VarParent != &( _Bar->m_VarRoot ) &&
                     _VarParent->m_Vars.size() <= 0 )
        ...
        */
        case V_READWRITE: // for backward compatibility
            ...
            return 1;
        case V_ORDER:
            // a special case for compatibility with deprecated command
            // 'option=ogl/dx'
            if ( IsGroup() && _Value != NULL &&
                 static_cast< CTwVarGroup* >( this )->m_SummaryCallback ==
                     CColorExt::SummaryCB &&
                 static_cast< CTwVarGroup* >( this )->m_StructValuePtr !=
                     NULL ) // is tw_type_color?
            {
                if ( _stricmp( _Value, "ogl" ) == 0 ) {
                    static_cast< CColorExt* >(
                        static_cast< CTwVarGroup* >( this )->m_StructValuePtr )
                        ->m_OGL = true;
                    return 1;
        ...
        case V_VISIBLE:
            if ( _Value != NULL && strlen( _Value ) > 0 ) {
                if ( _stricmp( _Value, "true" ) == 0 ||
                    ...
                    return 1;
                } else if ( _stricmp( _Value, "false" ) == 0 ||
                            _stricmp( _Value, "0" ) == 0 ) {
                    if ( m_Visible ) {
                        m_Visible = false;
                        _Bar->NotUpToDate();
        ...
        case V_READONLY:
            if ( _Value == NULL ||
                 strlen( _Value ) ==
                     0 // no value is acceptable (for backward compatibility)
                 || _stricmp( _Value, "true" ) == 0 ||
                 _stricmp( _Value, "1" ) == 0 ) {
                if ( !IsReadOnly() ) {
                    SetReadOnly( true );
                    _Bar->NotUpToDate();
...
ERetType CTwVar::GetAttrib( int _AttribID,
                            TwBar* /*_Bar*/,
                            CTwVarGroup* _VarParent,
                            int /*_VarIndex*/,
...
int CTwVarAtom::HasAttrib( const char* _Attrib, bool* _HasValue ) const {
    *_HasValue = true;
    if ( _stricmp( _Attrib, "keyincr" ) == 0 ||
         _stricmp( _Attrib, "key" ) == 0 )
        return VA_KEY_INCR;
        ...
        return VA_HEXA;
    else if ( _stricmp( _Attrib, "decimal" ) ==
...
int CTwVarAtom::SetAttrib( int _AttribID,
                           const char* _Value,
                           TwBar* _Bar,
                           struct CTwVarGroup* _VarParent,
                           int _VarIndex ) {
    switch ( _AttribID ) {
        ...
        case VA_STEP:
            if ( _Value && strlen( _Value ) > 0 ) {
                ...
                int d = 0;
                unsigned int u = 0;
                int Num = ( _AttribID == VA_STEP )
                              ? 2
                              : ( ( _AttribID == VA_MAX ) ? 1 : 0 );
                switch ( m_Type ) {
                    case TW_TYPE_CHAR:
                ...
                if ( Fmt != NULL && Ptr != NULL &&
                     sscanf( _Value, Fmt, Ptr ) == 1 ) {
                    if ( m_Type == TW_TYPE_CHAR )
                        *( ( &m_Val.m_Char.m_Min ) + Num ) =
                            ( unsigned char )( u );
                    else if ( m_Type == TW_TYPE_INT8 )
                        *( ( &m_Val.m_Int8.m_Min ) + Num ) =
                            ( signed char )( d );
                    else if ( m_Type == TW_TYPE_UINT8 )
                        *( ( &m_Val.m_UInt8.m_Min ) + Num ) =
                            ( unsigned char )( u );

                    // set precision
                    if ( _AttribID == VA_STEP &&
                         ( ( m_Type == TW_TYPE_FLOAT &&
                             m_Val.m_Float32.m_Precision < 0 ) ||
                           ( m_Type == TW_TYPE_DOUBLE &&
                             m_Val.m_Float64.m_Precision < 0 ) ) ) {
                        double Step = fabs( ( m_Type == TW_TYPE_FLOAT )
                                                ? m_Val.m_Float32.m_Step
                                                : m_Val.m_Float64.m_Step );
                        signed char* Precision =
                            ( m_Type == TW_TYPE_FLOAT )
                                ? &m_Val.m_Float32.m_Precision
                                : &m_Val.m_Float64.m_Precision;
        ...
        case VA_PRECISION:
            if ( _Value && strlen( _Value ) > 0 ) {
                int Precision = 0;
                if ( sscanf( _Value, "%d", &Precision ) == 1 &&
        ...
        case VA_DECIMAL: {
            ...
            if ( _AttribID == VA_HEXA ) {
                if ( _Value == NULL ||
                     strlen( _Value ) == 0 // no value is acceptable (for
        ...
        case VA_ENUM:
            if ( _Value && strlen( _Value ) > 0 && IsEnumType( m_Type ) ) {
                ...
                bool Cont;
                g_TwMgr->m_Enums[ m_Type - TW_TYPE_ENUM_BASE ]
                    .m_Entries.clear(); // anyway reset entries
                do {
                    Cont = false;
                    i = 0;
                    char Sep;
                    ...
                    if ( n == 2 && i > 0 &&
                        ...
                        if ( s[ i ] == Sep ) {
                            // if( m_Val.m_Enum.m_Entries==NULL )
                            //   m_Val.m_Enum.m_Entries = new
                            //   UVal::CEnumVal::CEntries;
                            ...
                            ret = g_TwMgr->m_Enums[ m_Type - TW_TYPE_ENUM_BASE ]
                                      .m_Entries.insert( v );
                            if ( !ret.second ) // force overwrite if element
                                               // already exists
                            {
                                g_TwMgr->m_Enums[ m_Type - TW_TYPE_ENUM_BASE ]
                                    .m_Entries.erase( ret.first );
                                g_TwMgr->m_Enums[ m_Type - TW_TYPE_ENUM_BASE ]
                                    .m_Entries.insert( v );
                            }
        ...
        case VA_VALUE:
                                       ...
                                       // ) )
                {
                    if ( m_Type == TW_TYPE_CDSTRING ||
                        ...
                        if ( m_SetCallback != NULL ) {
                            ...
                            if ( g_TwMgr !=
                                 NULL ) // Mgr might have been destroyed by the
                                        // client inside a callback call
                                _Bar->NotUpToDate();
                            return 1;
                        } else if ( m_Type != TW_TYPE_CDSTDSTRING ) {
                            char** StringPtr = ( char** )m_Ptr;
                            if ( StringPtr != NULL &&
                                 g_TwMgr->m_CopyCDStringToClient != NULL ) {
                                g_TwMgr->m_CopyCDStringToClient( StringPtr,
                                                                 _Value );
                                _Bar->NotUpToDate();
                                return 1;
                            }
                        ...
                        if ( n > 0 ) {
                            ...
                            if ( m_SetCallback != NULL ) {
                                ...
                                if ( g_TwMgr !=
                                     NULL ) // Mgr might have been destroyed by
                                            // the client inside a callback call
                                    _Bar->NotUpToDate();
                                return 1;
                            } else if ( m_Ptr != NULL ) {
                                if ( n > 1 )
                                    strncpy( ( char* )m_Ptr, str.c_str(),
                                             n - 1 );
                                ( ( char* )m_Ptr )[ n - 1 ] = '\0';
                                _Bar->NotUpToDate();
                                return 1;
            ...
            return 0;
        default:
            return CTwVar::SetAttrib( _AttribID, _Value, _Bar, _VarParent,
                                      _VarIndex );
    }
}

ERetType CTwVarAtom::GetAttrib( int _AttribID,
                                TwBar* _Bar,
                                CTwVarGroup* _VarParent,
                                int _VarIndex,
                                std::vector< double >& outDoubles,
    ...
    switch ( _AttribID ) {
            ...
            switch ( m_Type ) {
                ...
                case TW_TYPE_UINT16:
                    outDoubles.push_back(
                        *( ( &m_Val.m_UInt16.m_Min ) + num ) );
                    return RET_DOUBLE;
                case TW_TYPE_UINT32:
                    outDoubles.push_back(
                        *( ( &m_Val.m_UInt32.m_Min ) + num ) );
                    return RET_DOUBLE;
                case TW_TYPE_FLOAT:
                    outDoubles.push_back(
                        *( ( &m_Val.m_Float32.m_Min ) + num ) );
                    return RET_DOUBLE;
                case TW_TYPE_DOUBLE:
                    outDoubles.push_back(
                        *( ( &m_Val.m_Float64.m_Min ) + num ) );
                    return RET_DOUBLE;
                default:
                    g_TwMgr->SetLastError( g_ErrInvalidAttrib );
                    return RET_ERROR;
        ...
        case VA_ENUM:
            if ( IsEnumType( m_Type ) ) {
                CTwMgr::CEnum::CEntries::iterator it =
                    g_TwMgr->m_Enums[ m_Type - TW_TYPE_ENUM_BASE ]
                        .m_Entries.begin();
                for ( ; it != g_TwMgr->m_Enums[ m_Type - TW_TYPE_ENUM_BASE ]
                                  .m_Entries.end();
                      ++it ) {
                    if ( it != g_TwMgr->m_Enums[ m_Type - TW_TYPE_ENUM_BASE ]
                                   .m_Entries.begin() )
                        outString << ',';
                    outString << it->first << ' ';
                    if ( it->second.find_first_of( "{}" ) == std::string::npos )
                        outString << '{' << it->second << '}';
                    else if ( it->second.find_first_of( "<>" ) ==
                              std::string::npos )
                        outString << '<' << it->second << '>';
                    else if ( it->second.find_first_of( "()" ) ==
                              std::string::npos )
        ...
        case VA_VALUE:
            ...
            return RET_ERROR;
        default:
            return CTwVar::GetAttrib( _AttribID, _Bar, _VarParent, _VarIndex,
                                      outDoubles, outString );
    }
}
    ...
    switch ( m_Type ) {
        ...
        case TW_TYPE_INT32: {
            ...
            if ( m_Ptr != NULL )
                ...
                m_GetCallback( &v, m_ClientData );
            double dv =
                ( double )_Step * ( double )m_Val.m_Int32.m_Step + ( double )v;
        ...
        case TW_TYPE_UINT32: {
            ...
            if ( m_Ptr != NULL )
                ...
                m_GetCallback( &v, m_ClientData );
            double dv =
                ( double )_Step * ( double )m_Val.m_UInt32.m_Step + ( double )v;
        ...
        case TW_TYPE_ENUM8:
            {
                ...
                {
                    UVal::CEnumVal::CEntries::iterator It =
        m_Val.m_Enum.m_Entries->find(v); if( It==m_Val.m_Enum.m_Entries->end() )
        ...
        default:
            if ( m_Type == TW_TYPE_BUTTON ) {
                if ( m_Val.m_Button.m_Callback != NULL ) {
                    ...
                    if ( g_TwMgr == NULL ) // Mgr might have been destroyed by
                                           // the client inside a callback call
                        return;
                }
...
int CTwVarAtom::DefineEnum(const TwEnumVal *_EnumValues, unsigned int _NbValues)
{
    ...
    if( m_Type!=TW_TYPE_ENUM8 && m_Type!=TW_TYPE_ENUM16 &&
m_Type!=TW_TYPE_ENUM32 )
    {
    ...
    {
        UVal::CEnumVal::CEntries::value_type Entry(_EnumValues[i].Value,
(_EnumValues[i].Label!=NULL)?_EnumValues[i].Label:"");
        pair<UVal::CEnumVal::CEntries::iterator, bool> Result =
m_Val.m_Enum.m_Entries->insert(Entry); if( !Result.second )
...
int CTwVarGroup::HasAttrib( const char* _Attrib, bool* _HasValue ) const {
    *_HasValue = false;
    if ( _stricmp( _Attrib, "open" ) == 0 ) // for backward compatibility
        ...
        *_HasValue = true;
        return VG_VALPTR;
    } else if ( _stricmp( _Attrib, "alpha" ) ==
                0 ) // for backward compatibility
        return VG_ALPHA;
    else if ( _stricmp( _Attrib, "noalpha" ) ==
              0 ) // for backward compatibility
        return VG_NOALPHA;
...
int CTwVarGroup::SetAttrib( int _AttribID,
                            const char* _Value,
                            TwBar* _Bar,
                            struct CTwVarGroup* _VarParent,
                            int _VarIndex ) {
    switch ( _AttribID ) {
        ...
        case VG_TYPEID: {
            ...
            if ( _Value != NULL && sscanf( _Value, "%d", &type ) == 1 ) {
                ...
                if ( idx >= 0 && idx < ( int )g_TwMgr->m_Structs.size() ) {
                    m_SummaryCallback =
                        g_TwMgr->m_Structs[ idx ].m_SummaryCallback;
                    m_SummaryClientData =
                        g_TwMgr->m_Structs[ idx ].m_SummaryClientData;
        ...
        case VG_VALPTR: {
            ...
            if ( _Value != NULL &&
                 sscanf( _Value, "%p", &structValuePtr ) == 1 ) {
        ...
        case VG_ALPHA: // for backward compatibility
            if ( m_SummaryCallback == CColorExt::SummaryCB &&
                 m_StructValuePtr != NULL ) // is tw_type_color?
                if ( static_cast< CColorExt* >( m_StructValuePtr )
                         ->m_CanHaveAlpha ) {
                    static_cast< CColorExt* >( m_StructValuePtr )->m_HasAlpha =
                        true;
                    _Bar->NotUpToDate();
                    return 1;
                }
                     ...
                     m_StructValuePtr != NULL ) // is tw_type_color?
                {
                    if ( _stricmp( _Value, "true" ) == 0 ||
                        ...
                        if ( static_cast< CColorExt* >( m_StructValuePtr )
                            ...
                            if ( !static_cast< CColorExt* >( m_StructValuePtr )
                                      ->m_HasAlpha ) {
                                static_cast< CColorExt* >( m_StructValuePtr )
                                    ->m_HasAlpha = true;
                                _Bar->NotUpToDate();
                            }
                            return 1;
                 ...
                 m_StructValuePtr != NULL ) // is tw_type_color?
            {
                if ( _Value != NULL ) {
                    if ( _stricmp( _Value, "rgba" ) == 0 )
                        static_cast< CColorExt* >( m_StructValuePtr )->m_OGL =
                            true;
                ...
                return 0;
        case VG_ARROW:
            if ( m_SummaryCallback == CQuaternionExt::SummaryCB &&
                 m_StructValuePtr != NULL ) // is tw_type_quat?
            {
                if ( _Value != NULL ) {
                    double* dir =
                        static_cast< CQuaternionExt* >( m_StructValuePtr )
                            ->m_Dir;
                    double x, y, z;
                 ...
                 m_StructValuePtr != NULL ) // is tw_type_quat?
            {
                if ( _Value != NULL ) {
                    ...
                    if ( sscanf( _Value, "%d %d %d", &r, &g, &b ) == 3 )
                        static_cast< CQuaternionExt* >( m_StructValuePtr )
                            ->m_DirColor = Color32FromARGBi( 255, r, g, b );
                    else
                        return 0;
                    return 1;
                }
                 ...
                 m_StructValuePtr != NULL ) // is tw_type_quat?
            {
                if ( _Value != NULL ) {
                    ...
                    if ( _stricmp( _Value, "x" ) == 0 ||
                        ...
                        return 0;
                    int i = ( _AttribID == VG_AXISX )
                                ? 0
                                : ( ( _AttribID == VG_AXISY ) ? 1 : 2 );
                    static_cast< CQuaternionExt* >( m_StructValuePtr )
                        ->m_Permute[ i ][ 0 ] = x;
                    static_cast< CQuaternionExt* >( m_StructValuePtr )
                        ->m_Permute[ i ][ 1 ] = y;
...
ERetType CTwVarGroup::GetAttrib( int _AttribID,
                                 TwBar* _Bar,
                                 struct CTwVarGroup* _VarParent,
                                 int _VarIndex,
                                 std::vector< double >& outDoubles,
    ...
    switch ( _AttribID ) {
        ...
        case VG_ARROW:
            ...
            {
                double* dir =
                    static_cast< CQuaternionExt* >( m_StructValuePtr )->m_Dir;
        ...
        case VG_AXISZ:
                 ...
                 m_StructValuePtr != NULL ) // is tw_type_quat?
            {
                int i = ( _AttribID == VG_AXISX )
                            ? 0
                            : ( ( _AttribID == VG_AXISY ) ? 1 : 2 );
                float x = static_cast< CQuaternionExt* >( m_StructValuePtr )
                              ->m_Permute[ i ][ 0 ];
                float y = static_cast< CQuaternionExt* >( m_StructValuePtr )
                              ->m_Permute[ i ][ 1 ];
                float z = static_cast< CQuaternionExt* >( m_StructValuePtr )
                              ->m_Permute[ i ][ 2 ];
                if ( x > 0 )
                    outString << "+x";
                else if ( x < 0 )
...
const CTwVar* CTwVarGroup::Find( const char* _Name,
                                 CTwVarGroup** _Parent,
                                 int* _Index ) const {
    if ( strcmp( _Name, m_Name.c_str() ) == 0 ) {
        if ( _Parent != NULL )
...
size_t CTwVar::GetDataSize( TwType _Type ) {
    switch ( _Type ) {
        ...
        case TW_TYPE_CDSTRING:
            return sizeof( char* );
        case TW_TYPE_STDSTRING:
            return ( g_TwMgr != 0 ) ? g_TwMgr->m_ClientStdStringStructSize
                                    : sizeof( std::string );
        default:
            if ( g_TwMgr && _Type >= TW_TYPE_STRUCT_BASE &&
                 _Type <
                     TW_TYPE_STRUCT_BASE + ( int )g_TwMgr->m_Structs.size() ) {
                const CTwMgr::CStruct& s =
                    g_TwMgr->m_Structs[ _Type - TW_TYPE_STRUCT_BASE ];
                return s.m_Size;
                /*
                size_t size = 0;
...
CTwBar::CTwBar( const char* _Name ) {
    ...
    if ( g_TwMgr->m_UseOldColorScheme ) {
        ColorHLSToRGBi( g_TwMgr->m_BarInitColorHue % 256, 180, 200, &cr, &cg,
                        &cb );
        m_Color = Color32FromARGBi( 0xf0, cr, cg, cb );
        m_DarkText = true;
    } else {
        ColorHLSToRGBi( g_TwMgr->m_BarInitColorHue % 256, 80, 200, &cr, &cg,
                        &cb );
...
const CTwVar* CTwBar::Find( const char* _Name,
                            CTwVarGroup** _Parent,
                            int* _Index ) const {
...
CTwVar* CTwBar::Find( const char* _Name, CTwVarGroup** _Parent, int* _Index ) {
    return const_cast< CTwVar* >(
        const_cast< const CTwBar* >( this )->Find( _Name, _Parent, _Index ) );
}
...
int CTwBar::HasAttrib( const char* _Attrib, bool* _HasValue ) const {
    ...
    *_HasValue = false;
    if ( _stricmp( _Attrib, "show" ) == 0 ) // for backward compatibility
        ...
        return BAR_HIDE;
    else if ( _stricmp( _Attrib, "iconify" ) ==
              0 ) // for backward compatibility
        return BAR_ICONIFY;
    ...
    switch ( _AttribID ) {
        ...
        case BAR_COLOR:
            if ( _Value && strlen( _Value ) > 0 ) {
                ...
                if ( n == 3 && v0 >= 0 && v0 <= 255 && v1 >= 0 && v1 <= 255 &&
                     v2 >= 0 && v2 <= 255 )
                    c = Color32FromARGBi( alpha, v0, v1, v2 );
                else if ( n == 4 && v0 >= 0 && v0 <= 255 && v1 >= 0 &&
                          v1 <= 255 && v2 >= 0 && v2 <= 255 && v3 >= 0 &&
                          v3 <= 255 )
        ...
        case BAR_RESIZABLE:
            if ( _Value && strlen( _Value ) > 0 ) {
                if ( _stricmp( _Value, "1" ) == 0 ||
                     _stricmp( _Value, "true" ) == 0 ) {
                    m_Resizable = true;
                    return 1;
                } else if ( _stricmp( _Value, "0" ) == 0 ||
                            _stricmp( _Value, "false" ) == 0 ) {
                    m_Resizable = false;
                    return 1;
                } else {
                    g_TwMgr->SetLastError( g_ErrBadValue );
        ...
        case BAR_ALWAYS_TOP:
            if ( _Value && strlen( _Value ) > 0 ) {
                if ( _stricmp( _Value, "1" ) == 0 ||
                    ...
                    if ( g_TwMgr->m_BarAlwaysOnBottom.length() > 0 &&
                         strcmp( g_TwMgr->m_BarAlwaysOnBottom.c_str(),
                                 m_Name.c_str() ) == 0 )
                    ...
                    return 1;
                            ...
                            _stricmp( _Value, "false" ) == 0 ) {
                    if ( g_TwMgr->m_BarAlwaysOnTop.length() > 0 &&
                         strcmp( g_TwMgr->m_BarAlwaysOnTop.c_str(),
                                 m_Name.c_str() ) == 0 )
...
ERetType CTwBar::GetAttrib( int _AttribID,
                            std::vector< double >& outDoubles,
                            std::ostringstream& outString ) const {
    outDoubles.clear();
    outString.clear();
    ...
    switch ( _AttribID ) {
        ...
        case BAR_ICONIFIABLE:
            ...
            return RET_DOUBLE;
        case BAR_FONT_RESIZABLE:
            return g_TwMgr->GetAttrib( MGR_FONT_RESIZABLE, outDoubles,
                                       outString );
        case BAR_ALWAYS_TOP:
        ...
        case BAR_ICONIFIED:
            ...
            return RET_DOUBLE;
        case BAR_COLOR_SCHEME:
            return g_TwMgr->GetAttrib( MGR_COLOR_SCHEME, outDoubles,
                                       outString );
        case BAR_CONTAINED:
...
void CTwBar::UpdateColors() {
    ...
    m_ColHighBg0 = lightText ? Color32FromARGBf( 0.4f, 0.9f, 0.9f, 0.9f )
                             : Color32FromARGBf( 0.4f, 1.0f, 1.0f, 1.0f );
    m_ColHighBg1 = lightText ? Color32FromARGBf( 0.4f, 0.2f, 0.2f, 0.2f )
                             : Color32FromARGBf( 0.1f, 0.7f, 0.7f, 0.7f );
    ...
    m_ColValBg = lightText
                     ? Color32FromARGBf( 0.2f + 0.3f * a, 0.1f, 0.1f, 0.1f )
                     : Color32FromARGBf( 0.2f + 0.3f * a, 1, 1, 1 );
    m_ColStructBg = lightText ? Color32FromARGBf( 0.4f * a, 0, 0, 0 )
                              : Color32FromARGBf( 0.4f * a, 1, 1, 1 );

    m_ColLine = lightText ? Color32FromARGBf( 0.6f, 1, 1, 1 )
                          : Color32FromARGBf( 0.6f, 0.3f, 0.3f, 0.3f );
    m_ColLineShadow = lightText ? Color32FromARGBf( 0.6f, 0, 0, 0 )
                                : Color32FromARGBf( 0.6f, 0, 0, 0 );
    m_ColUnderline = lightText ? 0xffd0d0d0 : 0xff202000;

    m_ColGrpBg = lightText ? Color32FromARGBf( 0.1f + 0.25f * a, 1, 1, 1 )
                           : Color32FromARGBf( 0.1f + 0.05f * a, 0, 0, 0 );
    m_ColGrpText = lightText ? 0xffffff80 : 0xff000000;

    m_ColShortcutText = lightText ? 0xffffb060 : 0xff802000;
    m_ColShortcutBg = lightText
                          ? Color32FromARGBf( 0.4f * a, 0.2f, 0.2f, 0.2f )
                          : Color32FromARGBf( 0.4f * a, 0.8f, 0.8f, 0.8f );
    m_ColInfoText = Color32FromARGBf( 1.0f, 0.5f, 0.5f, 0.5f );

    m_ColRoto = lightText ? Color32FromARGBf( 0.8f, 0.85f, 0.85f, 0.85f )
                          : Color32FromARGBf( 0.8f, 0.1f, 0.1f, 0.1f );
    m_ColRotoVal = Color32FromARGBf( 1, 1.0f, 0.2f, 0.2f );
    m_ColRotoBound = lightText ? Color32FromARGBf( 0.8f, 0.6f, 0.6f, 0.6f )
                               : Color32FromARGBf( 0.8f, 0.3f, 0.3f, 0.3f );
    ...
    m_ColHelpText = lightText ? Color32FromARGBf( 1, 0.2f, 1.0f, 0.2f )
                              : Color32FromARGBf( 1, 0, 0.4f, 0 );
...
void CTwBar::UpdateColors()
{
    ...
    ColorHLSToRGBf(h, lightText ? (min(l+0.2f, 0.3f)) : (max(l-0.2f, 0.6f)), s,
&r, &g, &b); m_ColValBg = Color32FromARGBf(0.4f*a, 0, 0, 0); m_ColStructBg =
Color32FromARGBf(0.4f*a, 0, 0, 0);
...
CTwVarGroup::~CTwVarGroup() {
    for ( vector< CTwVar* >::iterator it = m_Vars.begin(); it != m_Vars.end();
          ++it )
        if ( *it != NULL ) {
            CTwVar* Var = *it;
            delete Var;
...
void CTwBar::BrowseHierarchy( int* _CurrLine,
                              int _CurrLevel,
                              const CTwVar* _Var,
                              int _First,
                              int _Last ) {
    ...
    if ( _Var->IsGroup() ) {
        ...
        if ( Grp->m_Open )
            for ( vector< CTwVar* >::const_iterator it = Grp->m_Vars.begin();
                  it != Grp->m_Vars.end(); ++it )
                if ( ( *it )->m_Visible )
                    BrowseHierarchy( _CurrLine, _CurrLevel + 1, *it, _First,
                                     _Last );
...
void CTwBar::ListLabels( vector< string >& _Labels,
                         vector< color32 >& _Colors,
                         vector< color32 >& _BgColors,
                         bool* _HasBgColors,
                         const CTexFont* _Font,
                         int _AtomWidthMax,
                         int _GroupWidthMax ) {
    const int NbEtc = 2;
    string ValStr;
    ...
    int LevelSpace =
        max( _Font->m_CharHeight - 6, 4 ); // space used by DrawHierHandles

    int nh = ( int )m_HierTags.size();
    for ( int h = 0; h < nh; ++h ) {
        Len = ( int )m_HierTags[ h ].m_Var->m_Label.length();
        if ( Len > 0 )
            Text = ( const unsigned char* )( m_HierTags[ h ]
                                                 .m_Var->m_Label.c_str() );
        else {
            Text = ( const unsigned char* )( m_HierTags[ h ]
                                                 .m_Var->m_Name.c_str() );
        ...
        if ( !m_HierTags[ h ].m_Var->IsGroup() &&
             static_cast< const CTwVarAtom* >( m_HierTags[ h ].m_Var )
                     ->m_Type == TW_TYPE_BUTTON &&
             static_cast< const CTwVarAtom* >( m_HierTags[ h ].m_Var )
                 ->m_ReadOnly &&
             static_cast< const CTwVarAtom* >( m_HierTags[ h ].m_Var )
                     ->m_Val.m_Button.m_Callback != NULL )
            _Colors.push_back(
                m_ColValTextRO ); // special case for read-only buttons
        else
            _Colors.push_back( m_HierTags[ h ].m_Var->m_ColorPtr != NULL
                                   ? *( m_HierTags[ h ].m_Var->m_ColorPtr )
                                   : COLOR32_WHITE );
        color32 bg = m_HierTags[ h ].m_Var->m_BgColorPtr != NULL
                         ? *( m_HierTags[ h ].m_Var->m_BgColorPtr )
                         : 0;
        ...
        bool IsCustom =
            m_HierTags[ h ]
                .m_Var
                ->IsCustom(); // !m_HierTags[h].m_Var->IsGroup() &&
                              // (static_cast<const CTwVarAtom
        ...
        if ( !IsCustom ) {
            ...
            if ( m_HierTags[ h ].m_Var->IsGroup() &&
                 static_cast< const CTwVarGroup* >( m_HierTags[ h ].m_Var )
                         ->m_SummaryCallback == NULL )
                WidthMax = _GroupWidthMax;
            else if ( !m_HierTags[ h ].m_Var->IsGroup() &&
                      static_cast< const CTwVarAtom* >( m_HierTags[ h ].m_Var )
                              ->m_Type == TW_TYPE_BUTTON ) {
                if ( static_cast< const CTwVarAtom* >( m_HierTags[ h ].m_Var )
                         ->m_Val.m_Button.m_Callback == NULL )
                    WidthMax = _GroupWidthMax;
                else if ( m_ButtonAlign == BUTTON_ALIGN_RIGHT )
                    WidthMax = _GroupWidthMax -
                               2 * IncrBtnWidth( m_Font->m_CharHeight );
                else
            ...
            //   WidthMax = _AtomWidthMax - IncrBtnWidth(m_Font->m_CharHeight);
                ...
                WidthMax = _AtomWidthMax;
            if ( Space > 0 )
                for ( s = 0; s < m_HierTags[ h ].m_Level * LevelSpace;
                      s += Space ) {
                    CurrentLabel += ' ';
                    x += Space;
                }
            if ( x + ( NbEtc + 2 ) * _Font->m_CharWidth[ ( int )'.' ] <
                     WidthMax ||
                 m_HierTags[ h ].m_Var->m_DontClip )
                for ( i = 0; i < Len; ++i ) {
                    ch = ( Etc == 0 ) ? Text[ i ] : '.';
                    CurrentLabel += ch;
...
void CTwBar::ListValues( vector< string >& _Values,
                         vector< color32 >& _Colors,
                         vector< color32 >& _BgColors,
                         const CTexFont* _Font,
                         int _WidthMax ) {
    ...
    size_t SummaryMaxLength =
        max( _WidthMax / _Font->m_CharWidth[ ( int )'I' ], 4 );
            ...
            if ( !m_HierTags[ h ].m_Var->IsGroup() ) {
                ...
                if ( !m_IsHelpBar ||
                     ( Atom->m_Type == TW_TYPE_SHORTCUT &&
                       ( Atom->m_Val.m_Shortcut.m_Incr[ 0 ] > 0 ||
                         Atom->m_Val.m_Shortcut.m_Decr[ 0 ] > 0 ) ) )
                ...
                if ( Atom->m_Type == TW_TYPE_BOOLCPP ||
                     Atom->m_Type == TW_TYPE_BOOL8 ||
                     Atom->m_Type == TW_TYPE_BOOL16 ||
                     Atom->m_Type == TW_TYPE_BOOL32 ) {
                    if ( ValStr == "1" )
                        ValStr = "\x7f"; // check sign
                    else if ( ValStr == "0" )
                        ValStr = " -"; //"\x97"; // uncheck sign
                }
                if ( ( Atom->m_Type == TW_TYPE_CDSTRING &&
                       Atom->m_SetCallback == NULL &&
                       g_TwMgr->m_CopyCDStringToClient == NULL ) ||
                     ( Atom->m_Type == TW_TYPE_CDSTDSTRING &&
                       Atom->m_SetCallback == NULL ) ||
                     ( Atom->m_Type == TW_TYPE_STDSTRING &&
                       Atom->m_SetCallback == NULL &&
                ...
                AcceptEdit = EditInPlaceAcceptVar( Atom ) ||
                        ...
                        static_cast< const CTwVarGroup* >(
                            m_HierTags[ h ].m_Var )
                                ->m_SummaryCallback != NULL ) {
                const CTwVarGroup* Grp =
                    static_cast< const CTwVarGroup* >( m_HierTags[ h ].m_Var );
                // force internal value update
                for ( size_t v = 0; v < Grp->m_Vars.size(); v++ )
                    if ( Grp->m_Vars[ v ] != NULL &&
                         !Grp->m_Vars[ v ]->IsGroup() &&
                         Grp->m_Vars[ v ]->m_Visible )
                        static_cast< CTwVarAtom* >( Grp->m_Vars[ v ] )
                            ->ValueToDouble();

                Summary[ 0 ] = '\0';
                if ( Grp->m_SummaryCallback == CTwMgr::CStruct::DefaultSummary )
                    Grp->m_SummaryCallback( &Summary[ 0 ], SummaryMaxLength,
                                            Grp, Grp->m_SummaryClientData );
                else
                    Grp->m_SummaryCallback( &Summary[ 0 ], SummaryMaxLength,
                                            Grp->m_StructValuePtr,
                                            Grp->m_SummaryClientData );
...
static int ClampText( string& _Text, const CTexFont* _Font, int _WidthMax ) {
    ...
    for ( i = 0; i < Len; ++i ) {
        ch = _Text.at( i );
        if ( i < Len - 1 &&
             Width + _Font->m_CharWidth[ ( int )'.' ] >= _WidthMax )
            break;
...
void CTwBar::Update() {
    ...
    int NbLines =
        ( m_VarY1 - m_VarY0 + 1 ) / ( m_Font->m_CharHeight + m_LineSep );
    if ( NbLines <= 0 )
        NbLines = 1;
    if ( !m_IsMinimized ) {
        int LineNum = 0;
        BrowseHierarchy(
            &LineNum, 0, &m_VarRoot, m_FirstLine,
            m_FirstLine + NbLines ); // add a dummy tag at the end to avoid
                                     // wrong 'tag-closing' problems
        ...
        if ( ValuesWidthFit ) {
            ...
            if ( m_ValuesWidth < 2 * m_Font->m_CharHeight )
                m_ValuesWidth =
                    2 * m_Font->m_CharHeight; // enough to draw buttons
            if ( m_ValuesWidth > m_VarX2 - m_VarX0 )
                m_ValuesWidth =
                    max( m_VarX2 - m_VarX0 - m_Font->m_CharHeight, 0 );
            m_VarX1 = m_VarX2 - m_ValuesWidth;
            if ( m_VarX1 < m_VarX0 + 32 )
                m_VarX1 = m_VarX0 + 32;
            if ( m_VarX1 > m_VarX2 )
    ...
    int w = x1 - x0 + 1;
    int h = y1 - y0 - 2 * w;
    int hscr = ( m_NbHierLines > 0 )
                   ? ( ( h * m_NbDisplayedLines ) / m_NbHierLines )
                   : h;
    if ( hscr <= 4 )
        hscr = 4;
    if ( hscr > h )
        hscr = h;
    int yscr =
        ( m_NbHierLines > 0 ) ? ( ( h * m_FirstLine ) / m_NbHierLines ) : 0;
    if ( yscr <= 0 )
        yscr = 0;
    if ( yscr > h - 4 )
        yscr = h - 4;
    ...
    if ( m_Label.size() > 0 )
        ...
        Title = m_Name;
    m_TitleWidth =
        ClampText( Title, m_Font,
                   ( !m_IsMinimized ) ? ( m_Width - 5 * m_Font->m_CharHeight )
                                      : ( 16 * m_Font->m_CharHeight ) );
    ...
    if ( !m_IsMinimized ) {
        ...
        ListLabels( Labels, Colors, BgColors, &HasBgColors, m_Font,
                    m_VarX1 - m_VarX0, m_VarX2 - m_VarX0 );
        assert( Labels.size() == Colors.size() &&
                Labels.size() == BgColors.size() );
        if ( Labels.size() > 0 )
            Gr->BuildText(
                m_LabelsTextObj, &( Labels[ 0 ] ), &( Colors[ 0 ] ),
                &( BgColors[ 0 ] ), ( int )Labels.size(), m_Font, m_LineSep,
                HasBgColors ? m_VarX1 - m_VarX0 - m_Font->m_CharHeight + 2
                            : 0 );
        else
            Gr->BuildText( m_LabelsTextObj, NULL, NULL, NULL, 0, m_Font,
                           m_LineSep, 0 );

        // Should draw click button?
        m_DrawClickBtn =
            ( m_VarX2 - m_VarX1 > 4 * IncrBtnWidth( m_Font->m_CharHeight ) &&
              m_HighlightedLine >= 0 &&
              ...
              ( static_cast< CTwVarAtom* >(
                    ...
                    ->m_Type == TW_TYPE_BUTTON ) );
        //     || static_cast<CTwVarAtom
        //     *>(m_HierTags[m_HighlightedLine].m_Var)->m_Type==TW_TYPE_BOOLCPP
        //     || static_cast<CTwVarAtom
        //     *>(m_HierTags[m_HighlightedLine].m_Var)->m_Type==TW_TYPE_BOOL8
        //     || static_cast<CTwVarAtom
        //     *>(m_HierTags[m_HighlightedLine].m_Var)->m_Type==TW_TYPE_BOOL16
        //     || static_cast<CTwVarAtom
        //     *>(m_HierTags[m_HighlightedLine].m_Var)->m_Type==TW_TYPE_BOOL32
        //     ));
        ...
        m_DrawIncrDecrBtn =
            ( m_VarX2 - m_VarX1 > 4 * IncrBtnWidth( m_Font->m_CharHeight ) &&
              ...
              !static_cast< CTwVarAtom* >(
                   m_HierTags[ m_HighlightedLine ].m_Var )
                   ->m_NoSlider &&
              !( m_EditInPlace.m_Active &&
                 m_EditInPlace.m_Var ==
                     m_HierTags[ m_HighlightedLine ].m_Var ) );

        // Should draw [v] button (list)?
        m_DrawListBtn =
            ( m_VarX2 - m_VarX1 > 2 * IncrBtnWidth( m_Font->m_CharHeight ) &&
              m_HighlightedLine >= 0 &&
              m_HighlightedLine < ( int )m_HierTags.size() &&
              m_HierTags[ m_HighlightedLine ].m_Var != NULL &&
              !m_HierTags[ m_HighlightedLine ].m_Var->IsGroup() &&
              IsEnumType( static_cast< CTwVarAtom* >(
                              m_HierTags[ m_HighlightedLine ].m_Var )
                              ->m_Type ) &&
              !static_cast< CTwVarAtom* >(
                   m_HierTags[ m_HighlightedLine ].m_Var )
        ...
        m_DrawBoolBtn =
            ( m_VarX2 - m_VarX1 > 4 * IncrBtnWidth( m_Font->m_CharHeight ) &&
              ...
              ( static_cast< CTwVarAtom* >(
                ...
                static_cast< CTwVarAtom* >(
                    m_HierTags[ m_HighlightedLine ].m_Var )
                        ->m_Type == TW_TYPE_BOOL8 ||
                static_cast< CTwVarAtom* >(
                    m_HierTags[ m_HighlightedLine ].m_Var )
                        ->m_Type == TW_TYPE_BOOL16 ||
                static_cast< CTwVarAtom* >(
                    m_HierTags[ m_HighlightedLine ].m_Var )
                        ->m_Type == TW_TYPE_BOOL32 ) );

        // Should draw [o] button?
        // m_DrawRotoBtn     = m_DrawIncrDecrBtn;
        /*
        m_DrawRotoBtn     = ( m_HighlightedLine>=0 &&
        m_HighlightedLine<(int)m_HierTags.size()
                              && m_HierTags[m_HighlightedLine].m_Var!=NULL
                              && !m_HierTags[m_HighlightedLine].m_Var->IsGroup()
                              && static_cast<CTwVarAtom
        *>(m_HierTags[m_HighlightedLine].m_Var)->m_Type!=TW_TYPE_BUTTON
                              && !static_cast<CTwVarAtom
        *>(m_HierTags[m_HighlightedLine].m_Var)->m_ReadOnly
                              && !static_cast<CTwVarAtom
        *>(m_HierTags[m_HighlightedLine].m_Var)->m_NoSlider );
        */
        ...
        if ( Values.size() > 0 )
            Gr->BuildText( m_ValuesTextObj, &( Values[ 0 ] ), &( Colors[ 0 ] ),
                           &( BgColors[ 0 ] ), ( int )Values.size(), m_Font,
                           m_LineSep, m_VarX2 - m_VarX1 );
        else
            Gr->BuildText( m_ValuesTextObj, NULL, NULL, NULL, 0, m_Font,
                           m_LineSep, m_VarX2 - m_VarX1 );
        ...
        if ( m_HighlightedLine >= 0 &&
             ...
             m_HierTags[ m_HighlightedLine ].m_Var != NULL &&
             !m_HierTags[ m_HighlightedLine ].m_Var->IsGroup() ) {
            const CTwVarAtom* Atom = static_cast< const CTwVarAtom* >(
                m_HierTags[ m_HighlightedLine ].m_Var );
            if ( Atom->m_KeyIncr[ 0 ] > 0 || Atom->m_KeyDecr[ 0 ] > 0 ) {
                ...
                if ( Atom->m_KeyDecr[ 0 ] > 0 )
                    TwGetKeyString( &Shortcut, Atom->m_KeyDecr[ 0 ],
                                    Atom->m_KeyDecr[ 1 ] );
                if ( Atom->m_KeyIncr[ 0 ] > 0 ) {
                    Shortcut += "  ";
                    TwGetKeyString( &Shortcut, Atom->m_KeyIncr[ 0 ],
                                    Atom->m_KeyIncr[ 1 ] );
                }
        ...
        Gr->BuildText( m_ShortcutTextObj, &Shortcut, NULL, NULL, 1, m_Font, 0,
                       0 );

        // build headers text
        if ( m_HighlightLabelsHeader || m_HighlightValuesHeader ) {
            std::string HeadersText = "Fit column content";
            ClampText( HeadersText, m_Font,
                       m_Width - 3 * m_Font->m_CharHeight );
            Gr->BuildText( m_HeadersTextObj, &HeadersText, NULL, NULL, 1,
                           m_Font, 0, 0 );
        }
    }
...
void CTwBar::DrawHierHandle() {
        ...
        for ( h = 0; h < nh; ++h ) {
            ...
            if ( Grp ) {
                if ( m_ColGrpBg != 0 && Grp->m_StructValuePtr == NULL ) {
                    color32 cb = ( Grp->m_StructType == TW_TYPE_HELP_STRUCT )
                                     ? m_ColStructBg
                                     : m_ColGrpBg;
                    // Gr->DrawRect(x0+dx-1, y0, m_PosX+m_VarX2,
                    // y0+m_Font->m_CharHeight-1, cb);
                    Gr->DrawRect( x2 + dx + 3, y0, m_PosX + m_VarX2,
                                  y0 + m_Font->m_CharHeight - 1 + m_LineSep - 1,
                                  cb );
                }

                if ( m_DrawHandles ) {
                    Gr->DrawLine( dx + x2 + 1, y0 + dh0 + 1, dx + x2 + 1,
                                  y0 + dh1 + 1, m_ColLineShadow );
                    Gr->DrawLine( dx + x0 + 1, y0 + dh1 + 1, dx + x2 + 2,
                                  y0 + dh1 + 1, m_ColLineShadow );
                }

                // Gr->DrawRect(x0+1,y0+dh0+1,x2-1,y0+dh1-1,
                // (h==m_HighlightedLine) ? m_ColHighBtn : m_ColBtn);
                Gr->DrawRect(
                    dx + x0, y0 + dh0, dx + x2, y0 + dh1,
                    ( h == m_HighlightedLine ) ? m_ColHighFold : m_ColFold );
                if ( m_DrawHandles ) {
                    Gr->DrawLine( dx + x0, y0 + dh0, dx + x2, y0 + dh0,
                                  m_ColLine );
                    Gr->DrawLine( dx + x2, y0 + dh0, dx + x2, y0 + dh1 + 1,
                                  m_ColLine );
                    Gr->DrawLine( dx + x2, y0 + dh1, dx + x0, y0 + dh1,
                                  m_ColLine );
                    Gr->DrawLine( dx + x0, y0 + dh1, dx + x0, y0 + dh0,
                                  m_ColLine );
                }

                Gr->DrawLine( dx + x0 + 2, y0 + dh0 + w / 2, dx + x2 - 1,
                              y0 + dh0 + w / 2, m_ColTitleText );
                if ( !Grp->m_Open )
                    Gr->DrawLine( dx + x1, y0 + dh0 + 2, dx + x1, y0 + dh1 - 1,
                                  m_ColTitleText );
                ...
                {
                    color32 cb = (Grp->m_StructType==TW_TYPE_HELP_STRUCT) ?
                m_ColStructBg : m_ColGrpBg;
                    //int decal =
                m_Font->m_CharHeight/2-2+2*m_HierTags[h].m_Level;
                    //if( decal>m_Font->m_CharHeight-3 )
                    //  decal = m_Font->m_CharHeight-3;
                    int margin = dx; //m_Font->m_CharWidth[(int)'
                ']*m_HierTags[h].m_Level;
                    //Gr->DrawRect(m_PosX+m_VarX0+margin, y0+decal,
                m_PosX+m_VarX2, y0+m_Font->m_CharHeight-1, cb);
                    Gr->DrawRect(m_PosX+m_VarX0+margin-1, y0+1, m_PosX+m_VarX2,
                y0+m_Font->m_CharHeight, cb);// m_ColHierBg);
                    //Gr->DrawRect(m_PosX+m_VarX0-4,
                y0+m_Font->m_CharHeight/2-1, m_PosX+m_VarX0+margin-2,
                y0+m_Font->m_CharHeight/2, m_ColHierBg);
                }
                */
            } else if ( static_cast< CTwVarAtom* >( m_HierTags[ h ].m_Var )
                                ->m_Type == TW_TYPE_HELP_GRP &&
                        m_ColHelpBg != 0 )
                Gr->DrawRect(
                    m_PosX + m_VarX0 + m_HierTags[ h ].m_Var->m_LeftMargin,
                    y0 + m_HierTags[ h ].m_Var->m_TopMargin, m_PosX + m_VarX2,
                    y0 + m_Font->m_CharHeight - 1, m_ColHelpBg );
            // else if( static_cast<CTwVarAtom
            // *>(m_HierTags[h].m_Var)->m_Type==TW_TYPE_HELP_HEADER &&
            // m_ColHelpBg!=0 )
            //   Gr->DrawRect(m_PosX+m_VarX0+m_HierTags[h].m_Var->m_LeftMargin,
            //   y0+m_HierTags[h].m_Var->m_TopMargin, m_PosX+m_VarX2,
            //   y0+m_Font->m_CharHeight-1, m_ColHelpBg);
            /*
            else if( static_cast<CTwVarAtom
            *>(m_HierTags[h].m_Var)->m_Type==TW_TYPE_BUTTON && m_ColBtn!=0 )
            {
                // draw button
                int cbx0 = m_PosX+m_VarX2-2*bw+bw/2, cby0 = y0+2, cbx1 =
            m_PosX+m_VarX2-2-bw/2, cby1 = y0+m_Font->m_CharHeight-4; if(
            m_HighlightClickBtn )
                {
    ...
    if ( m_NbDisplayedLines < m_NbHierLines ) {
        ...
        Gr->DrawRect( x0 + 2, y0 + w, x1 - 2, y1 - 1 - w,
                      ( m_ColBg & 0xffffff ) | 0x11000000 );
        if ( m_DrawHandles || m_IsPopupList ) {
            // scroll handle shadow lines
            Gr->DrawLine( x1 - 1, m_ScrollY0 + 1, x1 - 1, m_ScrollY1 + 1,
                          m_ColLineShadow );
            Gr->DrawLine( x0 + 2, m_ScrollY1 + 1, x1, m_ScrollY1 + 1,
                          m_ColLineShadow );

            // up & down arrow
            for ( i = 0; i < ( x1 - x0 - 2 ) / 2; ++i ) {
                Gr->DrawLine( x0 + 2 + i, y0 + w - 2 * i, x1 - i,
                              y0 + w - 2 * i, m_ColLineShadow );
                Gr->DrawLine( x0 + 1 + i, y0 + w - 1 - 2 * i, x1 - 1 - i,
                              y0 + w - 1 - 2 * i,
                              m_HighlightUpScroll
                                  ? ( ( m_ColLine & 0xffffff ) | 0x4f000000 )
                                  : m_ColLine );

                Gr->DrawLine( x0 + 2 + i, y1 - w + 2 + 2 * i, x1 - i,
                              y1 - w + 2 + 2 * i, m_ColLineShadow );
                Gr->DrawLine( x0 + 1 + i, y1 - w + 1 + 2 * i, x1 - 1 - i,
                              y1 - w + 1 + 2 * i,
                              m_HighlightDnScroll
                                  ? ( ( m_ColLine & 0xffffff ) | 0x4f000000 )
                                  : m_ColLine );
            ...
            // middle lines
            Gr->DrawLine( ( x0 + x1 ) / 2 - 1, y0 + w, ( x0 + x1 ) / 2 - 1,
                          m_ScrollY0, m_ColLine );
            Gr->DrawLine( ( x0 + x1 ) / 2, y0 + w, ( x0 + x1 ) / 2, m_ScrollY0,
                          m_ColLine );
            Gr->DrawLine( ( x0 + x1 ) / 2 + 1, y0 + w, ( x0 + x1 ) / 2 + 1,
                          m_ScrollY0, m_ColLineShadow );
            Gr->DrawLine( ( x0 + x1 ) / 2 - 1, m_ScrollY1, ( x0 + x1 ) / 2 - 1,
                          y1 - w + 1, m_ColLine );
            Gr->DrawLine( ( x0 + x1 ) / 2, m_ScrollY1, ( x0 + x1 ) / 2,
                          y1 - w + 1, m_ColLine );
            Gr->DrawLine( ( x0 + x1 ) / 2 + 1, m_ScrollY1, ( x0 + x1 ) / 2 + 1,
                          y1 - w + 1, m_ColLineShadow );
            // scroll handle lines
            Gr->DrawRect( x0 + 2, m_ScrollY0 + 1, x1 - 3, m_ScrollY1 - 1,
                          m_HighlightScroll ? m_ColHighBtn : m_ColBtn );
            ...
            Gr->DrawRect( x0 + 3, m_ScrollY0 + 1, x1 - 3, m_ScrollY1 - 1,
                          m_ColBtn );
    }
    ...
    if ( m_DrawHandles && !m_IsPopupList ) {
        ...
        {
            //   lower-left
            Gr->DrawLine( m_PosX + 3,
                          m_PosY + m_Height - m_Font->m_CharHeight + 3,
                          m_PosX + 3, m_PosY + m_Height - 4, m_ColLine );
            Gr->DrawLine( m_PosX + 4,
                          m_PosY + m_Height - m_Font->m_CharHeight + 4,
                          m_PosX + 4, m_PosY + m_Height - 3, m_ColLineShadow );
            Gr->DrawLine( m_PosX + 3, m_PosY + m_Height - 4,
                          m_PosX + m_Font->m_CharHeight - 4,
                          m_PosY + m_Height - 4, m_ColLine );
            Gr->DrawLine( m_PosX + 4, m_PosY + m_Height - 3,
                          m_PosX + m_Font->m_CharHeight - 3,
                          m_PosY + m_Height - 3, m_ColLineShadow );
            //   lower-right
            Gr->DrawLine( m_PosX + m_Width - 4,
                          m_PosY + m_Height - m_Font->m_CharHeight + 3,
                          m_PosX + m_Width - 4, m_PosY + m_Height - 4,
                          m_ColLine );
            Gr->DrawLine( m_PosX + m_Width - 3,
                          m_PosY + m_Height - m_Font->m_CharHeight + 4,
                          m_PosX + m_Width - 3, m_PosY + m_Height - 3,
                          m_ColLineShadow );
            Gr->DrawLine( m_PosX + m_Width - 4, m_PosY + m_Height - 4,
                          m_PosX + m_Width - m_Font->m_CharHeight + 3,
                          m_PosY + m_Height - 4, m_ColLine );
            Gr->DrawLine( m_PosX + m_Width - 3, m_PosY + m_Height - 3,
                          m_PosX + m_Width - m_Font->m_CharHeight + 4,
                          m_PosY + m_Height - 3, m_ColLineShadow );
            //   upper-left
            Gr->DrawLine( m_PosX + 3, m_PosY + m_Font->m_CharHeight - 4,
                          m_PosX + 3, m_PosY + 3, m_ColLine );
            Gr->DrawLine( m_PosX + 4, m_PosY + m_Font->m_CharHeight - 3,
                          m_PosX + 4, m_PosY + 4, m_ColLineShadow );
            Gr->DrawLine( m_PosX + 3, m_PosY + 3,
                          m_PosX + m_Font->m_CharHeight - 4, m_PosY + 3,
                          m_ColLine );
            Gr->DrawLine( m_PosX + 4, m_PosY + 4,
                          m_PosX + m_Font->m_CharHeight - 3, m_PosY + 4,
                          m_ColLineShadow );
            //   upper-right
            Gr->DrawLine( m_PosX + m_Width - 4, m_PosY + 3,
                          m_PosX + m_Width - m_Font->m_CharHeight + 3,
                          m_PosY + 3, m_ColLine );
            Gr->DrawLine( m_PosX + m_Width - 3, m_PosY + 4,
                          m_PosX + m_Width - m_Font->m_CharHeight + 4,
                          m_PosY + 4, m_ColLineShadow );
            Gr->DrawLine( m_PosX + m_Width - 4,
                          m_PosY + m_Font->m_CharHeight - 4,
                          m_PosX + m_Width - 4, m_PosY + 3, m_ColLine );
            Gr->DrawLine( m_PosX + m_Width - 3,
                          m_PosY + m_Font->m_CharHeight - 3,
                          m_PosX + m_Width - 3, m_PosY + 4, m_ColLineShadow );
        }

        int xm = m_PosX + m_Width - 2 * m_Font->m_CharHeight,
            wm = m_Font->m_CharHeight - 6;
        wm = ( wm < 6 ) ? 6 : wm;
        if ( m_Iconifiable ) // Draw minimize button
        {
            Gr->DrawRect( xm + 1, m_PosY + 4, xm + wm - 1, m_PosY + 3 + wm,
                          m_HighlightMinimize
                              ? m_ColHighBtn
                              : ( ( m_ColBtn & 0xffffff ) | 0x4f000000 ) );
            Gr->DrawLine( xm, m_PosY + 3, xm + wm, m_PosY + 3, m_ColLine );
            Gr->DrawLine( xm + wm, m_PosY + 3, xm + wm, m_PosY + 3 + wm,
                          m_ColLine );
            Gr->DrawLine( xm + wm, m_PosY + 3 + wm, xm, m_PosY + 3 + wm,
                          m_ColLine );
            Gr->DrawLine( xm, m_PosY + 3 + wm, xm, m_PosY + 3, m_ColLine );
            Gr->DrawLine( xm + wm + 1, m_PosY + 4, xm + wm + 1, m_PosY + 4 + wm,
                          m_ColLineShadow );
            Gr->DrawLine( xm + wm + 1, m_PosY + 4 + wm, xm, m_PosY + 4 + wm,
                          m_ColLineShadow );
            Gr->DrawLine( xm + wm / 3 + ( ( wm < 9 ) ? 1 : 0 ) - 1,
                          m_PosY + 4 + wm / 3 - ( ( wm < 9 ) ? 0 : 1 ),
                          xm + wm / 2, m_PosY + 2 + wm - 1, m_ColTitleText,
                          true );
            Gr->DrawLine( xm + wm - wm / 3 + ( ( wm < 9 ) ? 0 : 1 ),
                          m_PosY + 4 + wm / 3 - ( ( wm < 9 ) ? 0 : 1 ),
                          xm + wm / 2, m_PosY + 2 + wm - 1, m_ColTitleText,
                          true );
        }
        ...
        if ( g_TwMgr->m_FontResizable ) // Draw font button
        {
            ...
            Gr->DrawLine( xm + wm + 1, m_PosY + 4 + wm, xm, m_PosY + 4 + wm,
                          m_ColLineShadow );
            Gr->DrawLine( xm + wm / 2 - wm / 6, m_PosY + 3 + wm / 3,
                          xm + wm / 2 + wm / 6 + 1, m_PosY + 3 + wm / 3,
                          m_ColTitleText );
            Gr->DrawLine( xm + wm / 2 - wm / 6, m_PosY + 3 + wm / 3,
                          xm + wm / 2 - wm / 6,
                          m_PosY + 4 + wm - wm / 3 + ( wm > 11 ? 1 : 0 ),
                          m_ColTitleText );
            Gr->DrawLine(
                xm + wm / 2 - wm / 6, m_PosY + 3 + wm / 2 + ( wm > 11 ? 1 : 0 ),
                xm + wm / 2 + wm / 6, m_PosY + 3 + wm / 2 + ( wm > 11 ? 1 : 0 ),
                m_ColTitleText );
        }
    }
...
void CTwBar::Draw( int _DrawPart ) {
    ...
    if ( !m_IsMinimized ) {
        int y = m_PosY + 1;
        int LevelSpace =
            max( m_Font->m_CharHeight - 6, 4 ); // space used by DrawHierHandles
        ...
        if ( !m_IsPopupList ) {
            ...
            if ( _DrawPart & DRAW_BG ) {
                // Gr->DrawRect(m_PosX, m_PosY, m_PosX+m_Width-1,
                // m_PosY+m_Font->m_CharHeight+1,
                // (m_HighlightTitle||m_MouseDragTitle) ? m_ColTitleHighBg :
                // (m_DrawHandles ? m_ColTitleBg : m_ColTitleUnactiveBg));
                if ( m_HighlightTitle || m_MouseDragTitle )
                    ...
                    Gr->DrawRect( m_PosX, m_PosY, m_PosX + m_Width - 1,
                                  m_PosY + m_Font->m_CharHeight + 1,
                                  m_ColTitleBg, m_ColTitleBg, colBg2, colBg1 );
                else
                    Gr->DrawRect( m_PosX, m_PosY, m_PosX + m_Width - 1,
                                  m_PosY + m_Font->m_CharHeight + 1,
                                  m_ColTitleBg, m_ColTitleBg, colBg2, colBg1 );
            ...
            if ( _DrawPart & DRAW_CONTENT ) {
                ...
                Gr->DrawRect( m_PosX, m_PosY, m_PosX + m_Width - 1, y, COL0,
                              COL0, COL1, COL1 );
                if ( m_ColTitleShadow != 0 )
                    Gr->DrawText( m_TitleTextObj,
                                  m_PosX + ( m_Width - m_TitleWidth ) / 2 + 1,
                                  m_PosY + 1, m_ColTitleShadow, 0 );
                Gr->DrawText( m_TitleTextObj,
                              m_PosX + ( m_Width - m_TitleWidth ) / 2, m_PosY,
                              m_ColTitleText, 0 );
            }
            y = m_PosY + m_Font->m_CharHeight + 1;
            if ( _DrawPart & DRAW_CONTENT && m_DrawHandles )
                Gr->DrawLine( m_PosX, y, m_PosX + m_Width - 1, y,
                              0x30ffffff ); // 0x80afafaf);
            y++;
            PERF( DT = Timer.GetTime();
                  printf( "Title=%.4fms ", 1000.0 * DT ); )
        }

        // Draw background
        PERF( Timer.Reset(); )
        if ( _DrawPart & DRAW_BG ) {
            Gr->DrawRect( m_PosX, y, m_PosX + m_Width - 1,
                          m_PosY + m_Height - 1, colBg2, colBg1, colBg1,
                          colBg );
            // Gr->DrawRect(m_PosX, y, m_PosX+m_VarX0-5, m_PosY+m_Height-1,
            // m_ColHierBg);
            Gr->DrawRect( m_PosX + m_VarX2 + 3, y, m_PosX + m_Width - 1,
                          m_PosY + m_Height - 1, m_ColHierBg );
        }
        ...
        if ( _DrawPart & DRAW_CONTENT ) {
                                                   ...
                                                   // )
            {
                int y0 =
                    m_PosY + m_VarY0 +
                    m_HighlightedLine * ( m_Font->m_CharHeight + m_LineSep );
                Gr->DrawRect(
                    m_PosX + LevelSpace + 6 +
                        LevelSpace * m_HierTags[ m_HighlightedLine ].m_Level,
                    y0 + 1, m_PosX + m_VarX2,
                    y0 + m_Font->m_CharHeight - 1 + m_LineSep - 1, m_ColHighBg0,
                    m_ColHighBg0, m_ColHighBg1, m_ColHighBg1 );
                int eps = ( g_TwMgr->m_GraphAPI == TW_OPENGL ||
                ...
                if ( !m_EditInPlace.m_Active )
                    Gr->DrawLine(
                        m_PosX + LevelSpace + 6 +
                            LevelSpace *
                                m_HierTags[ m_HighlightedLine ].m_Level,
                        y0 + m_Font->m_CharHeight + m_LineSep - 1 + eps,
                        m_PosX + m_VarX2,
                        y0 + m_Font->m_CharHeight + m_LineSep - 1 + eps,
                        m_ColUnderline );
                ...
                CTwVarAtom* Atom = static_cast< CTwVarAtom* >(
                    m_HierTags[ m_HighlightedLine ].m_Var );
                if (
                    !Atom->IsCustom() // !(Atom->m_Type>=TW_TYPE_CUSTOM_BASE &&
                                      // Atom->m_Type<TW_TYPE_CUSTOM_BASE+(int)g_TwMgr->m_Customs.size())
                    && !( Atom->m_Type == TW_TYPE_BUTTON &&
                          Atom->m_Val.m_Button.m_Callback == NULL ) )
                    Gr->DrawRect(
                        m_PosX + LevelSpace + 6 +
                            LevelSpace *
                                m_HierTags[ m_HighlightedLine ].m_Level,
                        y0 + 1, m_PosX + m_VarX2,
                        y0 + m_Font->m_CharHeight - 1 + m_LineSep - 1, col );
                else
                    Gr->DrawRect(
                        m_PosX + LevelSpace + 6 +
                            LevelSpace *
            ...
            Gr->DrawLine( m_PosX + m_Width, m_PosY, m_PosX + m_Width,
                          m_PosY + m_Height, clight );
            Gr->DrawLine( m_PosX, m_PosY + m_Height, m_PosX + m_Width,
                          m_PosY + m_Height, clight );
            int dshad = 3; // bar shadows
            color32 cshad = ( ( ( m_Color >> 24 ) / 2 ) << 24 ) & 0xFF000000;
            Gr->DrawRect( m_PosX, m_PosY + m_Height, m_PosX + dshad,
                          m_PosY + m_Height + dshad, 0, cshad, 0, 0 );
            Gr->DrawRect( m_PosX + dshad + 1, m_PosY + m_Height,
                          m_PosX + m_Width - 1, m_PosY + m_Height + dshad,
                          cshad, cshad, 0, 0 );
            Gr->DrawRect( m_PosX + m_Width, m_PosY + m_Height,
                          m_PosX + m_Width + dshad, m_PosY + m_Height + dshad,
                          cshad, 0, 0, 0 );
            Gr->DrawRect( m_PosX + m_Width, m_PosY, m_PosX + m_Width + dshad,
                          m_PosY + dshad, 0, 0, cshad, 0 );
            Gr->DrawRect( m_PosX + m_Width, m_PosY + dshad + 1,
                          m_PosX + m_Width + dshad, m_PosY + m_Height - 1,
                          cshad, 0, cshad, 0 );
            ...
            DrawHierHandle();
            PERF( DT = Timer.GetTime();
                  printf( "Handles=%.4fms ", 1000.0 * DT ); )

            // Draw labels
            PERF( Timer.Reset(); )
            Gr->DrawText( m_LabelsTextObj, m_PosX + LevelSpace + 6,
                          m_PosY + m_VarY0, 0 /*m_ColLabelText*/, 0 );
            PERF( DT = Timer.GetTime();
                  printf( "Labels=%.4fms ", 1000.0 * DT ); )

            // Draw values
            if ( !m_IsPopupList ) {
                PERF( Timer.Reset(); )
                Gr->DrawText( m_ValuesTextObj, m_PosX + m_VarX1,
                              m_PosY + m_VarY0, 0 /*m_ColValText*/,
                              0 /*m_ColValBg*/ );
                PERF( DT = Timer.GetTime();
                      printf( "Values=%.4fms ", 1000.0 * DT ); )
            }
            ...
            for ( h = 0; h < nh; ++h ) {
                if ( m_HierTags[ h ].m_Var->IsGroup() ) {
                    const CTwVarGroup* Grp = static_cast< const CTwVarGroup* >(
                        m_HierTags[ h ].m_Var );
                    if ( Grp->m_SummaryCallback == CColorExt::SummaryCB &&
                         Grp->m_StructValuePtr != NULL ) {
                        // draw color value
                        if ( Grp->m_Vars.size() > 0 &&
                             Grp->m_Vars[ 0 ] != NULL &&
                             !Grp->m_Vars[ 0 ]->IsGroup() )
                            static_cast< CTwVarAtom* >( Grp->m_Vars[ 0 ] )
                                ->ValueToDouble(); // force ext update
                        int ydecal = ( g_TwMgr->m_GraphAPI == TW_OPENGL ||
                                       g_TwMgr->m_GraphAPI == TW_OPENGL_CORE )
                                         ? 1
                                         : 0;
                        const int checker = 8;
                        for ( int c = 0; c < checker; ++c )
                            Gr->DrawRect(
                                m_PosX + m_VarX1 +
                                    ( c * ( m_VarX2 - m_VarX1 ) ) / checker,
                                yh + 1 + ydecal +
                                    ( ( c % 2 ) *
                                      ( m_Font->m_CharHeight - 2 ) ) /
                                        2,
                                m_PosX + m_VarX1 - 1 +
                                    ( ( c + 1 ) * ( m_VarX2 - m_VarX1 ) ) /
                                        checker,
                                yh + ydecal +
                                    ( ( ( c % 2 ) + 1 ) *
                                      ( m_Font->m_CharHeight - 2 ) ) /
                                        2,
                                0xffffffff );
                        Gr->DrawRect( m_PosX + m_VarX1, yh + 1 + ydecal,
                                      m_PosX + m_VarX2 - 1,
                                      yh + ydecal + m_Font->m_CharHeight - 2,
                                      0xbfffffff );
                        const CColorExt* colExt =
                            static_cast< const CColorExt* >(
                                Grp->m_StructValuePtr );
                        color32 col = Color32FromARGBi(
                            ( colExt->m_HasAlpha ? colExt->A : 255 ), colExt->R,
                            colExt->G, colExt->B );
                        if ( col != 0 )
                            Gr->DrawRect(
                                m_PosX + m_VarX1, yh + 1 + ydecal,
                                m_PosX + m_VarX2 - 1,
                                yh + ydecal + m_Font->m_CharHeight - 2, col );
                        /*
                        Gr->DrawLine(m_PosX+m_VarX1-1, yh, m_PosX+m_VarX2+1, yh,
                        0xff000000); Gr->DrawLine(m_PosX+m_VarX1-1,
                        yh+m_Font->m_CharHeight, m_PosX+m_VarX2+1,
                        yh+m_Font->m_CharHeight, 0xff000000);
                        Gr->DrawLine(m_PosX+m_VarX1-1, yh, m_PosX+m_VarX1-1,
                        yh+m_Font->m_CharHeight, 0xff000000);
                        Gr->DrawLine(m_PosX+m_VarX2, yh, m_PosX+m_VarX2,
                        yh+m_Font->m_CharHeight, 0xff000000);
                        */
                    }
                    // else if( Grp->m_SummaryCallback==CustomTypeSummaryCB &&
                    // Grp->m_StructValuePtr!=NULL )
                    //{
                    // }
                } else if ( static_cast< CTwVarAtom* >( m_HierTags[ h ].m_Var )
                    ...
                    if ( !static_cast< CTwVarAtom* >( m_HierTags[ h ].m_Var )
                              ->m_ReadOnly ) {
                        double BtnAutoDelta = g_TwMgr->m_Timer.GetTime() -
                                              m_HighlightClickBtnAuto;
                        if ( ( m_HighlightClickBtn ||
                               ( BtnAutoDelta >= 0 && BtnAutoDelta < 0.1 ) ) &&
                             h == m_HighlightedLine ) {
                            cbx0--;
                            cby0--;
                            cbx1--;
                            cby1--;
                            Gr->DrawRect( cbx0 + 2, cby0 + 2, cbx1 + 2,
                                          cby1 + 2, m_ColHighBtn );
                            Gr->DrawLine( cbx0 + 3, cby1 + 3, cbx1 + 4,
                                          cby1 + 3, 0xAF000000 );
                            Gr->DrawLine( cbx1 + 3, cby0 + 3, cbx1 + 3,
                                          cby1 + 3, 0xAF000000 );
                            Gr->DrawLine( cbx0 + 2, cby0 + 2, cbx0 + 2,
                                          cby1 + 2, m_ColLine );
                            Gr->DrawLine( cbx0 + 2, cby1 + 2, cbx1 + 2,
                                          cby1 + 2, m_ColLine );
                            Gr->DrawLine( cbx1 + 2, cby1 + 2, cbx1 + 2,
                                          cby0 + 2, m_ColLine );
                            Gr->DrawLine( cbx1 + 2, cby0 + 2, cbx0 + 2,
                                          cby0 + 2, m_ColLine );
                        } else {
                            Gr->DrawRect(
                                cbx0 + 2, cby1 + 1, cbx1 + 2, cby1 + 2,
                                ( h == m_HighlightedLine ) ? 0xAF000000
                                                           : 0x7F000000 );
                            Gr->DrawRect( cbx1 + 1, cby0 + 2, cbx1 + 2, cby1,
                                          ( h == m_HighlightedLine )
                                              ? 0xAF000000
                                              : 0x7F000000 );
                            Gr->DrawRect( cbx0, cby0, cbx1, cby1,
                                          ( h == m_HighlightedLine )
                                              ? m_ColHighBtn
                                              : m_ColBtn );
                            ...
                            Gr->DrawLine( cbx1, cby0, cbx0, cby0, m_ColLine );
                                    ...
                                    m_HierTags[ h ].m_Var )
                                    ->m_Val.m_Button.m_Callback != NULL ) {
                        Gr->DrawRect( cbx0 + 1, cby0 + 1, cbx1 + 1, cby1 + 1,
                                      m_ColBtn );
                    } else if ( static_cast< CTwVarAtom* >(
                                    m_HierTags[ h ].m_Var )
                                    ->m_Val.m_Button.m_Separator == 1 ) {
                        int LevelSpace =
                            max( m_Font->m_CharHeight - 6,
                                 4 ); // space used by DrawHierHandles
                        Gr->DrawLine(
                            m_PosX + m_VarX0 +
                                m_HierTags[ h ].m_Level * LevelSpace,
                ...
                { // record custom types
                    CTwMgr::CMemberProxy* mProxy =
                        static_cast< CTwVarAtom* >( m_HierTags[ h ].m_Var )
                            ->m_Val.m_Custom.m_MemberProxy;
                    if ( mProxy != NULL && mProxy->m_StructProxy != NULL ) {
                        CustomMap::iterator it =
                            m_CustomRecords.find( mProxy->m_StructProxy );
                        int xMin = m_PosX + m_VarX0 +
                                   m_HierTags[ h ].m_Level * LevelSpace;
                        int xMax = m_PosX + m_VarX2 - 2;
                        int yMin = yh + 1;
                        int yMax = yh + m_Font->m_CharHeight;
                        if ( it == m_CustomRecords.end() ) {
                            std::pair< CTwMgr::CStructProxy*, CCustomRecord >
                                pr;
                            pr.first = mProxy->m_StructProxy;
                            pr.second.m_IndexMin = pr.second.m_IndexMax =
                                mProxy->m_MemberIndex;
                            pr.second.m_XMin = xMin;
                            pr.second.m_XMax = xMax;
                            pr.second.m_YMin = yMin;
                            pr.second.m_YMax = yMax;
                            pr.second.m_Y0 =
                                0; // will be filled by the draw loop below
                            pr.second.m_Y1 =
                                0; // will be filled by the draw loop below
                            pr.second.m_Var = mProxy->m_VarParent;
                            m_CustomRecords.insert( pr );
                        } else {
                            it->second.m_IndexMin = min(
                                it->second.m_IndexMin, mProxy->m_MemberIndex );
                            it->second.m_IndexMax = min(
                                it->second.m_IndexMax, mProxy->m_MemberIndex );
            ...
            // Draw custom types
            for ( CustomMap::iterator it = m_CustomRecords.begin();
                  it != m_CustomRecords.end(); ++it ) {
                ...
                if ( sProxy->m_CustomDrawCallback != NULL ) {
                    int y0 =
                        r.m_YMin -
                        max( r.m_IndexMin - sProxy->m_CustomIndexFirst, 0 ) *
                            ( m_Font->m_CharHeight + m_LineSep );
                    int y1 = y0 +
                             max( sProxy->m_CustomIndexLast -
                    ...
                    if ( y0 < y1 ) {
                        ...
                        r.m_Y1 = y1;
                        Gr->ChangeViewport(
                            r.m_XMin, r.m_YMin, r.m_XMax - r.m_XMin + 1,
                            r.m_YMax - r.m_YMin + 1, 0, y0 - r.m_YMin + 1 );
                        sProxy->m_CustomDrawCallback(
                            r.m_XMax - r.m_XMin, y1 - y0,
                            sProxy->m_StructExtData, sProxy->m_StructClientData,
                            this, r.m_Var );
                        Gr->RestoreViewport();
                    }
                }
            ...
            if ( m_DrawHandles && !m_IsPopupList ) {
                ...
                if ( ( m_DrawIncrDecrBtn || m_DrawClickBtn || m_DrawListBtn ||
                       m_DrawBoolBtn || m_DrawRotoBtn ) &&
                     m_HighlightedLine >= 0 &&
                     m_HighlightedLine < ( int )m_HierTags.size() ) {
                    int y0 = m_PosY + m_VarY0 +
                             m_HighlightedLine *
                    ...
                    if ( m_DrawIncrDecrBtn ) {
                        bool IsMin = false;
                        bool IsMax = false;
                        if ( !m_HierTags[ m_HighlightedLine ]
                                  .m_Var->IsGroup() ) {
                            const CTwVarAtom* Atom =
                                static_cast< const CTwVarAtom* >(
                                    m_HierTags[ m_HighlightedLine ].m_Var );
                            double v, vmin, vmax;
                            v = Atom->ValueToDouble();
                        ...
                        /*
                        Gr->DrawRect(m_PosX+m_VarX2-2*bw+1, y0+1,
                        m_PosX+m_VarX2-bw-1, y0+m_Font->m_CharHeight-2,
                        (m_HighlightDecrBtn && !IsMin)?m_ColHighBtn:m_ColBtn);
                        Gr->DrawRect(m_PosX+m_VarX2-bw+1, y0+1,
                        m_PosX+m_VarX2-1, y0+m_Font->m_CharHeight-2,
                        (m_HighlightIncrBtn && !IsMax)?m_ColHighBtn:m_ColBtn);
                        // [-]
                        Gr->DrawLine(m_PosX+m_VarX2-2*bw+3+(bw>8?1:0),
                        y0+m_Font->m_CharHeight/2,
                        m_PosX+m_VarX2-bw-2-(bw>8?1:0),
                        y0+m_Font->m_CharHeight/2,
                        IsMin?m_ColValTextRO:m_ColTitleText);
                        // [+]
                        Gr->DrawLine(m_PosX+m_VarX2-bw+3,
                        y0+m_Font->m_CharHeight/2, m_PosX+m_VarX2-2,
                        y0+m_Font->m_CharHeight/2,
                        IsMax?m_ColValTextRO:m_ColTitleText);
                        Gr->DrawLine(m_PosX+m_VarX2-bw/2,
                        y0+m_Font->m_CharHeight/2-bw/2+2, m_PosX+m_VarX2-bw/2,
                        y0+m_Font->m_CharHeight/2+bw/2-1,
                        IsMax?m_ColValTextRO:m_ColTitleText);
                        */
                        Gr->DrawRect( m_PosX + m_VarX2 - 2 * bw + 1, y0 + 1,
                                      m_PosX + m_VarX2 - bw - 1,
                                      y0 + m_Font->m_CharHeight - 2,
                                      ( m_HighlightDecrBtn && !IsMin )
                                          ? m_ColHighBtn
                                          : m_ColBtn );
                        Gr->DrawRect(
                            m_PosX + m_VarX2 - bw + 1, y0 + 1,
                            m_PosX + m_VarX2 - 1, y0 + m_Font->m_CharHeight - 2,
                            ( m_HighlightIncrBtn && !IsMax ) ? m_ColHighBtn
                                                             : m_ColBtn );
                        // [-]
                        Gr->DrawLine(
                            m_PosX + m_VarX2 - 2 * bw + 3 + ( bw > 8 ? 1 : 0 ),
                            y0 + m_Font->m_CharHeight / 2,
                            m_PosX + m_VarX2 - bw - 2 - ( bw > 8 ? 1 : 0 ),
                            y0 + m_Font->m_CharHeight / 2,
                            IsMin ? m_ColValTextRO : m_ColTitleText );
                        ...
                        Gr->DrawLine(
                            m_PosX + m_VarX2 - bw / 2,
                            y0 + m_Font->m_CharHeight / 2 - bw / 2 + 2,
                            m_PosX + m_VarX2 - bw / 2,
                            y0 + m_Font->m_CharHeight / 2 + bw / 2 - 1,
                            IsMax ? m_ColValTextRO : m_ColTitleText );
                        ...
                        Gr->DrawRect(
                            m_PosX + m_VarX2 - bw + 1, y0 + 1,
                            m_PosX + m_VarX2 - 1, y0 + m_Font->m_CharHeight - 2,
                            m_HighlightListBtn ? m_ColHighBtn : m_ColBtn );
                        Gr->DrawLine( m_PosX + m_VarX2 - bw + 4 + dx,
                                      y0 + m_Font->m_CharHeight / 2 - eps,
                                      m_PosX + m_VarX2 - bw / 2 + 1 + dx,
                                      y0 + m_Font->m_CharHeight - 4,
                                      m_ColTitleText, true );
                        Gr->DrawLine( m_PosX + m_VarX2 - bw / 2 + 1 + dx,
                                      y0 + m_Font->m_CharHeight - 4,
                                      m_PosX + m_VarX2 - 2 + dx,
                                      y0 + m_Font->m_CharHeight / 2 - 1,
                                      m_ColTitleText, true );
                    } else if ( m_DrawBoolBtn ) {
                        Gr->DrawRect(
                            m_PosX + m_VarX2 - bw + 1, y0 + 1,
                            m_PosX + m_VarX2 - 1, y0 + m_Font->m_CharHeight - 2,
                            m_HighlightBoolBtn ? m_ColHighBtn : m_ColBtn );
                        // [x]
                        // Gr->DrawLine(m_PosX+m_VarX2-bw/2-bw/6,
                        // y0+m_Font->m_CharHeight/2-bw/6,
                        // m_PosX+m_VarX2-bw/2+bw/6,
                        // y0+m_Font->m_CharHeight/2+bw/6, m_ColTitleText,
                        // true); Gr->DrawLine(m_PosX+m_VarX2-bw/2-bw/6,
                        // y0+m_Font->m_CharHeight/2+bw/6,
                        // m_PosX+m_VarX2-bw/2+bw/6,
                        // y0+m_Font->m_CharHeight/2-bw/6, m_ColTitleText,
                        // true);
                        ...
                        int s = bw / 4;
                        int eps = 1;
                        Gr->DrawLine( m_PosX + m_VarX2 - bw / 2 - 1,
                                      y0 + m_Font->m_CharHeight / 2 - s,
                                      m_PosX + m_VarX2 - bw / 2 - s - 1,
                                      y0 + m_Font->m_CharHeight / 2,
                                      m_ColTitleText, true );
                        Gr->DrawLine(
                            m_PosX + m_VarX2 - bw / 2 - s - 1,
                            y0 + m_Font->m_CharHeight / 2,
                            m_PosX + m_VarX2 - bw / 2 - eps,
                            y0 + m_Font->m_CharHeight / 2 + s + 1 - eps,
                            m_ColTitleText, true );
                        // Gr->DrawLine(m_PosX+m_VarX2-bw/2+1,
                        // y0+m_Font->m_CharHeight/2+s, m_PosX+m_VarX2-bw/2+s+1,
                        // y0+m_Font->m_CharHeight/2, m_ColTitleText, true);
                        // Gr->DrawLine(m_PosX+m_VarX2-bw/2+s+1,
                        // y0+m_Font->m_CharHeight/2, m_PosX+m_VarX2-bw/2+1,
                        // y0+m_Font->m_CharHeight/2-s, m_ColTitleText, true);
                        Gr->DrawLine( m_PosX + m_VarX2 - bw / 2 + 2,
                                      y0 + m_Font->m_CharHeight / 2 - s,
                                      m_PosX + m_VarX2 - bw / 2 + s + 2,
                                      y0 + m_Font->m_CharHeight / 2,
                                      m_ColTitleText, true );
                        Gr->DrawLine(
                            m_PosX + m_VarX2 - bw / 2 + s + 2,
                            y0 + m_Font->m_CharHeight / 2,
                            m_PosX + m_VarX2 - bw / 2 + 1 + eps,
                            y0 + m_Font->m_CharHeight / 2 + s + 1 - eps,
                            m_ColTitleText, true );
                    }
                    ...
                    if ( m_DrawRotoBtn ) {
                        // [o] rotoslider button
                        /*
                        Gr->DrawRect(m_PosX+m_VarX1-bw-1, y0+1,
                        m_PosX+m_VarX1-3, y0+m_Font->m_CharHeight-2,
                        m_HighlightRotoBtn?m_ColHighBtn:m_ColBtn);
                        Gr->DrawLine(m_PosX+m_VarX1-bw+bw/2-2,
                        y0+m_Font->m_CharHeight/2-1, m_PosX+m_VarX1-bw+bw/2-1,
                        y0+m_Font->m_CharHeight/2-1, m_ColTitleText);
                        Gr->DrawLine(m_PosX+m_VarX1-bw+bw/2-3,
                        y0+m_Font->m_CharHeight/2+0, m_PosX+m_VarX1-bw+bw/2+0,
                        y0+m_Font->m_CharHeight/2+0, m_ColTitleText);
                        Gr->DrawLine(m_PosX+m_VarX1-bw+bw/2-3,
                        y0+m_Font->m_CharHeight/2+1, m_PosX+m_VarX1-bw+bw/2+0,
                        y0+m_Font->m_CharHeight/2+1, m_ColTitleText);
                        Gr->DrawLine(m_PosX+m_VarX1-bw+bw/2-2,
                        y0+m_Font->m_CharHeight/2+2, m_PosX+m_VarX1-bw+bw/2-1,
                        y0+m_Font->m_CharHeight/2+2, m_ColTitleText);
                        */
                        /*
                        Gr->DrawRect(m_PosX+m_VarX2-3*bw+1, y0+1,
                        m_PosX+m_VarX2-2*bw-1, y0+m_Font->m_CharHeight-2,
                        m_HighlightRotoBtn?m_ColHighBtn:m_ColBtn);
                        Gr->DrawLine(m_PosX+m_VarX2-3*bw+bw/2+0,
                        y0+m_Font->m_CharHeight/2-1, m_PosX+m_VarX2-3*bw+bw/2+1,
                        y0+m_Font->m_CharHeight/2-1, m_ColTitleText);
                        Gr->DrawLine(m_PosX+m_VarX2-3*bw+bw/2-1,
                        y0+m_Font->m_CharHeight/2+0, m_PosX+m_VarX2-3*bw+bw/2+2,
                        y0+m_Font->m_CharHeight/2+0, m_ColTitleText);
                        Gr->DrawLine(m_PosX+m_VarX2-3*bw+bw/2-1,
                        y0+m_Font->m_CharHeight/2+1, m_PosX+m_VarX2-3*bw+bw/2+2,
                        y0+m_Font->m_CharHeight/2+1, m_ColTitleText);
                        Gr->DrawLine(m_PosX+m_VarX2-3*bw+bw/2+0,
                        y0+m_Font->m_CharHeight/2+2, m_PosX+m_VarX2-3*bw+bw/2+1,
                        y0+m_Font->m_CharHeight/2+2, m_ColTitleText);
                        */
                        int dy = 0;
                        Gr->DrawRect(
                            ...
                            m_PosX + m_VarX2 - 1, y0 + m_Font->m_CharHeight - 2,
                            m_HighlightRotoBtn ? m_ColHighBtn : m_ColBtn );
                        Gr->DrawLine( m_PosX + m_VarX2 - bw + bw / 2 + 0,
                                      y0 + m_Font->m_CharHeight / 2 - 1 + dy,
                                      m_PosX + m_VarX2 - bw + bw / 2 + 1,
                                      y0 + m_Font->m_CharHeight / 2 - 1 + dy,
                                      m_ColTitleText, true );
                        Gr->DrawLine( m_PosX + m_VarX2 - bw + bw / 2 - 1,
                                      y0 + m_Font->m_CharHeight / 2 + 0 + dy,
                                      m_PosX + m_VarX2 - bw + bw / 2 + 2,
                                      y0 + m_Font->m_CharHeight / 2 + 0 + dy,
                                      m_ColTitleText, true );
                        Gr->DrawLine( m_PosX + m_VarX2 - bw + bw / 2 - 1,
                                      y0 + m_Font->m_CharHeight / 2 + 1 + dy,
                ...
                if ( !m_HighlightValWidth ) {
                    // color32 col = m_DarkText ? COLOR32_WHITE :
                    // m_ColTitleText; Gr->DrawRect(m_PosX+m_VarX1-2,
                    // m_PosY+m_VarY0-8, m_PosX+m_VarX1-1, m_PosY+m_VarY0-4,
                    // col); Gr->DrawLine(m_PosX+m_VarX1-1, m_PosY+m_VarY0-3,
                    // m_PosX+m_VarX1, m_PosY+m_VarY0-3, m_ColLineShadow);
                    // Gr->DrawLine(m_PosX+m_VarX1, m_PosY+m_VarY0-3,
                    // m_PosX+m_VarX1, m_PosY+m_VarY0-8, m_ColLineShadow);
                } else {
                    color32 col = m_DarkText ? COLOR32_WHITE : m_ColTitleText;
                    Gr->DrawRect( m_PosX + m_VarX1 - 2, m_PosY + m_VarY0 - 8,
                                  m_PosX + m_VarX1 - 1, m_PosY + m_VarY1, col );
                    Gr->DrawLine( m_PosX + m_VarX1 - 1, m_PosY + m_VarY1 + 1,
                                  m_PosX + m_VarX1, m_PosY + m_VarY1 + 1,
                                  m_ColLineShadow );
                    Gr->DrawLine( m_PosX + m_VarX1, m_PosY + m_VarY1 + 1,
                                  m_PosX + m_VarX1, m_PosY + m_VarY0 - 8,
                                  m_ColLineShadow );
                }

                // Draw labels & values headers
                if ( m_HighlightLabelsHeader ) {
                    Gr->DrawRect( m_PosX + m_VarX0,
                                  m_PosY + m_Font->m_CharHeight + 2,
                                  m_PosX + m_VarX1 - 4, m_PosY + m_VarY0 - 1,
                                  m_ColHighBg0, m_ColHighBg0, m_ColHighBg1,
                                  m_ColHighBg1 );
                }
                if ( m_HighlightValuesHeader ) {
                    Gr->DrawRect(
                        m_PosX + m_VarX1 + 2, m_PosY + m_Font->m_CharHeight + 2,
                        m_PosX + m_VarX2, m_PosY + m_VarY0 - 1, m_ColHighBg0,
                        m_ColHighBg0, m_ColHighBg1, m_ColHighBg1 );
                }
            }
            ...
            if ( m_HighlightedLine >= 0 &&
                 m_HighlightedLine == m_ShortcutLine && !m_IsPopupList &&
                 !m_EditInPlace.m_Active ) {
                PERF( Timer.Reset(); )
                Gr->DrawRect( m_PosX + m_Font->m_CharHeight - 2,
                              m_PosY + m_VarY1 + 1,
                              m_PosX + m_Width - m_Font->m_CharHeight - 2,
                              m_PosY + m_VarY1 + 1 + m_Font->m_CharHeight,
                              m_ColShortcutBg );
                Gr->DrawText( m_ShortcutTextObj, m_PosX + m_Font->m_CharHeight,
                              m_PosY + m_VarY1 + 1, m_ColShortcutText, 0 );
                PERF( DT = Timer.GetTime();
                      printf( "Shortcut=%.4fms ", 1000.0 * DT ); )
            } else if ( ( m_HighlightLabelsHeader ||
                          m_HighlightValuesHeader ) &&
                        !m_IsPopupList && !m_EditInPlace.m_Active ) {
                Gr->DrawRect( m_PosX + m_Font->m_CharHeight - 2,
                              m_PosY + m_VarY1 + 1,
                              m_PosX + m_Width - m_Font->m_CharHeight - 2,
                              m_PosY + m_VarY1 + 1 + m_Font->m_CharHeight,
                              m_ColShortcutBg );
                Gr->DrawText( m_HeadersTextObj, m_PosX + m_Font->m_CharHeight,
                              m_PosY + m_VarY1 + 1, m_ColShortcutText, 0 );
            } else if ( m_IsHelpBar ) {
                if ( g_TwMgr->m_KeyPressedTextObj &&
                     g_TwMgr->m_KeyPressedStr.size() > 0 ) // Draw key pressed
                {
                    if ( g_TwMgr->m_KeyPressedBuildText ) {
                        string Str = g_TwMgr->m_KeyPressedStr;
                        ClampText( Str, m_Font,
                                   m_Width - 2 * m_Font->m_CharHeight );
                        g_TwMgr->m_Graph->BuildText(
                            g_TwMgr->m_KeyPressedTextObj, &Str, NULL, NULL, 1,
                            g_TwMgr->m_HelpBar->m_Font, 0, 0 );
                        g_TwMgr->m_KeyPressedBuildText = false;
                        g_TwMgr->m_KeyPressedTime =
                            ( float )g_BarTimer.GetTime();
                    }
                    if ( ( float )g_BarTimer.GetTime() >
                         g_TwMgr->m_KeyPressedTime +
                             1.0f ) // draw key pressed at least 1 second
                    ...
                    Gr->DrawRect( m_PosX + m_Font->m_CharHeight - 2,
                                  ...
                                  m_PosY + m_VarY1 + 1 + m_Font->m_CharHeight,
                                  m_ColShortcutBg );
                    Gr->DrawText( g_TwMgr->m_KeyPressedTextObj,
                                  m_PosX + m_Font->m_CharHeight,
                                  m_PosY + m_VarY1 + 1, m_ColShortcutText, 0 );
                    PERF( DT = Timer.GetTime();
                          printf( "KeyPressed=%.4fms ", 1000.0 * DT ); )
                } else {
                    if ( g_TwMgr->m_InfoBuildText ) {
                        string Info = "atb ";
                        char Ver[ 64 ];
                        sprintf( Ver, " %d.%02d", TW_VERSION / 100,
                                 TW_VERSION % 100 );
                        Info += Ver;
                        ClampText( Info, m_Font,
                                   m_Width - 2 * m_Font->m_CharHeight );
                        g_TwMgr->m_Graph->BuildText(
                            g_TwMgr->m_InfoTextObj, &Info, NULL, NULL, 1,
                            g_TwMgr->m_HelpBar->m_Font, 0, 0 );
                    ...
                    Gr->DrawRect( m_PosX + m_Font->m_CharHeight - 2,
                                  ...
                                  m_PosY + m_VarY1 + 1 + m_Font->m_CharHeight,
                                  m_ColShortcutBg );
                    Gr->DrawText( g_TwMgr->m_InfoTextObj,
                                  m_PosX + m_Font->m_CharHeight,
                                  m_PosY + m_VarY1 + 1, m_ColInfoText, 0 );
                    PERF( DT = Timer.GetTime();
                          printf( "Info=%.4fms ", 1000.0 * DT ); )
                }
            }
    ...
    } else // minimized
    {
        ...
        if ( g_TwMgr->m_IconPos == 3 ) // top-right
        {
            if ( g_TwMgr->m_IconAlign == 1 ) // horizontal
            {
                ...
                m_MinPosX = vpx + vpw -
                            ( ( m_MinNumber % n ) + 1 ) * m_Font->m_CharHeight;
        ...
        if ( m_HighlightMaximize ) {
            ...
            if ( _DrawPart & DRAW_BG ) {
                Gr->DrawRect(
                    m_MinPosX, m_MinPosY, m_MinPosX + m_Font->m_CharHeight,
                    m_MinPosY + m_Font->m_CharHeight, m_ColTitleUnactiveBg );
                Gr->DrawRect( m_MinPosX + MinXOffset, m_MinPosY + MinYOffset,
                              m_MinPosX + MinXOffset + m_TitleWidth +
                                  m_Font->m_CharHeight,
                              m_MinPosY + MinYOffset + m_Font->m_CharHeight,
                              m_ColTitleUnactiveBg );
            }
            if ( _DrawPart & DRAW_CONTENT ) {
                if ( m_ColTitleShadow != 0 )
                    Gr->DrawText(
                        m_TitleTextObj,
                        m_MinPosX + MinXOffset + m_Font->m_CharHeight / 2,
                        m_MinPosY + 1 + MinYOffset, m_ColTitleShadow, 0 );
                Gr->DrawText( m_TitleTextObj,
                              m_MinPosX + MinXOffset + m_Font->m_CharHeight / 2,
                              m_MinPosY + MinYOffset, m_ColTitleText, 0 );
            }
        }
        ...
        if ( !m_IsHelpBar ) {
            ...
            wm = ( wm < 6 ) ? 6 : wm;
            if ( _DrawPart & DRAW_BG )
                Gr->DrawRect( xm + 1, m_MinPosY + 4, xm + wm - 1,
                              m_MinPosY + 3 + wm,
                              m_HighlightMaximize ? m_ColHighBtn : m_ColBtn );
            if ( _DrawPart & DRAW_CONTENT ) {
                Gr->DrawLine( xm, m_MinPosY + 3, xm + wm, m_MinPosY + 3,
                              m_ColLine );
                Gr->DrawLine( xm + wm, m_MinPosY + 3, xm + wm,
                              m_MinPosY + 3 + wm, m_ColLine );
                Gr->DrawLine( xm + wm, m_MinPosY + 3 + wm, xm,
                              m_MinPosY + 3 + wm, m_ColLine );
                Gr->DrawLine( xm, m_MinPosY + 3 + wm, xm, m_MinPosY + 3,
                              m_ColLine );
                Gr->DrawLine( xm + wm + 1, m_MinPosY + 4, xm + wm + 1,
                              m_MinPosY + 4 + wm, m_ColLineShadow );
                Gr->DrawLine( xm + wm + 1, m_MinPosY + 4 + wm, xm,
                              m_MinPosY + 4 + wm, m_ColLineShadow );
                Gr->DrawLine( xm + wm / 3 - 1, m_MinPosY + 3 + wm - wm / 3,
                              xm + wm / 2, m_MinPosY + 6, m_ColTitleText,
                              true );
                Gr->DrawLine( xm + wm - wm / 3 + 1, m_MinPosY + 3 + wm - wm / 3,
                              xm + wm / 2, m_MinPosY + 6, m_ColTitleText,
                              true );
            }
        } else {
            ...
            if ( _DrawPart & DRAW_CONTENT ) {
                ...
                Gr->DrawLine( xm + wm + 1, m_MinPosY + 4 + wm, xm,
                              m_MinPosY + 4 + wm, m_ColLineShadow );
                Gr->DrawLine( xm + wm / 2 - wm / 6, m_MinPosY + 3 + wm / 4,
                              xm + wm - wm / 3, m_MinPosY + 3 + wm / 4,
                              m_ColTitleText );
                Gr->DrawLine( xm + wm - wm / 3, m_MinPosY + 3 + wm / 4,
                              xm + wm - wm / 3, m_MinPosY + 3 + wm / 2,
                              m_ColTitleText );
                Gr->DrawLine( xm + wm - wm / 3, m_MinPosY + 3 + wm / 2,
                              xm + wm / 2, m_MinPosY + 3 + wm / 2,
                              m_ColTitleText );
                Gr->DrawLine( xm + wm / 2, m_MinPosY + 3 + wm / 2, xm + wm / 2,
                              m_MinPosY + 3 + wm - wm / 4, m_ColTitleText );
                Gr->DrawLine( xm + wm / 2, m_MinPosY + 3 + wm - wm / 4 + 1,
                              xm + wm / 2, m_MinPosY + 3 + wm - wm / 4 + 2,
                              m_ColTitleText );
            }
        }
    }
...
bool CTwBar::MouseMotion( int _X, int _Y ) {
    assert( g_TwMgr->m_Graph && g_TwMgr->m_WndHeight > 0 &&
            g_TwMgr->m_WndWidth > 0 );
    if ( !m_UpToDate )
        Update();
    ...
    if ( !m_IsMinimized ) {
        bool InBar = ( _X >= m_PosX && _X < m_PosX + m_Width && _Y >= m_PosY &&
                       _Y < m_PosY + m_Height );
        ...
        if ( !m_MouseDrag ) {
            ...
            m_HighlightDecrBtn = false;
            m_HighlightRotoBtn = false;
            if ( abs( m_MouseOriginX - _X ) > 6 ||
                 abs( m_MouseOriginY - _Y ) > 6 )
                m_HighlightClickBtn = false;
            m_HighlightListBtn = false;
            m_HighlightTitle = false;
            m_HighlightScroll = false;
            ...
            m_HighlightLabelsHeader = false;
            m_HighlightValuesHeader = false;
            // if( InBar && _X>m_PosX+m_Font->m_CharHeight+1 &&
            // _X<m_PosX+m_VarX2 && _Y>=m_PosY+m_VarY0 && _Y<m_PosY+m_VarY1 )
            if ( InBar && _X > m_PosX + 2 && _X < m_PosX + m_VarX2 &&
                 _Y >= m_PosY + m_VarY0 &&
                 _Y < m_PosY + m_VarY1 ) { // mouse over var line
                m_HighlightedLine = ( _Y - m_PosY - m_VarY0 ) /
                                    ( m_Font->m_CharHeight + m_LineSep );
                ...
                if ( m_HighlightedLine < 0 ||
                    ...
                    } else if ( m_DrawRotoBtn &&
                                ( _X >= m_PosX + m_VarX2 -
                                            IncrBtnWidth(
                                                m_Font->m_CharHeight ) ||
                                  _X < m_PosX + m_VarX1 ) ) // [o] button
                    // else if( m_DrawRotoBtn && _X<m_PosX+m_VarX1 ) // [o]
                    // button
                    {
                        m_HighlightRotoBtn = true;
                        ANT_SET_CURSOR( Point );
                    } else if ( m_DrawIncrDecrBtn &&
                                _X >=
                                    m_PosX + m_VarX2 -
                                        IncrBtnWidth(
                                            m_Font
                                                                       ...
                                                                       // button
                    {
                        ...
                        ANT_SET_CURSOR( Arrow );
                    } else if ( !m_HierTags[ m_HighlightedLine ]
                                     .m_Var->IsGroup() &&
                                static_cast< CTwVarAtom* >(
                                    m_HierTags[ m_HighlightedLine ].m_Var )
                                    ->m_ReadOnly ) {
            ...
            } else if ( InBar && m_Movable && !m_IsPopupList &&
                        _X >= m_PosX + 2 * m_Font->m_CharHeight &&
                        _X < m_PosX + m_Width - 2 * m_Font->m_CharHeight &&
                        _Y < m_PosY +
                                 m_Font->m_CharHeight ) { // mouse over title
                m_HighlightTitle = true;
                ANT_SET_CURSOR( Move );
            } else if ( InBar && !m_IsPopupList && _X >= m_PosX + m_VarX1 - 5 &&
                        _X < m_PosX + m_VarX1 + 5 &&
                        _Y > m_PosY + m_Font->m_CharHeight &&
                        _Y < m_PosY +
                                 m_VarY0 ) { // mouse over ValuesWidth handle
                m_HighlightValWidth = true;
                ANT_SET_CURSOR( WE );
            } else if ( InBar && !m_IsPopupList && !m_IsHelpBar &&
                        _X >= m_PosX + m_VarX0 && _X < m_PosX + m_VarX1 - 5 &&
                        _Y > m_PosY + m_Font->m_CharHeight &&
                        _Y < m_PosY +
                                 m_VarY0 ) { // mouse over left column header
                ...
                ANT_SET_CURSOR( Arrow );
                        ...
                        _Y > m_PosY + m_Font->m_CharHeight &&
                        _Y < m_PosY +
                                 m_VarY0 ) { // mouse over right column header
                m_HighlightValuesHeader = true;
                ANT_SET_CURSOR( Arrow );
            }
            // else if( InBar && m_NbDisplayedLines<m_NbHierLines && _X>=m_PosX
            // && _X<m_PosX+m_Font->m_CharHeight && _Y>=m_ScrollY0 &&
            // _Y<m_ScrollY1 )
            else if ( InBar && m_NbDisplayedLines < m_NbHierLines &&
                      _X >= m_PosX + m_VarX2 + 2 && _X < m_PosX + m_Width - 2 &&
                      _Y >= m_ScrollY0 && _Y < m_ScrollY1 ) {
                ...
                ANT_SET_CURSOR( Arrow );
#endif
            } else if ( InBar && _X >= m_PosX + m_VarX2 + 2 &&
                        _X < m_PosX + m_Width - 2 && _Y >= m_PosY + m_VarY0 &&
                        _Y < m_ScrollY0 ) {
                m_HighlightUpScroll = true;
                ANT_SET_CURSOR( Arrow );
            } else if ( InBar && _X >= m_PosX + m_VarX2 + 2 &&
                        _X < m_PosX + m_Width - 2 && _Y >= m_ScrollY1 &&
                        _Y < m_PosY + m_VarY1 ) {
                m_HighlightDnScroll = true;
                ANT_SET_CURSOR( Arrow );
            } else if ( InBar && m_Resizable && !m_IsPopupList &&
                        _X >= m_PosX && _X < m_PosX + m_Font->m_CharHeight &&
                        _Y >= m_PosY && _Y < m_PosY + m_Font->m_CharHeight )
                ANT_SET_CURSOR( TopLeft );
            else if ( InBar && m_Resizable && !m_IsPopupList && _X >= m_PosX &&
                      _X < m_PosX + m_Font->m_CharHeight &&
                      _Y >= m_PosY + m_Height - m_Font->m_CharHeight &&
                      _Y < m_PosY + m_Height )
                ANT_SET_CURSOR( BottomLeft );
            else if ( InBar && m_Resizable && !m_IsPopupList &&
                      _X >= m_PosX + m_Width - m_Font->m_CharHeight &&
                      _X < m_PosX + m_Width && _Y >= m_PosY &&
                      _Y < m_PosY + m_Font->m_CharHeight )
            ...
            else if ( InBar && g_TwMgr->m_FontResizable && !m_IsPopupList &&
                      _X >= m_PosX + m_Font->m_CharHeight &&
                      _X < m_PosX + 2 * m_Font->m_CharHeight &&
                      _Y < m_PosY + m_Font->m_CharHeight ) {
                m_HighlightFont = true;
                ANT_SET_CURSOR( Arrow );
            } else if ( InBar && m_Iconifiable && !m_IsPopupList &&
                        _X >= m_PosX + m_Width - 2 * m_Font->m_CharHeight &&
                        _X < m_PosX + m_Width - m_Font->m_CharHeight &&
                        _Y < m_PosY + m_Font->m_CharHeight ) {
                m_HighlightMinimize = true;
                ANT_SET_CURSOR( Arrow );
            } else if ( m_IsHelpBar && InBar && _X >= m_PosX + m_VarX0 &&
                        _X < m_PosX + m_Width - m_Font->m_CharHeight &&
                        _Y > m_PosY + m_Height - m_Font->m_CharHeight &&
                        _Y < m_PosY + m_Height )
            ...
            if ( m_MouseDragVar && m_HighlightedLine >= 0 &&
                 m_HighlightedLine < ( int )m_HierTags.size() &&
                 m_HierTags[ m_HighlightedLine ].m_Var &&
                 !m_HierTags[ m_HighlightedLine ].m_Var->IsGroup() ) {
                /*
                CTwVarAtom *Var = static_cast<CTwVarAtom
                *>(m_HierTags[m_HighlightedLine].m_Var); int Delta =
                _X-m_MouseOriginX; if( Delta!=0 )
                {
                ...
                */
                ...
                if ( m_ScrollYH > 0 ) {
                    int dl = ( ( _Y - m_MouseOriginY ) * m_NbHierLines ) /
                             m_ScrollYH;
                    if ( m_FirstLine0 + dl < 0 )
                        m_FirstLine = 0;
                    else if ( m_FirstLine0 + dl + m_NbDisplayedLines >
                              m_NbHierLines )
                ...
                m_MouseOriginX = _X;
                m_MouseOriginY = _Y;
                int vpx = 0, vpy = 0, vpw = g_TwMgr->m_WndWidth,
                    vph = g_TwMgr->m_WndHeight;
                if ( !m_Contained ) {
                    if ( m_PosX + ContainedMargin > vpx + vpw )
                        m_PosX = vpx + vpw - ContainedMargin;
    ...
    {
        if ( m_Iconifiable && _X >= m_MinPosX + 2 &&
             _X < m_MinPosX + m_Font->m_CharHeight && _Y > m_MinPosY &&
             _Y < m_MinPosY + m_Font->m_CharHeight - 2 ) {
    ...
    if ( g_TwMgr != NULL && ( !Handled || CustomArea ) && !m_IsMinimized &&
         m_CustomRecords.size() > 0 ) {
        bool CustomHandled = false;
        for ( int s = 0; s < 2;
              ++s ) // 2 iterations: first for custom widget having focus,
                    // second for others if no focused widget.
            for ( CustomMap::iterator it = m_CustomRecords.begin();
                  it != m_CustomRecords.end(); ++it ) {
                CTwMgr::CStructProxy* sProxy = it->first;
                const CCustomRecord& r = it->second;
                if ( ( s == 1 || sProxy->m_CustomCaptureFocus ) &&
                     !CustomHandled && sProxy != NULL &&
                     sProxy->m_CustomMouseMotionCallback != NULL &&
                     r.m_XMin < r.m_XMax && r.m_Y0 < r.m_Y1 &&
                     r.m_YMin <= r.m_YMax && r.m_YMin >= r.m_Y0 &&
                     r.m_YMax <= r.m_Y1 ) {
                    if ( sProxy->m_CustomCaptureFocus ||
                         ( _X >= r.m_XMin && _X < r.m_XMax && _Y >= r.m_YMin &&
                           _Y < r.m_YMax ) ) {
                        CustomHandled = sProxy->m_CustomMouseMotionCallback(
                            _X - r.m_XMin, _Y - r.m_Y0, r.m_XMax - r.m_XMin,
                            r.m_Y1 - r.m_Y0, sProxy->m_StructExtData,
                            sProxy->m_StructClientData, this, r.m_Var );
                        currentCustomActiveStructProxy = sProxy;
                        s = 2; // force s-loop exit
                    }
                } else if ( sProxy != NULL ) {
                    sProxy->m_CustomCaptureFocus =
                        false; // force free focus, just in case.
                    ANT_SET_CURSOR( Arrow );
                }
            }
        if ( CustomHandled )
    ...
    if ( g_TwMgr != NULL && m_CustomActiveStructProxy != NULL &&
         m_CustomActiveStructProxy != currentCustomActiveStructProxy ) {
        bool found = false;
        for ( list< CTwMgr::CStructProxy >::iterator it =
                  g_TwMgr->m_StructProxies.begin();
              it != g_TwMgr->m_StructProxies.end() && !found; ++it )
            found = ( &( *it ) == m_CustomActiveStructProxy );
        if ( found &&
             m_CustomActiveStructProxy->m_CustomMouseLeaveCallback != NULL )
            m_CustomActiveStructProxy->m_CustomMouseLeaveCallback(
                m_CustomActiveStructProxy->m_StructExtData,
                m_CustomActiveStructProxy->m_StructClientData, this );
    }
    m_CustomActiveStructProxy = currentCustomActiveStructProxy;
...
#ifdef ANT_WINDOWS
#pragma optimize( "", off )
//  disable optimizations because the conversion of Enum from unsigned int to
//  double is not always exact if optimized and GraphAPI=DirectX !
#endif
static void ANT_CALL PopupCallback( void* _ClientData ) {
    ...
    if ( g_TwMgr != NULL && g_TwMgr->m_PopupBar != NULL ) {
        ...
        if ( Bar != NULL && Var != NULL && !Var->m_ReadOnly &&
             IsEnumType( Var->m_Type ) ) {
            Var->ValueFromDouble( Enum );
            // Bar->UnHighlightLine();
            Bar->HaveFocus( true );
            Bar->NotUpToDate();
        }
        if ( g_TwMgr->m_PopupBar != NULL ) // check again because it might have
                                           // been destroyed by an enum callback
...
bool CTwBar::MouseButton( ETwMouseButtonID _Button,
                          bool _Pressed,
                          int _X,
                          int _Y ) {
    assert( g_TwMgr->m_Graph && g_TwMgr->m_WndHeight > 0 &&
    ...
    if ( !m_IsMinimized ) {
        Handled = ( _X >= m_PosX && _X < m_PosX + m_Width && _Y >= m_PosY &&
                    _Y < m_PosY + m_Height );
        if ( _Button == TW_MOUSE_LEFT && m_HighlightedLine >= 0 &&
             m_HighlightedLine < ( int )m_HierTags.size() &&
             m_HierTags[ m_HighlightedLine ].m_Var ) {
            bool OnFocus = ( m_HighlightedLine ==
                                 ( _Y - m_PosY - m_VarY0 ) /
                                     ( m_Font->m_CharHeight + m_LineSep ) &&
                             Handled );
            if ( m_HierTags[ m_HighlightedLine ].m_Var->IsGroup() ) {
                if ( _Pressed && !g_TwMgr->m_IsRepeatingMousePressed &&
                     OnFocus ) {
                    CTwVarGroup* Grp = static_cast< CTwVarGroup* >(
                        m_HierTags[ m_HighlightedLine ].m_Var );
                ...
                static_cast< CTwVarAtom* >(
                    m_HierTags[ m_HighlightedLine ].m_Var )
                    ->Increment( 1 );
                if ( g_TwMgr == NULL ) // Mgr might have been destroyed by the
                                       // client inside a callback call
                    return 1;
                NotUpToDate();
                ...
                if ( !Var->m_NoSlider && !Var->m_ReadOnly &&
                    ...
                    if ( _X > m_PosX + m_VarX1 && OnFocus )
                        RotoOnLButtonDown(
                            m_PosX + m_VarX2 -
                                ( 1 * IncrBtnWidth( m_Font->m_CharHeight ) ) /
                                    2,
                            _Y );
                    else
                    ...
                    m_MouseDrag = true;
                    m_MouseDragVar = true;
                } else if ( ( Var->m_Type == TW_TYPE_BOOL8 ||
                              Var->m_Type == TW_TYPE_BOOL16 ||
                              Var->m_Type == TW_TYPE_BOOL32 ||
                              Var->m_Type == TW_TYPE_BOOLCPP ) &&
                            !Var->m_ReadOnly && OnFocus ) {
                    Var->Increment( 1 );
                    ...
                    m_MouseDrag = false;
                    NotUpToDate();
                } else if ( Var->m_Type == TW_TYPE_BUTTON &&
                            !Var->m_ReadOnly ) {
                    m_HighlightClickBtn = true;
                    m_MouseDragVar = false;
                    m_MouseDrag = false;
                }
                // else if( (Var->m_Type==TW_TYPE_ENUM8 ||
                // Var->m_Type==TW_TYPE_ENUM16 || Var->m_Type==TW_TYPE_ENUM32)
                // && !Var->m_ReadOnly )
                else if ( IsEnumType( Var->m_Type ) && !Var->m_ReadOnly &&
                          !g_TwMgr->m_IsRepeatingMousePressed && OnFocus ) {
                    m_MouseDragVar = false;
                    m_MouseDrag = false;
                    ...
                    // popup list
                    CTwMgr::CEnum& e =
                        g_TwMgr->m_Enums[ Var->m_Type - TW_TYPE_ENUM_BASE ];
                    ...
                    g_TwMgr->m_PopupBar->m_PosY =
                        m_PosY + m_VarY0 +
                        ( m_HighlightedLine + 1 ) *
                            ( m_Font->m_CharHeight + m_LineSep );
                    g_TwMgr->m_PopupBar->m_Width =
                        m_Width - 2 * m_Font->m_CharHeight;
                    g_TwMgr->m_PopupBar->m_LineSep = g_TwMgr->m_PopupBar->m_Sep;
                    int popHeight0 = ( int )e.m_Entries.size() *
                                         ( m_Font->m_CharHeight + m_Sep ) +
                                     m_Font->m_CharHeight / 2 + 2;
                    int popHeight = popHeight0;
                    if ( g_TwMgr->m_PopupBar->m_PosY + popHeight + 2 >
                         g_TwMgr->m_WndHeight )
                        popHeight = g_TwMgr->m_WndHeight -
                                    g_TwMgr->m_PopupBar->m_PosY - 2;
                    if ( popHeight < popHeight0 / 2 &&
                         popHeight < g_TwMgr->m_WndHeight / 2 )
                    ...
                    unsigned int CurrentEnumValue =
                        ( unsigned int )( ( int )Var->ValueToDouble() );
                    for ( CTwMgr::CEnum::CEntries::iterator It =
                              e.m_Entries.begin();
                          It != e.m_Entries.end(); ++It ) {
                        char ID[ 64 ];
                        sprintf( ID, "%u", It->first );
                        // ultoa(It->first, ID, 10);
                        TwAddButton( g_TwMgr->m_PopupBar, ID, PopupCallback,
                                     *( void** )&( It->first ), NULL );
                    ...
                    g_TwMgr->m_HelpBarNotUpToDate = false;
                              ...
                              ( Var->m_Type == TW_TYPE_CDSTRING ||
                                Var->m_Type == TW_TYPE_CDSTDSTRING ||
                                Var->m_Type == TW_TYPE_STDSTRING ||
                                IsCSStringType( Var->m_Type ) ) &&
                              EditInPlaceAcceptVar( Var ) ) ||
                            ( !Var->m_ReadOnly &&
                              EditInPlaceAcceptVar( Var ) ) ) {
                    int dw = 0;
                    // if( m_DrawIncrDecrBtn )
                    ...
                    if ( !m_EditInPlace.m_Active ||
                        ...
                        EditInPlaceStart(
                            Var, m_VarX1,
                            m_VarY0 + ( m_HighlightedLine ) *
                                          ( m_Font->m_CharHeight + m_LineSep ),
                            m_VarX2 - m_VarX1 - dw - 1 );
                ...
                if( !m_VarHasBeenIncr && !static_cast<CTwVarAtom
                *>(m_HierTags[m_HighlightedLine].m_Var)->m_ReadOnly )
                {
                    if( _Button==TW_MOUSE_LEFT )
                        static_cast<CTwVarAtom
                *>(m_HierTags[m_HighlightedLine].m_Var)->Increment(-1); else if(
                _Button==TW_MOUSE_RIGHT ) static_cast<CTwVarAtom
                *>(m_HierTags[m_HighlightedLine].m_Var)->Increment(1);
                    NotUpToDate();
                }
                */
                ...
                if ( static_cast< CTwVarAtom* >(
                    ...
                    CustomArea = true;
                        ...
                        m_HighlightClickBtn ) // a button variable is activated
            {
                m_HighlightClickBtn = false;
                m_MouseDragVar = false;
                m_MouseDrag = false;
                ...
                if ( !m_HierTags[ m_HighlightedLine ].m_Var->IsGroup() ) {
                    CTwVarAtom* Var = static_cast< CTwVarAtom* >(
                        m_HierTags[ m_HighlightedLine ].m_Var );
                    if ( !Var->m_ReadOnly && Var->m_Type == TW_TYPE_BUTTON &&
                         Var->m_Val.m_Button.m_Callback != NULL ) {
                ...
                CustomArea = true;
                    ...
                    ( ( _Button == TW_MOUSE_LEFT &&
                        _X >= m_PosX + 2 * m_Font->m_CharHeight &&
                        _X < m_PosX + m_Width - 2 * m_Font->m_CharHeight &&
                        _Y >= m_PosY && _Y < m_PosY + m_Font->m_CharHeight ) ||
                      ( _Button == TW_MOUSE_MIDDLE && _X >= m_PosX &&
                        _X < m_PosX + m_Width && _Y >= m_PosY &&
                        _Y < m_PosY + m_Height ) ) ) {
            m_MouseDrag = true;
            ...
            m_MouseDragTitle = false;
            ANT_SET_CURSOR( Arrow );
        } else if ( _Pressed && !m_MouseDrag && !m_IsPopupList &&
                    _Button == TW_MOUSE_LEFT && _X >= m_PosX + m_VarX1 - 3 &&
                    _X < m_PosX + m_VarX1 + 3 &&
                    _Y > m_PosY + m_Font->m_CharHeight &&
                    _Y < m_PosY + m_VarY0 ) {
            m_MouseDrag = true;
            ...
            ANT_SET_CURSOR( Arrow );
                    ...
                    m_NbDisplayedLines < m_NbHierLines &&
                    _Button == TW_MOUSE_LEFT && _X >= m_PosX + m_VarX2 + 2 &&
                    _X < m_PosX + m_Width - 2 && _Y >= m_ScrollY0 &&
                    _Y < m_ScrollY1 ) {
            m_MouseDrag = true;
            ...
            m_MouseDragScroll = false;
            ANT_SET_CURSOR( Arrow );
        } else if ( _Pressed && _Button == TW_MOUSE_LEFT &&
                    _X >= m_PosX + m_VarX2 + 2 && _X < m_PosX + m_Width - 2 &&
                    _Y >= m_PosY + m_VarY0 && _Y < m_ScrollY0 ) {
            if ( m_FirstLine > 0 ) {
                --m_FirstLine;
                NotUpToDate();
            }
                    ...
                    _X >= m_PosX + m_VarX2 + 2 && _X < m_PosX + m_Width - 2 &&
                    _Y >= m_ScrollY1 && _Y < m_PosY + m_VarY1 ) {
            if ( m_FirstLine < m_NbHierLines - m_NbDisplayedLines ) {
                ++m_FirstLine;
                NotUpToDate();
            }
        } else if ( _Pressed && !m_MouseDrag && m_Resizable && !m_IsPopupList &&
                    _Button == TW_MOUSE_LEFT && _X >= m_PosX &&
                    _X < m_PosX + m_Font->m_CharHeight && _Y >= m_PosY &&
                    _Y < m_PosY + m_Font->m_CharHeight ) {
            m_MouseDrag = true;
            ...
            m_MouseOriginY = _Y;
            m_ValuesWidthRatio =
                ( m_Width > 0 ) ? ( double )m_ValuesWidth / m_Width : 0;
            ANT_SET_CURSOR( TopLeft );
            ...
            m_MouseDragResizeLR = false;
            ANT_SET_CURSOR( Arrow );
        } else if ( _Pressed && !m_IsPopupList && _Button == TW_MOUSE_LEFT &&
                    m_HighlightLabelsHeader ) {
            ...
            NotUpToDate();
            ANT_SET_CURSOR( Arrow );
        } else if ( _Pressed && g_TwMgr->m_FontResizable && !m_IsPopupList &&
                    _X >= m_PosX + m_Font->m_CharHeight &&
                    _X < m_PosX + 2 * m_Font->m_CharHeight && _Y > m_PosY &&
                    _Y < m_PosY + m_Font->m_CharHeight ) {
            // change font
            if ( _Button == TW_MOUSE_LEFT ) {
            ...
            ANT_SET_CURSOR( Arrow );
        } else if ( _Pressed && m_Iconifiable && !m_IsPopupList &&
                    _Button == TW_MOUSE_LEFT &&
                    _X >= m_PosX + m_Width - 2 * m_Font->m_CharHeight &&
                    _X < m_PosX + m_Width - m_Font->m_CharHeight &&
                    _Y > m_PosY && _Y < m_PosY + m_Font->m_CharHeight ) {
            ...
            ANT_SET_CURSOR( Arrow );
        } else if ( m_IsHelpBar && _Pressed &&
                    !g_TwMgr->m_IsRepeatingMousePressed &&
                    _X >= m_PosX + m_VarX0 &&
                    _X < m_PosX + m_Width - m_Font->m_CharHeight &&
                    _Y > m_PosY + m_Height - m_Font->m_CharHeight &&
                    _Y < m_PosY + m_Height ) {
            ...
            const char *WebPage = "http://";
                ...
                // brute force: try all the possible browsers (I don't know how
            to find the default one; someone?) char DefaultBrowsers[] =
            "firefox,chrome,opera,mozilla,konqueror,galeon,dillo,netscape"; char
            *browser = strtok(DefaultBrowsers, ","); char cmd[256];
                while(browser)
                {
                    snprintf(cmd, sizeof(cmd), "%s \"%s\" 1>& null &", browser,
            WebPage); if( system(cmd) ) {} // avoiding warn_unused_result
    ...
    if ( g_TwMgr != NULL ) // Mgr might have been destroyed by the client inside
                           // a callback call
            ...
            for ( CustomMap::iterator it = m_CustomRecords.begin();
                ...
                if ( ( s == 1 || sProxy->m_CustomCaptureFocus ) &&
                    ...
                    if ( sProxy->m_CustomCaptureFocus ||
                        ...
                        CustomHandled = sProxy->m_CustomMouseButtonCallback(
                            _Button, _Pressed, _X - r.m_XMin, _Y - r.m_Y0,
                            r.m_XMax - r.m_XMin, r.m_Y1 - r.m_Y0,
                            sProxy->m_StructExtData, sProxy->m_StructClientData,
                            this, r.m_Var );
                        s = 2; // force s-loop exit
                    }
...
bool CTwBar::MouseWheel( int _Pos, int _PrevPos, int _MouseX, int _MouseY ) {
    ...
    bool Handled = false;
    if ( !m_IsMinimized && _MouseX >= m_PosX && _MouseX < m_PosX + m_Width &&
         _MouseY >= m_PosY && _MouseY < m_PosY + m_Height ) {
...
CTwVarAtom* CTwVarGroup::FindShortcut( int _Key,
                                       int _Modifiers,
                                       bool* _DoIncr ) {
    CTwVarAtom* Atom;
    int Mask = 0xffffffff;
    ...
    Mask &=
        ~( 0x2000 ); // 0x2000 is the KMOD_CAPS value defined in SDL_keysym.h
    ...
    for ( size_t i = 0; i < m_Vars.size(); ++i )
        if ( m_Vars[ i ] != NULL ) {
            if ( m_Vars[ i ]->IsGroup() ) {
                Atom = static_cast< CTwVarGroup* >( m_Vars[ i ] )
                           ->FindShortcut( _Key, _Modifiers, _DoIncr );
                if ( Atom != NULL )
                    return Atom;
            } else {
                Atom = static_cast< CTwVarAtom* >( m_Vars[ i ] );
                if ( Atom->m_KeyIncr[ 0 ] == _Key &&
                     ( Atom->m_KeyIncr[ 1 ] & Mask ) ==
                         ( _Modifiers & Mask ) ) {
                    if ( _DoIncr != NULL )
                        *_DoIncr = true;
                    return Atom;
                } else if ( Atom->m_KeyDecr[ 0 ] == _Key &&
                            ( Atom->m_KeyDecr[ 1 ] & Mask ) ==
                                ( _Modifiers & Mask ) ) {
                    if ( _DoIncr != NULL )
                        *_DoIncr = false;
                    return Atom;
...
bool CTwBar::KeyPressed( int _Key, int _Modifiers ) {
    ...
    if ( _Key > 0 && _Key < TW_KEY_LAST ) {
        ...
        // CTRL special cases
        if( (_Modifiers&TW_KMOD_CTRL) && !(_Modifiers&TW_KMOD_ALT ||
        _Modifiers&TW_KMOD_META) && _Key>0 && _Key<32 ) _Key += 'a'-1;

        // PAD translation (for SDL keysym)
        if( _Key>=256 && _Key<=272 ) // 256=SDLK_KP0 ... 272=SDLK_KP_EQUALS
        {
            bool Num = ((_Modifiers&TW_KMOD_SHIFT) && !(_Modifiers&0x1000)) ||
        (!(_Modifiers&TW_KMOD_SHIFT) && (_Modifiers&0x1000)); // 0x1000 is SDL's
        KMOD_NUM _Modifiers &= ~TW_KMOD_SHIFT;   // remove shift modifier if(
        _Key==266 )          // SDLK_KP_PERIOD _Key = Num ? '.' : TW_KEY_DELETE;
        ...
        */
        ...
        if ( m_EditInPlace.m_Active ) {
            ...
            bool BarActive =
                ( m_DrawHandles || m_IsPopupList ) && !m_IsMinimized;
            bool DoIncr = true;
            CTwVarAtom* Atom =
                m_VarRoot.FindShortcut( _Key, _Modifiers, &DoIncr );
            if ( Atom != NULL && Atom->m_Visible ) {
                ...
                CTwVarAtom* Var = static_cast< CTwVarAtom* >(
                    m_HierTags[ m_HighlightedLine ].m_Var );
                if ( ( Var->m_Type == TW_TYPE_CDSTRING ||
                       Var->m_Type == TW_TYPE_CDSTDSTRING ||
                       Var->m_Type == TW_TYPE_STDSTRING ||
                       IsCSStringType( Var->m_Type ) ) &&
                     EditInPlaceAcceptVar( Var ) && !m_EditInPlace.m_Active ) {
                    EditInPlaceStart(
                        Var, m_VarX1,
                        m_VarY0 + ( m_HighlightedLine ) *
                                      ( m_Font->m_CharHeight + m_LineSep ),
                ...
                Handled = true;
            } else if ( BarActive && m_HighlightedLine >= 0 &&
                        m_HighlightedLine < ( int )m_HierTags.size() &&
                        m_HierTags[ m_HighlightedLine ].m_Var ) {
                if ( _Key == TW_KEY_RIGHT ) {
                    if ( !m_HierTags[ m_HighlightedLine ].m_Var->IsGroup() ) {
                        CTwVarAtom* Atom = static_cast< CTwVarAtom* >(
                            m_HierTags[ m_HighlightedLine ].m_Var );
                        bool Accept = !Atom->m_NoSlider ||
                                      Atom->m_Type == TW_TYPE_BUTTON ||
                                      Atom->m_Type == TW_TYPE_BOOL8 ||
                                      Atom->m_Type == TW_TYPE_BOOL16 ||
                                      Atom->m_Type == TW_TYPE_BOOL32 ||
                                      Atom->m_Type == TW_TYPE_BOOLCPP ||
                    ...
                    if ( !m_HierTags[ m_HighlightedLine ].m_Var->IsGroup() ) {
                        ...
                        if ( !Atom->IsReadOnly() ) {
                            if ( Atom->m_Type == TW_TYPE_BUTTON ||
                                ...
                                Atom->Increment( +1 );
                                if ( g_TwMgr == NULL // Mgr might have been
                                                     // destroyed by the client
                                                     // inside a callback call
                    ...
                    if ( m_HighlightedLine >= ( int )m_HierTags.size() ) {
                        ...
                        if ( m_FirstLine <
                             m_NbHierLines - m_NbDisplayedLines ) {
                            ++m_FirstLine;
                            NotUpToDate();
                        }
                    }
                ...
                if ( _Key == TW_KEY_UP || _Key == TW_KEY_DOWN ||
                     _Key == TW_KEY_LEFT || _Key == TW_KEY_RIGHT ||
                     _Key == TW_KEY_RETURN ) {
                    if ( m_HighlightedLineLastValid >= 0 &&
                         m_HighlightedLineLastValid < ( int )m_HierTags.size() )
        ...
        if ( m_EditInPlace.m_Active )
            ...
            if ( Atom != NULL && Atom->m_Visible )
                Handled = true;
                      ...
                      ( _Key == TW_KEY_RIGHT || _Key == TW_KEY_LEFT ||
                        _Key == TW_KEY_UP || _Key == TW_KEY_DOWN ||
                        _Key == TW_KEY_RETURN ||
                        ( _Key == TW_KEY_ESCAPE && m_IsPopupList ) ) )
                Handled = true;
        }
    }
    return Handled;
    ...
    for ( size_t i = 0; i < _Root->m_Vars.size(); ++i )
        if ( _Root->m_Vars[ i ] != NULL ) {
            if ( _Var == _Root->m_Vars[ i ] ||
                 ( _Root->m_Vars[ i ]->IsGroup() &&
                   OpenHier( static_cast< CTwVarGroup* >( _Root->m_Vars[ i ] ),
                             _Var ) ) ) {
                _Root->m_Open = true;
                NotUpToDate();
                return true;
...
int CTwBar::LineInHier( CTwVarGroup* _Root, CTwVar* _Var ) {
    ...
    for ( size_t i = 0; i < _Root->m_Vars.size(); ++i )
        if ( _Root->m_Vars[ i ] != NULL && _Root->m_Vars[ i ]->m_Visible ) {
            if ( _Var == _Root->m_Vars[ i ] )
                return l;
            else if ( _Root->m_Vars[ i ]->IsGroup() &&
                      static_cast< CTwVarGroup* >( _Root->m_Vars[ i ] )
                          ->m_Open ) {
                ++l;
                int ll = LineInHier(
                    static_cast< CTwVarGroup* >( _Root->m_Vars[ i ] ), _Var );
                if ( ll >= 0 )
                    return l + ll;
                else
                    l += -ll - 2;
...
void DrawArc( int _X,
              ...
              float _EndAngleDeg,
              color32 _Color ) // angles in degree
{
    ITwGraph* Gr = g_TwMgr->m_Graph;
    if ( Gr == NULL || !Gr->IsDrawing() || _Radius == 0 ||
         _StartAngleDeg == _EndAngleDeg )
        return;
    ...
    bool fullCircle = fabsf( endAngle - startAngle ) >=
                      2.0f * ( float )M_PI + fabsf( stepAngle );
    int numSteps;
    if ( fullCircle ) {
...
void CTwBar::RotoDraw() {
    ...
    if ( m_Roto.m_Active ) {
        ...
        DrawArc( m_Roto.m_Origin.x + 1, m_Roto.m_Origin.y, 32, 0, 360,
                 m_ColRoto );
        DrawArc( m_Roto.m_Origin.x, m_Roto.m_Origin.y + 1, 32, 0, 360,
                 m_ColRoto );
        ...
        if ( m_Roto.m_HasPrevious ) {
            ...
            if ( varMax < DOUBLE_MAX && varMin > -DOUBLE_MAX &&
                 fabs( varStep ) > DOUBLE_EPS && m_Roto.m_Subdiv > 0 ) {
                double dtMax = 360.0 * ( varMax - m_Roto.m_ValueAngle0 ) /
                               ( ( double )m_Roto.m_Subdiv * varStep ); //+2;
                double dtMin = 360.0 * ( varMin - m_Roto.m_ValueAngle0 ) /
                               ( ( double )m_Roto.m_Subdiv * varStep ); //-2;

                if ( dtMax >= 0 && dtMax < 360 && dtMin <= 0 && dtMin > -360 &&
                     fabs( dtMax - dtMin ) <= 360 ) {
                    int x1, y1, x2, y2;
                    double da = 2.0 * M_PI / m_Roto.m_Subdiv;

                    x1 =
                        m_Roto.m_Origin.x +
                        ( int )( 40 * cos( -M_PI * ( m_Roto.m_Angle0 + dtMax ) /
                                               180 -
                                           da ) );
                    y1 =
                        m_Roto.m_Origin.y +
                        ( int )( 40 * sin( -M_PI * ( m_Roto.m_Angle0 + dtMax ) /
                                               180 -
                                           da ) +
                                 0.5 );
                    x2 =
                        m_Roto.m_Origin.x +
                        ( int )( 40 *
                                 cos( -M_PI * ( m_Roto.m_Angle0 + dtMax - 10 ) /
                                          180 -
                                      da ) );
                    y2 =
                        m_Roto.m_Origin.y +
                        ( int )( 40 *
                                     sin( -M_PI *
                                              ( m_Roto.m_Angle0 + dtMax - 10 ) /
                                              180 -
                                          da ) +
                                 0.5 );
                    Gr->DrawLine( m_Roto.m_Origin.x, m_Roto.m_Origin.y, x1, y1,
                                  m_ColRotoBound, true );
                    Gr->DrawLine( m_Roto.m_Origin.x + 1, m_Roto.m_Origin.y,
                                  x1 + 1, y1, m_ColRotoBound, true );
                    Gr->DrawLine( m_Roto.m_Origin.x, m_Roto.m_Origin.y + 1, x1,
                                  y1 + 1, m_ColRotoBound, true );
                    Gr->DrawLine( x1, y1, x2, y2, m_ColRotoBound, true );
                    Gr->DrawLine( x1 + 1, y1, x2 + 1, y2, m_ColRotoBound,
                                  true );
                    Gr->DrawLine( x1, y1 + 1, x2, y2 + 1, m_ColRotoBound,
                                  true );

                    x1 =
                        m_Roto.m_Origin.x +
                        ( int )( 40 * cos( -M_PI * ( m_Roto.m_Angle0 + dtMin ) /
                                               180 +
                                           da ) );
                    y1 =
                        m_Roto.m_Origin.y +
                        ( int )( 40 * sin( -M_PI * ( m_Roto.m_Angle0 + dtMin ) /
                                               180 +
                                           da ) +
                                 0.5 );
                    x2 =
                        m_Roto.m_Origin.x +
                        ( int )( 40 *
                                 cos( -M_PI * ( m_Roto.m_Angle0 + dtMin + 10 ) /
                                          180 +
                                      da ) );
                    y2 =
                        m_Roto.m_Origin.y +
        ...
        Gr->DrawLine( m_Roto.m_Origin.x + 1, m_Roto.m_Origin.y,
                      m_Roto.m_Current.x + 1, m_Roto.m_Current.y, m_ColRotoVal,
                      true );
        Gr->DrawLine( m_Roto.m_Origin.x, m_Roto.m_Origin.y + 1,
                      m_Roto.m_Current.x, m_Roto.m_Current.y + 1, m_ColRotoVal,
                      true );
        Gr->DrawLine( m_Roto.m_Origin.x, m_Roto.m_Origin.y, m_Roto.m_Current.x,
                      m_Roto.m_Current.y, m_ColRotoVal, true );

        if ( fabs( m_Roto.m_AngleDT ) >= 1 ) {
            DrawArc( m_Roto.m_Origin.x, m_Roto.m_Origin.y, 32,
                     float( m_Roto.m_Angle0 ),
                     float( m_Roto.m_Angle0 + m_Roto.m_AngleDT - 1 ),
                     m_ColRotoVal );
            DrawArc( m_Roto.m_Origin.x + 1, m_Roto.m_Origin.y, 32,
                     float( m_Roto.m_Angle0 ),
                     float( m_Roto.m_Angle0 + m_Roto.m_AngleDT - 1 ),
                     m_ColRotoVal );
            DrawArc( m_Roto.m_Origin.x, m_Roto.m_Origin.y + 1, 32,
                     float( m_Roto.m_Angle0 ),
                     float( m_Roto.m_Angle0 + m_Roto.m_AngleDT - 1 ),
                     m_ColRotoVal );
        }
...
void CTwBar::RotoOnMouseMove( int _X, int _Y ) {
    ...
    if ( m_Roto.m_Active ) {
        ...
        double t = 0;
        float r =
            sqrtf( float( ( m_Roto.m_Current.x - m_Roto.m_Origin.x ) *
                              ( m_Roto.m_Current.x - m_Roto.m_Origin.x ) +
                          ( m_Roto.m_Current.y - m_Roto.m_Origin.y ) *
                              ( m_Roto.m_Current.y - m_Roto.m_Origin.y ) ) );
        if ( r > m_RotoMinRadius ) {
            t = -atan2( double( m_Roto.m_Current.y - m_Roto.m_Origin.y ),
                        double( m_Roto.m_Current.x - m_Roto.m_Origin.x ) );
            ti = ( int( ( t / ( 2.0 * M_PI ) + 1.0 ) * NB_ROTO_CURSORS +
                        0.5 ) ) %
                 NB_ROTO_CURSORS;
            if ( m_Roto.m_HasPrevious ) {
                ...
                double dt = acos(
                    max( -1 + 1.0e-30,
                         min( 1 - 1.0e-30, double( v0.x * v1.x + v0.y * v1.y ) /
                                               ( l0 * l1 ) ) ) );
                if ( v0.x * v1.y - v0.y * v1.x > 0 )
                    dt = -dt;
                double preciseInc = double( m_Roto.m_Subdiv ) * dt /
                                    ( 2.0 * M_PI ) * RotoGetStep();
                if ( preciseInc > RotoGetStep() ||
                     preciseInc < -RotoGetStep() ) {
                    ...
                    if ( m_Roto.m_PreciseValue > RotoGetMax() ) {
                        ...
                        double da =
                            360 * ( RotoGetMax() - m_Roto.m_ValueAngle0 ) /
                            ( double( m_Roto.m_Subdiv ) * RotoGetStep() );
                        m_Roto.m_Angle0 =
                            ( ( int( ( t / ( 2.0 * M_PI ) + 1.0 ) * 360.0 +
                                     0.5 ) ) %
                              360 ) -
                            da;
                        m_Roto.m_AngleDT = da;
                        ...
                        double da =
                            360 * ( RotoGetMin() - m_Roto.m_ValueAngle0 ) /
                            ( double( m_Roto.m_Subdiv ) * RotoGetStep() );
                        m_Roto.m_Angle0 =
                            ( ( int( ( t / ( 2.0 * M_PI ) + 1.0 ) * 360.0 +
                                     0.5 ) ) %
                ...
                m_Roto.m_Angle0 =
                    ( int( ( t / ( 2.0 * M_PI ) + 1.0 ) * 360.0 + 0.5 ) ) % 360;
...
void CTwBar::RotoOnLButtonDown( int _X, int _Y ) {
    CPoint p( _X, _Y );
    if ( !m_Roto.m_Active && m_HighlightedLine >= 0 &&
         m_HighlightedLine < ( int )m_HierTags.size() &&
         m_HierTags[ m_HighlightedLine ].m_Var &&
         !m_HierTags[ m_HighlightedLine ].m_Var->IsGroup() ) {
        m_Roto.m_Var =
            static_cast< CTwVarAtom* >( m_HierTags[ m_HighlightedLine ].m_Var );
        int y = m_PosY + m_VarY0 +
                m_HighlightedLine * ( m_Font->m_CharHeight + m_LineSep ) +
                m_Font->m_CharHeight / 2;
...
bool CTwBar::EditInPlaceIsReadOnly() {
    if ( m_EditInPlace.m_Var == NULL )
        ...
        return true;
    else if ( m_EditInPlace.m_Var->m_Type == TW_TYPE_CDSTRING &&
              ( ( m_EditInPlace.m_Var->m_Ptr == NULL &&
                  m_EditInPlace.m_Var->m_SetCallback == NULL ) ||
                ( m_EditInPlace.m_Var->m_Ptr != NULL &&
                  g_TwMgr->m_CopyCDStringToClient == NULL ) ) )
        return true;
...
void CTwBar::EditInPlaceDraw() {
    ...
    int SubstrWidth = 0;
    for ( i = min( m_EditInPlace.m_CaretPos, StringLen - 1 );
          i >= 0 && SubstrWidth < m_EditInPlace.m_Width; --i ) {
    ...
    if ( m_EditInPlace.m_CaretPos == m_EditInPlace.m_FirstChar &&
         m_EditInPlace.m_FirstChar > 0 )
        --m_EditInPlace.m_FirstChar;
    SubstrWidth = 0;
    for ( i = m_EditInPlace.m_FirstChar;
          i < StringLen && SubstrWidth < m_EditInPlace.m_Width; ++i ) {
        unsigned char u = m_EditInPlace.m_String.c_str()[ i ];
        SubstrWidth += m_Font->m_CharWidth[ u ];
    }
    int LastChar = i;
    if ( SubstrWidth >= m_EditInPlace.m_Width )
        --LastChar;
    string Substr = m_EditInPlace.m_String.substr(
        m_EditInPlace.m_FirstChar, LastChar - m_EditInPlace.m_FirstChar );

    // compute caret x pos
    int CaretX = m_PosX + m_EditInPlace.m_X;
    for ( i = m_EditInPlace.m_FirstChar;
          i < m_EditInPlace.m_CaretPos && i < StringLen; ++i ) {
        unsigned char u = m_EditInPlace.m_String.c_str()[ i ];
    ...
    g_TwMgr->m_Graph->BuildText( m_EditInPlace.m_EditTextObj, &Substr, NULL,
                                 NULL, 1, m_Font, 0, m_EditInPlace.m_Width );
    g_TwMgr->m_Graph->DrawText( m_EditInPlace.m_EditTextObj,
                                m_PosX + m_EditInPlace.m_X,
                                m_PosY + m_EditInPlace.m_Y, ColText, ColBg );

    // draw selected text
    string StrSelected = "";
    if ( m_EditInPlace.m_CaretPos > m_EditInPlace.m_SelectionStart ) {
        int FirstSel =
            max( m_EditInPlace.m_SelectionStart, m_EditInPlace.m_FirstChar );
        int LastSel = min( m_EditInPlace.m_CaretPos, LastChar );
        StrSelected =
            m_EditInPlace.m_String.substr( FirstSel, LastSel - FirstSel );
    } else {
        int FirstSel =
            max( m_EditInPlace.m_CaretPos, m_EditInPlace.m_FirstChar );
    ...
    if ( SelWidth > 0 && StrSelected.length() > 0 ) {
        color32 ColSelBg =
            EditInPlaceIsReadOnly() ? m_ColValTextRO : m_ColEditSelBg;
        g_TwMgr->m_Graph->BuildText( m_EditInPlace.m_EditSelTextObj,
                                     &StrSelected, NULL, NULL, 1, m_Font, 0,
                                     SelWidth );
        if ( m_EditInPlace.m_CaretPos > m_EditInPlace.m_SelectionStart )
            g_TwMgr->m_Graph->DrawText(
                m_EditInPlace.m_EditSelTextObj, CaretX - SelWidth,
                m_PosY + m_EditInPlace.m_Y, m_ColEditSelText, ColSelBg );
        else
            g_TwMgr->m_Graph->DrawText( m_EditInPlace.m_EditSelTextObj, CaretX,
                                        m_PosY + m_EditInPlace.m_Y,
                                        m_ColEditSelText, ColSelBg );
    }

    // draw caret
    if ( CaretX <= m_PosX + m_EditInPlace.m_X + m_EditInPlace.m_Width )
        g_TwMgr->m_Graph->DrawLine(
            CaretX, m_PosY + m_EditInPlace.m_Y + 1, CaretX,
            m_PosY + m_EditInPlace.m_Y + m_Font->m_CharHeight, m_ColEditText );
}
...
bool CTwBar::EditInPlaceAcceptVar( const CTwVarAtom* _Var ) {
    ...
    if ( _Var->m_Type == TW_TYPE_CDSTRING ||
         _Var->m_Type == TW_TYPE_CDSTDSTRING ||
         _Var->m_Type == TW_TYPE_STDSTRING )
        return true;
...
void CTwBar::EditInPlaceEnd( bool _Commit ) {
    if ( _Commit && m_EditInPlace.m_Active && m_EditInPlace.m_Var != NULL ) {
        if ( m_EditInPlace.m_Var->m_Type == TW_TYPE_CDSTRING ||
             m_EditInPlace.m_Var->m_Type == TW_TYPE_CDSTDSTRING ) {
            if ( m_EditInPlace.m_Var->m_SetCallback != NULL ) {
                const char* String = m_EditInPlace.m_String.c_str();
                m_EditInPlace.m_Var->m_SetCallback(
                    &String, m_EditInPlace.m_Var->m_ClientData );
            } else if ( m_EditInPlace.m_Var->m_Type != TW_TYPE_CDSTDSTRING ) {
                char** StringPtr = ( char** )m_EditInPlace.m_Var->m_Ptr;
                if ( StringPtr != NULL &&
                     g_TwMgr->m_CopyCDStringToClient != NULL )
                    g_TwMgr->m_CopyCDStringToClient(
                        StringPtr, m_EditInPlace.m_String.c_str() );
            }
        } else if ( m_EditInPlace.m_Var->m_Type == TW_TYPE_STDSTRING ) {
            // this case should never happened: TW_TYPE_STDSTRING are converted
            // to TW_TYPE_CDSTDSTRING by TwAddVar
            if ( m_EditInPlace.m_Var->m_SetCallback != NULL )
                m_EditInPlace.m_Var->m_SetCallback(
                    &( m_EditInPlace.m_String ),
                    m_EditInPlace.m_Var->m_ClientData );
            else {
                string* StringPtr = ( string* )m_EditInPlace.m_Var->m_Ptr;
                if ( StringPtr != NULL &&
                     g_TwMgr->m_CopyStdStringToClient != NULL )
                    g_TwMgr->m_CopyStdStringToClient( *StringPtr,
                                                      m_EditInPlace.m_String );
            }
            ...
            if ( n > 0 ) {
                ...
                if ( m_EditInPlace.m_Var->m_SetCallback != NULL )
                    m_EditInPlace.m_Var->m_SetCallback(
                        m_EditInPlace.m_String.c_str(),
                        m_EditInPlace.m_Var->m_ClientData );
                else if ( m_EditInPlace.m_Var->m_Ptr != NULL ) {
                    if ( n > 1 )
                        strncpy( ( char* )m_EditInPlace.m_Var->m_Ptr,
                                 m_EditInPlace.m_String.c_str(), n - 1 );
        ...
        if ( g_TwMgr != NULL ) // Mgr might have been destroyed by the client
                               // inside a callback call
            NotUpToDate();
    }
...
bool CTwBar::EditInPlaceKeyPressed( int _Key, int _Modifiers ) {
    ...
    switch ( _Key ) {
        ...
        case TW_KEY_LEFT:
            if ( _Modifiers == TW_KMOD_SHIFT )
                m_EditInPlace.m_CaretPos =
                    max( 0, m_EditInPlace.m_CaretPos - 1 );
            else {
                if ( m_EditInPlace.m_SelectionStart !=
                     m_EditInPlace.m_CaretPos )
                    m_EditInPlace.m_CaretPos =
                        min( m_EditInPlace.m_SelectionStart,
                             m_EditInPlace.m_CaretPos );
                else
                    m_EditInPlace.m_CaretPos =
                        max( 0, m_EditInPlace.m_CaretPos - 1 );
        ...
        case TW_KEY_RIGHT:
            if ( _Modifiers == TW_KMOD_SHIFT )
                m_EditInPlace.m_CaretPos =
                    min( ( int )m_EditInPlace.m_String.length(),
                         m_EditInPlace.m_CaretPos + 1 );
            else {
                if ( m_EditInPlace.m_SelectionStart !=
                     m_EditInPlace.m_CaretPos )
                    m_EditInPlace.m_CaretPos =
                        max( m_EditInPlace.m_SelectionStart,
                             m_EditInPlace.m_CaretPos );
                else
                    m_EditInPlace.m_CaretPos =
                        min( ( int )m_EditInPlace.m_String.length(),
        ...
        case TW_KEY_BACKSPACE:
            if ( !EditInPlaceIsReadOnly() ) {
                if ( m_EditInPlace.m_SelectionStart ==
                     m_EditInPlace.m_CaretPos )
                    m_EditInPlace.m_SelectionStart =
                        max( 0, m_EditInPlace.m_CaretPos - 1 );
        ...
        case TW_KEY_DELETE:
            if ( !EditInPlaceIsReadOnly() ) {
                if ( m_EditInPlace.m_SelectionStart ==
                     m_EditInPlace.m_CaretPos )
                    m_EditInPlace.m_SelectionStart =
                        min( m_EditInPlace.m_CaretPos + 1,
                             ( int )m_EditInPlace.m_String.length() );
                EditInPlaceEraseSelect();
            }
            break;
        case TW_KEY_HOME:
        ...
        default:
            if ( _Modifiers == TW_KMOD_CTRL ) {
                ...
                if ( !EditInPlaceIsReadOnly() &&
                    ...
                    m_EditInPlace.m_String.insert( m_EditInPlace.m_CaretPos,
                                                   Str );
                    ++m_EditInPlace.m_CaretPos;
                    m_EditInPlace.m_SelectionStart = m_EditInPlace.m_CaretPos;
                }
    ...
    if ( DoCopy ) {
        string Str = "";
        if ( m_EditInPlace.m_CaretPos > m_EditInPlace.m_SelectionStart )
            Str = m_EditInPlace.m_String.substr(
                m_EditInPlace.m_SelectionStart,
                m_EditInPlace.m_CaretPos - m_EditInPlace.m_SelectionStart );
...
bool CTwBar::EditInPlaceEraseSelect() {
    ...
    if ( !EditInPlaceIsReadOnly() &&
         m_EditInPlace.m_SelectionStart != m_EditInPlace.m_CaretPos ) {
        int PosMin =
            min( m_EditInPlace.m_CaretPos, m_EditInPlace.m_SelectionStart );
        m_EditInPlace.m_String.erase(
            PosMin,
            abs( m_EditInPlace.m_CaretPos - m_EditInPlace.m_SelectionStart ) );
...
bool CTwBar::EditInPlaceMouseMove( int _X, int _Y, bool _Select ) {
    if ( !m_EditInPlace.m_Active || _Y < m_PosY + m_EditInPlace.m_Y ||
         _Y > m_PosY + m_EditInPlace.m_Y + m_Font->m_CharHeight )
        return false;

    int i, CaretX = m_PosX + m_EditInPlace.m_X;
    for ( i = m_EditInPlace.m_FirstChar;
          i < ( int )m_EditInPlace.m_String.length() &&
          CaretX < m_PosX + m_EditInPlace.m_X + m_EditInPlace.m_Width;
          ++i ) {
        unsigned char u = m_EditInPlace.m_String.c_str()[ i ];
    ...
    *_OutString = m_EditInPlace.m_Clipboard; // default implementation
    ...
    if ( TextHandle != NULL ) {
        const char* TextString =
            static_cast< char* >( GlobalLock( TextHandle ) );
        if ( TextHandle != NULL ) {
...
bool CTwBar::EditInPlaceSetClipboard( const std::string& _String ) {
    ...
    if ( g_TwMgr->m_CurrentXDisplay != NULL ) {
        XSetSelectionOwner( g_TwMgr->m_CurrentXDisplay, XA_PRIMARY, None,
                            CurrentTime );
