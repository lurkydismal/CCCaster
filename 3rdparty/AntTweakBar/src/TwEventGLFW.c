...
int TW_CALL TwEventMouseButtonGLFW( int glfwButton, int glfwAction ) {
    ...
    TwMouseAction action =
        ( glfwAction == GLFW_PRESS ) ? TW_MOUSE_PRESSED : TW_MOUSE_RELEASED;
...
int TW_CALL TwEventKeyGLFW( int glfwKey, int glfwAction ) {
    ...
    if ( glfwAction == GLFW_PRESS ) {
        int mod = g_KMod;
        int testkp =
            ( ( mod & TW_KMOD_CTRL ) || ( mod & TW_KMOD_ALT ) ) ? 1 : 0;

        if ( ( mod & TW_KMOD_CTRL ) && glfwKey > 0 &&
             glfwKey < GLFW_KEY_SPECIAL ) // CTRL cases
            ...
            else if ( testkp && glfwKey >= GLFW_KEY_KP_0 &&
                      glfwKey <= GLFW_KEY_KP_9 )
