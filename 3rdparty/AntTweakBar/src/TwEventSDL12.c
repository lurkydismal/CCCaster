...
int TW_CALL TwEventSDL12( const void* sdlEvent ) {
    ...
    switch ( event->type ) {
        case SDL_KEYDOWN:
            // This is broken
            // if( event->key.keysym.unicode!=0 && (event->key.keysym.unicode &
            // 0xFF00)==0 )
            // {
            // if( (event->key.keysym.unicode & 0xFF)<32 &&
            // (event->key.keysym.unicode & 0xFF)!=event->key.keysym.sym )
            // handled = TwKeyPressed((event->key.keysym.unicode & 0xFF)+'a'-1,
            // event->key.keysym.mod);
            // else
            // handled = TwKeyPressed(event->key.keysym.unicode & 0xFF,
            // event->key.keysym.mod);
            // }
            // else
            // handled = TwKeyPressed(event->key.keysym.sym,
            // event->key.keysym.mod);
        ...
        case SDL_MOUSEBUTTONDOWN:
                   ...
                   event->button.button == 5 ) ) // mouse wheel
            {
                ...
                handled = TwMouseButton(
                    ( event->type == SDL_MOUSEBUTTONUP ) ? TW_MOUSE_RELEASED
                                                         : TW_MOUSE_PRESSED,
