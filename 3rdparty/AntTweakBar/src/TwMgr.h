...
// var states -> use TwDefine instead: visible/iconified implemented only as
// string commands
// ANT_TWEAK_BAR_API int     ANT_CALL TwSetVarState(TwBar *bar, const char
// *name, TwState state); ANT_TWEAK_BAR_API TwState ANT_CALL TwGetVarState(const
// TwBar *bar, const char *name);

struct CTwVarGroup;
typedef void( ANT_CALL* TwStructExtInitCallback )( void* structExtValue,
                                                   void* clientData );
typedef void( ANT_CALL* TwCopyVarFromExtCallback )(
    void* structValue,
    const void* structExtValue,
    unsigned int structExtMemberIndex,
    void* clientData );
typedef void( ANT_CALL* TwCopyVarToExtCallback )(
    const void* structValue,
    void* structExtValue,
    unsigned int structExtMemberIndex,
    void* clientData );
/*ANT_TWEAK_BAR_API*/ TwType ANT_CALL
TwDefineStructExt( const char* name,
                   const TwStructMember* structExtMembers,
                   unsigned int nbExtMembers,
                   size_t structSize,
                   size_t structExtSize,
                   TwStructExtInitCallback structExtInitCallback,
                   TwCopyVarFromExtCallback copyVarFromExtCallback,
                   TwCopyVarToExtCallback copyVarToExtCallback,
                   TwSummaryCallback summaryCallback,
                   void* clientData,
                   const char* help );
typedef void( ANT_CALL* TwCustomDrawCallback )( int w,
                                                int h,
                                                void* structExtValue,
                                                void* clientData,
                                                TwBar* bar,
                                                CTwVarGroup* varGrp );
typedef bool( ANT_CALL* TwCustomMouseMotionCallback )( int mouseX,
                                                       int mouseY,
                                                       int w,
                                                       int h,
                                                       void* structExtValue,
                                                       ...
                                                       TwBar* bar,
                                                       CTwVarGroup* varGrp );
typedef bool( ANT_CALL* TwCustomMouseButtonCallback )( TwMouseButtonID button,
                                                       bool pressed,
                                                       int mouseX,
                                                       int mouseY,
                                                       int w,
                                                       ...
                                                       TwBar* bar,
                                                       CTwVarGroup* varGrp );
typedef void( ANT_CALL* TwCustomMouseLeaveCallback )( void* structExtValue,
                                                      void* clientData,
                                                      TwBar* bar );

enum ERetType { RET_ERROR = 0, RET_DOUBLE, RET_STRING };
...
struct CTwMgr {
    ...
    ERetType GetAttrib( int _AttribID,
                        std::vector< double >& outDouble,
                        std::ostringstream& outString ) const;
    void SetLastError(
        const char* _StaticErrorMesssage ); // _StaticErrorMesssage must be a
                                            // static string
    const char* GetLastError(); // returns a static string describing the error,
                                // and set LastError to NULL
    const char* CheckLastError()
        const; // returns the LastError, but does not set it to NULL
    void SetCurrentDbgParams( const char* file, int line );
    TwBar* m_PopupBar;
    // bool              IsProcessing() const            { return m_Processing);
    // void              SetProcessing(bool processing)  { m_Processing =
    // processing; }
...
const TwType TW_TYPE_CSSTRING_BASE =
    TW_TYPE_CSSTRING( 0 ); // defined as 0x30000000 (see AntTweakBar.h)
...
// const TwGraphAPI TW_OPENGL_CORE = (TwGraphAPI)5; // WIP (note: OpenGL Core
// Profil requires OpenGL 3.2 or later)
...
struct CColorExt {
    ...
    static void ANT_CALL CopyVarFromExtCB( void* _VarValue,
                                           const void* _ExtValue,
                                           unsigned int _ExtMemberIndex,
                                           void* _ClientData );
    static void ANT_CALL CopyVarToExtCB( const void* _VarValue,
                                         void* _ExtValue,
                                         unsigned int _ExtMemberIndex,
                                         void* _ClientData );
    static void ANT_CALL SummaryCB( char* _SummaryString,
                                    size_t _SummaryMaxLength,
                                    const void* _ExtValue,
                                    void* _ClientData );
...
struct CQuaternionExt {
    ...
    static void ANT_CALL SummaryCB( char* _SummaryString,
                                    ...
                                    const void* _ExtValue,
                                    void* _ClientData );
    static void ANT_CALL DrawCB( int _W,
                                 int _H,
                                 void* _ExtValue,
                                 void* _ClientData,
                                 TwBar* _Bar,
                                 CTwVarGroup* varGrp );
    static bool ANT_CALL MouseMotionCB( int _MouseX,
                                        int _MouseY,
                                        int _W,
                                        int _H,
                                        void* _StructExtValue,
                                        void* _ClientData,
                                        TwBar* _Bar,
                                        CTwVarGroup* varGrp );
    static bool ANT_CALL MouseButtonCB( TwMouseButtonID _Button,
                                        bool _Pressed,
                                        int _MouseX,
                                        int _MouseY,
                                        int _W,
                                        ...
                                        TwBar* _Bar,
                                        CTwVarGroup* varGrp );
    static void ANT_CALL MouseLeaveCB( void* _StructExtValue,
                                       void* _ClientData,
                                       TwBar* _Bar );
    static void CreateTypes();
    ...
    static void ApplyQuat( float* outX,
                           ...
                           float qz,
                           float qs );
    static void QuatFromDir( double* outQx,
                             double* outQy,
                             double* outQz,
                             double* outQs,
                             double dx,
                             double dy,
                             double dz );
    inline void Permute( float* outX,
                         float* outY,
                         float* outZ,
                         float x,
                         float y,
                         float z );
    inline void PermuteInv( float* outX,
                            float* outY,
                            float* outZ,
                            float x,
                            float y,
                            float z );
    inline void Permute( double* outX,
                         double* outY,
                         double* outZ,
                         double x,
                         double y,
                         double z );
    inline void PermuteInv( double* outX,
                            double* outY,
                            double* outZ,
                            double x,
                            double y,
...
//  (could be useful because DirectX changes it and AntTweakBar requires default
//  double precision)
//  ---------------------------------------------------------------------------
