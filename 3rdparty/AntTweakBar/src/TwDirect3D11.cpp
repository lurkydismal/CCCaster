...
static void BindFont( ID3D11Device* _Dev,
...
static void UnbindFont( ID3D11Device* _Dev,
                        ID3D11Texture2D* _Tex,
                        ID3D11ShaderResourceView* _TexRV ) {
    ( void )_Dev;
...
struct CState11 {
    ...
    ID3D11ShaderResourceView*
        m_PSShaderResourceView; // backup the first shader resource only
...
void CState11::Save() {
    ...
    // First call GetShader with NULL ClassInstances to get the number of class
    // instances. Second, if not zero allocate an array of class instances and
    // call GetShader again with this array ptr to get the class instances and
    // release the shader since its ref count has been incremented a second
    // time.

    m_CSShader = NULL;
    m_CSClassInstances = NULL;
    m_CSNumClassInstances = 0;
    m_D3DDevImmContext->CSGetShader( &m_CSShader, NULL,
                                     &m_CSNumClassInstances );
    if ( m_CSNumClassInstances > 0 ) {
        ...
        m_D3DDevImmContext->CSGetShader( &m_CSShader, m_CSClassInstances,
                                         &m_CSNumClassInstances );
        if ( m_CSShader != NULL )
            m_CSShader->Release();
    }
    ...
    m_D3DDevImmContext->DSGetShader( &m_DSShader, NULL,
                                     &m_DSNumClassInstances );
    if ( m_DSNumClassInstances > 0 ) {
        ...
        m_D3DDevImmContext->DSGetShader( &m_DSShader, m_DSClassInstances,
                                         &m_DSNumClassInstances );
        if ( m_DSShader != NULL )
            m_DSShader->Release();
    }
    ...
    m_D3DDevImmContext->GSGetShader( &m_GSShader, NULL,
                                     &m_GSNumClassInstances );
    if ( m_GSNumClassInstances > 0 ) {
        ...
        m_D3DDevImmContext->GSGetShader( &m_GSShader, m_GSClassInstances,
                                         &m_GSNumClassInstances );
        if ( m_GSShader != NULL )
            m_GSShader->Release();
    }
    ...
    m_D3DDevImmContext->HSGetShader( &m_HSShader, NULL,
                                     &m_HSNumClassInstances );
    if ( m_HSNumClassInstances > 0 ) {
        ...
        m_D3DDevImmContext->HSGetShader( &m_HSShader, m_HSClassInstances,
                                         &m_HSNumClassInstances );
        if ( m_HSShader != NULL )
            m_HSShader->Release();
    }
    ...
    m_D3DDevImmContext->PSGetShader( &m_PSShader, NULL,
                                     &m_PSNumClassInstances );
    if ( m_PSNumClassInstances > 0 ) {
        ...
        m_D3DDevImmContext->PSGetShader( &m_PSShader, m_PSClassInstances,
                                         &m_PSNumClassInstances );
        if ( m_PSShader != NULL )
            m_PSShader->Release();
    }
    ...
    m_D3DDevImmContext->VSGetShader( &m_VSShader, NULL,
                                     &m_VSNumClassInstances );
    if ( m_VSNumClassInstances > 0 ) {
        ...
        m_D3DDevImmContext->VSGetShader( &m_VSShader, m_VSClassInstances,
                                         &m_VSNumClassInstances );
        if ( m_VSShader != NULL )
            m_VSShader->Release();
    }
    ...
    m_D3DDevImmContext->IAGetIndexBuffer(
        &m_IAIndexBuffer, &m_IAIndexBufferFormat, &m_IAIndexBufferOffset );
    m_D3DDevImmContext->IAGetInputLayout( &m_IAInputLayout );
    m_D3DDevImmContext->IAGetPrimitiveTopology( &m_IATopology );
    m_D3DDevImmContext->IAGetVertexBuffers( 0, 1, &m_IAVertexBuffer,
                                            &m_IAVertexBufferStride,
                                            &m_IAVertexBufferOffset );

    // Save Ouput-Merger states
    m_D3DDevImmContext->OMGetBlendState( &m_OMBlendState, m_OMBlendFactor,
                                         &m_OMSampleMask );
    m_D3DDevImmContext->OMGetDepthStencilState( &m_OMDepthStencilState,
                                                &m_OMStencilRef );

    // Save Rasterizer states
    m_D3DDevImmContext->RSGetScissorRects( &m_RSScissorNumRects, NULL );
    if ( m_RSScissorNumRects > 0 )
        m_D3DDevImmContext->RSGetScissorRects( &m_RSScissorNumRects,
                                               m_RSScissorRects );
...
void CState11::Restore() {
    // Restore shaders
    m_D3DDevImmContext->CSSetShader( m_CSShader, m_CSClassInstances,
                                     m_CSNumClassInstances );
    m_D3DDevImmContext->DSSetShader( m_DSShader, m_DSClassInstances,
                                     m_DSNumClassInstances );
    m_D3DDevImmContext->GSSetShader( m_GSShader, m_GSClassInstances,
                                     m_GSNumClassInstances );
    m_D3DDevImmContext->HSSetShader( m_HSShader, m_HSClassInstances,
                                     m_HSNumClassInstances );
    m_D3DDevImmContext->PSSetShader( m_PSShader, m_PSClassInstances,
                                     m_PSNumClassInstances );
    ...
    m_D3DDevImmContext->VSSetShader( m_VSShader, m_VSClassInstances,
                                     m_VSNumClassInstances );
    m_D3DDevImmContext->VSSetConstantBuffers( 0, 1, &m_VSConstantBuffer );

    // Restore Input-Assembler
    m_D3DDevImmContext->IASetIndexBuffer(
        m_IAIndexBuffer, m_IAIndexBufferFormat, m_IAIndexBufferOffset );
    m_D3DDevImmContext->IASetInputLayout( m_IAInputLayout );
    m_D3DDevImmContext->IASetPrimitiveTopology( m_IATopology );
    m_D3DDevImmContext->IASetVertexBuffers( 0, 1, &m_IAVertexBuffer,
                                            &m_IAVertexBufferStride,
                                            &m_IAVertexBufferOffset );

    // Restore Ouput-Merger
    m_D3DDevImmContext->OMSetBlendState( m_OMBlendState, m_OMBlendFactor,
                                         m_OMSampleMask );
    m_D3DDevImmContext->OMSetDepthStencilState( m_OMDepthStencilState,
                                                m_OMStencilRef );

    // Restore Rasterizer states
    m_D3DDevImmContext->RSSetScissorRects( m_RSScissorNumRects,
                                           m_RSScissorRects );
...
int CTwGraphDirect3D11::Init() {
    ...
    // Create shaders
    HRESULT hr = m_D3DDev->CreateVertexShader(
        g_LineRectVS, sizeof( g_LineRectVS ), NULL, &m_LineRectVS );
    if ( FAILED( hr ) ) {
        g_TwMgr->SetLastError( g_ErrCreateVS11 );
        Shut();
        return 0;
    }
    hr = m_D3DDev->CreateVertexShader( g_LineRectCstColorVS,
                                       sizeof( g_LineRectCstColorVS ), NULL,
    ...
    hr = m_D3DDev->CreateVertexShader( g_TextVS, sizeof( g_TextVS ), NULL,
                                       &m_TextVS );
    if ( FAILED( hr ) ) {
        g_TwMgr->SetLastError( g_ErrCreateVS11 );
        Shut();
    ...
    hr = m_D3DDev->CreateVertexShader(
        g_TextCstColorVS, sizeof( g_TextCstColorVS ), NULL, &m_TextCstColorVS );
    if ( FAILED( hr ) ) {
        g_TwMgr->SetLastError( g_ErrCreateVS11 );
        Shut();
        return 0;
    }
    hr = m_D3DDev->CreatePixelShader( g_TextPS, sizeof( g_TextPS ), NULL,
                                      &m_TextPS );
    if ( FAILED( hr ) ) {
        g_TwMgr->SetLastError( g_ErrCreatePS11 );
        Shut();
    ...
    D3D11_INPUT_ELEMENT_DESC lineRectLayout[] = {
        ...
        { "COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
          offsetof( CLineRectVtx, m_Color ), D3D11_INPUT_PER_VERTEX_DATA, 0 } };
    hr = m_D3DDev->CreateInputLayout(
        lineRectLayout,
        sizeof( lineRectLayout ) / sizeof( lineRectLayout[ 0 ] ), g_LineRectVS,
        sizeof( g_LineRectVS ), &m_LineRectVertexLayout );
    if ( FAILED( hr ) ) {
    ...
    sd.BorderColor[ 0 ] = sd.BorderColor[ 1 ] = sd.BorderColor[ 2 ] =
        sd.BorderColor[ 3 ] = 0;
    ...
    D3D11_INPUT_ELEMENT_DESC textLayout[] = {
        { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0,
          D3D11_INPUT_PER_VERTEX_DATA, 0 },
        { "COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
          offsetof( CTextVtx, m_Color ), D3D11_INPUT_PER_VERTEX_DATA, 0 },
        { "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
          offsetof( CTextVtx, m_UV ), D3D11_INPUT_PER_VERTEX_DATA, 0 } };
    hr = m_D3DDev->CreateInputLayout(
        textLayout, sizeof( textLayout ) / sizeof( textLayout[ 0 ] ), g_TextVS,
        sizeof( g_TextVS ), &m_TextVertexLayout );
    if ( FAILED( hr ) ) {
        g_TwMgr->SetLastError( g_ErrCreateLayout11 );
        Shut();
        return 0;
...
int CTwGraphDirect3D11::Shut() {
    ...
    if ( m_DepthStencilState ) {
        ...
        // assert( rc==0 ); // no assert: the client can use a similar (then
        // shared) state
        ( void )rc;
        m_DepthStencilState = NULL;
    }
...
void CTwGraphDirect3D11::BeginDraw( int _WndWidth, int _WndHeight ) {
    ...
    *static_cast< D3D11_VIEWPORT* >( m_ViewportInit ) = vp;
    ...
    m_D3DDevImmContext->OMSetBlendState( m_BlendState, blendFactors,
                                         0xffffffff );
...
static inline color32 ToR8G8B8A8( color32 col ) {
    return ( col & 0xff00ff00 ) | ( ( col >> 16 ) & 0xff ) |
           ( ( col << 16 ) & 0xff0000 );
}

//  ---------------------------------------------------------------------------

void CTwGraphDirect3D11::DrawLine( int _X0,
                                   int _Y0,
                                   int _X1,
                                   int _Y1,
                                   color32 _Color0,
    ...
    HRESULT hr = m_D3DDevImmContext->Map(
        m_LineVertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource );
    if ( SUCCEEDED( hr ) ) {
        ...
        hr = m_D3DDevImmContext->Map(
            m_ConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource );
        if ( SUCCEEDED( hr ) ) {
        ...
        UINT offset = 0;
        m_D3DDevImmContext->IASetVertexBuffers( 0, 1, &m_LineVertexBuffer,
                                                &stride, &offset );

        // Set primitive topology
        m_D3DDevImmContext->IASetPrimitiveTopology(
        ...
        if ( _AntiAliased )
            m_D3DDevImmContext->RSSetState(
                m_RasterState ); // restore default raster state
    }
}

//  ---------------------------------------------------------------------------

void CTwGraphDirect3D11::DrawRect( int _X0,
                                   int _Y0,
                                   int _X1,
                                   int _Y1,
                                   color32 _Color00,
    ...
    D3D11_MAPPED_SUBRESOURCE mappedResource;
    HRESULT hr = m_D3DDevImmContext->Map(
        m_RectVertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource );
    if ( SUCCEEDED( hr ) ) {
        CLineRectVtx* vertices = ( CLineRectVtx* )mappedResource.pData;
        // Fill vertex buffer
        vertices[ 0 ].m_Pos[ 0 ] = x0;
        ...
        UINT stride = sizeof( CLineRectVtx );
        UINT offset = 0;
        m_D3DDevImmContext->IASetVertexBuffers( 0, 1, &m_RectVertexBuffer,
                                                &stride, &offset );
...
void CTwGraphDirect3D11::BuildText( void* _TextObj,
                                    const std::string* _TextLines,
                                    color32* _LineColors,
                                    color32* _LineBgColors,
                                    int _NbLines,
                                    const CTexFont* _Font,
                                    int _Sep,
                                    int _BgWidth ) {
    ...
    if ( nbTextVerts > 0 ) {
        if ( textObj->m_TextVertexBuffer == NULL ||
            ...
            textObj->m_TextVertexBufferSize =
                nbTextVerts + 6 * 256; // add a reserve of 256 characters
            D3D11_BUFFER_DESC bd;
            bd.Usage = D3D11_USAGE_DYNAMIC;
        ...
        if ( textObj->m_TextVertexBuffer != NULL ) {
            D3D11_MAPPED_SUBRESOURCE mappedResource;
            m_D3DDevImmContext->Map( textObj->m_TextVertexBuffer, 0,
    ...
    if ( nbBgVerts > 0 ) {
        if ( textObj->m_BgVertexBuffer == NULL ||
            ...
            textObj->m_BgVertexBufferSize =
                nbBgVerts + 6 * 32; // add a reserve of 32 rects
            D3D11_BUFFER_DESC bd;
            bd.Usage = D3D11_USAGE_DYNAMIC;
            bd.ByteWidth =
                textObj->m_BgVertexBufferSize * sizeof( CLineRectVtx );
            bd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
            bd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
            bd.MiscFlags = 0;
            bd.StructureByteStride = 0;
        ...
        if ( textObj->m_BgVertexBuffer != NULL ) {
            D3D11_MAPPED_SUBRESOURCE mappedResource;
            m_D3DDevImmContext->Map( textObj->m_BgVertexBuffer, 0,
                                     D3D11_MAP_WRITE_DISCARD, 0,
                                     &mappedResource );
...
void CTwGraphDirect3D11::DrawText( void* _TextObj,
                                   int _X,
                                   int _Y,
                                   color32 _Color,
                                   color32 _BgColor ) {
    ...
    if ( textObj->m_NbBgVerts >= 4 && textObj->m_BgVertexBuffer != NULL ) {
        ...
        if ( SUCCEEDED( hr ) ) {
            ...
            constants->m_Offset[ 3 ] = 0;
            Color32ToARGBf(
                _BgColor, constants->m_CstColor + 3, constants->m_CstColor + 0,
                constants->m_CstColor + 1, constants->m_CstColor + 2 );
            m_D3DDevImmContext->Unmap( m_ConstantBuffer, 0 );
        }
        ...
        UINT stride = sizeof( CLineRectVtx );
        UINT offset = 0;
        m_D3DDevImmContext->IASetVertexBuffers(
            0, 1, &textObj->m_BgVertexBuffer, &stride, &offset );
        ...
        m_D3DDevImmContext->VSSetConstantBuffers( 0, 1, &m_ConstantBuffer );
        if ( _BgColor != 0 ||
             !textObj->m_LineBgColors ) // use a constant bg color
    ...
    if ( textObj->m_NbTextVerts >= 4 && textObj->m_TextVertexBuffer != NULL ) {
        ...
        if ( SUCCEEDED( hr ) ) {
            ...
            constants->m_Offset[ 3 ] = 0;
            Color32ToARGBf(
                _Color, constants->m_CstColor + 3, constants->m_CstColor + 0,
                constants->m_CstColor + 1, constants->m_CstColor + 2 );
            m_D3DDevImmContext->Unmap( m_ConstantBuffer, 0 );
        }
        ...
        UINT offset = 0;
        m_D3DDevImmContext->IASetVertexBuffers(
            0, 1, &textObj->m_TextVertexBuffer, &stride, &offset );
...
void CTwGraphDirect3D11::ChangeViewport( int _X0,
                                         int _Y0,
                                         int _Width,
                                         int _Height,
                                         int _OffsetX,
    ...
    */
        ...
        if ( RectIsFull( m_ViewportAndScissorRects[ 1 ] ) )
            m_D3DDevImmContext->RSSetScissorRects(
                1, m_ViewportAndScissorRects ); // viewport clipping only
        else
            m_D3DDevImmContext->RSSetScissorRects( 2,
                                                   m_ViewportAndScissorRects );
...
void CTwGraphDirect3D11::RestoreViewport() {
    // m_D3DDevImmContext->RSSetViewports(1, static_cast<D3D11_VIEWPORT
    // *>(m_ViewportInit));
    m_ViewportAndScissorRects[ 0 ] = FullRect;
    m_D3DDevImmContext->RSSetScissorRects(
        1, m_ViewportAndScissorRects + 1 ); // scissor only
...
void CTwGraphDirect3D11::SetScissor( int _X0,
                                     int _Y0,
                                     int _Width,
                                     int _Height ) {
    if ( _Width > 0 && _Height > 0 ) {
        ...
        if ( RectIsFull( m_ViewportAndScissorRects[ 0 ] ) )
            m_D3DDevImmContext->RSSetScissorRects(
                1, m_ViewportAndScissorRects + 1 ); // no viewport clipping
        else
            m_D3DDevImmContext->RSSetScissorRects( 2,
                                                   m_ViewportAndScissorRects );
    } else {
        m_ViewportAndScissorRects[ 1 ] = FullRect;
        m_D3DDevImmContext->RSSetScissorRects(
            1, m_ViewportAndScissorRects ); // apply viewport clipping only
    }
}

//  ---------------------------------------------------------------------------

void CTwGraphDirect3D11::DrawTriangles( int _NumTriangles,
                                        int* _Vertices,
                                        color32* _Colors,
                                        Cull _CullMode ) {
    assert( m_Drawing == true );
    ...
    D3D11_MAPPED_SUBRESOURCE mappedResource;
    HRESULT hr =
        m_D3DDevImmContext->Map( m_TrianglesVertexBuffer, 0,
    ...
    if ( SUCCEEDED( hr ) ) {
        ...
        for ( int i = 0; i < 3 * _NumTriangles; ++i ) {
            vertices[ i ].m_Pos[ 0 ] =
                ToNormScreenX( _Vertices[ 2 * i + 0 ] + m_OffsetX, m_WndWidth );
            vertices[ i ].m_Pos[ 1 ] = ToNormScreenY(
                _Vertices[ 2 * i + 1 ] + m_OffsetY, m_WndHeight );
        ...
        UINT stride = sizeof( CLineRectVtx );
        UINT offset = 0;
        m_D3DDevImmContext->IASetVertexBuffers( 0, 1, &m_TrianglesVertexBuffer,
                                                &stride, &offset );
