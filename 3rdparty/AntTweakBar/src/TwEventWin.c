...
int TW_CALL TwEventWin( HWND wnd,
    ...
    switch ( msg ) {
        case WM_MOUSEMOVE:
            ...
            handled = TwMouseMotion( ( short )LOWORD( lParam ),
                                     ( short )HIWORD( lParam ) );
            break;
        case WM_LBUTTONDOWN:
        ...
        case WM_SYSKEYDOWN: {
            ...
            if ( k != 0 )
                ...
                // if the key will be handled at next WM_CHAR report this event
                // as handled
                int key = ( int )( wParam & 0xff );
        ...
        case WM_SYSKEYUP: {
            ...
            if ( GetAsyncKeyState( VK_MENU ) < 0 )
                kmod |= TW_KMOD_ALT;
            // if the key has been handled at previous WM_KEYDOWN report this
            // event as handled
            if ( s_PrevKeyDown == wParam && s_PrevKeyDownMod == kmod )
                handled = s_PrevKeyDownHandled;
            else {
                // if the key would have been handled report this event as
                // handled
                int key = ( int )( wParam & 0xff );
                if ( kmod & TW_KMOD_CTRL && key > 0 && key < 27 )
                    key += 'a' - 1;
        ...
        case WM_SIZE:
            ...
            // do not set 'handled', WM_SIZE may be also processed by the
            // calling application
            break;
    }
...
extern "C" {
#endif // __cplusplus
TW_EXPORT_API int TW_CALL TwEventWin32( void* wnd,
                                        unsigned int msg,
