...
#pragma warning( disable \
                 : 4505 ) // glut generates 'unreferenced function' warnings
...
int TW_GLUT_CALL TwEventMouseButtonGLUT( int glutButton,
    ...
    TwMouseAction action =
        ( glutState == GLUT_DOWN ) ? TW_MOUSE_PRESSED : TW_MOUSE_RELEASED;
...
int TwEventMouseWheelGLUT( int glutWheel, int direction, int x, int y ) {
    // We only care about the first mouse wheel for simplicity
    if ( glutWheel != 0 )
        return 0;
    ...
    return TwMouseWheel( s_WheelPos );
}
...
int TW_GLUT_CALL TwEventKeyboardGLUT( unsigned char glutKey,
                                      int mouseX,
                                      int mouseY ) {
    int kmod = 0;
    ...
    if ( ( kmod & TW_KMOD_CTRL ) &&
         ( glutKey > 0 && glutKey < 27 ) ) // CTRL special case
        glutKey += 'a' - 1;
