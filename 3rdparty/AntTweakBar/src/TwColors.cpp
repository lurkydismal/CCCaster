...
void ColorRGBToHLSi( int _R,
                     ...
                     int* _Saturation ) {
    float h, l, s;
    ColorRGBToHLSf( ( 1.0f / 255.0f ) * float( _R ),
                    ( 1.0f / 255.0f ) * float( _G ),
                    ( 1.0f / 255.0f ) * float( _B ), &h, &l, &s );
    if ( _Hue )
        *_Hue = ( int )TClamp( h * ( 256.0f / 360.0f ), 0.0f, 255.0f );
    if ( _Light )
        *_Light = ( int )TClamp( l * 256.0f, 0.0f, 255.0f );
    if ( _Saturation )
        *_Saturation = ( int )TClamp( s * 256.0f, 0.0f, 255.0f );
}
...
void ColorHLSToRGBi( int _Hue,
                     ...
                     int* _B ) {
    float r, g, b;
    ColorHLSToRGBf( ( 360.0f / 255.0f ) * float( _Hue ),
                    ( 1.0f / 255.0f ) * float( _Light ),
                    ( 1.0f / 255.0f ) * float( _Saturation ), &r, &g, &b );
    if ( _R )
...
color32 ColorBlend( color32 _Color1, color32 _Color2, float _S ) {
    ...
    float t = 1.0f - _S;
    return Color32FromARGBf( t * a1 + _S * a2, t * r1 + _S * r2,
                             t * g1 + _S * g2, t * b1 + _S * b2 );
}
