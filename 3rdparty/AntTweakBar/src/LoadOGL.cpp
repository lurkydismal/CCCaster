...
int LoadOpenGL() {
    ...
    if ( g_OGLModule ) {
        ...
        for ( int i = 0; i < g_NbOGLFunc; ++i ) {
            ...
            *( g_OGLFuncRec[ i ].m_FuncPtr ) =
                reinterpret_cast< GL::PFNOpenGL >(
                    GetProcAddress( g_OGLModule, g_OGLFuncRec[ i ].m_Name ) );
            if ( *( g_OGLFuncRec[ i ].m_FuncPtr ) == NULL )
                Res = 0; // Error("cannot find OpenGL function");
        }

        _glGetProcAddress =
            reinterpret_cast< GL::PFNGLGetProcAddress >( _wglGetProcAddress );
...
PFNOpenGL Record( const char* _FuncName, PFNOpenGL* _FuncPtr ) {
    if ( g_NbOGLFunc >= ANT_NB_OGL_FUNC_MAX ) {
        fprintf(
            stderr,
            "Too many OpenGL functions declared. Change ANT_NB_OGL_FUNC_MAX." );
        exit( -1 );
    }
...
int LoadOpenGL() {
    _glGetProcAddress =
        reinterpret_cast< GL::PFNGLGetProcAddress >( glXGetProcAddressARB );
...
int LoadOpenGL() {
    _glGetProcAddress =
        reinterpret_cast< GL::PFNGLGetProcAddress >( NSGLGetProcAddressNew );
