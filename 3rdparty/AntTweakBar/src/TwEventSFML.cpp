...
int TW_CALL TwEventSFML( const void* sfmlEvent,
                         unsigned char majorVersion,
                         unsigned char minorVersion ) {
    // Assume version 1.6 (will possibly not work for version != 1.6, but give
    // it a chance)
    /*
    ...
    */
    ...
    switch ( event->Type ) {
        ...
        case sf::Event::TextEntered:
            if ( !s_PreventTextHandling && event->Text.Unicode != 0 &&
                 ( event->Text.Unicode & 0xFF00 ) == 0 ) {
                if ( ( event->Text.Unicode & 0xFF ) < 32 ) // CTRL+letter
                    handled =
                        TwKeyPressed( ( event->Text.Unicode & 0xFF ) + 'a' - 1,
                                      TW_KMOD_CTRL | s_KMod );
                else
        ...
        case sf::Event::MouseButtonReleased:
            mouseAction = ( event->Type == sf::Event::MouseButtonPressed )
                              ? TW_MOUSE_PRESSED
                              : TW_MOUSE_RELEASED;
        ...
        case sf::Event::Resized:
            ...
            // do not set 'handled', sf::Event::Resized may be also processed by
