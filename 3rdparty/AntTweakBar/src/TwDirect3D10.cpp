...
typedef HRESULT( WINAPI* D3D10CompileEffectFromMemoryProc )(
    void* pData,
    SIZE_T DataLength,
    LPCSTR pSrcFileName,
    ...
    ID3D10Blob** ppErrors );
typedef HRESULT( WINAPI* D3D10CreateEffectFromMemoryProc )(
    void* pData,
    SIZE_T DataLength,
    UINT FXFlags,
...
typedef HRESULT( WINAPI* D3D10StateBlockMaskEnableAllProc )(
    D3D10_STATE_BLOCK_MASK* pMask );
typedef HRESULT( WINAPI* D3D10CreateStateBlockProc )(
    ID3D10Device* pDevice,
...
static int LoadDirect3D10() {
    ...
    if ( g_D3D10Module ) {
        ...
        _D3D10CompileEffectFromMemory =
            reinterpret_cast< D3D10CompileEffectFromMemoryProc >(
                GetProcAddress( g_D3D10Module,
                                "D3D10CompileEffectFromMemory" ) );
        if ( _D3D10CompileEffectFromMemory == NULL )
            res = 0;
        _D3D10CreateEffectFromMemory =
            reinterpret_cast< D3D10CreateEffectFromMemoryProc >( GetProcAddress(
                g_D3D10Module, "D3D10CreateEffectFromMemory" ) );
        if ( _D3D10CreateEffectFromMemory == NULL )
            res = 0;
        _D3D10StateBlockMaskEnableAll =
            reinterpret_cast< D3D10StateBlockMaskEnableAllProc >(
                GetProcAddress( g_D3D10Module,
                                "D3D10StateBlockMaskEnableAll" ) );
        if ( _D3D10StateBlockMaskEnableAll == NULL )
            res = 0;
        _D3D10CreateStateBlock = reinterpret_cast< D3D10CreateStateBlockProc >(
            GetProcAddress( g_D3D10Module, "D3D10CreateStateBlock" ) );
...
static ID3D10ShaderResourceView* BindFont(
    ID3D10Device* _Dev,
    ID3D10EffectShaderResourceVariable* _ResVar,
...
static void UnbindFont( ID3D10Device* _Dev,
                        ID3D10EffectShaderResourceVariable* _ResVar,
                        ID3D10ShaderResourceView* _TexRV ) {
    ( void )_Dev;
...
char g_ShaderFX[] =
    ...
    " } }\n"
    " Texture2D Font; \n"
    " SamplerState FontSampler { Filter = MIN_MAG_MIP_POINT; AddressU = "
    "BORDER; AddressV = BORDER; BorderColor=float4(0, 0, 0, 0); }; \n"
    " struct TextPSInput { float4 Pos : SV_POSITION; float4 Color : COLOR0; "
    "float2 Tex : TEXCOORD0; }; \n"
    " TextPSInput TextVS(float4 pos : POSITION, float4 color : COLOR, float2 "
    "tex : TEXCOORD0, uniform bool useCstColor) { \n"
    "   TextPSInput ps; ps.Pos = pos + g_Offset; \n"
    "   ps.Color = useCstColor ? g_CstColor : color; ps.Tex = tex; return ps; "
    "} \n"
    " float4 TextPS(TextPSInput input) : SV_Target { return "
    "Font.Sample(FontSampler, input.Tex)*input.Color; } \n"
...
int CTwGraphDirect3D10::Init() {
    ...
    HRESULT hr = _D3D10CompileEffectFromMemory(
        g_ShaderFX, strlen( g_ShaderFX ), "AntTweakBarFX", NULL, NULL,
        shaderFlags, 0, &compiledFX, &errors );
    if ( FAILED( hr ) ) {
        const size_t ERR_MSG_MAX_LEN = 4096;
        static char s_ErrorMsg[ ERR_MSG_MAX_LEN ]; // must be static to be sent
                                                   // to SetLastError
    ...
    hr = _D3D10CreateEffectFromMemory( compiledFX->GetBufferPointer(),
                                       compiledFX->GetBufferSize(), 0, m_D3DDev,
                                       NULL, &m_Effect );
    compiledFX->Release();
    if ( FAILED( hr ) ) {
    ...
    if ( m_LineRectTech == NULL || m_TextTech == NULL ||
         m_LineRectCstColorTech == NULL || m_TextCstColorTech == NULL ) {
    ...
    D3D10_INPUT_ELEMENT_DESC lineRectLayout[] = {
        ...
        { "COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
          offsetof( CLineRectVtx, m_Color ), D3D10_INPUT_PER_VERTEX_DATA, 0 } };
    ...
    D3D10_INPUT_ELEMENT_DESC textLayout[] = {
        { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0,
          D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
          offsetof( CTextVtx, m_Color ), D3D10_INPUT_PER_VERTEX_DATA, 0 },
        { "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
          offsetof( CTextVtx, m_UV ), D3D10_INPUT_PER_VERTEX_DATA, 0 } };
    hr = m_TextTech->GetPassByIndex( 0 )->GetDesc( &passDesc );
    if ( SUCCEEDED( hr ) )
        hr = m_D3DDev->CreateInputLayout(
            textLayout, sizeof( textLayout ) / sizeof( textLayout[ 0 ] ),
            passDesc.pIAInputSignature, passDesc.IAInputSignatureSize,
            &m_TextVertexLayout );
    if ( FAILED( hr ) ) {
        g_TwMgr->SetLastError( g_ErrCreateLayout10 );
    ...
    if ( m_Effect->GetVariableByName( "Font" ) )
        m_FontD3DResVar =
            m_Effect->GetVariableByName( "Font" )->AsShaderResource();
...
void CTwGraphDirect3D10::DrawLine( int _X0,
                                   int _Y0,
                                   int _X1,
                                   int _Y1,
                                   color32 _Color0,
    ...
    HRESULT hr = m_LineVertexBuffer->Map( D3D10_MAP_WRITE_DISCARD, 0,
                                          ( void** )&vertices );
    if ( SUCCEEDED( hr ) ) {
        // Fill vertex buffer
        ...
        UINT offset = 0;
        m_D3DDev->IASetVertexBuffers( 0, 1, &m_LineVertexBuffer, &stride,
                                      &offset );
...
void CTwGraphDirect3D10::DrawRect( int _X0,
                                   int _Y0,
                                   int _X1,
                                   int _Y1,
                                   color32 _Color00,
    ...
    CLineRectVtx* vertices = NULL;
    HRESULT hr = m_RectVertexBuffer->Map( D3D10_MAP_WRITE_DISCARD, 0,
                                          ( void** )&vertices );
    if ( SUCCEEDED( hr ) ) {
        // Fill vertex buffer
        vertices[ 0 ].m_Pos[ 0 ] = x0;
        ...
        UINT stride = sizeof( CLineRectVtx );
        UINT offset = 0;
        m_D3DDev->IASetVertexBuffers( 0, 1, &m_RectVertexBuffer, &stride,
                                      &offset );

        // Set primitive topology
        m_D3DDev->IASetPrimitiveTopology(
...
void CTwGraphDirect3D10::BuildText( void* _TextObj,
    ...
    if ( nbTextVerts > 0 ) {
        ...
        if ( textObj->m_TextVertexBuffer != NULL )
            textObj->m_TextVertexBuffer->Map( D3D10_MAP_WRITE_DISCARD, 0,
                                              ( void** )&textVerts );
    }
    ...
    if ( nbBgVerts > 0 ) {
        ...
        if ( textObj->m_BgVertexBuffer != NULL )
            textObj->m_BgVertexBuffer->Map( D3D10_MAP_WRITE_DISCARD, 0,
                                            ( void** )&bgVerts );
    }
...
void CTwGraphDirect3D10::DrawText( void* _TextObj,
                                   int _X,
                                   int _Y,
                                   color32 _Color,
                                   color32 _BgColor ) {
    ...
    if ( textObj->m_NbBgVerts >= 4 && textObj->m_BgVertexBuffer != NULL ) {
        ...
        UINT stride = sizeof( CLineRectVtx );
        UINT offset = 0;
        m_D3DDev->IASetVertexBuffers( 0, 1, &textObj->m_BgVertexBuffer, &stride,
                                      &offset );
    ...
    if ( textObj->m_NbTextVerts >= 4 && textObj->m_TextVertexBuffer != NULL ) {
        ...
        UINT offset = 0;
        m_D3DDev->IASetVertexBuffers( 0, 1, &textObj->m_TextVertexBuffer,
                                      &stride, &offset );
...
void CTwGraphDirect3D10::ChangeViewport( int _X0,
                                         int _Y0,
                                         int _Width,
                                         int _Height,
                                         int _OffsetX,
...
void CTwGraphDirect3D10::RestoreViewport() {
    // m_D3DDev->RSSetViewports(1, static_cast<D3D10_VIEWPORT
    // *>(m_ViewportInit));
    m_ViewportAndScissorRects[ 0 ] = FullRect;
    m_D3DDev->RSSetScissorRects(
...
void CTwGraphDirect3D10::SetScissor( int _X0,
                                     int _Y0,
                                     int _Width,
                                     int _Height ) {
    if ( _Width > 0 && _Height > 0 ) {
...
void CTwGraphDirect3D10::DrawTriangles( int _NumTriangles,
                                        int* _Vertices,
                                        color32* _Colors,
                                        Cull _CullMode ) {
    assert( m_Drawing == true );
    ...
    CLineRectVtx* vertices = NULL;
    HRESULT hr = m_TrianglesVertexBuffer->Map( D3D10_MAP_WRITE_DISCARD, 0,
                                               ( void** )&vertices );
    if ( SUCCEEDED( hr ) ) {
        // Fill vertex buffer
        ...
        UINT stride = sizeof( CLineRectVtx );
        UINT offset = 0;
        m_D3DDev->IASetVertexBuffers( 0, 1, &m_TrianglesVertexBuffer, &stride,
                                      &offset );
