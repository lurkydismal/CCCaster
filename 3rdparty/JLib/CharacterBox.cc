...
CharacterBox::CharacterBox( COORD upperLeft,
                            COORD lowerRight,
...
void CharacterBox::Draw() const {
    /*
        ...
        for(x = 0; x < xRange; x++)
        {
                pCore->Prints(buffer,FALSE,&m_border,m_upperLeft.X +
       x,m_upperLeft.Y); pCore->Prints(buffer,FALSE,&m_border,m_upperLeft.X +
       x,m_lowerRight.Y-1);
        }
        for(y = 1; y < yRange; y++)
        {
                pCore->Prints(buffer,FALSE,&m_border,m_upperLeft.X,
       m_upperLeft.Y + y);
                pCore->Prints(buffer,FALSE,&m_border,m_lowerRight.X-1,
       m_upperLeft.Y + y);
        }
        ...
        for(x = 1; x < (xRange-1); x++)
        {
                for(y = 1; y < (yRange-1); y++)
                        pCore->Prints(" ",FALSE,&m_client,m_upperLeft.X + x,
       m_upperLeft.Y + y);
        }
...
void CharacterBox::Draw( COORD upperLeft, COORD lowerRight, char fill ) {
    /*
        ...
        for(y = 1; y < yRange; y++)
        {
                ...
                pCore->Prints(buffer,FALSE,NULL,lowerRight.X-1, upperLeft.Y +
       y);
        }
        for(x = 1; x < (xRange-1); x++)
        {
                for(y = 1; y < (yRange-1); y++)
                        pCore->Prints(" ",FALSE,NULL ,upperLeft.X + x,
       upperLeft.Y + y);
        }
    */
...
void CharacterWindow::Draw() const {
    ...
    clientBox.Draw();
    if ( m_title.size() )
        ConsoleCore::GetInstance()->Prints( m_title, FALSE, &ClientColor(),
