...
int main( int argc, char* argv[] ) {
    ...
    WindowedMenu menu( menuOrigin, numberOfMainMenuItems, "JLib Demo",
                       ConsoleFormat::BRIGHTRED );
...
void WriteString() {
    ...
    pCore->Prints( "Press any key to return to the main menu.", FALSE, &ccf, 1,
                   2 );
    pCore->Color( &ccfOld );
...
void PrintNumber() {
    ConsoleCore* pCore = ConsoleCore::GetInstance();
    ConsoleFormat ccfOld, border = ConsoleFormat::BRIGHTYELLOW,
                          client = ConsoleFormat::ONBLUE;
    ConsoleFormat inputColoring =
        ( ConsoleFormat::BRIGHTYELLOW | ConsoleFormat::ONBLUE );
    COORD ul = COORD(), br = { 43, 4 };
    CharacterBox box( ul, br, border, client );
    DWORD number = 12;
    box.Draw();
    ccfOld = pCore->Color();
    pCore->Printn( number, FALSE, &inputColoring, 1, 1 );
    pCore->Prints( "Press any key to return to the main menu.", FALSE,
                   &inputColoring, 1, 2 );
    pCore->Color( &ccfOld );
    pCore->Wait();
}
...
void Coloring() {
    ...
    pCore->Prints( "These are the palette indices for all color combinations.",
                   FALSE, NULL, 6, 21 );
    pCore->Prints( "Use them with Prints(\"$###\",...) auto format syntax.",
                   FALSE, NULL, 6, 22 );
    pCore->Prints( "Press any key to return to the main menu.", FALSE, NULL, 6,
                   23 );
    pCore->Wait();
}
...
void FormatOutput() {
    ...
    ConsoleFormat ccfString( ConsoleFormat::YELLOW | ConsoleFormat::ONBLUE ),
        ...
        ccf( ConsoleFormat::BRIGHTYELLOW | ConsoleFormat::ONBLUE ), ccfOld, red,
        green, blue;
    ...
    COORD ul = COORD(), br = { 49, 7 };
    CharacterBox box( ul, br, ConsoleFormat::BRIGHTYELLOW,
                      ConsoleFormat::ONBLUE );
    ccfOld = pCore->Color();
    box.Draw();
...
void ReadString() {
    ...
    ConsoleFormat ccf( ConsoleFormat::BRIGHTYELLOW | ConsoleFormat::ONBLUE ),
        ccfOld;
    COORD scanPosition = { 1, 2 }, ul = { 0, 0 }, br = { 43, 6 };
    CharacterBox box( ul, br, ConsoleFormat::BRIGHTYELLOW,
                      ConsoleFormat::ONBLUE );
    box.Draw();
    ccfOld = pCore->Color( &ccf );
    pCore->Prints( "Enter a string of up to 20 characters.", FALSE, NULL, 1,
                   1 );
...
void ReadNumber() {
    ...
    ConsoleFormat ccf( ConsoleFormat::BRIGHTYELLOW | ConsoleFormat::ONBLUE ),
        ccfOld;
    COORD scanPosition = { 1, 2 }, ul = { 0, 0 }, br = { 46, 6 };
    CharacterBox box( ul, br, ConsoleFormat::BRIGHTYELLOW,
                      ConsoleFormat::ONBLUE );
    int num = 0;
    box.Draw();
    ccfOld = pCore->Color( &ccf );
    pCore->Prints( "Enter a number of up to 10 digits in length.", TRUE, NULL,
                   1, 1 );
    pCore->CursorPosition( &scanPosition );
...
void DrawBox() {
    ...
    pCore->Color( &old );
    pCore->Prints( "A charbox encompassing the entire screen (79,24)", TRUE,
                   NULL, 1, 21 );
    ...
    CharacterBox box =
        CharacterBox( upperLeft, lowerRight, ccfBorder, ccfClient, fill );
    box.Draw();
    pCore->Prints(
        "A charbox encompassing the entire screen using specific "
        "client/borders",
        TRUE, NULL, 1, 21 );
    pCore->Prints( "Press any key to return to the main menu.", FALSE, NULL,
                   1 );
    pCore->Wait();
}
...
void DrawWindowTitle() {
    ...
    COORD ul = { 31, 0 }, br = { 37, 6 };
    ConsoleFormat client = ConsoleFormat::BRIGHTYELLOW, old = pCore->Color();
    ...
    ConsoleFormat border =
        ConsoleFormat::BRIGHTRED | ConsoleFormat::ONBRIGHTPURPLE;
    client = ConsoleFormat::BRIGHTYELLOW | ConsoleFormat::ONBRIGHTCYAN;
    window.ClientColor( client );
    window.BorderColor( border );
    window.Draw();
    pCore->Color( &old );
    pCore->Prints( "Press any key to return to the main menu.", FALSE, NULL, 0,
                   23 );
    pCore->Wait();
}
...
void DoMenu() {
    COORD origin = { 31, 0 };
    ConsoleFormat ccfRed = ConsoleFormat::BRIGHTRED, ccfOld;
    ...
    do {
        switch ( menu.Show() ) {
            case USERESC:
                ...
                pCore->Prints( "Escape was pressed", FALSE, NULL, origin.X,
                               origin.Y + 5 );
                pCore->Prints( "Press any key to return to the main menu.",
                               FALSE, NULL, origin.X, origin.Y + 6 );
                pCore->Color( &ccfOld );
                pCore->Wait();
                return;
            case BADMENU:
                ccfOld = pCore->Color( &ccfRed );
                pCore->Prints( "This menu sucks and cannot be shown", FALSE,
                               NULL, origin.X, origin.Y + 5 );
                pCore->Prints( "Press any key to return to the main menu.",
                               FALSE, NULL, origin.X, origin.Y + 6 );
                pCore->Color( &ccfOld );
                pCore->Wait();
            ...
            default:
                ...
                pCore->Prints( "You selected the item with the label: ", FALSE,
                               NULL, origin.X, origin.Y + 3 );
                pCore->Prints( selection.Text() );
                pCore->Prints( "Press any key to restart the menu.", FALSE,
                               NULL, origin.X, origin.Y + 5 );
                pCore->Wait();
                break;
        }
...
void DoWindowedMenu() {
    ConsoleFormat ccfRed = ConsoleFormat::BRIGHTRED,
                  ccfClient = ConsoleFormat::BRIGHTWHITE, ccfOld;
    COORD origin = { 31, 0 };
        ...
        switch ( menu.Show() ) {
            case USERESC:
                ...
                pCore->Prints( "Escape was pressed", FALSE, NULL, origin.X,
                               origin.Y + 12 );
                pCore->Prints( "Press any key to return to the main menu.",
                               FALSE, NULL, origin.X, origin.Y + 13 );
                pCore->Color( &ccfOld );
                pCore->Wait();
                return;
            case BADMENU:
                ccfOld = pCore->Color( &ccfRed );
                pCore->Prints( "This menu sucks and cannot be shown", FALSE,
                               NULL, origin.X, origin.Y + 10 );
                pCore->Prints( "Press any key to return to the main menu.",
                               FALSE, NULL, origin.X, origin.Y + 11 );
                pCore->Color( &ccfOld );
                pCore->Wait();
                return;
            default:
...
void CustomBuffers() {
    COORD bufferSize = { 6, 4 }, saveOrigin = { 31, 0 }, loadOrigin = { 31, 8 },
          lowerRight;
    lowerRight.X = saveOrigin.X + bufferSize.X;
    lowerRight.Y = bufferSize.Y;
    ConsoleCore* pCore = ConsoleCore::GetInstance();
    CharacterBox box( saveOrigin, lowerRight, ConsoleFormat::BRIGHTYELLOW,
                      ConsoleFormat::ONBLUE );
    ...
    pCore->Prints( "...and the copies.", FALSE, NULL, loadOrigin.X,
                   loadOrigin.Y - 1 );
    for ( loadOrigin.X; loadOrigin.X + bufferSize.X < MAXSCREENX - 1;
          loadOrigin.X += bufferSize.X ) {
        for ( loadOrigin.Y = 8; loadOrigin.Y + 4 < MAXSCREENY - 1;
              loadOrigin.Y += bufferSize.Y ) {
            pCore->LoadScreen( buffer, bufferSize, loadOrigin );
        }
    }
    pCore->Prints( "Press any key to return to the main menu.", FALSE, NULL,
                   saveOrigin.X, loadOrigin.Y );
    pCore->Wait();
    delete[] buffer;
}
...
void ScanFloat() {
    ...
    ConsoleFormat ccf( ConsoleFormat::BRIGHTYELLOW | ConsoleFormat::ONBLUE ),
        ccfOld;
    COORD ul = { 0, 0 }, br = { 44, 6 }, org = { 1, 2 };
    CharacterBox box( ul, br, ConsoleFormat::BRIGHTYELLOW,
                      ConsoleFormat::ONBLUE );
    box.Draw();
    ccfOld = pCore->Color( &ccf );
    pCore->Prints( "argument digits is set to 4; enter a float", FALSE, NULL, 1,
                   1 );
