...
vector< unsigned > FindCodeLocations( string text ) {
    ...
    for ( unsigned x = 0; x < text.length(); x++ ) {
        if ( text[ x ] == '$' ) {
            nextCode = text.begin();
            advance( nextCode, x );
            string::iterator end = nextCode, codeStart = nextCode;
            advance( codeStart, 1 );
            if ( ( x + 3 ) >= text.length() ) {
...
CodeFinder<>::CodePhraseVector RemoveCodes(
    ...
    for ( ; it != end; it++ ) {
        ...
        string toRemove;
        string::iterator strIt = text.begin(), strEnd;
        advance( strIt, *it );
        strEnd = strIt;
        advance( strEnd, 4 );
        copy( strIt, strEnd, inserter( toRemove, toRemove.begin() ) );
        copy( strEnd, text.end(),
              inserter( remainingPhrase, remainingPhrase.begin() ) );
        CodeFinder< string >::CodePhrasePair next( toRemove, remainingPhrase );
        codesAndPhrases.push_back( next );
        text.erase( strIt, text.end() );
    }
    if ( text.length() != 0 ) {
        // Remaining text will have no code (means use whatever the default
        // color is for the console)
