...
void ConsoleCore::ClearScreen() {
    ...
    memset( ( void* )lpAttributes, 0x00,
            sizeof( WORD ) * MAXSCREENX * MAXSCREENY );
...
#endif
    UpdateWindowSize();
    COORD bufferSize = { MAXSCREENX, MAXSCREENY }, bufferOrigin = { 0, 0 };
    SMALL_RECT rectToRead = { 0, 0, MAXSCREENX, MAXSCREENY };
    if ( m_screenBuffer.size() != ( size_t )MAXSCREENX * MAXSCREENY )
        m_screenBuffer.resize( MAXSCREENX * MAXSCREENY );
    ReadConsoleOutput( m_consoleHandle, &m_screenBuffer[ 0 ], bufferSize,
                       bufferOrigin, &rectToRead );
#ifdef JLIB_MUTEXED
    pthread_mutex_unlock( &mutex );
#endif
}

void ConsoleCore::SaveScreen( PCHAR_INFO buffer,
                              COORD bufferSize,
                              COORD saveOrigin ) {
#ifdef JLIB_MUTEXED
    pthread_mutex_lock( &mutex );
...
void ConsoleCore::LoadScreen() {
    ...
    WriteConsoleOutput( m_consoleHandle, &m_screenBuffer[ 0 ], bufferSize,
                        bufferOrigin, &rectToWrite );
#ifdef JLIB_MUTEXED
    pthread_mutex_unlock( &mutex );
#endif
}

void ConsoleCore::LoadScreen( PCHAR_INFO buffer,
                              COORD bufferSize,
                              COORD loadOrigin ) {
#ifdef JLIB_MUTEXED
    pthread_mutex_lock( &mutex );
...
void ConsoleCore::Printn( int number,
                          BOOL endLine,
                          ConsoleFormat* color,
                          SHORT x,
                          SHORT y ) {
...
void ConsoleCore::Prints( const string& text,
                          BOOL endLine,
...
void ConsoleCore::_Prints( const string& text,
                           BOOL endLine,
                           const ConsoleFormat* color,
                           SHORT x,
                           SHORT y ) {
    ...
    if ( stringLength != 0 ) {
        ...
        CodeFinder<>::PhraseLocationVector::iterator
            it = codes.m_phrasesAndLocations.begin(),
            end = codes.m_phrasesAndLocations.end();
        for ( ; it != end; ++it ) {
            CodeFinder<>::CodePhraseVector codesAndPairs = RemoveCodes( it );
            CodeFinder<>::CodePhraseVector::reverse_iterator
                rIt = codesAndPairs.rbegin(),
            ...
            for ( ; rIt != rEnd; ++rIt ) {
                ...
                memset( ( void* )lpAttributes, colorCode.Color(),
                        sizeof( WORD ) * stringLength );
                WriteConsoleOutputAttribute( m_consoleHandle, lpAttributes,
                                             stringLength, m_cursorPosition,
                                             &whatWasWritten );

                delete[] lpAttributes;
                WriteConsoleOutputCharacter(
                    m_consoleHandle, rIt->second.c_str(), stringLength,
                    m_cursorPosition, &whatWasWritten );
...
bool ConsoleCore::ScanNumber( COORD origin,
                              int& number,
                              int width,
                              bool allowNegative,
                              bool hasDefault,
        ...
        switch ( ch ) {
            ...
            case KB_EXTENDED_KEY: // control key
                ...
                if ( ch == HOME_KEY ) {
                    ...
                    buffer.clear();
                    pos = 0;
                } else if ( !buffer.empty() && pos < buffer.size() &&
                            ch == DELETE_KEY ) {
                    buffer.erase( pos, 1 );
                }
                break;
            default: // digit or first '-'
                ...
                if ( ( isdigit( ch ) && buffer.size() < width ) ||
                     ( ch == '-' && pos == 0 ) )
