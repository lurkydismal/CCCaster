...
public:
    ...
    //		Wait
    // Hangs program execution until any key is pressed, returns false if
    // ESCAPE_KEY was pressed
    bool Wait();
    ...
    // If you enter a digit outside of the range you may not get the desired
    // result. Additionally, because FPUs are fickle the number you enter may
    // not always be the precisely the one that is returned through the out
    // parameter.
...
private:
    ...
    ConsoleFormat
        m_currentFormat; // What format will be used when none is specified.
