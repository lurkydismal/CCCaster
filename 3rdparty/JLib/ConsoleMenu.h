...
public:
    ...
    //      ConsoleFormat selectionFormat:  The color of the item that is
    //      selected.
    //
    ConsoleMenu( COORD origin,
    ...
    //      Iterator where: the item preceeding the location to put the new
    //      item.
    //  Notes:
    ...
    //      GetEnd
    //  Gets a constant iterator into the items collection that is one past the
    //  end of the item collection elements. Returns: a constant iterator.
    //  Notes:
    ...
    //      SelectedValue
    //  After calling Show, this will return the user defined value of the
    //  selected item. Returns: the user defined value of the selected item.
    //  Notes:
    //      You should always check the return value of Show before
    //  calling this method.  Otherwise the behavior is undefined.
...
protected:
    ...
    int m_enableDelete;  // Enables deleting menu items, 1 for just delete/BS, 2
                         // for delete/BS/left/right
};
...
public:
    ...
    //      unsigned maxToShow: The number of menu items that can be displayed
    //      at one time. ConsoleFormat format:   The color of unselected items.
    //      ConsoleFormat selectionFormat:  The color of the selected item.
    //  Notes:
    //      If the maxToShow is equal to or less than the number of items in the
    //      menu
    //  then the behavior of Show is undefined.
    ScrollingMenu( COORD origin,
                   unsigned maxToShow,
                   ConsoleFormat format = ConsoleFormat::SYSTEM,
                   ConsoleFormat selectionFormat = ~ConsoleFormat::SYSTEM );
    ...
    //  Notes:
    //      If this value is less than or equal to the number of menu items the
    //      behavior
...
private:
    unsigned
        m_maxToShow; // The number of items that can be displayed at one time.
...
public:
    ...
    //      ConsoleFormat clientColor:  The color used to draw the inside of the
    //      window and the title.
    //  Notes:
    //      If maxToShow is zero, the window will be created as unscrollable,
    //      otherwise
    //  it will be marked as scrollable.
    //  If the maxToShow is equal to or less than the number of items in the
    //  menu then the behavior of Show is undefined.
    WindowedMenu( COORD origin,
                  unsigned maxToShow,
    ...
    //  Arguments:
    //      ConsoleFormat color:    the color used to draw the border of the
    //      window.
    ...
    //  Arguments:
    //      ConsoleFormat color:    the color used to draw the inside of the
    //      window and the title.
    ...
    //  If maxToShow is less than or equal to the number of menu items the
    //  behavior of Show will be undefined.
    virtual DWORD Show();
...
private:
    ...
    std::string m_title;
    BOOL m_scrollable;
    ConsoleFormat m_windowColor, m_clientColor;
    char m_fill;
};
#endif
