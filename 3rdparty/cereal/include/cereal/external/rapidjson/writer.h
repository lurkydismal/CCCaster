...
/*! Writer implements the concept Handler.
        ...
        User may programmatically calls the functions of a writer to generate
   JSON text.
...
#if _MSC_VER
        ( void )sprintf_s( double_format, sizeof( double_format ), "%%0.%dg",
                           precision );
        ( void )sprintf_s( long_double_format, sizeof( long_double_format ),
                           "%%0.%dLg", precision );
#else
...
#undef Z16
        ...
        for ( const Ch* p = str; p != str + length; ++p ) {
            if ( ( sizeof( Ch ) == 1 || characterOk( *p ) ) &&
                 escape[ ( unsigned char )*p ] ) {
                // if ((sizeof(Ch) == 1 || *p < 256) && escape[(unsigned
                // char)*p])  {
                stream_.Put( '\\' );
    ...
    void Prefix( Type type ) {
        ...
        if ( level_stack_.GetSize() != 0 ) { // this value is not at root
            ...
            if ( level->valueCount > 0 ) {
                if ( level->inArray )
                    stream_.Put( ',' ); // add comma if it is not the first
                                        // element in array
                else                    // in object
