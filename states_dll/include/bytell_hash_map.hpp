...
template < typename T >
constexpr size_t
    sherwood_v8_constants< T >::jump_distances[ num_jump_distances ];
...
struct sherwood_v8_block {
    ...
    int first_empty_index() const {
        for ( int i = 0; i < BlockSize; ++i ) {
            if ( control_bytes[ i ] ==
                 sherwood_v8_constants<>::magic_for_empty )
                return i;
        }
        return -1;
    }
    ...
    void fill_control_bytes( int8_t value ) {
        std::fill( std::begin( control_bytes ), std::end( control_bytes ),
                   value );
    }
};
...
public:
    ...
    sherwood_v8_table( size_type bucket_count, const ArgumentAlloc& alloc )
        : sherwood_v8_table( bucket_count,
                             ArgumentHash(),
                             ArgumentEqual(),
                             alloc ) {}
    ...
    void rehash( size_t num_items ) {
        ...
        BlockPointer new_buckets =
            reinterpret_cast< BlockPointer >( new_memory );
        ...
        for ( BlockPointer it = entries, end = it + num_blocks; it != end;
            ...
            for ( int i = 0; i < BlockSize; ++i ) {
                if ( it->control_bytes[ i ] != Constants::magic_for_empty ) {
                    AllocatorTraits::destroy( *this,
                                              std::addressof( it->data[ i ] ) );
    ...
    void swap( sherwood_v8_table& other ) {
        ...
        if ( AllocatorTraits::propagate_on_container_swap::value )
            swap( static_cast< ByteAlloc& >( *this ),
                  static_cast< ByteAlloc& >( other ) );
    }
...
private:
    ...
    struct LinkedListIt {
        ...
        void set_next( int8_t jump_index ) {
            int8_t& metadata = block->control_bytes[ index_in_block() ];
            metadata =
                ( metadata & ~Constants::bits_for_distance ) | jump_index;
        }
    ...
    emplace_direct_hit( LinkedListIt block, Args&&... args ) {
        ...
        if ( block.metadata() == Constants::magic_for_empty ) {
            ...
            for ( LinkedListIt it = block;; ) {
                ...
                free_block.second.set_metadata(
                    Constants::magic_for_list_entry );
                if ( !it.has_next() ) {
    ...
    std::pair< int8_t, LinkedListIt > find_free_index(
        LinkedListIt parent ) const {
    ...
    void deallocate_data( BlockPointer begin, size_t num_slots_minus_one ) {
        ...
        AllocatorTraits::deallocate(
            *this, typename AllocatorTraits::pointer( as_byte_pointer ),
            memory );
    }
...
template < typename T >
struct AlignmentOr8Bytes< T,
                          typename std::enable_if< alignof( T ) >= 1 >::type > {
...
public:
    ...
    std::pair< typename Table::iterator, bool > insert_or_assign(
        ...
        M&& m ) {
        auto emplace_result =
            emplace( std::move( key ), std::forward< M >( m ) );
        if ( !emplace_result.second )
            emplace_result.first->second = std::forward< M >( m );
        return emplace_result;
    }
    ...
    typename Table::iterator insert_or_assign( typename Table::const_iterator,
                                               key_type&& key,
                                               M&& m ) {
        return insert_or_assign( std::move( key ), std::forward< M >( m ) )
            .first;
    }
