...
public:
    ...
    sherwood_v3_table( size_type bucket_count, const ArgumentAlloc& alloc )
        : sherwood_v3_table( bucket_count,
                             ArgumentHash(),
                             ArgumentEqual(),
                             alloc ) {}
    ...
    iterator end() {
        return { entries + static_cast< ptrdiff_t >( num_slots_minus_one +
                                                     max_lookups ) };
    }
    ...
    iterator erase( const_iterator begin_it, const_iterator end_it ) {
        ...
        for ( EntryPointer it = end_it.current;
            ...
            ++it;
            num_to_move =
                std::min( static_cast< ptrdiff_t >( it->distance_from_desired ),
                          num_to_move );
        }
        return { to_return };
    }
...
struct prime_number_hash_policy {
    ...
    mod_function next_size_over( size_t& size ) const {
        ...
        // ClosestPrime(p * 2^(1/3)) and ClosestPrime(p * 2^(2/3)) and put those
        // in the gaps
