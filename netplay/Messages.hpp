...
struct ErrorMessage : public SerializableSequence {
    ...
    std::string str() const override {
        return format( "ErrorMessage[%s]", error );
    }
...
struct ClientMode : public SerializableSequence {
    ...
    ClientMode( const ClientMode& other )
        : ClientMode( other.value, other.flags ) {}

    ClientMode& operator=( const ClientMode& other ) {
        value = other.value;
        flags = other.flags;
        return *this;
    }
...
struct PingStats : public SerializableSequence {
    ...
    PingStats( const Statistics& latency, uint8_t packetLoss )
        : latency( latency ), packetLoss( packetLoss ) {}
...
struct InitialConfig : public SerializableSequence {
    ...
    PROTOCOL_MESSAGE_BOILERPLATE( InitialConfig,
                                  mode,
                                  dataPort,
                                  localName,
                                  remoteName,
...
struct NetplayConfig : public SerializableSequence {
    ...
    void setNames( const std::string& localName,
...
struct InitialGameState : public SerializableSequence {
    ...
    InitialGameState( IndexedFrame indexedFrame )
        : indexedFrame( indexedFrame ) {}

    InitialGameState( IndexedFrame indexedFrame,
                      uint8_t netplayState,
                      bool isTraining );

    std::string formatCharaName( uint8_t player,
                                 CharaNameFunc charaNameFunc ) const {
        ...
        const char moonCh = ( moon[ player - 1 ] == 0
                                  ? 'C'
                                  : ( moon[ player - 1 ] == 1 ? 'F' : 'H' ) );
...
struct SpectateConfig : public SerializableSequence {
    ...
    SpectateConfig( const NetplayConfig& netplayConfig, uint8_t state )
        : mode( netplayConfig.mode ),
          delay( netplayConfig.rollback ? netplayConfig.rollbackDelay
                                        : netplayConfig.delay ),
          rollback( netplayConfig.rollback ),
          winCount( netplayConfig.winCount ),
          hostPlayer( netplayConfig.hostPlayer ),
          names( netplayConfig.names ),
          sessionId( netplayConfig.sessionId ),
          initial( { 0, 0 }, state, netplayConfig.mode.isTraining() ) {}

    std::string formatPlayer( uint8_t player,
                              CharaNameFunc charaNameFunc ) const {
        ASSERT( player == 1 || player == 2 );

        return format( ( names[ player - 1 ].empty() ? "%s%s" : "%s (%s)" ),
                       names[ player - 1 ],
                       initial.formatCharaName( player, charaNameFunc ) );
    }
...
struct RngState : public SerializableSequence {
    ...
    PROTOCOL_MESSAGE_BOILERPLATE( RngState,
                                  index,
                                  rngState0,
                                  rngState1,
                                  rngState2,
...
struct SyncHash : public SerializableSequence {
    ...
    bool operator==( const SyncHash& other ) const {
        ...
        if ( memcmp( ( ( char* )&chara[ 0 ] ) + 8,
                     ( ( char* )&other.chara[ 0 ] ) + 8,
                     sizeof( CharaHash ) - 8 ) )
            return false;

        if ( memcmp( ( ( char* )&chara[ 1 ] ) + 8,
                     ( ( char* )&other.chara[ 1 ] ) + 8,
                     sizeof( CharaHash ) - 8 ) )
            return false;
        ...
        // Special case, since we don't care if seq 0, the neutral sequence, is
        // out of sync
        if ( chara[ 0 ].seq != 0 &&
             chara[ 0 ].seqState != other.chara[ 0 ].seqState )
            return false;

        if ( chara[ 1 ].seq != 0 &&
             chara[ 1 ].seqState != other.chara[ 1 ].seqState )
            return false;
    ...
    std::string str() const override {
        return format( "SyncHash[%s]", indexedFrame );
    }
    ...
    std::string dump() const {
        ...
        for ( uint8_t i = 0; i < 2; ++i ) {
            str += format(
                "; P%u: C=%u; M=%u seq=%u; st=%u; hp=%u; rh=%u; gb=%.1f; "
                "gq=%.1f; mt=%u; ht=%u; x=%d; y=%d",
                i + 1, chara[ i ].chara, chara[ i ].moon, chara[ i ].seq,
                chara[ i ].seqState, chara[ i ].health, chara[ i ].redHealth,
                chara[ i ].guardBar, chara[ i ].guardQuality, chara[ i ].meter,
                chara[ i ].heat, chara[ i ].x, chara[ i ].y );
        }
...
struct MenuIndex : public SerializableSequence {
    ...
    MenuIndex( uint32_t index, int8_t menuIndex )
        : index( index ), menuIndex( menuIndex ) {}

    std::string str() const override {
        return format( "MenuIndex[%u,%d]", index, menuIndex );
    }
...
struct ChangeConfig : public SerializableSequence {
    ...
    PROTOCOL_MESSAGE_BOILERPLATE( ChangeConfig,
                                  value,
                                  indexedFrame.value,
                                  delay,
                                  rollbackDelay,
...
struct TransitionIndex : public SerializableMessage {
    ...
    std::string str() const override {
        return format( "TransitionIndex[%u]", index );
    }
...
struct PlayerInputs : public SerializableMessage, public BaseInputs {
    ...
    PlayerInputs( IndexedFrame indexedFrame ) {
        this->indexedFrame = indexedFrame;
    }

    std::string str() const override {
        return format( "PlayerInputs[%s]", indexedFrame );
    }
...
struct BothInputs : public SerializableSequence, public BaseInputs {
    ...
    BothInputs( IndexedFrame indexedFrame ) {
        this->indexedFrame = indexedFrame;
    }

    std::string str() const override {
        return format( "BothInputs[%s]", indexedFrame );
    }
