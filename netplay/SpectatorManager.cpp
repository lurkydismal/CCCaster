...
void SpectatorManager::pushPendingSocket( Timer::Owner* owner,
...
SocketPtr SpectatorManager::popPendingSocket( Socket* socketPtr ) {
    ...
    ASSERT( _pendingSocketTimers.find( socketPtr ) !=
            _pendingSocketTimers.end() );

    SocketPtr socket = it->second;
    Timer* timerPtr = _pendingSocketTimers[ socketPtr ].get();

    ASSERT( _pendingTimerToSocket.find( timerPtr ) !=
            _pendingTimerToSocket.end() );
...
void SpectatorManager::timerExpired( Timer* timerPtr ) {
    ...
    ASSERT( _pendingSocketTimers.find( it->second ) !=
            _pendingSocketTimers.end() );
