        ...
        while ( fin >> gameMode >> netplayState >> index >> frame >> tag ) {
            ...
            if ( gameMode == CC_GAME_MODE_LOADING ) {
                if ( _initialStates.empty() )
                    _initialStates.push_back(
                        MsgPtr( new InitialGameState( { 0, index } ) ) );

                ASSERT( _initialStates.back().get() != 0 );

                if ( _initialStates.back()
                         ->getAs< InitialGameState >()
                         .indexedFrame.parts.index != index )
                    _initialStates.push_back(
                        MsgPtr( new InitialGameState( { 0, index } ) ) );
            }
            ...
            if ( tag == "Inputs" || ( real && tag == "Reinputs" ) ) {
                ...
                if ( ss.str().size() == 707 ) // Old RngState hex dump size
                {
                    ...
                    char
                        data[ sizeof( uint32_t ) * 3 + 4 + CC_RNG_STATE3_SIZE ];
                    ...
                    memcpy( &rngState->rngState0, &data[ 0 ],
                            sizeof( uint32_t ) );
                    memcpy( &rngState->rngState1, &data[ 4 ],
                            sizeof( uint32_t ) );
                    memcpy( &rngState->rngState2, &data[ 8 ],
                            sizeof( uint32_t ) );
                    copy( &data[ 16 ], &data[ 16 + CC_RNG_STATE3_SIZE ],
                          rngState->rngState3.begin() );
                } else if ( ss.str().size() ==
                            695 ) // New RngState hex dump size
                {
                    rngState = new RngState( 0 );
                    ...
                    memcpy( &rngState->rngState2, &data[ 8 ],
                            sizeof( uint32_t ) );
                    copy( &data[ 12 ], &data[ 12 + CC_RNG_STATE3_SIZE ],
                          rngState->rngState3.begin() );
                } else {
                    THROW_EXCEPTION( "Unknown RngState size: %u",
                                     "Invalid replay file!", ss.str().size() );
                }
                ...
                _initialStates.back()->getAs< InitialGameState >().chara[ 0 ] =
                    chara;
                _initialStates.back()->getAs< InitialGameState >().moon[ 0 ] =
                    moon;
                _initialStates.back()->getAs< InitialGameState >().color[ 0 ] =
                    color;
            } else if ( tag == "P2" ) {
                if ( gameMode != CC_GAME_MODE_IN_GAME )
                    continue;
                ...
                _initialStates.back()->getAs< InitialGameState >().chara[ 1 ] =
                    chara;
                _initialStates.back()->getAs< InitialGameState >().moon[ 1 ] =
                    moon;
                _initialStates.back()->getAs< InitialGameState >().color[ 1 ] =
                    color;
            } else {
                THROW_EXCEPTION( "Unhandled tag: '%s'", "Invalid replay file!",
                                 tag );
            }
        }

        LOG( "Processed up to [%u:%u]", _inputs.size() - 1,
             _inputs.back().size() - 1 );
    }
...
const ReplayManager::Inputs& ReplayManager::getInputs(
    IndexedFrame indexedFrame ) {
    static const Inputs confirm = { MaxIndexedFrame, CC_BUTTON_CONFIRM << 4,
                                    CC_BUTTON_CONFIRM << 4 };
    ...
    if ( indexedFrame.parts.index >= _inputs.size() ||
         indexedFrame.parts.frame >=
             _inputs[ indexedFrame.parts.index ].size() ) {
        return empty;
    }
...
IndexedFrame ReplayManager::getRollbackTarget( IndexedFrame indexedFrame ) {
    if ( indexedFrame.parts.index >= _rollbacks.size() ||
         indexedFrame.parts.frame >=
             _rollbacks[ indexedFrame.parts.index ].size() ) {
...
const vector< ReplayManager::Inputs >& ReplayManager::getReinputs(
    IndexedFrame indexedFrame ) {
    if ( indexedFrame.parts.index >= _reinputs.size() ||
         indexedFrame.parts.frame >=
             _reinputs[ indexedFrame.parts.index ].size() ) {
