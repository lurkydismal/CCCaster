...
#define CC_TITLE \
    "MELTY BLOOD Actress Again Current Code Ver.1.07 Rev." CC_VERSION
...
#define CC_DAMAGE_LEVEL_ADDR \
    ( ( uint32_t* )0x553FCC ) // Damage level: default 2
...
#define CC_AUTO_REPLAY_SAVE_ADDR \
    ( ( uint32_t* )0x553FE8 ) // Auto replay saving: 0 to disable, 1 to enable
#define CC_WORLD_TIMER_ADDR \
    ( ( uint32_t* )0x55D1D4 ) // Frame step timer, always counting up
#define CC_PAUSE_FLAG_ADDR ( ( uint8_t* )0x55D203 ) // 1 when paused
#define CC_SKIP_FRAMES_ADDR \
    ( ( uint32_t* )0x55D25C ) // Set to N to skip rendering for N frames
#define CC_ROUND_TIMER_ADDR \
    ( ( uint32_t* )0x562A3C ) // Counts down from 4752, may stop
#define CC_REAL_TIMER_ADDR \
    ( ( uint32_t* )0x562A40 ) // Counts up from 0 after round start
...
#define CC_P1_GAME_POINT_FLAG_ADDR \
    ( ( uint32_t* )0x559548 ) // P1 game point flag
#define CC_P2_GAME_POINT_FLAG_ADDR \
    ( ( uint32_t* )0x55954C )                         // P2 game point flag
...
#define CC_STAGE_ANIMATION_OFF_ADDR \
    ( ( uint32_t* )0x554124 ) // 1 if stage animations are off
#define CC_INTRO_STATE_ADDR \
    ( ( uint8_t* )0x55D20B ) // 2 (character intros), 1 (pre-game), 0 (in-game)
#define CC_HIT_SPARKS_ADDR ( ( uint32_t* )0x67BD78 ) // Number of hit sparks?

#define CC_STAGE_SELECTOR_ADDR \
    ( ( uint32_t* )0x74FD98 ) // Currently selected stage, can be assigned to
                              // directly
#define CC_FPS_COUNTER_ADDR \
    ( ( uint32_t* )0x774A70 ) // Value of the displayed FPS counter
#define CC_PERF_FREQ_ADDR \
    ( ( uint64_t* )0x774A80 ) // Value of QueryPerformanceFrequency for game FPS
#define CC_SKIPPABLE_FLAG_ADDR \
    ( ( uint32_t* )0x74D99C ) // Flag that indicates a skippable state when
                              // in-game
#define CC_ALIVE_FLAG_ADDR \
    ( ( uint8_t* )0x76E650 ) // Flag that indicates the game is alive
#define CC_REPLAY_CREATED_ADDR \
    ( ( uint32_t* )0x774C30 ) // Flag that indicates whether a replay file has
                              // been written
#define CC_D3DX9_OBJ_ADDR \
    ( ( uint32_t* )0x76E7D4 ) // The address for the IDirect3DDevice9

// Some menu state counter, incremented for each open menu, decremented when
// menu closes
#define CC_MENU_STATE_COUNTER_ADDR ( ( uint32_t* )0x767440 )

#define CC_DUMMY_STATUS_ADDR \
    ( ( int32_t* )0x74D7F8 ) // Training mode dummy status
...
#define CC_PTR_TO_WRITE_INPUT_ADDR \
    ( ( char* )0x76E6AC ) // Pointer to the location to write game input
...
#define CC_GAME_MODE_ADDR \
    ( ( uint32_t* )0x54EEE8 ) // Current game mode, constants below
...
#define CC_GAME_STATE_ADDR \
    ( ( uint32_t* )0x74d598 ) // Intermediate game states, constants below
...
// Character select data, can be assigned to directly at the character select
// screen
...
#define CC_P1_RANDOM_COLOR_ADDR \
    ( ( uint8_t* )( ( *( uint32_t* )0x74D808 ) + 0 * 0x1DC + 0x2C + 0x0C ) )
...
#define CC_P2_RANDOM_COLOR_ADDR \
    ( ( uint8_t* )( ( *( uint32_t* )0x74D808 ) + 1 * 0x1DC + 0x2C + 0x0C ) )
...
#define CC_P1_NO_INPUT_FLAG_ADDR \
    ( ( uint8_t* )0x5552A7 ) // Indicates when input is disabled, ie KO or time
                             // over
#define CC_P1_PUPPET_STATE_ADDR \
    ( ( uint8_t* )0x5552A8 ) // 0 is not puppet, 1 is puppet, 2 is puppet w/
                             // hurtbox?
...
#define CC_P1_FACING_FLAG_ADDR \
    ( ( uint8_t* )0x555444 ) // 0 facing left, 1 facing right
#define CC_P1_COMBO_OFFSET_ADDR \
    ( ( uint8_t* )0x557E59 ) // Index into combo structure array
#define CC_P1_COMBO_HIT_BASE_ADDR \
    ( ( uint32_t* )0x557E5C ) // Base for combo strucure array

#define CC_P2_ENABLED_FLAG_ADDR                            \
    ( ( uint8_t* )( ( ( char* )CC_P1_ENABLED_FLAG_ADDR ) + \
                    CC_PLR_STRUCT_SIZE ) )
#define CC_P2_SEQUENCE_ADDR \
    ( ( uint32_t* )( ( ( char* )CC_P1_SEQUENCE_ADDR ) + CC_PLR_STRUCT_SIZE ) )
#define CC_P2_SEQ_STATE_ADDR \
    ( ( uint32_t* )( ( ( char* )CC_P1_SEQ_STATE_ADDR ) + CC_PLR_STRUCT_SIZE ) )
#define CC_P2_HEALTH_ADDR \
    ( ( uint32_t* )( ( ( char* )CC_P1_HEALTH_ADDR ) + CC_PLR_STRUCT_SIZE ) )
#define CC_P2_RED_HEALTH_ADDR \
    ( ( uint32_t* )( ( ( char* )CC_P1_RED_HEALTH_ADDR ) + CC_PLR_STRUCT_SIZE ) )
#define CC_P2_GUARD_BAR_ADDR \
    ( ( float* )( ( ( char* )CC_P1_GUARD_BAR_ADDR ) + CC_PLR_STRUCT_SIZE ) )
#define CC_P2_GUARD_QUALITY_ADDR \
    ( ( float* )( ( ( char* )CC_P1_GUARD_QUALITY_ADDR ) + CC_PLR_STRUCT_SIZE ) )
#define CC_P2_METER_ADDR \
    ( ( uint32_t* )( ( ( char* )CC_P1_METER_ADDR ) + CC_PLR_STRUCT_SIZE ) )
#define CC_P2_HEAT_ADDR \
    ( ( uint32_t* )( ( ( char* )CC_P1_HEAT_ADDR ) + CC_PLR_STRUCT_SIZE ) )
#define CC_P2_NO_INPUT_FLAG_ADDR                            \
    ( ( uint8_t* )( ( ( char* )CC_P1_NO_INPUT_FLAG_ADDR ) + \
                    CC_PLR_STRUCT_SIZE ) )
#define CC_P2_PUPPET_STATE_ADDR                            \
    ( ( uint8_t* )( ( ( char* )CC_P1_PUPPET_STATE_ADDR ) + \
                    CC_PLR_STRUCT_SIZE ) )
#define CC_P2_X_POSITION_ADDR \
    ( ( int32_t* )( ( ( char* )CC_P1_X_POSITION_ADDR ) + CC_PLR_STRUCT_SIZE ) )
#define CC_P2_Y_POSITION_ADDR \
    ( ( int32_t* )( ( ( char* )CC_P1_Y_POSITION_ADDR ) + CC_PLR_STRUCT_SIZE ) )
#define CC_P2_FACING_FLAG_ADDR \
    ( ( uint8_t* )( ( ( char* )CC_P1_FACING_FLAG_ADDR ) + CC_PLR_STRUCT_SIZE ) )

#define CC_P3_ENABLED_FLAG_ADDR                            \
    ( ( uint8_t* )( ( ( char* )CC_P2_ENABLED_FLAG_ADDR ) + \
                    CC_PLR_STRUCT_SIZE ) )
#define CC_P3_SEQUENCE_ADDR \
    ( ( uint32_t* )( ( ( char* )CC_P2_SEQUENCE_ADDR ) + CC_PLR_STRUCT_SIZE ) )
#define CC_P3_NO_INPUT_FLAG_ADDR                            \
    ( ( uint8_t* )( ( ( char* )CC_P2_NO_INPUT_FLAG_ADDR ) + \
                    CC_PLR_STRUCT_SIZE ) )
#define CC_P3_PUPPET_STATE_ADDR                            \
    ( ( uint8_t* )( ( ( char* )CC_P2_PUPPET_STATE_ADDR ) + \
                    CC_PLR_STRUCT_SIZE ) )

#define CC_P4_ENABLED_FLAG_ADDR                            \
    ( ( uint8_t* )( ( ( char* )CC_P3_ENABLED_FLAG_ADDR ) + \
                    CC_PLR_STRUCT_SIZE ) )
#define CC_P4_NO_INPUT_FLAG_ADDR                            \
    ( ( uint8_t* )( ( ( char* )CC_P3_NO_INPUT_FLAG_ADDR ) + \
                    CC_PLR_STRUCT_SIZE ) )
#define CC_P4_PUPPET_STATE_ADDR                            \
    ( ( uint8_t* )( ( ( char* )CC_P3_PUPPET_STATE_ADDR ) + \
                    CC_PLR_STRUCT_SIZE ) )
...
// Array of sound effect flags, each byte corresponds to a specific SFX, set to
// 1 to start
...
// Asm hacks are prefixed MM (for modified memory), they should be written to
// safe locations
...
inline std::ostream& operator<<( std::ostream& os,
    ...
    return ( os << indexedFrame.parts.index << ':'
