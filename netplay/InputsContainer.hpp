...
public:
    ...
    // Set a single input for the given index:frame, CANNOT change existing
    // inputs.
    ...
    // Assign a single input for the given index:frame, CAN change existing
    // inputs
    ...
    // Fill n inputs with the same given value starting from the given
    // index:frame, CAN change existing inputs.
    ...
    // Set n inputs starting from the given index:frame, CAN change existing
    // inputs.
    void set( uint32_t index,
              uint32_t frame,
              const T* t,
              size_t n,
              uint32_t checkStartingFromIndex = UINT_MAX ) {
        if ( index >= checkStartingFromIndex ) {
            IndexedFrame f;
            size_t i;

            for ( i = 0, f = { { frame, index } }; i < n;
                  ++i, ++f.parts.frame ) {
                if ( get( f.parts.index, f.parts.frame ) == t[ i ] )
                    continue;

                // Indicate changed if the input is different from the last
                // known input
                _lastChangedFrame.value =
                    std::min( _lastChangedFrame.value, f.value );
                break;
            }
        }
    ...
    uint32_t getEndIndex() const { return _inputs.size(); }
...
private:
    ...
    // Get the last known input BEFORE the given index. Defaults to 0 if
    // unknown.
