...
struct MainApp : public Main,
    ...
    /* Connect protocol
        ...
        9 - Client confirms NetplayConfig and sends ConfirmConfig before
       starting

       10 - Reconnect dataSocket in-game, and also don't need ctrlSocket for
    ...
    */
    ...
    void startNetplay() {
        ...
        if ( clientMode.isHost() ) {
            ...
            address.port = serverCtrlSocket->address
                               .port; // Update port in case it was initially 0
            address.invalidate();
    ...
    void gotVersionConfig( Socket* socket,
                           const VersionConfig& versionConfig ) {
        const Version RemoteVersion = versionConfig.version;

        LOG( "LocalVersion='%s'; revision='%s'; buildTime='%s'", LocalVersion,
             LocalVersion.revision, LocalVersion.buildTime );

        LOG( "RemoteVersion='%s'; revision='%s'; buildTime='%s'", RemoteVersion,
             RemoteVersion.revision, RemoteVersion.buildTime );

        LOG( "VersionConfig: mode=%s; flags={ %s }", versionConfig.mode,
             versionConfig.mode.flagString() );

        if ( !LocalVersion.isSimilar(
                 RemoteVersion, 1 + options[ Options::StrictVersion ] ) ) {
            ...
            if ( clientMode.isHost() )
                socket->send(
                    new ErrorMessage( "Incompatible host version: " + local ) );
            else
        ...
        if ( clientMode.isHost() ) {
            if ( versionConfig.mode.isSpectate() ) {
                socket->send(
                    new ErrorMessage( "Not in a game yet, cannot spectate!" ) );
                return;
            }
            ...
            try {
                ...
                serverDataSocket = SmartSocket::listenUDP( this, 0 );
            }
    ...
    void gotInitialConfig( const InitialConfig& initialConfig ) {
        ...
        if ( clientMode.isClient() ) {
            ...
            dataSocket = SmartSocket::connectUDP(
                this, { address.addr, this->initialConfig.dataPort },
                ctrlSocket->getAsSmart().isTunnel() );
            LOG( "dataSocket=%08x", dataSocket.get() );

            ui.display( "Connecting to " + this->initialConfig.remoteName +
                        "\n\n" +
                        ( this->initialConfig.mode.isTraining() ? "Training"
                                                                : "Versus" ) +
                        " mode" + "\n\nCalculating delay..." );
        }

        LOG( "InitialConfig: mode=%s; flags={ %s }; dataPort=%u; "
             "localName='%s'; remoteName='%s'; winCount=%u",
             initialConfig.mode, initialConfig.mode.flagString(),
             initialConfig.dataPort, initialConfig.localName,
             initialConfig.remoteName, initialConfig.winCount );
    }
    ...
    void mergePingStats() {
        LOG( "PingStats (local): latency=%.2f ms; worst=%.2f ms; stderr=%.2f "
             "ms; stddev=%.2f ms; packetLoss=%d%%",
             pinger.getStats().getMean(), pinger.getStats().getWorst(),
             pinger.getStats().getStdErr(), pinger.getStats().getStdDev(),
             pinger.getPacketLoss() );

        LOG( "PingStats (remote): latency=%.2f ms; worst=%.2f ms; stderr=%.2f "
             "ms; stddev=%.2f ms; packetLoss=%d%%",
             pingStats.latency.getMean(), pingStats.latency.getWorst(),
             pingStats.latency.getStdErr(), pingStats.latency.getStdDev(),
             pingStats.packetLoss );

        pingStats.latency.merge( pinger.getStats() );
        pingStats.packetLoss =
            ( pingStats.packetLoss + pinger.getPacketLoss() ) / 2;

        LOG( "PingStats (merged): latency=%.2f ms; worst=%.2f ms; stderr=%.2f "
             "ms; stddev=%.2f ms; packetLoss=%d%%",
             pingStats.latency.getMean(), pingStats.latency.getWorst(),
             pingStats.latency.getStdErr(), pingStats.latency.getStdDev(),
             pingStats.packetLoss );
    ...
    void gotSpectateConfig( const SpectateConfig& spectateConfig ) {
        ...
        LOG( "SpectateConfig: %s; flags={ %s }; delay=%d; rollback=%d; "
             ...
             spectateConfig.delay, spectateConfig.rollback,
             spectateConfig.winCount, spectateConfig.hostPlayer,
             spectateConfig.names[ 0 ], spectateConfig.names[ 1 ] );

        LOG( "InitialGameState: %s; stage=%u; isTraining=%u; %s vs %s",
             NetplayState(
                 ( NetplayState::Enum )spectateConfig.initial.netplayState ),
    ...
    void checkDelayAndContinue() {
        ...
        if ( delay > maxDelay ) {
            const string error =
                MainUi::formatStats( this->pingStats ) +
    ...
    void getUserConfirmation() {
        ...
        if ( options[ Options::Dummy ] || options[ Options::SyncTest ] ) {
            ...
            if ( clientMode.isHost() ) {
                // TODO parse these from SyncTest args
                netplayConfig.delay =
                    computeDelay( pingStats.latency.getWorst() ) + 1;
        ...
        uiSendSocket =
            UdpSocket::bind( 0, { "127.0.0.1", uiRecvSocket->address.port } );
        isWaitingForUser = true;
    ...
    void gotUserConfirmation() {
        ...
        switch ( clientMode.value ) {
            ...
            case ClientMode::Host:
                // Waiting again
                KeyboardManager::get().keyboardWindow =
                    MainUi::getConsoleWindow();
            ...
            case ClientMode::Client:
                ...
                netplayConfig.rollbackDelay =
                    ui.getNetplayConfig().rollbackDelay;
    ...
    void gotDummyMsg( const MsgPtr& msg ) {
        ...
        switch ( msg->getMsgType() ) {
            case MsgType::InitialGameState:
                LOG( "InitialGameState: %s; stage=%u; isTraining=%u; %s vs %s",
                     NetplayState(
                         ( NetplayState::Enum )msg->getAs< InitialGameState >()
                             .netplayState ),
                     msg->getAs< InitialGameState >().stage,
                     msg->getAs< InitialGameState >().isTraining,
                     msg->getAs< InitialGameState >().formatCharaName(
                         1, getFullCharaName ),
                     msg->getAs< InitialGameState >().formatCharaName(
                         2, getFullCharaName ) );
                return;
            ...
            case MsgType::PlayerInputs: {
                ...
                PlayerInputs inputs(
                    msg->getAs< PlayerInputs >().indexedFrame );
                ...
                for ( uint32_t i = 0; i < inputs.size(); ++i ) {
                    ...
                    inputs.inputs[ i ] =
                        ( ( frame % 5 )
                              ? 0
                              : COMBINE_INPUT(
                                    0, CC_BUTTON_A | CC_BUTTON_CONFIRM ) );
                }
            ...
            case MsgType::MenuIndex:
                ...
                // since the higher option always takes priority, the host
                // effectively takes priority.
                if ( clientMode.isClient() )
                    dataSocket->send(
                        new MenuIndex( msg->getAs< MenuIndex >().index, 0 ) );
                return;
            ...
            case MsgType::BothInputs: {
                ...
                if ( both.getIndex() > last.parts.index ) {
                    for ( uint32_t i = 0; i < both.getStartFrame(); ++i )
                        LOG_TO( syncLog, "Dummy [%u:%u] Inputs: 0x%04x 0x%04x",
                                both.getIndex(), i, 0, 0 );
                }

                for ( uint32_t i = 0; i < both.size(); ++i ) {
                    const IndexedFrame current = {
                        { i + both.getStartFrame(), both.getIndex() } };

                    if ( current.value <= last.value )
                        continue;

                    LOG_TO( syncLog, "Dummy [%s] Inputs: 0x%04x 0x%04x",
                            current, both.inputs[ 0 ][ i ],
                            both.inputs[ 1 ][ i ] );
                }
    ...
    void startGame() {
        ...
        if ( clientMode.isClient() && ctrlSocket->isSmart() &&
             ctrlSocket->getAsSmart().isTunnel() )
        ...
        if ( clientMode.isNetplay() ) {
            ...
            netplayConfig.mode.flags = clientMode.flags =
                initialConfig.mode.flags;
            netplayConfig.winCount = initialConfig.winCount;
            netplayConfig.setNames( initialConfig.localName,
                                    initialConfig.remoteName );

            LOG( "NetplayConfig: %s; flags={ %s }; delay=%d; rollback=%d; "
                 "rollbackDelay=%d; winCount=%d; "
                 "hostPlayer=%d; names={ '%s', '%s' }",
                 netplayConfig.mode, netplayConfig.mode.flagString(),
                 netplayConfig.delay, netplayConfig.rollback,
                 netplayConfig.rollbackDelay, netplayConfig.winCount,
                 netplayConfig.hostPlayer, netplayConfig.names[ 0 ],
                 netplayConfig.names[ 1 ] );
        }
        ...
        if ( options[ Options::Dummy ] ) {
            ASSERT( clientMode.value == ClientMode::Client ||
                    clientMode.isSpectate() == true );

            ui.display( format( "Dummy is ready%s",
                                clientMode.isTraining() ? " (training)" : "" ),
                        false ); // Don't replace last message

            isDummyReady = true;

            // We need to send an IpAddrPort message to indicate our
            // serverCtrlSocket address, here it is a fake
            if ( ctrlSocket && ctrlSocket->isConnected() )
            ...
            if ( clientMode.isClient() ) {
                dataSocket = SmartSocket::connectUDP(
                    this, address, ctrlSocket->getAsSmart().isTunnel() );
                LOG( "dataSocket=%08x", dataSocket.get() );
            }
            ...
            syncLog.sessionId =
                ( clientMode.isSpectate() ? spectateConfig.sessionId
                                          : netplayConfig.sessionId );

            if ( options[ Options::PidLog ] )
                syncLog.initialize( ProcessManager::appDir + SYNC_LOG_FILE,
                                    PID_IN_FILENAME );
            else
        ...
        ui.display(
            format( "Starting %s mode...", getGameModeString() ),
            clientMode.isNetplay() ); // Only replace last message if netplaying

        // Start game (and disconnect sockets) after a small delay since the
        // final configs are still in flight
    ...
    void pingerCompleted( Pinger* pinger,
                          const Statistics& stats,
                          uint8_t packetLoss ) override {
        ASSERT( pinger == &this->pinger );
    ...
    void socketAccepted( Socket* serverSocket ) override {
        ...
        } else if ( serverSocket == serverDataSocket.get() && ctrlSocket &&
                    ctrlSocket->isConnected() && !dataSocket ) {
    ...
    void socketDisconnected( Socket* socket ) override {
        ...
        if ( socket == ctrlSocket.get() || socket == dataSocket.get() ) {
            ...
            LOG( "%s disconnected!",
                 ( socket == ctrlSocket.get() ? "ctrlSocket" : "dataSocket" ) );
            ...
            if ( !( userConfirmed && isFinalConfigReady ) || isDummyReady ) {
                if ( lastError.empty() )
                    lastError = ( isInitialConfigReady ? "Disconnected!"
                                                       : "Timed out!" );
    ...
    void socketRead( Socket* socket,
        ...
        if ( msg->getMsgType() == MsgType::IpAddrPort &&
             socket == ctrlSocket.get() ) {
            this->address = msg->getAs< IpAddrPort >();
            ctrlSocket = SmartSocket::connectTCP( this, this->address,
                                                  options[ Options::Tunnel ] );
            return;
        } else if ( msg->getMsgType() == MsgType::VersionConfig &&
                    ( ( clientMode.isHost() && !ctrlSocket ) ||
                      clientMode.isClient() ) ) {
        ...
        if ( clientMode.isHost() &&
             msg->getMsgType() == MsgType::VersionConfig ) {
            if ( msg->getAs< VersionConfig >().mode.isSpectate() )
                socket->send(
                    new ErrorMessage( "Not in a game yet, cannot spectate!" ) );
            else
                socket->send( new ErrorMessage(
    ...
    void ipcRead( const MsgPtr& msg ) override {
        ...
        switch ( msg->getMsgType() ) {
            ...
            case MsgType::ChangeConfig:
                ...
                if ( msg->getAs< ChangeConfig >().value ==
                ...
                if ( delayChanged && rollbackChanged )
                    ui.display(
                        format( "Input delay was changed to %u\nRollback was "
                                "changed to %u",
                                msg->getAs< ChangeConfig >().delay,
                                msg->getAs< ChangeConfig >().rollback ) );
                    ...
                    ui.display(
                        format( "Rollback was changed to %u",
                                msg->getAs< ChangeConfig >().rollback ) );
                return;
    ...
    void timerExpired( Timer* timer ) override {
        if ( timer == stopTimer.get() ) {
            ...
            if ( !clientMode.isSpectate() ) {
                // We must disconnect the sockets before the game process is
                // created, otherwise Windows say conflicting ports EVEN if they
                // are created later.
                dataSocket.reset();
                serverDataSocket.reset();
                ctrlSocket.reset();
            ...
            DWORD val = GetFileAttributes(
                ( ProcessManager::appDir + "framestep.dll" ).c_str() );

            bool hasFramestep = true;
            bool loadFramestep =
                ( GetAsyncKeyState( VK_F8 ) & 0x8000 ) == 0x8000;
            ...
            procMan.openGame(
                ui.getConfig().getInteger( "highCpuPriority" ),
                ( clientMode.isTraining() || clientMode.isReplay() ) &&
                    hasFramestep );
        } else {
    ...
    void externalIpAddrFound( ExternalIpAddress* extIpAddr,
    ...
    MainApp( const IpAddrPort& addr, const Serializable& config )
        ...
        LOG( "clientMode=%s; flags={ %s }; address='%s'; config=%s", clientMode,
             clientMode.flagString(), addr, config.getMsgType() );
        ...
        if ( ui.getConfig().getDouble( "heldStartDuration" ) > 0 ) {
            options.set(
                Options::HeldStartDuration, 1,
                format( "%u", uint32_t( 60 * ui.getConfig().getDouble(
                                                 "heldStartDuration" ) ) ) );
        }
...
private:
    ...
    const char* getGameModeString() const {
        return netplayConfig.tournament
                   ? "tournament"
                   : ( clientMode.isTraining() ? "training" : "versus" );
    }
    ...
    void updateStatusMessage() const {
        ...
        const uint16_t port =
            ( clientMode.isBroadcast() ? netplayConfig.broadcastPort
                                       : address.port );
        if ( ui.isServer() ) {
            ui.display( format(
                "%s at server%s\n",
        ...
        } else if ( externalIpAddress.address.empty() ||
                    externalIpAddress.address == ExternalIpAddress::Unknown ) {
            ui.display(
                format(
                    "%s on port %u%s\n",
                    ( clientMode.isBroadcast() ? "Broadcasting" : "Hosting" ),
            ...
            ui.display( format(
                ...
                ( clientMode.isBroadcast() ? "Broadcasting" : "Hosting" ),
                externalIpAddress.address, port,
                ( clientMode.isTraining() ? " (training mode)" : "" ) ) );
        }
        ui.hostReady();
    }
    ...
    void resetHost() {
        ...
        isBroadcastPortReady = isFinalConfigReady = isWaitingForUser =
            userConfirmed = false;
    }
};
