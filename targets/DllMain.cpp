...
struct DllMain : public Main,
    ...
    void frameStepNormal() {
        switch ( netMan.getState().value ) {
            ...
            case NetplayState::ReplayMenu: {
                            ...
                            clientMode.isLocal() ) // Netplay + local controls
                {
                    ...
                    //                         randomDelay ? "Enabled random
                    //                         delay" : "Disabled random delay"
                    //                         );
                    //                     }
                ...
                // #ifndef RELEASE
                //                 DllOverlayUi::debugText = format ( "%+d
                //                 [%s]", netMan.getRemoteFrameDelta(),
                //                 netMan.getIndexedFrame() );
                ...
                //                 if ( replayInputs )
                //                 {
                //                     if ( repMan.getGameMode (
                //                     netMan.getIndexedFrame() ) )
                //                         ASSERT ( repMan.getGameMode (
                //                         netMan.getIndexedFrame() ) ==
                //                         *CC_GAME_MODE_ADDR );

                //                     if ( ! repMan.getStateStr (
                //                     netMan.getIndexedFrame() ).empty() )
                //                         ASSERT ( repMan.getStateStr (
                //                         netMan.getIndexedFrame() ) ==
                //                         netMan.getState().str() );

                //                     // Inputs
                //                     const auto& inputs = repMan.getInputs (
                //                     netMan.getIndexedFrame() );
                //                     netMan.setInput ( 1, inputs.p1 );
                //                     netMan.setInput ( 2, inputs.p2 );

                //                     const IndexedFrame target =
                //                     repMan.getRollbackTarget (
                //                     netMan.getIndexedFrame() );
                ...
                //                         const auto& reinputs =
                //                         repMan.getReinputs (
                //                         netMan.getIndexedFrame() ); for (
                //                         const auto& inputs : reinputs )
                //                         {
                //                             netMan.assignInput ( 1,
                //                             inputs.p1, inputs.indexedFrame );
                //                             netMan.assignInput ( 2,
                //                             inputs.p2, inputs.indexedFrame );
                //                         }

                //                         const string before = format ( "%s
                //                         [%u] %s [%s]",
                //                                                        gameModeStr
                //                                                        (
                //                                                        *CC_GAME_MODE_ADDR
                ...
                //                         // Indicate we're re-running to the
                //                         current frame fastFwdStopFrame =
                //                         netMan.getIndexedFrame();

                //                         // Reset the game state (this resets
                //                         game state AND netMan state) if (
                ...
                //                             LOG_TO ( syncLog, "%s Rollback:
                //                             target=[%s]; actual=[%s]",
                //                                      before, target,
                //                                      netMan.getIndexedFrame()
                //                                      );

                //                             LOG_SYNC ( "Reinputs: 0x%04x
                //                             0x%04x", netMan.getRawInput ( 1
                //                             ), netMan.getRawInput ( 2 ) );
                //                             return;
                //                         }

                //                         LOG_TO ( syncLog, "%s Rollback to
                //                         target=[%s] failed!", before, target
                //                         );
                ...
                //                     localInputs [ clientMode.isLocal() ? 1 :
                //                     0 ] = 0; DllOverlayUi::showMessage (
                //                     randomInputs ? "Enabled random inputs" :
                ...
                //                     if ( netMan.isInRollback() )
                //                         shouldRandomize &= (
                //                         netMan.getFrame() % 150 < 120 );

                //                     if ( shouldRandomize )
                //                     {
                //                         uint16_t direction = ( rand() % 10 );

                //                         // Reduce the chances of moving the
                //                         cursor at retry menu if (
                //                         netMan.getState() ==
                //                         NetplayState::RetryMenu && ( rand() %
                //                         2 ) )
                //                             direction = 0;
                ...
                //                         // Reduce the chances of hitting the
                //                         D button if ( rand() % 100 < 98 )
                //                             buttons &= ~ CC_BUTTON_D;

                //                         // Prevent hitting some non-essential
                //                         buttons buttons &= ~ ( CC_BUTTON_FN1
                //                         | CC_BUTTON_FN2 | CC_BUTTON_START );

                //                         // Prevent going back at character
                //                         select if ( netMan.getState() ==
                //                         NetplayState::CharaSelect )
                //                             buttons &= ~ ( CC_BUTTON_B |
                //                             CC_BUTTON_CANCEL );

                //                         localInputs [ clientMode.isLocal() ?
                //                         1 : 0 ] = COMBINE_INPUT ( direction,
                //                         buttons );
                //                     }
                //                 }
                // #endif // NOT RELEASE
                ...
                if ( !clientMode.isSpectate() ) {
                    // #ifndef RELEASE
                    //                     if ( netMan.isInRollback() )
                    //                         netMan.assignInput ( localPlayer,
                    //                         localInputs[0], netMan.getFrame()
                    //                         + netMan.getDelay() );
                    //                     else
                    // #endif // NOT RELEASE
        ...
        //                     // Reset the game state (this resets game state
        //                     AND netMan state) if ( rollMan.loadState (
        //                     target, netMan ) )
        //                     {
        //                         // Start fast-forwarding now
        //                         *CC_SKIP_FRAMES_ADDR = 1;

        //                         LOG_TO ( syncLog, "%s Rollback: target=[%s];
        //                         actual=[%s]",
        //                                  before,
        //                                  netMan.getLastChangedFrame(),
        //                                  netMan.getIndexedFrame() );
        ...
        //                         netMan.getRawInput ( 1 ), netMan.getRawInput
        //                         ( 2 ) ); return;
        //                     }
        //                 }
        //                 else
        ...
        //                 DllOverlayUi::showMessage ( randomRollback ? "Enabled
        //                 random rollback" : "Disabled random rollback" );
        //             }
        ...
        //                 netMan state) if ( rollMan.loadState ( target, netMan
        //                 ) )
        //                 {
        //                     // Start fast-forwarding now
        //                     *CC_SKIP_FRAMES_ADDR = 1;
        ...
        //                 LOG_TO ( syncLog, "%s Rollback to target=[%s]
        //                 failed!", before, target );
        //             }
        //         }
        ...
        //                 NetplayState::CharaSelect && netMan.getState() !=
        ...
        //             if ( !netMan.isInRollback()
        //                     || ( netMan.getFrame() == 0 )
        //                     || ( randomInputs && netMan.getFrame() % 150 ==
        //                     149 ) )
        //             {
        //                 MsgPtr msgSyncHash ( new SyncHash (
        //                 netMan.getIndexedFrame() ) ); dataSocket->send (
        //                 msgSyncHash ); localSync.push_back ( msgSyncHash );
        //             }
        //         }
        ...
        //             while ( !remoteSync.empty() && L.indexedFrame.value >
        //             R.indexedFrame.value )
        //                 remoteSync.pop_front();

        //             if ( remoteSync.empty() )
        //                 break;

        //             while ( !localSync.empty() && R.indexedFrame.value >
        //             L.indexedFrame.value )
        //                 localSync.pop_front();
        ...
        //         if ( replayInputs && netMan.getIndex() >=
        //         repMan.getLastIndex() && netMan.getFrame() >=
        //         repMan.getLastFrame() )
        //         {
        //             replayInputs = false;
        ...
        //             {
        //                 LOG_SYNC ( "RngState: %s",
        //                 msgRngState->getAs<RngState>().dump() ); LOG_TO (
        //                 syncLog, "Desync!" ); syncLog.deinitialize();
        ...
        //         )
        //         // {
        //         //     if ( *CC_P1_HEALTH_ADDR != 11400 || *CC_P1_METER_ADDR
        //         != 0
        //         //             || *CC_P2_HEALTH_ADDR != 10121 ||
        //         *CC_P2_METER_ADDR != 10638 )
        //         //     {
    ...
    void ipcRead( const MsgPtr& msg ) override {
        ...
        switch ( msg->getMsgType() ) {
            case MsgType::OptionsMessage:
                ...
                //                 {
                //                     LOG ( "Replay: '%s'", options.arg (
                //                     Options::Replay ) );

                //                     // Parse arguments
                //                     const vector<string> args = split (
                //                     options.arg ( Options::Replay ), "," );
                //                     ASSERT ( args.empty() == false );

                //                     const string replayFile =
                //                     ProcessManager::appDir + args[0]; const
                //                     bool real = find ( args.begin(),
                //                     args.end(), "real" ) != args.end();

                //                     // Parse replay speed
                //                     auto it = find ( args.begin(),
                //                     args.end(), "speed" ); if ( it !=
                //                     args.end() )
                //                         ++it;
                //                     if ( it != args.end() )
                //                         replaySpeed = lexical_cast<uint32_t>
                //                         ( *it );

                //                     // Parse stop index and frame
                //                     it = find ( args.begin(), args.end(),
                //                     "stop" ); if ( it != args.end() )
                //                         ++it;
                //                     if ( it != args.end() && ( args.end() -
                //                     it ) >= 2 )
                //                     {
                ...
                //                     // Parse replay file
                //                     const bool good = repMan.load (
                //                     replayFile, real ); ASSERT ( good == true
                //                     );

                //                     // Parse start index
                //                     it = find ( args.begin(), args.end(),
                //                     "start" ); if ( it != args.end() )
                //                         ++it;
                //                     if ( it != args.end() )
                //                     {
                //                         MsgPtr msgInitialState =
                //                         repMan.getInitialStateBefore (
                //                         lexical_cast<int> ( *it ) );
    ...
    DllMain()
        ...
        //         Variable::MenuConfirmState ), AsmHacks::menuConfirmState );
        //         ChangeMonitor::get().addRef ( this, Variable (
        //         Variable::CurrentMenuIndex ), AsmHacks::currentMenuIndex );
        //         // ChangeMonitor::get().addRef ( this, Variable (
        //         Variable::GameStateCounter ), *CC_MENU_STATE_COUNTER_ADDR );
        //         // ChangeMonitor::get().addPtrToRef ( this, Variable (
        //         Variable::AutoReplaySave ),
        //         //                                    const_cast<const
        //         uint32_t *&> ( AsmHacks::autoReplaySaveStatePtr ), 0u );
        // #endif // NOT RELEASE
    }
