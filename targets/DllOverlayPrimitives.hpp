...
static inline void DrawRectangle( IDirect3DDevice9* device,
                                  int x1,
                                  int y1,
                                  int x2,
                                  int y2,
...
static inline void DrawBox( IDirect3DDevice9* device,
                            int x1,
                            int y1,
                            int x2,
                            int y2,
...
template < size_t N >
static inline void DrawCircle( IDirect3DDevice9* device,
                               float x,
                               float y,
                               float r,
                               const D3DCOLOR& color ) {
    ...
    for ( size_t i = 0; i < verts.size(); ++i ) {
        verts[ i ].x =
            x + r * cos( float( 2 * M_PI * i ) / ( verts.size() - 1 ) );
        verts[ i ].y =
            y + r * sin( float( 2 * M_PI * i ) / ( verts.size() - 1 ) );
    }
...
static inline int DrawText( ID3DXFont* font,
...
static inline int DrawTextW( ID3DXFont* font,
...
static inline int TextCalcRect( ID3DXFont* font,
                                const std::string& text,
                                RECT& rect,
                                int flags,
                                const D3DCOLOR& color ) {
    ...
    return font->DrawText( 0,
                           &text[ 0 ], // text buffer
                           text.size(), &rect, DT_CALCRECT | flags, 0 );
}

static inline int TextCalcRectW( ID3DXFont* font,
                                 const std::wstring& text,
                                 RECT& rect,
                                 int flags,
                                 const D3DCOLOR& color ) {
...
// extern "C" int CallDrawText ( int width, int height, int xAddr, int yAddr,
// char* text, int textAlpha, int textShade, int textShade2, void* addr, int
// spacing, int unk, char* out );
