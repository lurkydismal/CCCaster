...
// Run macro that deinitializes controllers, runs, then reinitializes
// controllers
#define RUN( ADDRESS, CONFIG )                               \
    do {                                                     \
        ControllerManager::get().deinitialize();             \
        run( ADDRESS, CONFIG );                              \
        ControllerManager::get().loadMappings(               \
            ProcessManager::appDir + FOLDER, MAPPINGS_EXT ); \
...
void MainUi::netplay( RunFuncPtr run ) {
    ConsoleUi::Prompt* menu = new ConsoleUi::Prompt(
        ConsoleUi::Prompt::String,
    ...
    for ( ;; ) {
        ...
        try {
            ...
            _ui->pushBelow( new ConsoleUi::TextBox( exc.user ),
                            { 1, 0 } ); // Expand width
            continue;
        }
        ...
        if ( !sessionError.empty() ) {
            _ui->pushBelow( new ConsoleUi::TextBox( sessionError ),
                            { 1, 0 } ); // Expand width
            sessionError.clear();
        }
    }
...
void MainUi::server( RunFuncPtr run ) {
    serverMode = true;
    _ui->pushRight(
        new ConsoleUi::Menu( "Mode", { "Lobby", "Matchmaking" }, "Cancel" ) );
    _ui->top< ConsoleUi::Menu >()->setPosition(
        _config.getInteger( "lastServerMenuPosition" ) - 1 );
    try {
        for ( ;; ) {
...
void MainUi::lobby( RunFuncPtr run ) {
    ...
    ConsoleUi::Element* menu = new ConsoleUi::Menu(
        "Lobby", { "Public Lobbies", "Create Lobby", "Enter Lobby Code" },
        "Exit" );
    ...
    for ( ;; ) {
        ...
        _ui->pop();
        _ui->pushInFront(
            new ConsoleUi::Menu( lobbyDisplay, lobbyText, "Exit" ) );
        ...
        if ( _lobby->mode == MENU ) {
            if ( mode < 0 || mode >= 4 ) {
                ...
                _ui->pushInFront(
                    new ConsoleUi::Menu( "Room Type", { "Public", "Private" },
                                         "Exit" ),
                ...
                _ui->pushInFront(
                    new ConsoleUi::Prompt( ConsoleUi::Prompt::String,
                                           "Enter lobby code:" ),
                    { 1, 0 }, true ); // Expand width and clear top
                _ui->popUntilUserInput();
            ...
            if ( mode < 0 || mode >= numEntries ) {
                ...
                if ( _lobby->mode != CONCERTO_LOBBY )
                    lobbyDisplay =
                        "  | Room | Players | Error: " + _lobby->lobbyError;
            }
...
void MainUi::spectate( RunFuncPtr run ) {
    ConsoleUi::Prompt* menu = new ConsoleUi::Prompt(
        ConsoleUi::Prompt::String, "Enter/paste <ip>:<port> to spectate:" );
    ...
    for ( ;; ) {
        ...
        if ( _address.addr.empty() ) {
            _ui->pushBelow( new ConsoleUi::TextBox( ERROR_INVALID_ADDR_PORT ),
                            { 1, 0 } ); // Expand width
            continue;
        }
...
void MainUi::broadcast( RunFuncPtr run ) {
    ...
    _ui->top< ConsoleUi::Prompt >()->setInitial( _address.port ? _address.port
                                                               : INT_MIN );
    ...
    for ( ;; ) {
        ...
        if ( menu->resultInt > 0xFFFF ) {
            _ui->pushBelow( new ConsoleUi::TextBox( ERROR_INVALID_PORT ),
                            { 1, 0 } ); // Expand width
            continue;
        }
...
void MainUi::offline( RunFuncPtr run ) {
    ...
    _netplayConfig.trialFlashColor =
        _config.getInteger( "trialScreenFlashColor" );
    RUN( "", _netplayConfig );
...
bool MainUi::areYouSure() {
    _ui->pushRight(
        new ConsoleUi::Menu( "Are you sure?", { "Yes" }, "Cancel" ) );
...
bool MainUi::gameMode( bool below ) {
    ConsoleUi::Menu* menu =
        new ConsoleUi::Menu( "Mode", { "Versus", "Training" }, "Cancel" );
    ...
    _ui->popUntilUserInput(
        true ); // Clear other messages since we're starting the game now
...
bool MainUi::offlineGameMode() {
    _ui->pushRight( new ConsoleUi::Menu(
        "Mode",
        { "Training", "Versus", "Versus CPU", "Tournament", "Replay", "Trial" },
        "Cancel" ) );
    _ui->top< ConsoleUi::Menu >()->setPosition(
        _config.getInteger( "lastOfflineMenuPosition" ) - 1 );

    int mode = _ui->popUntilUserInput( true )
                   ->resultInt; // Clear other messages since
...
void MainUi::joystickToBeDetached( Controller* controller ) {
    LOG( "controller=%08x; currentController=%08x", controller,
         _currentController );
...
void MainUi::controls() {
    for ( ;; ) {
        ...
        vector< Controller* > controllers =
            ControllerManager::get().getControllers();
        ...
        int ret =
            _ui->popUntilUserInput( true )
                ->resultInt; // Clear popped since we don't care about messages
        _ui->pop();
        ...
        for ( ;; ) {
            ...
            for ( size_t i = 0; i < gameInputBits.size(); ++i ) {
                const string mapping =
                    controller.getMapping( gameInputBits[ i ].second );
                options[ i ] =
                    format( "%-11s: %s", gameInputBits[ i ].first, mapping );
            }
            ...
            if ( controller.isJoystick() )
                options.push_back( format( "Set joystick deadzone (%.2f)",
                                           controller.getDeadzone() ) );

            options.push_back(
                format( "Set SOCD method: %s", controller.getSocd() ) );
            ...
            if ( pos == ( int )gameInputBits.size() + 2 &&
                ...
                _ui->pushRight( new ConsoleUi::Prompt(
                    ConsoleUi::Prompt::String,
                    "Enter a value between 0.0 and 1.0:" ) );

                _ui->top< ConsoleUi::Prompt >()->setInitial(
                    format( "%.2f", controller.getDeadzone() ) );
            ...
            if ( pos == ( int )gameInputBits.size() + 2 ||
                ...
                _ui->pushRight( new ConsoleUi::Menu(
                    "SOCD settings",
                    { "Default", "L/R cancel", "U/D cancel", "L/R U/D cancel",
                      "U/D=U", "L/R cancel U/D=U" },
                    "Exit" ) );
                _ui->top< ConsoleUi::Menu >()->setPosition(
            ...
            // Map selected key
            _ui->top< ConsoleUi::Menu >()->overlayCurrentPosition(
                format( "%-11s: ...", gameInputBits[ pos ].first ) );
            ...
            try {
                ...
                if ( controller.isKeyboard() ) {
                    controller.startMapping( this,
                                             gameInputBits[ pos ].second );

                    EventManager::get().start();
                } else {
                    ControllerManager::get()
                        .check(); // Flush joystick events before mapping
...
void MainUi::settings() {
    ...
    for ( ;; ) {
        ConsoleUi::Element* menu = _ui->popUntilUserInput(
            true ); // Clear popped since we don't care about messages
        ...
        switch ( menu->resultInt ) {
            case 0: {
                _ui->pushInFront(
                    new ConsoleUi::Menu( "Alert when connected?",
                                         { "Focus window", "Play a sound",
                                           "Do both", "Don't alert" },
                                         "Cancel" ),
                    { 0, 0 }, true ); // Don't expand but DO clear top

                _ui->top< ConsoleUi::Menu >()->setPosition(
                    ( _config.getInteger( "alertOnConnect" ) + 3 ) % 4 );
                _ui->popUntilUserInput();
                ...
                _ui->pushInFront( new ConsoleUi::TextBox(
                                  ...
                                  { 1, 0 },
                                  true ); // Expand width and clear top
                ...
                _ui->top< ConsoleUi::Prompt >()->setInitial(
                    _config.getString( "alertWavFile" ) );
                _ui->popUntilUserInput();
                ...
                if ( _ui->top()->resultInt == 0 ) {
                    if ( _ui->top()->resultStr.empty() )
                        ...
                        _config.setString( "alertWavFile",
                                           _ui->top()->resultStr );
                    saveConfig();
                }
            ...
            case 1:
                _ui->pushInFront( new ConsoleUi::Prompt(
                                      ConsoleUi::Prompt::String,
            ...
            case 2:
                _ui->pushInFront(
                    new ConsoleUi::Menu(
                        "Show full character names when spectating?",
                        { "Yes", "No" }, "Cancel" ),
                    { 0, 0 }, true ); // Don't expand but DO clear top

                _ui->top< ConsoleUi::Menu >()->setPosition(
                    ( _config.getInteger( "fullCharacterName" ) + 1 ) % 2 );
                _ui->popUntilUserInput();

                if ( _ui->top()->resultInt >= 0 &&
                     _ui->top()->resultInt <= 1 ) {
                    _config.setInteger( "fullCharacterName",
                                        ( _ui->top()->resultInt + 1 ) % 2 );
                    saveConfig();
                }
            ...
            case 3:
                _ui->pushInFront(
                    new ConsoleUi::Menu( "Start game with high CPU priority?",
                                         { "Yes", "No" }, "Cancel" ),
                    { 0, 0 }, true ); // Don't expand but DO clear top

                _ui->top< ConsoleUi::Menu >()->setPosition(
                    ( _config.getInteger( "highCpuPriority" ) + 1 ) % 2 );
                _ui->popUntilUserInput();

                if ( _ui->top()->resultInt >= 0 &&
                     _ui->top()->resultInt <= 1 ) {
                    _config.setInteger( "highCpuPriority",
                                        ( _ui->top()->resultInt + 1 ) % 2 );
                    saveConfig();
                }
            ...
            case 4:
                _ui->pushInFront(
                    new ConsoleUi::Prompt( ConsoleUi::Prompt::Integer,
                                           "Enter versus mode win count:" ),
                    { 0, 0 }, true ); // Don't expand but DO clear top

                _ui->top< ConsoleUi::Prompt >()->allowNegative = false;
                _ui->top< ConsoleUi::Prompt >()->maxDigits = 1;
                _ui->top< ConsoleUi::Prompt >()->setInitial(
                    _config.getInteger( "versusWinCount" ) );
                ...
                for ( ;; ) {
                    ...
                    if ( _ui->top()->resultInt == 0 ) {
                        _ui->pushBelow( new ConsoleUi::TextBox(
                            "Win count can't be zero!" ) );
                        continue;
                    }

                    if ( _ui->top()->resultInt > 5 ) {
                        _ui->pushBelow( new ConsoleUi::TextBox(
                            "Win count can't be greater than 5!" ) );
                        continue;
                    }

                    _config.setInteger( "versusWinCount",
                                        _ui->top()->resultInt );
                    saveConfig();
                    break;
                }
            ...
            case 5:
                _ui->pushInFront(
                    new ConsoleUi::Menu( "Check for updates on startup?",
                                         { "Yes", "No" }, "Cancel" ),
                    { 0, 0 }, true ); // Don't expand but DO clear top

                _ui->top< ConsoleUi::Menu >()->setPosition(
                    ( _config.getInteger( "autoCheckUpdates" ) + 1 ) % 2 );
                _ui->popUntilUserInput();

                if ( _ui->top()->resultInt >= 0 &&
                     _ui->top()->resultInt <= 1 ) {
                    _config.setInteger( "autoCheckUpdates",
                                        ( _ui->top()->resultInt + 1 ) % 2 );
                    saveConfig();
                }
            ...
            case 6:
                ...
                _ui->top< ConsoleUi::Prompt >()->setInitial(
                    _config.getInteger( "maxRealDelay" ) );
                ...
                for ( ;; ) {
                    ...
                    if ( _ui->top()->resultInt == 0 ) {
                        _ui->pushBelow( new ConsoleUi::TextBox(
                            "Max network delay can't be zero!" ) );
                        continue;
                    }

                    if ( _ui->top()->resultInt >= 0xFF ) {
                        _ui->pushBelow(
                            new ConsoleUi::TextBox( ERROR_INVALID_DELAY ) );
                        continue;
                    }
            ...
            case 7:
                _ui->pushInFront(
                    new ConsoleUi::Prompt( ConsoleUi::Prompt::Integer,
                                           "Enter default rollback:" ),
                    { 0, 0 }, true ); // Don't expand but DO clear top

                _ui->top< ConsoleUi::Prompt >()->allowNegative = false;
                _ui->top< ConsoleUi::Prompt >()->maxDigits = 2;
                _ui->top< ConsoleUi::Prompt >()->setInitial(
                    _config.getInteger( "defaultRollback" ) );
                ...
                for ( ;; ) {
                    ...
                    if ( _ui->top()->resultInt > MAX_ROLLBACK ) {
                        _ui->pushBelow( new ConsoleUi::TextBox( format(
                            ERROR_INVALID_ROLLBACK, 1 + MAX_ROLLBACK ) ) );
                        continue;
                    }

                    _config.setInteger( "defaultRollback",
                                        _ui->top()->resultInt );
                    saveConfig();
                    break;
                }
            ...
            case 8: {
                ...
                for ( ;; ) {
                    ...
                    if ( !( ss >> value ) || value < 0.0 ) {
                        _ui->pushBelow( new ConsoleUi::TextBox(
                            "Must be a decimal number and at least 0.0!" ) );
                        continue;
                    }
            ...
            case 9: {
                _ui->pushInFront(
                    new ConsoleUi::Menu( "Turn on automatic replay saving?",
                                         { "Yes", "No" }, "Cancel" ),
                    { 0, 0 }, true ); // Don't expand but DO clear top

                _ui->top< ConsoleUi::Menu >()->setPosition(
                    ( _config.getInteger( "autoReplaySave" ) + 1 ) % 2 );
                _ui->popUntilUserInput();

                if ( _ui->top()->resultInt >= 0 &&
                     _ui->top()->resultInt <= 1 ) {
                    _config.setInteger( "autoReplaySave",
                                        ( _ui->top()->resultInt + 1 ) % 2 );
                    saveConfig();
                }
            ...
            case 10: {
                vector< string > regions = { "NA West", "NA East",     "SA",
                                             "EU",      "Middle East", "Asia",
                                             "OCE" };
                _ui->pushInFront( new ConsoleUi::Menu( "Set Matchmaking Region",
                                                       regions, "Cancel" ),
                                  { 0, 0 },
                                  true ); // Don't expand but DO clear top
                ...
                if ( _ui->top()->resultInt >= 0 &&
                     _ui->top()->resultInt < regions.size() ) {
                    _config.setString( "matchmakingRegion",
                                       regions[ _ui->top()->resultInt ] );
                    saveConfig();
                }
            ...
            case 11:
                _ui->pushInFront(
                    new ConsoleUi::Menu(
                        "Trial Input Guide Options",
                        { "Trial Audio Cue", "Trial Screen Flash Color" },
                        "Cancel" ),
                    { 0, 0 }, true ); // Don't expand but DO clear top
                while ( true ) {
                    _ui->popUntilUserInput();
                    ...
                    if ( trialSetting == 0 ) {
                        ...
                        _ui->pushBelow(
                            ...
                            { 1, 0 } ); // Expand width
                        _ui->top< ConsoleUi::Prompt >()->setInitial(
                            _config.getString( "trialAudioCueFile" ) );
                        _ui->popUntilUserInput();
                        if ( _ui->top()->resultInt == 0 ) {
                            if ( _ui->top()->resultStr.empty() )
                                _config.setString( "trialAudioCueFile",
                                                   "SystemDefault" );
                            else
                                _config.setString( "trialAudioCueFile",
                                                   _ui->top()->resultStr );
                        ...
                        _ui->pushBelow(
                            ...
                            { 1, 0 } ); // Expand width
                        stringstream stream;
                        stream << "0x" << setfill( '0' ) << setw( 8 ) << hex
                               << _config.getInteger( "trialScreenFlashColor" );
                        ...
                        if ( _ui->top()->resultInt == 0 ) {
                            if ( _ui->top()->resultStr.empty() )
                                _config.setInteger( "trialScreenFlashColor",
                                                    0xff0000ff );
                            else {
                                string resStr = _ui->top()->resultStr;
                                LOG( resStr );
                                uint32_t resInt =
                                    strtoul( resStr.c_str(), NULL, 16 );
                                LOG( resInt );
                                _config.setInteger( "trialScreenFlashColor",
                                                    resInt );
                            }
            ...
            case 12:
                _ui->pushInFront(
                    new ConsoleUi::Menu( "Experimental Options",
                ...
                while ( true ) {
                    ...
                    if ( setting == 0 ) {
                        ...
                        _ui->top< ConsoleUi::Menu >()->setPosition(
                            ( _config.getInteger( "frameLimiter" ) + 1 ) % 2 );
                        _ui->popUntilUserInput();
...
void MainUi::results() {
    ...
    for ( ;; ) {
        ConsoleUi::Element* menu = _ui->popUntilUserInput(
            false ); // Clear popped since we don't care about messages
...
void MainUi::initialize() {
    _ui.reset( new ConsoleUi( uiTitle + LocalVersion.suffix(),
    ...
    _config.setInteger( "updateChannel",
                        static_cast< int >( MainUpdater::Channel::Stable ) );
    ...
    // Load and save main config (this creates the config file on the first
    // time)
    loadConfig();
    saveConfig();
    ...
    ControllerManager::get().loadMappings( ProcessManager::appDir + FOLDER,
                                           MAPPINGS_EXT );
    ControllerManager::get().saveMappings( ProcessManager::appDir + FOLDER,
                                           MAPPINGS_EXT );

    // Initialize updater
    _updater.setTemporal( MainUpdater::Temporal::Latest );
    _updater.setChannel( static_cast< MainUpdater::Channel::Enum >(
        _config.getInteger( "updateChannel" ) ) );
}
...
void MainUi::saveMappings( const Controller& controller ) {
    const string file =
        ProcessManager::appDir + FOLDER + controller.getName() + MAPPINGS_EXT;
...
static bool isOnline() {
    ...
    return ( state & ( INTERNET_CONNECTION_LAN | INTERNET_CONNECTION_MODEM |
...
void MainUi::main( RunFuncPtr run ) {
    ...
    for ( ;; ) {
        const vector< string > options = {
            "Netplay",  "Spectate", "Broadcast", "Offline", "Server",
            "Controls", "Settings", "Update",    "Results",
        };
        ...
        _ui->pushRight( new ConsoleUi::TextBox( message ),
                        { 1, 0 } ); // Expand width
...
void MainUi::display( const string& message, bool replace ) {
    if ( replace && ( _ui->empty() || !_ui->top()->requiresUser ||
                      _ui->top() != mainMenu ) )
        _ui->pushInFront( new ConsoleUi::TextBox( message ), { 1, 0 },
                          true ); // Expand width and clear top
    else if ( _ui->top() == 0 || _ui->top()->expandWidth() )
        _ui->pushBelow( new ConsoleUi::TextBox( message ),
                        { 1, 0 } ); // Expand width
    else
        _ui->pushRight( new ConsoleUi::TextBox( message ),
                        { 1, 0 } ); // Expand width
...
bool MainUi::configure( const PingStats& pingStats ) {
    ...
    int rollback = clamped( delay + worst + variance, 0,
                            _config.getInteger( "defaultRollback" ) );
    ...
    _ui->pushBelow( new ConsoleUi::Prompt( ConsoleUi::Prompt::Integer,
                                           "Enter max frames of rollback:" ) );
    ...
    for ( ;; ) {
        ...
        _ui->top< ConsoleUi::Prompt >()->maxDigits = 3;
        _ui->top< ConsoleUi::Prompt >()->setInitial(
            clamped( delay - rollback, 0, delay ) );
...
#else
            _netplayConfig.hostPlayer =
                1; // Host is always player 1 for easier debugging
#endif
    ...
    if ( ret ) {
        _ui->pushBelow(
            new ConsoleUi::TextBox( format(
                "Using %u delay%s",
                ( _netplayConfig.rollback ? _netplayConfig.rollbackDelay
                                          : _netplayConfig.delay ),
                ( _netplayConfig.rollback
                      ? format( ", %u rollback", _netplayConfig.rollback )
                      : "" ) ) ),
            { 1, 0 } ); // Expand width
    }
...
bool MainUi::connected( const InitialConfig& initialConfig,
    ...
    const string connectString =
        ( initialConfig.mode.isHost()
              ? initialConfig.remoteName + " connected"
              : "Connected to " + initialConfig.remoteName );

    const string modeString =
        ( initialConfig.mode.isTraining()
              ? "Training mode"
              : format( "Versus mode, each game is %u rounds",
                        initialConfig.winCount ) );

    _ui->pushInFront(
        new ConsoleUi::TextBox( connectString + "\n\n" + modeString + "\n\n" +
                                formatStats( pingStats ) ),
        { 1, 0 }, true ); // Expand width and clear top

    if ( configure( pingStats ) ) {
        display( string( "Waiting for " ) +
                     ( initialConfig.mode.isHost() ? "client" : "host" ) +
                     "...",
                 false );
        return true;
    }
...
void MainUi::spectate( const SpectateConfig& spectateConfig ) {
    ...
    if ( spectateConfig.mode.isBroadcast() ) {
        text = format(
            "Spectating a %s mode broadcast (0 delay)\n\n",
            ( spectateConfig.mode.isTraining() ? "training" : "versus" ) );
    } else {
        text = format(
            "Spectating %s mode (%u delay%s)\n\n",
            ( spectateConfig.mode.isTraining() ? "training" : "versus" ),
            spectateConfig.delay,
            ( spectateConfig.rollback
                  ? format( ", %u rollback", spectateConfig.rollback )
                  : "" ) );
    }

    CharaNameFunc charaNameFunc =
        ( _config.getInteger( "fullCharacterName" ) ? getFullCharaName
                                                    : getShortCharaName );
    ...
    if ( spectateConfig.initial.netplayState <= NetplayState::CharaSelect ) {
        if ( spectateConfig.mode.isBroadcast() )
            ...
            text += format( "%s vs %s", spectateConfig.names[ 0 ],
                            spectateConfig.names[ 1 ] );
    } else {
        text += spectateConfig.formatPlayer( 1, charaNameFunc ) + " vs " +
                spectateConfig.formatPlayer( 2, charaNameFunc );
    }

    text += "\n\n(Tap the spacebar to toggle fast-forward)";

    _ui->pushInFront( new ConsoleUi::TextBox( text ), { 1, 0 },
                      true ); // Expand width and clear top
}
...
#endif
                   ,
                   format( "Network delay: %d",
                           computeDelay( pingStats.latency.getMean() ) ),
                   pingStats.latency.getMean()
#ifndef NDEBUG
                       ,
                   "", pingStats.latency.getWorst(), "",
                   pingStats.latency.getStdErr(), "",
                   pingStats.latency.getStdDev(), "", pingStats.packetLoss
#endif
    );
}
...
string MainUi::getUpdate( bool isStartup ) {
    ...
    if ( _updater.getTargetVersion().empty() ) {
        return "Cannot fetch info for " + _updater.getTargetDescName() +
               " version";
    }

    if ( LocalVersion.isSimilar(
             _updater.getTargetVersion(),
             ( _config.getInteger( "updateChannel" ) - 1 ) ? 3 : 2 ) ) {
        _upToDate = true;
        if ( !isStartup )
            return "You already have the " + _updater.getTargetDescName() +
                   " version";
        return "";
    }
    ...
    for ( ;; ) {
        ...
        _ui->pushBelow(
            new ConsoleUi::Menu( "Update?", { "Yes", "View changes" }, "No" ) );
...
void MainUi::update() {
    ...
    for ( ;; ) {
        if ( !msg.empty() )
            _ui->pushBelow( new ConsoleUi::TextBox( msg ) );
        ConsoleUi::Element* menu =
            _ui->popUntilUserInput( false ); // Don't clear popped elements yet
        ...
        switch ( menu->resultInt ) {
            case 0:
                _ui->pushBelow(
                    new ConsoleUi::TextBox( "Checking for updates..." ) );
                msg = getUpdate();
                sessionMessage = msg;
                _ui->pop();
                break;
            ...
            case 2:
                _ui->pushInFront(
                    new ConsoleUi::Menu( "Select update type",
                                         { "Stable", "Dev" }, "Cancel" ),
                    { 0, 0 }, true ); // Don't expand but DO clear top
                ...
                int p;
                switch (
                    static_cast< MainUpdater::Channel::Enum >( oldChannel ) ) {
...
void MainUi::openChangeLog() {
    const DWORD val = GetFileAttributes(
        ( ProcessManager::appDir + FOLDER CHANGELOG ).c_str() );
    ...
    if ( ProcessManager::isWine() )
        ...
        system( ( "\"start \"Viewing change log\" notepad " + file + "\"" )
                    .c_str() );
}
...
void MainUi::fetchCompleted( MainUpdater* updater,
                             const MainUpdater::Type& type ) {
...
void MainUi::fetchFailed( MainUpdater* updater,
                          const MainUpdater::Type& type ) {
    ASSERT( updater == &_updater );
    ...
    switch ( type.value ) {
        case MainUpdater::Type::Version:
            sessionMessage = "Cannot fetch info for " +
                             _updater.getTargetDescName() + " version";
            break;
        ...
        case MainUpdater::Type::Archive:
            sessionMessage =
                "Cannot download " + _updater.getTargetDescName() + " version";
            break;
...
void MainUi::fetchProgress( MainUpdater* updater,
                            const MainUpdater::Type& type,
                            double progress ) {
    if ( type == MainUpdater::Type::Version )
        return;
