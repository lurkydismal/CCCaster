...
static inline int getTextHeight( const array< string, 3 >& newText ) {
    ...
    for ( const string& text : newText )
        height =
            max( height, OVERLAY_FONT_HEIGHT *
                             ( 1 + count( text.begin(), text.end(), '\n' ) ) );
...
void updateSelector( uint8_t index, int position, const string& line ) {
    ...
    rect.top += OVERLAY_TEXT_BORDER + position * OVERLAY_FONT_HEIGHT -
                OVERLAY_SELECTOR_Y_BORDER + 1;
    rect.bottom += OVERLAY_TEXT_BORDER + position * OVERLAY_FONT_HEIGHT +
                   OVERLAY_SELECTOR_Y_BORDER;

    if ( index == 0 ) {
        rect.left += OVERLAY_TEXT_BORDER - OVERLAY_SELECTOR_X_BORDER;
        rect.right += OVERLAY_TEXT_BORDER + OVERLAY_SELECTOR_X_BORDER;
    } else {
        rect.left = ( *CC_SCREEN_WIDTH_ADDR ) - rect.right -
                    OVERLAY_TEXT_BORDER - OVERLAY_SELECTOR_X_BORDER;
        rect.right = ( *CC_SCREEN_WIDTH_ADDR ) - OVERLAY_TEXT_BORDER +
                     OVERLAY_SELECTOR_X_BORDER;
    }
...
void initOverlayText( IDirect3DDevice9* device ) {
    ...
    device->CreateVertexBuffer(
        4 * sizeof( Vertex ), // buffer size in bytes
...
void renderOverlayText( IDirect3DDevice9* device,
    ...
    //                    ( debugTextAlign == 0 ? DT_CENTER : ( debugTextAlign <
    //                    0 ? DT_LEFT : DT_RIGHT ) ), OVERLAY_DEBUG_COLOR );
    //     }
    ...
    if ( !TrialManager::dtext.empty() && !TrialManager::hideText ) {
        ...
        DrawText( font, TrialManager::dtext, rect2,
                  DT_WORDBREAK |
                      ( debugTextAlign == 0
                            ? DT_CENTER
                            : ( debugTextAlign < 0 ? DT_LEFT : DT_RIGHT ) ),
                  OVERLAY_DEBUG_COLOR );
    }
    if ( !TrialManager::comboTrialText.empty() && !TrialManager::hideText ) {
        ...
        if ( TrialManager::trialTextures == NULL ) {
            ...
            int (*loadTextureFromMemory) (int, char*, int, int) = (int(*)(int,
        char*, int, int)) 0x4bd2d0; D3DXCreateTextureFromFile( device, filename,
        &TrialManager::trialTextures ); TrialManager::trialTextures2 =
        loadTextureFromMemory(0, rawimg, imgsize, 0);
        }
        */
        RECT rect;
        rect.top = 70;
        ...
        wstring w =
            TrialManager::fullStrings[ TrialManager::currentTrialIndex ];
        ...
        for ( wstring text : TrialManager::comboTrialText ) {
            ...
            D3DCOLOR color = ( TrialManager::comboTrialPosition > i )
                                 ? OVERLAY_BUTTON_DONE_COLOR
                             : ( TrialManager::comboTrialPosition == i )
                                 ? OVERLAY_DEBUG_COLOR
                                 : OVERLAY_BUTTON_COLOR;
            DrawTextW(
                font, text, rect,
                DT_WORDBREAK | ( TrialManager::comboTrialTextAlign == 0
                                     ? DT_CENTER
        ...
        TextCalcRectW( font, TrialManager::comboName, rect3,
                       DT_LEFT | DT_WORDBREAK, 0 );
        rect3.left = 20;
    ...
    if ( isShowingMessage() ) {
        ...
        DrawText( font, text[ 1 ], rect, DT_CALCRECT,
                  D3DCOLOR_XRGB( 0, 0, 0 ) );
    ...
    const float scaleX =
        ( isShowingMessage() ? messageWidth / viewport.Width : 1.0f );
    const float scaleY =
        float( height + 2 * OVERLAY_TEXT_BORDER ) / viewport.Height;
    ...
    if ( !( text[ 0 ].empty() && text[ 1 ].empty() && text[ 2 ].empty() ) ) {
        ...
        RECT rect;
        rect.left =
            centerX - int( ( viewport.Width / 2 ) * 1.0 ) + OVERLAY_TEXT_BORDER;
        rect.right =
            centerX + int( ( viewport.Width / 2 ) * 1.0 ) - OVERLAY_TEXT_BORDER;
        ...
        if ( newHeight == height ) {
            if ( shouldDrawSelector[ 0 ] )
                DrawRectangle( device, INLINE_RECT( selector[ 0 ] ),
                               OVERLAY_SELECTOR_L_COLOR );

            if ( shouldDrawSelector[ 1 ] )
                DrawRectangle( device, INLINE_RECT( selector[ 1 ] ),
                               OVERLAY_SELECTOR_R_COLOR );
        }

        if ( !text[ 0 ].empty() )
            DrawText( font, text[ 0 ], rect, DT_WORDBREAK | DT_LEFT,
                      OVERLAY_TEXT_COLOR );

        if ( !text[ 1 ].empty() )
            DrawText( font, text[ 1 ], rect, DT_WORDBREAK | DT_CENTER,
                      OVERLAY_TEXT_COLOR );

        if ( !text[ 2 ].empty() )
            DrawText( font, text[ 2 ], rect, DT_WORDBREAK | DT_RIGHT,
                      OVERLAY_TEXT_COLOR );
    }
}
