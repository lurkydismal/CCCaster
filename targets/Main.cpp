...
static bool initDirsAndSanityCheck( bool checkGameExe = true ) {
    ...
    DWORD val =
        GetFileAttributes( ( ProcessManager::appDir + FOLDER ).c_str() );
    ...
    if ( val == INVALID_FILE_ATTRIBUTES ) {
        ...
        if ( search( istream_iterator< char >( dll ),
                     istream_iterator< char >(), code.begin(),
                     code.end() ) == istream_iterator< char >() ) {
    ...
    if ( checkGameExe ) {
        val =
            GetFileAttributes( ( ProcessManager::gameDir + MBAA_EXE ).c_str() );
...
int main( int argc, char* argv[] ) {
    ...
    static const Descriptor desc[] = {
        ...
        { Options::Help, 0, "h", "help", Arg::None,
          "  --help, -h           Print help and exit.\n" },
        { Options::GameDir, 0, "f", "folder", Arg::Required,
          "  --folder, -f F       Use game folder at F.\n" },

        { Options::Training, 0, "t", "training", Arg::None,
          "  --training, -t       Force training mode." },
        { Options::Broadcast, 0, "b", "broadcast", Arg::None,
          "  --broadcast, -b      Force broadcast mode." },
        { Options::Spectate, 0, "s", "spectate", Arg::None,
          "  --spectate, -s       Force spectator mode.\n" },
        ...
        { Options::Offline, 0, "o", "offline", Arg::OptionalNumeric,
          ...
          "                         D is the optional delay, defaults to "
          "0.\n" },
        ...
        // #ifndef RELEASE
        //         { Options::Unknown,   0,  "",       "", Arg::None, "Debug
        //         options:" }, { Options::Tests,     0,  "",  "tests",
        //         Arg::None,        "  --tests              Run unit tests and
        //         exit" }, { Options::Stdout,    0,  "", "stdout", Arg::None,
        //         "  --stdout,            Output logs to stdout"}, {
        //         Options::Tunnel,    0,  "", "tunnel", Arg::None,        "
        //         --tunnel             Force UDP tunnel" }, { Options::Dummy,
        //         0,  "",  "dummy", Arg::None,        "  --dummy Dummy with
        //         fake inputs" }, { Options::PidLog,    0,  "", "pidlog",
        //         Arg::None,        "  --pidlog             Tag log files with
        //         the PID" }, { Options::FakeUi,    0,  "",   "fake",
        //         Arg::None,        "  --fake               Fake UI mode\n" },

        //         {
        //             Options::StrictVersion, 0, "S", "strict", Arg::None,
        //             "  --strict, -S         Strict version match, can be
        //             stacked up to 3 times.\n" "                         -S
        //             means version suffix must match.\n" " -SS means commit ID
        //             must match.\n" "                         -SSS means build
        //             time must match.\n"
        //         },
        ...
        //             "  --replay, -R args    Replay the given file with
        //             options.\n" "                         TODO list possible
        //             arguments.\n"
        //         },
        // #else
        ...
        { Options::NoFork, 0, "", "no-fork", Arg::None,
          0 }, // Don't fork when inside Wine, ie when under wineconsole

        { Options::Unknown, 0, "", "", Arg::None,
          "Examples:\n"
          "  " BINARY " 12345                 Host on port 12345\n"
          "  " BINARY
          " 12.34.56.78 12345     Connect to 12.34.56.78 on port 12345\n"
          "  " BINARY
          " 12.34.56.78:12345     Connect to 12.34.56.78 on port 12345\n"
          "  " BINARY " -b 12345              Broadcast on port 12345\n"
          "  " BINARY " -ot                   Offline training mode\n"
          "  " BINARY
          " -o 4 -t               Offline training mode with 4 delay\n" },
    ...
    if ( !initDirsAndSanityCheck( !opt[ Options::Tests ] &&
                                  !opt[ Options::Dummy ] ) ) {
    ...
    if ( opt[ Options::Stdout ] )
        ...
        Logger::get().initialize( ProcessManager::appDir + LOG_FILE,
                                  LOG_DEFAULT_OPTIONS | PID_IN_FILENAME );
    else
        Logger::get().initialize( ProcessManager::appDir + LOG_FILE );
    ...
    // Initialize config
    ui.initialize();
    ui.initialConfig.mode.flags |=
        ( opt[ Options::Training ] && !opt[ Options::Tournament ]
              ? ClientMode::Training
              : 0 );
    ...
    if ( opt[ Options::Offline ] || opt[ Options::Tournament ] ) {
        ...
        if ( ui.initialConfig.mode.value == ClientMode::Unknown )
            ui.initialConfig.mode.value =
                ( address.addr.empty() ? ClientMode::Host
                                       : ClientMode::Client );
        ...
        const string str =
            string( parser.nonOption( 0 ) ) + ":" + parser.nonOption( 1 );
