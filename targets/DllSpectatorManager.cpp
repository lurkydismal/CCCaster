...
SpectatorManager::SpectatorManager( NetplayManager* netManPtr,
                                    const ProcessManager* procManPtr )
...
void SpectatorManager::pushSpectator( Socket* socketPtr,
    ...
    // This way whenever a new spectator causes a decrease in the broadcast
    // interval, later spectators will still get their next set of inputs late
    // enough that they don't need to wait another interval.
    //
    // Example:
    //
    // Spectators: [2] 1        interval is 15/2 = 7, so #1 will get a broadcast
    // in 7 frames
    //
    // Spectators: [2] 3 1      interval is 15/3 = 5, now #1 will get a
    // broadcast in 10 frames
    //
    // Spectators: 2 [3] 1      interval is 15/3 = 5, so #1 will get a broadcast
    // in 5 frames
    //
    // Spectators: 2 [3] 4 1    interval is 15/4 = 3, now #1 will get a
    // broadcast in 6 frames
    //
    ...
    if ( _spectatorList.empty() )
        ...
        it = _spectatorList.insert( incremented( _spectatorListPos ),
                                    socketPtr );
    ...
    _netManPtr->preserveStartIndex =
        min( _netManPtr->preserveStartIndex, spectator.pos.parts.index );

    LOG( "socket=%08x; spectator.pos=[%s]; preserveStartIndex=%u", socketPtr,
         spectator.pos, _netManPtr->preserveStartIndex );
    ...
    switch ( netplayState ) {
        case NetplayState::CharaSelect:
            newSocket->send(
                _netManPtr->getRngState( spectator.pos.parts.index ) );
            break;
        ...
        case NetplayState::RetryMenu:
            newSocket->send( _netManPtr->getRngState(
                spectator.pos.parts.index + ( isTraining ? 1 : 2 ) ) );
            break;
    }

    newSocket->send(
        new InitialGameState( spectator.pos, netplayState, isTraining ) );
}
...
void SpectatorManager::frameStepSpectators() {
    ...
    const uint32_t multiplier =
        1 + ( _spectatorList.size() * 2 ) / ( NUM_INPUTS + 1 );

    // Number of frames between each broadcast
    const uint32_t interval =
        ( multiplier * NUM_INPUTS / 2 ) / _spectatorList.size();
    ...
    for ( uint32_t i = 0; i < multiplier; ++i ) {
        ...
        LOG( "socket=%08x; spectator.pos=[%s]; preserveStartIndex=%u; "
             "sentRng=%d; oldIndex=%d",
             socket, spectator.pos, _netManPtr->preserveStartIndex,
             spectator.sentRngState, oldIndex );
