...
MH_WINAPI_HOOK( LRESULT,
    ...
    switch ( msg ) {
        ...
        case WM_KEYUP: {
            ...
            // Note: this doesn't actually eat the keyboard event, which is
            // actually acceptable for the in-game overlay UI, since we need to
            // mix usage with GetKeyState.
            KeyboardManager::get().owner->keyboardEvent( vkCode, scanCode,
                                                         isExtended, isDown );
            break;
        }
        ...
        case WM_DEVICECHANGE:
            switch ( wParam ) {
                ...
                case DBT_DEVICEREMOVECOMPLETE:
                    if ( ( ( DEV_BROADCAST_HDR* )lParam )->dbch_devicetype ==
        ...
        case WM_MOUSEMOVE: {
            ...
            MouseManager::get().owner->mouseEvent( x, y, isDown,
                                                   ( msg == WM_LBUTTONDOWN ),
                                                   ( msg == WM_LBUTTONUP ) );
            break;
        }
...
void initializePostLoad() {
    ...
    if ( !( notifyHandle = RegisterDeviceNotification(
                windowHandle, &dbh, DEVICE_NOTIFY_WINDOW_HANDLE ) ) )
        LOG( "RegisterDeviceNotification failed: %s",
...
// The following constructors should only be called when running in the DLL, ie
// MBAA's memory space
InitialGameState::InitialGameState( IndexedFrame indexedFrame,
...
#define SAVE_CHARA( N )                                           \
    ...
    chara[ N - 1 ].guardBar =                                     \
        ( *CC_INTRO_STATE_ADDR ? 0 : *CC_P##N##_GUARD_BAR_ADDR ); \
