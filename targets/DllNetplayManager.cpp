...
// Extra number to add to preserveStartIndex, this is a safety buffer for
// chained spectators.
...
uint16_t NetplayManager::getInitialInput( uint8_t player ) {
    ...
    // The host player should select the main menu, so that the host controls
    // training mode
...
uint16_t NetplayManager::getAutoCharaSelectInput( uint8_t player ) {
    *CC_P1_CHARA_SELECTOR_ADDR =
        ( uint32_t )charaToSelector( initial.chara[ 0 ] );
    *CC_P2_CHARA_SELECTOR_ADDR =
        ( uint32_t )charaToSelector( initial.chara[ 1 ] );
...
uint16_t NetplayManager::getCharaSelectInput( uint8_t player ) {
    ...
    // Prevent hitting Confirm until 150f after beginning of CharaSelect, this
    // is to workaround the moon selector desync
    if ( config.mode.isOnline() && getFrame() < 150 ) {
        input &= ~COMBINE_INPUT( 0, CC_BUTTON_A | CC_BUTTON_CONFIRM );
    }

    // Prevent exiting character select
    if ( ( *( player == 1 ? CC_P1_SELECTOR_MODE_ADDR
                          : CC_P2_SELECTOR_MODE_ADDR ) ) == CC_SELECT_CHARA ) {
        input &= ~COMBINE_INPUT( 0, CC_BUTTON_B | CC_BUTTON_CANCEL );
    }

    // Don't allow hitting Confirm or Cancel until 2f after we have stopped
    // changing the selector mode
    if ( hasButtonInHistory(
             player,
...
uint16_t NetplayManager::getSkippableInput( uint8_t player ) {
    ...
    if ( config.mode.isReplay() ) {
        ...
        if ( *CC_PAUSE_FLAG_ADDR ) {
            ...
            // Don't allow hitting Confirm until 3f after we have stopped moving
            // the cursor. This is a work around for the issue when select is
            // pressed after the cursor moves, but before currentMenuIndex is
            // updated.
...
uint16_t NetplayManager::getDemoInput( uint8_t player ) {
    ...
    if ( exitCountdown > 0 ) {
        exitCountdown--;
        if ( exitCountdown == 20 ) {
            *( player == 1 ? CC_P1_X_POSITION_ADDR : CC_P2_X_POSITION_ADDR ) =
                currentTrial.startingPositions[ 0 ];
            *( player == 1 ? CC_P2_X_POSITION_ADDR : CC_P1_X_POSITION_ADDR ) =
                currentTrial.startingPositions[ 1 ];
        }
        if ( exitCountdown == 0 )
            TrialManager::playDemo = false;
        return 0;
    ...
    if ( TrialManager::demoPosition == 0 && demoCountdown > 0 ) {
        ...
        if ( demoCountdown == 55 ) {
            ...
            LOG( "%d, %d", currentTrial.startingPositions[ 0 ],
                 currentTrial.startingPositions[ 1 ] );
            *( player == 1 ? CC_P1_X_POSITION_ADDR : CC_P2_X_POSITION_ADDR ) =
                currentTrial.startingPositions[ 0 ];
            *( player == 1 ? CC_P2_X_POSITION_ADDR : CC_P1_X_POSITION_ADDR ) =
...
uint16_t NetplayManager::getInGameInput( uint8_t player ) {
    ...
    // Disable pausing in netplay versus mode. Also only allow start button in
    // versus after holding it for a duration.
    if ( ( ( ( config.mode.isNetplay() && config.mode.isVersus() ) ||
             config.mode.isSpectate() ) ||
           ( !*CC_PAUSE_FLAG_ADDR && config.mode.isVersus() &&
             heldStartDuration &&
             !heldButtonInHistory( player, CC_BUTTON_START, 0,
                                   heldStartDuration ) ) ) &&
    ...
    if ( *CC_PAUSE_FLAG_ADDR ) {
        ...
        // Don't allow hitting Confirm until 3f after we have stopped moving the
        // cursor. This is a work around for the issue when select is pressed
        // after the cursor moves, but before currentMenuIndex is updated.
        if ( hasUpDownInHistory( player, 0, 3 ) )
            input &= ~COMBINE_INPUT( 0, CC_BUTTON_A | CC_BUTTON_CONFIRM );

        // Disable returning to main menu; 16 and 6 are the menu positions for
        // training and versus mode respectively
        if ( AsmHacks::currentMenuIndex ==
             ( config.mode.isTraining()
                   ? 16
                   : ( config.mode.isReplay() ? 11 : 6 ) ) )
            input &= ~COMBINE_INPUT( 0, CC_BUTTON_A | CC_BUTTON_CONFIRM );

    } else if ( config.mode.isTraining() && config.mode.isOffline() &&
                player == config.hostPlayer &&
        ...
        if ( _trainingResetState == -1 &&
             ( input &
               COMBINE_INPUT( 0, CC_BUTTON_FN2 ) ) ) // Initial reset input
        {
            ...
            *CC_P1_COMBO_GUARD_ADDR = 50;
                    ...
                    !( input &
                       COMBINE_INPUT( 0, CC_BUTTON_FN2 ) ) ) // Completed reset
        {
        ...
        } else if ( _trainingResetState > 10 ) // Check for held reset
        {
            ...
            if ( input & COMBINE_INPUT( 0, CC_BUTTON_FN2 ) ) {
                if ( _trainingResetType == 0 )
                    ...
                    *( player == 1 ? CC_P1_X_POSITION_ADDR
                                   : CC_P2_X_POSITION_ADDR ) = -65536;
                else if ( _trainingResetType == 1 )
                    *( player == 1 ? CC_P1_X_POSITION_ADDR
                                   : CC_P2_X_POSITION_ADDR ) = 65536;
        ...
        } else if ( _trainingResetState >= 0 ) // Reset in progress
        {
            if ( _trainingResetType == -1 ) {
                *( player == 1 ? CC_P1_X_POSITION_ADDR
                               : CC_P2_X_POSITION_ADDR ) = -45056;
                *( player == 1 ? CC_P2_X_POSITION_ADDR
                               : CC_P1_X_POSITION_ADDR ) = -61440;
                ...
                *( player == 1 ? CC_P1_X_POSITION_ADDR
                               : CC_P2_X_POSITION_ADDR ) = 16384;
                *( player == 1 ? CC_P2_X_POSITION_ADDR
                               : CC_P1_X_POSITION_ADDR ) = -16384;
            }
...
uint16_t NetplayManager::getRetryMenuInput( uint8_t player ) {
    ...
    if ( config.mode.isSpectateNetplay() ) {
        input =
            ( ( getFrame() % 2 ) ? 0 : COMBINE_INPUT( 0, CC_BUTTON_CONFIRM ) );
    } else {
    ...
    // Allow saving replays; when manual replay save is selected or any replay
    // save menu is open
    if ( AsmHacks::currentMenuIndex == 2 ||
         *CC_MENU_STATE_COUNTER_ADDR > _retryMenuStateCounter ) {
        AsmHacks::menuConfirmState = 2;
        return input;
    }
    ...
    if ( config.mode.isSpectateNetplay() ) {
        ...
        const bool doneAutoSave = !( *CC_AUTO_REPLAY_SAVE_ADDR ) ||
                                  ( AsmHacks::autoReplaySaveStatePtr &&
                                    *AsmHacks::autoReplaySaveStatePtr > 100 );

        // Navigate the menu when the menu index is ready AND we're done
        // auto-saving
        ...
        // Special netplay retry menu behaviour, only select final option after
        // both sides have selected
        if ( _remoteRetryMenuIndex != -1 && _localRetryMenuIndex != -1 ) {
            _targetMenuState = 0;
            _targetMenuIndex =
                max( _localRetryMenuIndex, _remoteRetryMenuIndex );
            _targetMenuIndex =
                min( _targetMenuIndex, ( int8_t )1 ); // Just in case...
...
MsgPtr NetplayManager::getRetryMenuIndex( uint32_t index ) const {
    ...
    return MsgPtr(
        new MenuIndex( index, _retryMenuIndicies[ index - _startIndex ] ) );
}

void NetplayManager::setRetryMenuIndex( uint32_t index, int8_t menuIndex ) {
    if ( config.mode.isOffline() || index == 0 || index < _startIndex ||
         menuIndex < 0 )
        return;
...
uint16_t NetplayManager::getMenuNavInput() {
    ...
    if ( _targetMenuState == 0 ) // Determined targetMenuIndex
    {
        ...
        _targetMenuState = 1;
    } else if ( _targetMenuState ==
                1 ) // Move up or down towards targetMenuIndex
    {
        _targetMenuState = 2;

        if ( _targetMenuIndex != ( int8_t )AsmHacks::currentMenuIndex )
            return COMBINE_INPUT(
                ( _targetMenuIndex < ( int8_t )AsmHacks::currentMenuIndex ? 8
                                                                          : 2 ),
                0 );
    } else if ( _targetMenuState >= 2 &&
                _targetMenuState <= 4 ) // Wait for currentMenuIndex to update
    {
        ++_targetMenuState;
    } else if ( _targetMenuState == 39 ) // Mash final menu selection
    {
        AsmHacks::menuConfirmState = 2;
        RETURN_MASH_INPUT( 0, CC_BUTTON_CONFIRM );
    } else if ( _targetMenuIndex !=
                ( int8_t )AsmHacks::currentMenuIndex ) // Keep navigating
    {
        _targetMenuState = 1;
    } else // Reached targetMenuIndex
    {
        LOG( "targetMenuIndex=%d; currentMenuIndex=%u", _targetMenuIndex,
...
bool NetplayManager::hasUpDownInHistory( uint8_t player,
                                         uint32_t start,
                                         uint32_t end ) const {
...
bool NetplayManager::hasButtonInHistory( uint8_t player,
                                         uint16_t button,
                                         uint32_t start,
                                         uint32_t end ) const {
    ASSERT( player == 1 || player == 2 );
...
bool NetplayManager::heldButtonInHistory( uint8_t player,
                                          uint16_t button,
                                          uint32_t start,
                                          uint32_t end ) const {
    ASSERT( player == 1 || player == 2 );
...
void NetplayManager::setState( NetplayState state ) {
    ...
    LOG( "indexedFrame=[%s]; previous=%s; current=%s", _indexedFrame, _state,
         state );
    ...
    if ( state.value >= NetplayState::CharaSelect ) {
        ...
        if ( state == NetplayState::Loading ) {
            ...
            const uint32_t newStartIndex =
                min( getBufferedPreserveStartIndex(), getIndex() );
            ...
            if ( newStartIndex > _startIndex ) {
                ...
                if ( offset >= _retryMenuIndicies.size() )
                    ...
                    _retryMenuIndicies.erase(
                        _retryMenuIndicies.begin(),
        ...
        if ( state == NetplayState::InGame ) {
            ...
            copy( CC_RNG_STATE3_ADDR, CC_RNG_STATE3_ADDR + CC_RNG_STATE3_SIZE,
                  rngState->rngState3.begin() );
            _roundRngStates.push_back( rngState );
        }

        // Entering RetryMenu
        if ( state == NetplayState::RetryMenu ) {
            // The actual retry menu is opened at position
            // *CC_MENU_STATE_COUNTER_ADDR + 1
...
uint16_t NetplayManager::getInput( uint8_t player ) {
    ...
    switch ( _state.value ) {
        ...
        case NetplayState::Skippable:
            ...
            if ( ( _startIndex + _inputs[ _remotePlayer - 1 ].getEndIndex() ) >
                 getIndex() + 1 ) {
                AsmHacks::menuConfirmState = 2;
                RETURN_MASH_INPUT( 0, CC_BUTTON_CONFIRM );
            }
...
void NetplayManager::setInput( uint8_t player, uint16_t input ) {
    ...
    if ( isInRollback() ) {
        ...
        _inputs[ player - 1 ].set( getIndex() - _startIndex, getFrame(),
                                   input );
    } else if ( config.mode.isOffline() && splitDelay ) {
        _inputs[ player - 1 ].set(
...
void NetplayManager::assignInput( uint8_t player,
                                  uint16_t input,
                                  uint32_t frame ) {
...
void NetplayManager::assignInput( uint8_t player,
    ...
    _inputs[ player - 1 ].assign( _indexedFrame.parts.index - _startIndex,
                                  _indexedFrame.parts.frame, input );
}

MsgPtr NetplayManager::getInputs( uint8_t player ) const {
    ASSERT( player == 1 || player == 2 );
    ASSERT( getIndex() >= _startIndex );
    ASSERT( _inputs[ player - 1 ].getEndFrame( getIndex() - _startIndex ) >=
            1 );

    PlayerInputs* playerInputs = new PlayerInputs(
        { _inputs[ player - 1 ].getEndFrame() - 1, getIndex() } );

    ASSERT( playerInputs->getIndex() >= _startIndex );

    _inputs[ player - 1 ].get(
        playerInputs->getIndex() - _startIndex, playerInputs->getStartFrame(),
        &playerInputs->inputs[ 0 ], playerInputs->size() );

    return MsgPtr( playerInputs );
}

void NetplayManager::setInputs( uint8_t player,
                                const PlayerInputs& playerInputs ) {
    // Only keep remote inputs at most 1 transition index old, but at least as
    // new as the startIndex
    ...
    const uint32_t checkStartingFromIndex =
        ( isInRollback() ? getIndex() - _startIndex : UINT_MAX );
...
MsgPtr NetplayManager::getBothInputs( IndexedFrame& pos ) const {
    ...
    // This is most recent frame, in the spectator's transition index, that the
    // spectator is allowed to "see"
    uint32_t commonEndFrame =
        min( _inputs[ 0 ].getEndFrame( orig.parts.index - _startIndex ),
    ...
    } else // During an older transition index
    {
        if ( orig.parts.frame + 1 <= commonEndFrame ) {
            ...
            // Since we're at the end of this transition index, increment to the
            // next one
    ...
    _inputs[ 0 ].get( bothInputs->getIndex() - _startIndex,
                      bothInputs->getStartFrame(),
                      &bothInputs->inputs[ 0 ][ 0 ], bothInputs->size() );

    _inputs[ 1 ].get( bothInputs->getIndex() - _startIndex,
                      bothInputs->getStartFrame(),
...
void NetplayManager::setBothInputs( const BothInputs& bothInputs ) {
    ...
    _inputs[ 0 ].set( bothInputs.getIndex() - _startIndex,
                      bothInputs.getStartFrame(), &bothInputs.inputs[ 0 ][ 0 ],
                      bothInputs.size() );

    _inputs[ 1 ].set( bothInputs.getIndex() - _startIndex,
                      bothInputs.getStartFrame(), &bothInputs.inputs[ 1 ][ 0 ],
                      bothInputs.size() );
}

bool NetplayManager::isRemoteInputReady() const {
    if ( _state.value < NetplayState::CharaSelect ||
         _state.value == NetplayState::Skippable ||
         _state.value == NetplayState::Loading ||
         _state.value == NetplayState::RetryMenu ||
    ...
    if ( _startIndex + _inputs[ _remotePlayer - 1 ].getEndIndex() - 1 <
         getIndex() ) {
        LOG( "[%s] remoteIndex=%u < localIndex=%u", _indexedFrame,
             _startIndex + _inputs[ _remotePlayer - 1 ].getEndIndex() - 1,
             getIndex() );
        return false;
    }

    // If remote index is ahead, we must be in an older state, so we don't need
    // to wait for inputs
    if ( _startIndex + _inputs[ _remotePlayer - 1 ].getEndIndex() - 1 >
         getIndex() )
        return true;
    ...
    if ( ( _inputs[ _remotePlayer - 1 ].getEndFrame() - 1 + maxFramesAhead ) <
         getFrame() ) {
        LOG( "[%s] remoteFrame = %u < localFrame=%u; delay=%u; rollback=%u; "
             "rollbackDelay=%u",
             _indexedFrame, _inputs[ _remotePlayer - 1 ].getEndFrame() - 1,
             getFrame(), config.delay, config.rollback, config.rollbackDelay );
...
void NetplayManager::setRngState( const RngState& rngState ) {
    ...
    _rngStates[ rngState.index - _startIndex ].reset(
        new RngState( rngState ) );
}

bool NetplayManager::isRngStateReady( bool shouldSyncRngState ) const {
    if ( !shouldSyncRngState || config.mode.isHost() ||
         config.mode.isBroadcast() || config.mode.isOffline() ||
    ...
    if ( ( _startIndex + _rngStates.size() - 1 ) < getIndex() ) {
        LOG( "[%s] remoteIndex=%u < localIndex=%u", _indexedFrame,
             _startIndex + _rngStates.size() - 1, getIndex() );
        return false;
    }
...
IndexedFrame NetplayManager::getLastChangedFrame() const {
    IndexedFrame _indexedFrame =
        _inputs[ _remotePlayer - 1 ].getLastChangedFrame();

    if ( _indexedFrame.value == MaxIndexedFrame.value )
        return MaxIndexedFrame;

    return { _indexedFrame.parts.frame,
             _startIndex + _indexedFrame.parts.index };
}
    ...
    static unordered_map< uint8_t, unordered_set< uint8_t > > validTransitions =
        {
            ...
            { NetplayState::Initial,
              { NetplayState::AutoCharaSelect, NetplayState::CharaSelect,
                NetplayState::ReplayMenu } },
            { NetplayState::AutoCharaSelect, { NetplayState::Loading } },
            { NetplayState::CharaSelect, { NetplayState::Loading } },
            { NetplayState::Loading,
              { NetplayState::Skippable, NetplayState::CharaIntro,
                NetplayState::InGame } },
            { NetplayState::CharaIntro, { NetplayState::InGame } },
            { NetplayState::Skippable,
              { NetplayState::InGame, NetplayState::RetryMenu } },
            { NetplayState::InGame,
              { NetplayState::Skippable, NetplayState::CharaSelect,
                NetplayState::ReplayMenu, NetplayState::RetryMenu } },
            { NetplayState::RetryMenu,
              { NetplayState::Loading, NetplayState::CharaSelect,
                NetplayState::ReplayMenu } },
...
void NetplayManager::exportInputs() {
    ...
    sprintf( namebuf, "ReplayVS/%sx%s_%s.repraw",
             ...
             getShortCharaName( *CC_P2_CHARACTER_ADDR ), timebuf );
...
void NetplayManager::exportResults() {
    ...
    if ( _localPlayer == 1 ) {
        sprintf( buf, "%s,%s-%s,%d,%s,%s-%s,%d,%d", n1.c_str(),
                 moon[ *CC_P1_MOON_SELECTOR_ADDR ].c_str(),
                 getShortCharaName( *CC_P1_CHARACTER_ADDR ), *CC_P1_WINS_ADDR,
                 n2.c_str(), moon[ *CC_P2_MOON_SELECTOR_ADDR ].c_str(),
                 getShortCharaName( *CC_P2_CHARACTER_ADDR ), *CC_P2_WINS_ADDR,
                 ( int )now );
    } else {
        sprintf( buf, "%s,%s-%s,%d,%s,%s-%s,%d,%d", n2.c_str(),
                 moon[ *CC_P2_MOON_SELECTOR_ADDR ].c_str(),
                 getShortCharaName( *CC_P2_CHARACTER_ADDR ), *CC_P2_WINS_ADDR,
...
void NetplayManager::findAndReplaceAll( string& data,
                                        string toSearch,
                                        string replaceStr ) {
