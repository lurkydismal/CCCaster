...
void ProcessManager::writeGameInput( uint8_t player,
                                     uint16_t direction,
                                     uint16_t buttons ) {
...
void ProcessManager::setRngState( const RngState& rngState ) {
    ...
    *CC_RNG_STATE2_ADDR = rngState.rngState2;
    ...
    copy( rngState.rngState3.begin(), rngState.rngState3.end(),
          CC_RNG_STATE3_ADDR );
}
...
void ProcessManager::connectPipe() {
    ...
    _pipe = CreateFile( NAMED_PIPE,                         // name of the pipe
    ...
    if ( _pipe == INVALID_HANDLE_VALUE ) {
        _pipe =
            CreateFile( NAMED_PIPE2,                        // name of the pipe
                        GENERIC_READ | GENERIC_WRITE,       // 2-way pipe
                        FILE_SHARE_READ | FILE_SHARE_WRITE, // R/W sharing mode
                        0,                                  // default security
                        OPEN_EXISTING,         // open existing pipe
    ...
    if ( _pipe == INVALID_HANDLE_VALUE )
        THROW_WIN_EXCEPTION( GetLastError(), "CreateFile failed",
                             ERROR_PIPE_START );
    ...
    if ( !WriteFile( _pipe, &( _ipcSocket->address.port ),
                     sizeof( _ipcSocket->address.port ), &bytes, 0 ) )
        THROW_WIN_EXCEPTION( GetLastError(), "WriteFile failed",
                             ERROR_PIPE_RW );

    if ( bytes != sizeof( _ipcSocket->address.port ) )
        THROW_EXCEPTION( "wrote %d bytes, expected %d", ERROR_PIPE_RW, bytes,
                         sizeof( _ipcSocket->address.port ) );
