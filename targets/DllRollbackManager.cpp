...
void DllRollbackManager::allocateStates() {
    if ( allAddrs.empty() ) {
        const size_t size = ( ( char* )&binary_res_rollback_bin_end ) -
                            ( char* )&binary_res_rollback_bin_start;
        allAddrs.load( ( char* )&binary_res_rollback_bin_start, size );
    }

    if ( allAddrs.empty() )
        THROW_EXCEPTION( "Failed to load rollback data!",
                         ERROR_BAD_ROLLBACK_DATA );

    if ( !_memoryPool )
        _memoryPool.reset( new char[ NUM_ROLLBACK_STATES * allAddrs.totalSize ],
                           deleteArray< char > );
...
void DllRollbackManager::saveState( const NetplayManager& netMan ) {
    if ( _freeStack.empty() ) {
        ...
        if ( _statesList.front().indexedFrame.parts.frame <=
             netMan.getRemoteFrame() ) {
    ...
    GameState state = { netMan._state, netMan._startWorldTime,
                        netMan._indexedFrame, fp_env,
    ...
    uint8_t* currentSfxArray =
        &_sfxHistory[ netMan.getFrame() % NUM_ROLLBACK_STATES ][ 0 ];
    memcpy( currentSfxArray, AsmHacks::sfxFilterArray, CC_SFX_ARRAY_LEN );
}

bool DllRollbackManager::loadState( IndexedFrame indexedFrame,
    ...
    LOG( "Trying to load state: indexedFrame=%s; _statesList={ %s ... %s }",
         indexedFrame, _statesList.front().indexedFrame,
         _statesList.back().indexedFrame );
    ...
    for ( auto it = _statesList.rbegin(); it != _statesList.rend(); ++it ) {
...
#endif
        {
            ...
            if ( !netMan.config.mode.isTraining() ) {
                rbFrames = _statesList.back().indexedFrame.value -
                           it->indexedFrame.value;
                LOG( "Rolled back %i frames", rbFrames );
            }
            // Erase all other states after the current one.
            // Note: it.base() returns 1 after the position of it, but moving
            // forward.
            ...
            if ( !netMan.config.mode.isTraining() ) {
                ...
                // Erase one frame of inputs from the game's replay structs for
                // each frame rolled back.
                for ( ; rbFrames > 0; rbFrames-- ) {
                    ...
                    RepRound* curRound =
                        ( *( RepRound** )CC_REPROUND_TBL_ENDPTR_ADDR - 1 );
                    LOG( "%d", curRound );
                    if ( !curRound->inputs ) {
                        LOG( "Missing inputs" );
                        break;
                    }
                    // Assumes there are always containers for 4 players in
                    // input container table; may not be true
                    for ( int i = 0; i < 4; i++ ) {
                        ...
                        RepInputState* state =
                            &( inputs->states[ inputs->activeIndex ] );
                        if ( !state->frameCount ) {
                        ...
                        if ( state->frameCount == 1 ) {
                            ...
                            LOG( "Replay state %i for p%i has frame count 1; "
                                 "decrementing index",
                                 inputs->activeIndex, i + 1 );
                            inputs->activeIndex--;
                        } else {
                            LOG( "Replay state %i for p%i has frame count %i; "
                                 "decrementing count",
                                 inputs->activeIndex, i + 1,
                                 state->frameCount );
                            state->frameCount--;
            ...
            // Initialize the SFX filter by flagging all played SFX flags in the
            // range (R,S), where R is the actual reset frame, and S is the
            // original starting frame. Note: we can skip frame S, because the
            // current SFX filter array is already initialized by frame S.
            for ( uint32_t i = netMan.getFrame() + 1; i < origFrame; ++i ) {
                for ( uint32_t j = 0; j < CC_SFX_ARRAY_LEN; ++j )
                    AsmHacks::sfxFilterArray[ j ] |=
                        _sfxHistory[ i % NUM_ROLLBACK_STATES ][ j ];
            }

            // We set the SFX filter flag to 0x80. Since played (but filtered)
            // SFX are incremented, unplayed sound effects in the filter will
            // stay as 0 or 0x80.
            for ( uint32_t j = 0; j < CC_SFX_ARRAY_LEN; ++j ) {
...
void DllRollbackManager::finishedRerunSounds() {
    ...
    for ( uint32_t j = 0; j < CC_SFX_ARRAY_LEN; ++j ) {
        // Filter flag 0x80 means the SFX didn't play after rollback since the
        // filter didn't get incremented
        if ( AsmHacks::sfxFilterArray[ j ] == 0x80 ) {
