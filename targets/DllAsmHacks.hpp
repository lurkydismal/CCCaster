...
#define WRITE_ASM_HACK( ASM_HACK )                              \
    do {                                                        \
        ...
        if ( error != 0 ) {                                     \
            LOG( "[%d] %s; %s failed; addr=%08x", error,        \
                 WinException::getAsString( error ), #ASM_HACK, \
                 ASM_HACK.addr );                               \
            exit( -1 );                                         \
        }                                                       \
    } while ( 0 )
...
// Auto replay save state, is set to 1 (loading), then 100 (saving), finally 255
// (saved)
extern uint32_t* autoReplaySaveStatePtr;

// Flag to enable / disable the Escape key function that exits game (initially
// true)
extern uint8_t enableEscapeToExit;

// Array of sound effects that were played last frame, set any SFX to 1 to
// prevent playback.
extern uint8_t sfxFilterArray[ CC_SFX_ARRAY_LEN ];

// Indicates the next playback of the sound effect should be muted.
// Can mute and play a sound effect at the same time to effectively cancel an
// existing playback.
...
void colorLoadCallback( uint32_t player,
                        uint32_t chara,
                        uint32_t* paletteData );
void colorLoadCallback( uint32_t player,
                        uint32_t chara,
                        uint32_t palette,
                        uint32_t* singlePaletteData );
...
// Add a call to the callback function just before the beginning of the game's
// main message loop. Note the message loop can get run multiple times per frame
// step, so be sure to check the world timer.
static const AsmList hookMainLoop = {
    { MM_HOOK_CALL1_ADDR,
      {
          0xE8,
          INLINE_DWORD( ( ( char* )&callback ) - MM_HOOK_CALL1_ADDR -
                        5 ), // call callback
          0xE9,
          INLINE_DWORD( MM_HOOK_CALL2_ADDR - MM_HOOK_CALL1_ADDR -
                        10 ) // jmp MM_HOOK_CALL2_ADDR
      } },
    { MM_HOOK_CALL2_ADDR,
      {
          0x6A, 0x01, // push 01
          ...
          0x6A, 0x00, // push 00
          0xE9,
          INLINE_DWORD( CC_LOOP_START_ADDR - MM_HOOK_CALL2_ADDR -
                        5 ) // jmp CC_LOOP_START_ADDR+6 (AFTER)
      } },
    // Write the jump location last, due to dependencies on the callback hook
    // code
    { CC_LOOP_START_ADDR,
      {
          0xE9,
          INLINE_DWORD( MM_HOOK_CALL1_ADDR - CC_LOOP_START_ADDR -
                        5 ), // jmp MM_HOOK_CALL1_ADDR
          0x90               // nop
                             // AFTER:
      } },
};
...
static const Asm disableFpsLimit = { CC_PERF_FREQ_ADDR,
                                     { INLINE_DWORD( 1 ), INLINE_DWORD( 0 ) } };

// Disable the code that updates the FPS counter
static const Asm disableFpsCounter = { ( void* )0x41FD43,
                                       INLINE_NOP_THREE_TIMES };
...
static const AsmList hijackMenu = {
    ...
    // This hack uses the strategy of jumping around the extra spaces left in
    // between code.
    { ( void* )0x4294D1,
      {
          0x8B, 0x7E, 0x40, // mov edi,[esi+40]
          0x89, 0x3D,
          INLINE_DWORD( &currentMenuIndex ), // mov [&currentMenuIndex],edi
    ...
    // The value of menuConfirmState is set to 1 if a menu confirm happens.
    // A menu confirm will only go through if menuConfirmState is greater
    // than 1.
    { ( void* )0x428F52,
      {
    ...
    { ( void* )0x428F82,
      {
          ...
          0x00,            // cmp [esp],0x4299F5 (return addr of menu call)
...
static const AsmList detectRoundStart = {
    { ( void* )0x440D16,
      {
          0xB9,
          INLINE_DWORD( &roundStartCounter ), // mov ecx,[&roundStartCounter]
...
static const Asm detectAutoReplaySave = {
    ...
    { 0x8D, 0x88, 0xD0, 0x00, 0x00, 0x00, // lea ecx,[eax+000000D0]
      0x89, 0x0D,
      INLINE_DWORD(
          &autoReplaySaveStatePtr ), // mov [&autoReplaySaveStatePtr],ecx
...
static const Asm multiWindow = { ( void* )MULTIPLE_MELTY,
                                 { 0xEB } }; // jmp 0040D262

// Force the game to go to a certain mode
static const Asm forceGotoVersus = { ( void* )0x42B475,
                                     { 0xEB, 0x3F } }; // jmp 0042B4B6
static const Asm forceGotoVersusCPU = { ( void* )0x42B475,
                                        { 0xEB, 0x5C } }; // jmp 0042B4D3
static const Asm forceGotoTraining = { ( void* )0x42B475,
                                       { 0xEB, 0x22 } }; // jmp 0042B499
static const Asm forceGotoReplay = {
    ( void* )0x42B475,
    { 0xE9, 0xC7, 0x00, 0x00, 0x00 } }; // jmp 0042B541
...
static const Asm hijackEscapeKey = {
    ( void* )0x4A0070,
    {
        0x80,
        0x3D,
        INLINE_DWORD( &enableEscapeToExit ),
        0x00, // cmp byte ptr [&enableEscapeToExit],00
        0xA0,
        INLINE_DWORD( 0x5544F1 ), // mov ax,[005544F1]
        0x75,
        0x03, // jne 0x4A0081 (AFTER)
        0x66,
        0x31,
        0xC0, // xor ax,ax
              // AFTER:
...
static const AsmList filterRepeatedSfx = {
    { ( void* )0x4DD836,
      {
          0xB8,
          INLINE_DWORD( sfxMuteArray ), // mov eax,sfxMuteArray
          0xEB,
          0x79, // jmp 0x4DD8B6
      } },
    { ( void* )0x4DD8B6,
    ...
    { ( void* )0x4DDFA4,
      {
          0x8B, 0x3C, 0xB5,
          INLINE_DWORD( 0x76C6F8 ),    // mov edi,[esi*4+0076C6F8]
...
static const AsmList muteSpecificSfx = {
    { ( void* )0x40EEA1,
      {
          ...
          0x81, 0xFA,
          INLINE_DWORD( CC_SFX_ARRAY_LEN ), // cmp edx,CC_SFX_ARRAY_LEN
    ...
    { ( void* )0x40F462,
      {
          0x8D, 0x92,
          INLINE_DWORD( sfxMuteArray ), // lea edx,[edx+sfxMuteArray]
...
// Disables the code that sets the intro state to 0. This is so we can manually
// set it during rollback
static const Asm hijackIntroState = { ( void* )0x45C1F2,
                                      INLINE_NOP_SEVEN_TIMES };

// Prevent training mode music from reseting
static const Asm disableTrainingMusicReset = { ( void* )0x472C6D,
                                               { 0xEB, 0x05 } }; // jmp 00472C74

// Fix the super flash overlays on the two boss stages.
// This is done by changing the string that the games searches for in the stage
// config .ini. Search for 'IsGiantStage' in bg/BgList.ini inside 0000.p.
static const Asm fixBossStageSuperFlashOverlay = { ( void* )0x53B3C8,
                                                   INLINE_FF_12_BYTES };
...
static const Asm hijackCharaSelectColors = {
    ...
    {
        0xE8,
        INLINE_DWORD( ( ( char* )&charaSelectColorCb ) - 0x489CD1 -
                      5 ), // call charaSelectColorCb
        0x90,
        0x90,
        0x90, // nops
    } };
    ...
    { ( void* )0x448202,
      {
          0x50, // push eax
          0xE8,
          INLINE_DWORD( ( ( char* )&loadingStateColorCb ) - 0x448202 - 1 -
                        5 ), // call loadingStateColorCb
          0x58,              // pop eax
          0x85,
          0xC0, // test eax,eax
          0xEB,
...
static const AsmList addExtraDraws = {
    { ( void* )0x432CD2,
      {
          0xE8,
          INLINE_DWORD( ( ( char* )&addExtraDrawCallsCb ) - 0x432CD2 -
                        5 ),           // call charaSelectColorCb
          0x6A, 0xFF,                  // Push -1
...
static const AsmList addExtraTextures = {
    /*{ ( void * ) 0x41c0f1, {
        0xE8, INLINE_DWORD ( ( ( char * ) &callbackExtraTextures ) - 0x41c0f1 -
       5 ),       // call addExtraTexturesCb 0x8B, 0x8C, 0x24, 0x1C, 0x01, 0x00,
       0x00 } },*/
    { ( void* )0x41BE38,
      { 0xE8,
        INLINE_DWORD( ( ( char* )&addExtraTexturesCb ) - 0x41BE38 -
                      5 ), // call addExtraTexturesCb
        0xC3 } },
};
