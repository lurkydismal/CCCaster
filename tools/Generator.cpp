...
#define CC_INPUT_STATE_ADDR \
    ( ( uint8_t* )0x562A6F ) // TODO figure out what the values mean
#define CC_SLOW_TIMER_INIT_ADDR \
    ( ( uint16_t* )0x562A6C ) // Initializes the slowdown timer
...
static const vector< MemDump > playerAddrs = {
    ...
    { 0x5552F0, 0x5552F4 },
    { 0x5552F4,
      0x555310 }, // ??? 0x5552F6, 2 bytes: Sion bullets, inverse counter
    { 0x555310, 0x55532C },
    ...
    { ( void* )0x55544C,
      4 }, // graphics pointer? this is accessed all the time even when paused
...
static const MemDump firstEffect(
    ...
    { MemDumpPtr( 0x320,
                  0x38,
                  4,
                  { MemDumpPtr( 0, 0, 4, { MemDumpPtr( 0, 0, 4 ) } ) } ) } );
...
int main( int argc, char* argv[] ) {
    ...
    for ( const MemDump& mem : allAddrs.addrs ) {
        ...
        for ( const MemDumpPtr& ptr0 : mem.ptrs ) {
            ...
            LOG( "  [0x%x]+0x%x -> { %u bytes }", ptr0.srcOffset,
                 ptr0.dstOffset, ptr0.size );

            for ( const MemDumpPtr& ptr1 : ptr0.ptrs ) {
                ASSERT( ptr1.parent == &ptr0 );

                LOG( "    [0x%x]+0x%x -> { %u bytes }", ptr1.srcOffset,
                     ptr1.dstOffset, ptr1.size );

                for ( const MemDumpPtr& ptr2 : ptr1.ptrs ) {
                    ASSERT( ptr2.parent == &ptr1 );

                    LOG( "      [0x%x]+0x%x -> { %u bytes }", ptr2.srcOffset,
                         ptr2.dstOffset, ptr2.size );
                }
            }
        }
    }
