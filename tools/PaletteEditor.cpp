...
bool PaletteEditor::init( const string& _palettesFolder,
...
void PaletteEditor::save( int paletteNumber ) {
    if ( paletteNumber >= 0 && paletteNumber != _paletteNumber ) {
        for ( int i = 0; i < 256; ++i ) {
            _palMans[ getChara() ].set(
                paletteNumber, i,
                _palMans[ getChara() ].get( _paletteNumber, i ) );
        }
...
int PaletteEditor::getChara() {
    return _frameDisp->get_character() /
           3; // Because FrameDisplay has each moon
}
...
int PaletteEditor::getCharaCount() {
    return _frameDisp->get_character_count() /
           3; // Because FrameDisplay has each moon
}
...
const char* PaletteEditor::getCharaName( int chara ) {
    return getShortCharaName( _frameDisp->get_character_index(
        chara * 3 ) ); // Because FrameDisplay has each moon
}
...
void PaletteEditor::loadCurrentChara() {
    if ( _palMans.find( getChara() ) == _palMans.end() ) {
        _palMans[ getChara() ].cache(
            static_cast< const MBAACC_FrameDisplay& >( *_frameDisp )
                .get_palette_data() );
...
void PaletteEditor::applyColor( uint32_t color ) {
    uint32_t& currColor =
        _frameDisp->get_palette_data()[ _paletteNumber ][ _colorNumber ];
    const uint32_t newColor =
        ( currColor & 0xFF000000 ) | ( SWAP_R_AND_B( color ) & 0xFFFFFF );
...
int PaletteEditor::getCharaIndex() {
    return _charaNumToIndex[ _frameDisp->get_character() /
                             3 ]; // Because FrameDisplay has each moon
}
