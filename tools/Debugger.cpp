...
void processInstruction( uint8_t buffer[ 64 ],
                         const _CodeInfo& ci,
                         const _DInst& di,
    ...
    //         case O_DISP: // memory dereference with displacement only,
    //         instruction.disp.
    //             addr = getRegVal ( di.segment, context ) + di.disp;
    //             ++numMemOperands;
    //             break;

    //         case O_SMEM: // simple memory dereference with optional
    //         displacement (a single register memory dereference).
    //             addr = getRegVal ( di.segment, context ) + getRegVal (
    //             op.index, context ) + di.disp;
    //             ++numMemOperands;
    //             break;

    //         case O_MEM:  // complex memory dereference (optional fields:
    //         s/i/b/disp).
    //             addr = getRegVal ( di.segment, context ) + getRegVal (
    //             di.base, context )
    //                    + getRegVal ( op.index, context ) * di.scale +
    //                    di.disp;
    //             ++numMemOperands;
    //             break;
    ...
    //         case O_PC:   // the relative address of a branch instruction
    //         (instruction.imm.addr). case O_PTR:  // the absolute target
    //         address of a far branch instruction
    //         (instruction.imm.ptr.seg/off).
    //             break;
    ...
    LOG( "%08X: %s:\t%s%s%s", ( uint32_t )context.Eip,
...
int main( int argc, char* argv[] ) {
    ...
    if ( !CreateProcessA( 0, buffer, 0, 0, TRUE, DEBUG_ONLY_THIS_PROCESS, 0,
    ...
    for ( bool debugging = true; debugging; ) {
        if ( !WaitForDebugEvent( &event, INFINITE ) ) {
            PRINT( "WaitForDebugEvent failed: %s",
        ...
        switch ( event.dwDebugEventCode ) {
            case CREATE_PROCESS_DEBUG_EVENT:
                ...
                WriteProcessMemory( process, CC_LOOP_START_ADDR,
                                    &breakpointConstant, 1, 0 );
            ...
            case EXCEPTION_DEBUG_EVENT:
                switch ( event.u.Exception.ExceptionRecord.ExceptionCode ) {
                    case EXCEPTION_BREAKPOINT:
                        ...
                        if ( !stepping &&
                             context.Eip == int( CC_LOOP_START_ADDR + 1 ) ) {
                            --context.Eip;
                            ...
                            WriteProcessMemory( process, CC_LOOP_START_ADDR,
                                                &original, 1, 0 );
                            FlushInstructionCache( process, CC_LOOP_START_ADDR,
                                                   1 );

                            stepping = true;

                            PRINT( "threadId = %u",
                                   ( uint32_t )event.dwThreadId );
                        }
                    ...
                    case EXCEPTION_SINGLE_STEP: {
                        ...
                        ReadProcessMemory( process, ( char* )context.Eip,
                                           buffer, sizeof( buffer ), 0 );
                        // WriteProcessMemory ( process, ( char * ) context.Eip,
                        // buffer, 1, 0 ); FlushInstructionCache ( process, (
                        // char * ) context.Eip, 1 );
                        ...
                        if ( distorm_decompose( &ci, &di, 1, &diCount ) ==
                                 DECRES_INPUTERR ||
                             diCount == 0 || di.flags == FLAG_NOT_DECODABLE ) {
                            PRINT( "decode failed @ eip = %08X",
                    ...
                    default:
                        // PRINT ( "Unexpected ExceptionCode: %d",
            ...
            case UNLOAD_DLL_DEBUG_EVENT:
                // PRINT ( "Unhandled dwDebugEventCode: %d",
                // event.dwDebugEventCode );
                break;

            default:
                PRINT( "Unexpected dwDebugEventCode: %d",
                       event.dwDebugEventCode );
                exit( -1 );
        }
        ...
        if ( !ContinueDebugEvent( event.dwProcessId, event.dwThreadId,
                                  DBG_CONTINUE ) ) {
            PRINT( "ContinueDebugEvent failed: %s",
                   WinException::getLastError() );
            exit( -1 );
        }
    }
