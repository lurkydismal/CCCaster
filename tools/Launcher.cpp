...
bool hookDLL( const string& dll_path, const PROCESS_INFORMATION* pi ) {
    ...
    void* dll_addr = VirtualAllocEx( pi->hProcess, 0, dll_path.size() + 1,
                                     MEM_COMMIT, PAGE_READWRITE );
    WriteProcessMemory( pi->hProcess, dll_addr, dll_path.c_str(),
                        dll_path.size() + 1, 0 );

    HANDLE hThread =
        CreateRemoteThread( pi->hProcess, 0, 0, pLoadLibrary, dll_addr, 0, 0 );
    if ( !hThread ) {
        char buffer[ 4096 ];
...
bool getbase( HANDLE hnd, DWORD* address, WORD* orig_code ) {
    ...
    ReadProcessMemory( hnd, ( LPCVOID )peAddress, &dosHeader,
                       sizeof( dosHeader ), 0 );
    ReadProcessMemory( hnd, ( LPCVOID )( peAddress + dosHeader.e_lfanew ),
                       &NTHeader, sizeof( NTHeader ), 0 );
...
bool hook( const string& exe_path,
           const string& dll_path,
           bool high_priority,
           bool framestep,
           const string& fsdll_path ) {
    // Initialize process
    STARTUPINFO si;
    ...
    if ( !GetFileAttributesEx( exe_path.c_str(), GetFileExInfoStandard,
                               &exe_info ) ) {
        char buffer[ 4096 ];
        snprintf( buffer, sizeof( buffer ),
                  "Couldn't find exe='%s'\nError [%d].", exe_path.c_str(),
                  ( int )GetLastError() );
    ...
    if ( !GetFileAttributesEx( dll_path.c_str(), GetFileExInfoStandard,
                               &dat_info ) ) {
        char buffer[ 4096 ];
        snprintf( buffer, sizeof( buffer ),
                  "Couldn't find dll='%s'\nError [%d].", dll_path.c_str(),
                  ( int )GetLastError() );
    ...
    const string dir_path =
        exe_path.substr( 0, exe_path.find_last_of( "/\\" ) );

    if ( !CreateProcessA( 0, buffer, 0, 0, TRUE, flags, 0, dir_path.c_str(),
                          &si, &pi ) ) {
        char buffer[ 4096 ];
        snprintf( buffer, sizeof( buffer ),
    ...
    if ( !getbase( pi.hProcess, &address, &orig_code ) ) {
        if ( popup_errors )
            MessageBox( 0, "Could not find entry point", "launcher error",
                        MB_OK | MB_ICONEXCLAMATION );
        return false;
    }

    WriteProcessMemory( pi.hProcess, ( void* )address, ( char* )&lock_code, 2,
                        0 );
    ...
    do {
        ...
        if ( !GetThreadContext( pi.hThread, &ct ) ) {
            ...
            if ( popup_errors )
                MessageBox( 0, "Could not get thread context.",
    ...
    WriteProcessMemory( pi.hProcess, ( void* )0x04A1D42, jmp1, sizeof( jmp1 ),
                        0 );

    static const char jmp2[] = { ( char )0xEB };
    WriteProcessMemory( pi.hProcess, ( void* )0x04A1D4A, jmp2, sizeof( jmp2 ),
                        0 );

    // Continue normal process action.
    WriteProcessMemory( pi.hProcess, ( void* )address, ( char* )&orig_code, 2,
                        0 );
...
int main( int argc, char* argv[] ) {
    ...
    if ( argc > 2 &&
         hook( argv[ 1 ], argv[ 2 ], options.find( "--high" ) != options.end(),
               options.find( "--framestep" ) != options.end(), argv[ 3 ] ) )
        return 0;
