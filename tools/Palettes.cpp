...
int GLFWCALL glfwMouseMoveEvent( int mouseX, int mouseY ) {
    ...
    if ( glfwGetMouseButton( GLFW_MOUSE_BUTTON_LEFT ) && lastX != INT_MIN &&
         lastY != INT_MIN ) {
...
int GLFWCALL glfwMouseWheelEvent( int wheelPos ) {
    ...
    if ( lastPos != INT_MIN ) {
        ...
        if ( deltaPos < 0 )
            zoom =
                clamped( zoom * pow( 0.95, ( double )-deltaPos ), 1.0, 10.0 );
        else
...
static void TW_CALL saveCurrentColorPrompt( void* clientData ) {
    ...
    TwDefine(
        " savePrompt label='Save Current Palette To' position='16 352' "
        "size='200 112' color='15 97 127' alpha=64"
        " resizable=false " );

    TwAddVarCB( savePrompt, "targetPalette", TW_TYPE_INT32, setTargetPalette,
                getTargetPalette, 0, " label='Palette' " );
...
int main( int argc, char* argv[] ) {
    ...
    if ( argc >= 2 )
        initialized = editor.init( PALETTES_FOLDER,
                                   string( argv[ 1 ] ) + "\\" DATA_FILE );
    ...
    if ( !glfwOpenWindow( screenWidth, screenHeight, videoMode.RedBits,
                          videoMode.GreenBits, videoMode.BlueBits, 0, 16, 0,
                          GLFW_WINDOW ) ) {
    ...
    bar = TwNewBar( "main" );
    TwDefine(
        " main label='Main Options' position='16 16' size='200 320' "
        "valueswidth=60"
    ...
    TwAddVarCB( bar, "spriteNumber", TW_TYPE_INT32, setSpriteNumber,
                getSpriteNumber, 0,
                " label='Sprite' keydecr=PGUP keyincr=PGDOWN " );

    TwAddVarCB( bar, "spriteFrame", TW_TYPE_INT32, setSpriteFrame,
                getSpriteFrame, 0, " label='Frame' keydecr=[ keyincr=] " );

    TwAddVarCB( bar, "paletteNumber", TW_TYPE_INT32, setPaletteNumber,
                getPaletteNumber, 0,
                " label='Palette' keydecr=UP keyincr=DOWN " );

    TwAddVarCB( bar, "colorNumber", TW_TYPE_INT32, setColorNumber,
                getColorNumber, 0,
    ...
    TwAddVarCB( bar, "currentColor", TW_TYPE_STDSTRING, setCurrentColor,
                getCurrentColor, 0, " label='Current Color' key=# " );
    ...
    TwAddVarCB( bar, "highlight", TW_TYPE_BOOLCPP, setHighlight, getHighlight,
                0, " label='Highlight' key=F2 " );

    TwAddButton( bar, "blank2", 0, 0, " label=' ' " );

    TwAddButton( bar, "save", saveCurrentColorPrompt, 0,
    ...
    while ( glfwGetWindowParam( GLFW_OPENED ) ) {
        ...
        editor.highlightCurrentColor( highlight &&
                                      ( editor.ticker % 60 ) < 30 );
